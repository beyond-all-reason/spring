...
{
	for (auto& ch: children) spring::SafeDelete(ch);
}
...
{
	GeometryChangeSelf();
	for (auto ch: children) ch->GeometryChange();
}

float GuiElement::DefaultOpacity() const { return 0.8f; }

void GuiElement::Move(float x, float y)
{
	pos[0] += x;
	pos[1] += y;
	for (auto ch: children) ch->Move(x, y);
}
...
void GuiElement::DrawBox(int primType, const SColor& color)
{
	...
	switch (primType) {
	case GL_QUADS: {
		rb.AddQuadTriangles({pos[0], pos[1], color}, {pos[0] + size[0], pos[1], color},
		    {pos[0] + size[0], pos[1] + size[1], color}, {pos[0], pos[1] + size[1], color});
