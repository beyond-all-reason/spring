...
void Window::DrawSelf()
{
	...
	rb.AddQuadTriangles({pos[0], pos[1] + size[1] - titleHeight, color}, {pos[0], pos[1] + size[1], color},
	    {pos[0] + size[0], pos[1] + size[1], color}, {pos[0] + size[0], pos[1] + size[1] - titleHeight, color});
	sh.Enable();
	...
	*/
	...
	font->glPrint(pos[0] + 0.01, pos[1] + size[1] - titleHeight / 2, 1.0,
	    FONT_VCENTER | FONT_SCALE | FONT_SHADOW | FONT_NORM, title);
	font->End();
}
#endif
