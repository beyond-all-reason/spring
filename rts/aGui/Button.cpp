...
void Button::DrawSelf()
{
	...
	font->glPrint(pos[0] + size[0] / 2, pos[1] + size[1] / 2, 1.0,
	    FONT_CENTER | FONT_VCENTER | FONT_SHADOW | FONT_SCALE | FONT_NORM, label);
	font->End();
}
#endif
...
bool Button::HandleEventSelf(const SDL_Event& ev)
{
	...
	case SDL_MOUSEBUTTONDOWN: {
		if ((ev.button.button == SDL_BUTTON_LEFT) && MouseOver(ev.button.x, ev.button.y) &&
