...
void List::SetFocus(bool focus) { hasFocus = focus; }

void List::RemoveAllItems() { items.clear(); }
...
int List::NumDisplay() { return std::max(1.0f, (size[1] - 2.0f * borderSpacing) / (itemHeight + itemSpacing)); }
...
float List::ScaleFactor() { return (float)std::max(1, globalRendering->winSizeY) * (size[1] - 2.0f * borderSpacing); }
...
bool List::MouseUpdate(int x, int y)
{
	...
	b.SetSize(
	    size[0] - 2.0f * borderSpacing - ((scrollbar.GetSize()[0] < 0) ? 0 : (itemHeight + itemSpacing)), itemHeight);

	// Get list started up here
	std::vector<std::string>::iterator ii = filteredItems->begin();
	UpdateTopIndex();

	while (nCurIndex < topIndex) {
		++ii;
		++nCurIndex;
	}
		...
		if (scrollbar.MouseOver(mx, my)) {
			...
			if (b.MouseOver(mx, my)) {
				if (my > scrollbar.GetPos()[1] + scrollbar.GetSize()[1])
					topIndex =
					    std::max(0, std::min(topIndex - NumDisplay(), (int)filteredItems->size() - NumDisplay()));
...
void List::DrawSelf()
{
	...
	for (/*ii = items.begin()*/; ii != filteredItems->end() && nDrawOffset < numDisplay; ++ii) {
		...
		font->glPrint(pos[0] + borderSpacing + 0.002f, b.GetMidY() - hf * 0.15f, itemFontScale,
		    FONT_BASELINE | FONT_SHADOW | FONT_SCALE | FONT_NORM, *ii);
	...
	if (nDrawOffset < filteredItems->size()) {
		...
		if (activeScrollbar) {
			topIndex = std::max(
			    0, std::min((int)(((float)filteredItems->size() *
			                          ((sbY1 - sbSize) - (my - std::min(scrollbarGrabPos, sbSize))) / sbHeight) +
			                      0.5f),
...
void List::UpOne() { place = std::max(0, place - 1); }

void List::DownOne() { place = std::max(0, std::min(place + 1, int(filteredItems->size()) - 1)); }

void List::UpPage() { place = std::max(0, place - NumDisplay()); }

void List::DownPage() { place = std::max(0, std::min(place + NumDisplay(), int(filteredItems->size()) - 1)); }
...
void List::CenterSelected() { topIndex = std::max(0, place - NumDisplay() / 2); }
