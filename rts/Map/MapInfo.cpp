...
static void FIND_MAP_TEXTURE(std::string* filePath, const std::string& defaultDir = "maps/")
{
	...
	if (CFileHandler::FileExists(
	        *filePath, SPRING_VFS_ZIP)) // no RawFS, cause it's also used for synced textures (typemap, metalmap, ...)
		return;

	*filePath = defaultDir + *filePath;
}

CMapInfo::CMapInfo(const std::string& mapFileName, const string& mapHumanName)
    : mapInfoParser(mapFileName)
{
	RECOIL_DETAILED_TRACY_ZONE;
	map.name = mapHumanName;

	if (!mapInfoParser.IsValid())
		throw content_error("[MapInfo] info-parser for map \"" + map.name + "\" (\"" + mapFileName + "\") invalid: \"" +
		                    mapInfoParser.GetErrorLog() + "\"");
...
void CMapInfo::ReadAtmosphere()
{
	...
	if (atmoTable.KeyExists("skyDir")) {
		LOG_L(L_DEPRECATED,
		    "[CMapInfo::%s] atmosphere.skyDir in mapinfo.lua was never used and now deprecated, use "
		    "atmosphere.skyAxisAngle instead",
		    __func__);
	}
...
void CMapInfo::ReadSMF()
{
	...
	if (sdnTexTable.IsValid()) {
		...
		for (int i = 0; true; i++) {
			if (!ParseSplatDetailNormalTexture(
			        mapResTable, "splatDetailNormalTex" + IntToString(i + 1), smf.splatDetailNormalTexNames)) {
				break;
			}
		}
	}
