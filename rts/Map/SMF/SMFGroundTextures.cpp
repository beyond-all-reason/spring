...
CONFIG(bool, SMFTextureStreaming)
    .defaultValue(false)
    .safemodeValue(true)
    .description("Dynamically load and unload SMF Diffuse textures. Saves VRAM, worse performance and image quality.");
CONFIG(float, SMFTextureLodBias)
    .defaultValue(0.0f)
    .safemodeValue(0.0f)
    .description("In case SMFTextureStreaming = false, this parameter controls the sampling lod bias applied to "
                 "diffuse texture");
...
void CSMFGroundTextures::LoadTiles(CSMFMapFile& file)
{
	...
	if ((mapDims.mapx != header.mapx) || (mapDims.mapy != header.mapy)) {
		std::string err = fmt::sprintf("[SMFGroundTextures::%s] header.{mapx=%d,mapy=%d} != mapDims.{mapx=%d,mapy=%d}",
		    __func__, header.mapx, header.mapy, mapDims.mapx, mapDims.mapy);
	...
	if (smfMap->tileCount <= 0) {
		std::string err =
		    fmt::sprintf("[SMFGroundTextures::%s] smfMap->tileCount=%d <= 0", __func__, smfMap->tileCount);
		throw content_error(err);
	}
	...
	if (!smf.smtFileNames.empty()) {
		if (!(smtHeaderOverride = (smf.smtFileNames.size() == tileHeader.numTileFiles))) {
			LOG_L(L_WARNING, "[SMFGroundTextures::%s] smtFileNames.size()=" _STPF_ " != tileHeader.numTileFiles=%d",
			    __func__, smf.smtFileNames.size(), tileHeader.numTileFiles);
		}
	}
	...
	for (int a = 0, curTile = 0; a < tileHeader.numTileFiles; ++a) {
		...
		if (!tileFile.FileExists()) {
			LOG_L(L_WARNING,
			    "[SMFGroundTextures::%s] could not find .smt tile-file %d (\"%s\"; ALL %d SMALL TILES WILL BE MADE "
			    "RED)",
		...
		if (strcmp(tfh.magic, "spring tilefile") != 0 || tfh.version != 1 || tfh.tileSize != 32 ||
			...
			std::string err = fmt::sprintf("[SMFGroundTextures::%s] tile-file %d (path=\"%s\" magic=\"%s\" version=%d "
			                               "tileSize=%d comprType=%d) does not match .smt format",
		...
		for (int x = 0; x < nbx; ++x) {
			...
			for (int x2 = x * mipSquareSize + 1; x2 < (x + 1) * mipSquareSize - 1; x2++) {
				for (int y2 = y * mipSquareSize + 1; y2 < (y + 1) * mipSquareSize - 1; y2++) {
					...
					// Gx sobel kernel
					const float gx = -1.0f * hdata[(y2 - 1) * mx + x2 - 1] + -2.0f * hdata[(y2)*mx + x2 - 1] +
					                 -1.0f * hdata[(y2 + 1) * mx + x2 - 1] + 1.0f * hdata[(y2 - 1) * mx + x2 + 1] +
					                 2.0f * hdata[(y2)*mx + x2 + 1] + 1.0f * hdata[(y2 + 1) * mx + x2 + 1];
					// Gy sobel kernel
					const float gy = -1.0f * hdata[(y2 + 1) * mx + x2 - 1] + -2.0f * hdata[(y2 + 1) * mx + x2] +
					                 -1.0f * hdata[(y2 + 1) * mx + x2 + 1] + 1.0f * hdata[(y2 - 1) * mx + x2 - 1] +
					                 2.0f * hdata[(y2 - 1) * mx + x2] + 1.0f * hdata[(y2 - 1) * mx + x2 + 1];
...
bool CSMFGroundTextures::RecompressTilesIfNeeded()
{
	...
	// note 2: Nvidia supports ETC but preprocesses the texture (on the CPU) each upload = slow -> makes no sense to add
	// it as another map compression format note 3: for both DXT1 & ETC1/2 blocksize is 8 bytes per 4x4 pixel block ->
	// perfect for us :)
...
inline bool CSMFGroundTextures::TexSquareInView(int btx, int bty) const
{
	...
	static const float bigTexSquareRadius =
	    fastmath::apxsqrt(smfMap->bigTexSize * smfMap->bigTexSize + smfMap->bigTexSize * smfMap->bigTexSize);
...
void CSMFGroundTextures::DrawUpdate()
{
	...
	for (int y = 0; y < smfMap->numBigTexY; ++y) {
		...
		for (int x = 0; x < smfMap->numBigTexX; ++x) {
			...
			const float hAvg =
			    (heightMaxima[y * smfMap->numBigTexX + x] + heightMinima[y * smfMap->numBigTexX + x]) / 2.0f;
			...
			int wantedLevel = 0;
			float heightDiff = heightMaxima[y * smfMap->numBigTexX + x] - heightMinima[y * smfMap->numBigTexX + x];
...
bool CSMFGroundTextures::GetSquareLuaTexture(int texSquareX,
    int texSquareY,
    int texID,
    int texSizeX,
    int texSizeY,
    ...
    int lodMax)
{
	...
	for (int lod = lodMin; lod <= lodMax; ++lod) {
		...
		pbo.Bind();
		pbo.New(numSqBytes);
		ExtractSquareTiles(texSquareX, texSquareY, lod,
		    reinterpret_cast<GLint*>(pbo.MapBuffer(0, pbo.GetSize(), access | pbo.mapUnsyncedBit)));
		pbo.UnmapBuffer();
...
void CSMFGroundTextures::ExtractSquareTiles(const int texSquareX,
...
void CSMFGroundTextures::LoadSquareTexture(int x, int y, int level)
{
	...
	pbo.New(numSqBytes);
	ExtractSquareTiles(
	    x, y, level, reinterpret_cast<GLint*>(pbo.MapBuffer(0, pbo.GetSize(), access | pbo.mapUnsyncedBit)));
	pbo.UnmapBuffer();
