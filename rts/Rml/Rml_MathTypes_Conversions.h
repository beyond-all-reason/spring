...
#ifndef RML_MATHTYPES_CONVERSIONS_H
#define RML_MATHTYPES_CONVERSIONS_H

// not ready yet, not sure if this will be useful or not anyhow
#if 0
...
#define RMLUI_COLOUR_USER_EXTRA                                                                         \
    ...
    operator SColor() const { return SColor(red, green, blue, alpha); }                                 \
...
#define RMLUI_VECTOR2_USER_EXTRA                                                                        \
    ...
    operator typename int2() const { return int2(x, y); }                                               \
    template<typename U = Type, typename std::enable_if_t<std::is_same_v<U, float>>* = nullptr>         \
    operator float2() const { return float2(x, y); }                                                    \
    template<typename U = Type, typename std::enable_if_t<std::is_same_v<U, int>>* = nullptr>           \
    Vector2(int2 value) : Vector2(value.x, value.y) { }                                                 \
    template<typename U = Type, typename std::enable_if_t<std::is_same_v<U, float>>* = nullptr>         \
    Vector2(float2 value) : Vector2(value.x, value.y) { }                                               \
...
#define RMLUI_VECTOR3_USER_EXTRA                                                                        \
    ...
    template<typename U = Type, typename std::enable_if_t<std::is_same_v<U, float>>* = nullptr>         \
    Vector3(float3 value) : Vector3(value.x, value.y, value.z) { }                                      \
...
#define RMLUI_VECTOR4_USER_EXTRA                                                                        \
    ...
    template<typename U = Type, typename std::enable_if_t<std::is_same_v<U, float>>* = nullptr>         \
    Vector4(float4 value) : Vector4(value.x, value.y, value.z, value.w) { }                             \

// Extra code to be inserted into RmlUi::Matrix4<> class body.
#define RMLUI_MATRIX4_USER_EXTRA                                                                            \
	template<typename U = ComponentType, typename std::enable_if_t<std::is_same_v<U, float>>* = nullptr>    \
	operator CMatrix44f() const { return CMatrix44f(data()); }                                              \
	template<typename U = ComponentType, typename std::enable_if_t<std::is_same_v<U, float>>* = nullptr>    \
	Matrix4(CMatrix44f value) : Matrix4(Matrix4::FromColumnMajor(value.m)) { }                              \
