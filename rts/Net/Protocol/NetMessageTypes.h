...
enum NETMSG {
	...
	NETMSG_AICOMMAND =
	    14, // uint8_t playerNum; uint8_t aiID; int16_t unitID; int32_t id; uint8_t options; std::vector<float> params;
	NETMSG_AICOMMANDS = 15, // uint8_t playerNum; uint8_t aiID; uint8_t pairwise, uint32_t sameCmdID, uint8_t
	                        // sameCmdOpt, uint16_t sameCmdParamSize; int16_t unitIDCount;  unitIDCount *
	                        // int16_t(unitID) int16_t commandCount; commandCount * { int32_t id; uint8_t options;
	                        // std::vector<float> params }
	NETMSG_AISHARE = 16, // uint8_t playerNum, uint8_t aiID, uint8_t sourceTeam, uint8_t destTeam, float metal, float
	                     // energy, std::vector<int16_t> unitIDs
	...
	NETMSG_MAPDRAW =
	    31, // uint8_t messageSize =  8, playerNum, command = MapDrawAction::NET_ERASE; int16_t x, z;
	...
	NETMSG_STARTPOS = 36,     // uint8_t playerNum, uint8_t myTeam, ready /*0: not ready, 1: ready, 2: don't update
	                          // readiness*/; float x, y, z;
	...
	NETMSG_LUAMSG = 50, // /* uint16_t messageSize */, uint8_t playerNum, uint16_t script, uint8_t mode,
	                    // std::vector<uint8_t> rawData

	NETMSG_TEAM = 51, // uint8_t playerNum, uint8_t action, uint8_t parameter1
	NETMSG_GAMEDATA =
	    52, // /* uint8_t messageSize */, std::string setupText, std::string script, std::string map, int32_t
	        // mapChecksum, std::string mod, int32_t modChecksum, int32_t randomSeed (each string ends with \0)
	NETMSG_ALLIANCE =
	    53, // uint8_t playerNum, uint8_t otherAllyTeam, uint8_t allianceState (0 = not allied / 1 = allied)
	NETMSG_CCOMMAND = 54,   // /* int16_t! messageSize */, int! playerNum, std::string command, std::string extra (each
	                        // string ends with \0)
	...
	NETMSG_ATTEMPTCONNECT =
	    65, // uint16_t msgsize, uint16_t netversion, string playername, string passwd, string VERSION_STRING_DETAILED
	NETMSG_REJECT_CONNECT = 66, // string reason

	NETMSG_AI_CREATED = 70, // /* uint8_t messageSize */, uint8_t playerNum, uint8_t whichSkirmishAI, uint8_t team,
	                        // std::string name (ends with \0)
	...
	NETMSG_CREATE_NEWPLAYER = 75, // uint8_t playerNum, uint8_t spectator, uint8_t teamNum, std::string playerName #used
	                              // for players not preset in script.txt#

	NETMSG_AICOMMAND_TRACKED = 76, // uint8_t playerNum; uint8_t aiID; int16_t unitID; int32_t id; uint8_t options;
	                               // int32_t aiCommandId, std::vector<float> params;

	NETMSG_GAME_FRAME_PROGRESS = 77, // int32_t frameNum # this special packet skips queue & cache entirely, indicates
	                                 // current game progress for clients fast-forwarding to current point the game #
