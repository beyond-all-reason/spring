...
static const std::string GetPathCacheDir()
{
	...
	return (FileSystem::GetCacheDir() + FileSystemAbstraction::GetNativePathSeparator() + "paths" +
	        FileSystemAbstraction::GetNativePathSeparator());
}

static const std::string
GetCacheFileName(const std::string& fileHashCode, const std::string& peFileName, const std::string& mapFileName)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
void PathingState::Init(std::vector<IPathFinder*> pathFinderlist,
    PathingState* parentState,
    ...
    const std::string& mapFileName)
{
...

	{
		...
		vertexCosts.clear();
		vertexCosts.resize(
		    moveDefHandler.GetNumMoveDefs() * blockStates.GetSize() * PATH_DIRECTION_VERTICES, PATHCOST_INFINITY);
		maxSpeedMods.clear();
	...
	// precalc for FindBlockPosOffset()
	{
		...
		std::stable_sort(offsetBlocksSortedByCost.begin(), offsetBlocksSortedByCost.end(),
		    [](const SOffsetBlock& a, const SOffsetBlock& b) { return (a.cost < b.cost); });
	}
...
void PathingState::InitEstimator(const std::string& peFileName, const std::string& mapFileName)
{
	...
	if (!ReadFile(peFileName, mapFileName)) {
		...
		for_mt(0, numThreads,
		    [this, &pathBarrier](int i) { CalcOffsetsAndPathCosts(ThreadPool::GetThreadNum(), &pathBarrier); });
...
void PathingState::InitBlocks()
{
	...
	for (unsigned int idx = 0; idx < moveDefHandler.GetNumMoveDefs(); idx++) {
		...
		// LOG("TK PathingState::InitBlocks: blockStates.peNodeOffsets %d now %d", idx,
		// blockStates.peNodeOffsets[idx].size());
	}
}
...
void PathingState::CalcOffsetsAndPathCosts(unsigned int threadNum, spring::barrier* pathBarrier)
{
	...
	while ((i = --offsetBlockNum) >= 0) CalculateBlockOffsets(maxBlockIdx - i, threadNum);

	pathBarrier->wait();

	while ((i = --costBlockNum) >= 0) EstimatePathCosts(maxBlockIdx - i, threadNum);
}
...
void PathingState::CalculateBlockOffsets(unsigned int blockIdx, unsigned int threadNum)
{
	...
	for (unsigned int i = 0; i < moveDefHandler.GetNumMoveDefs(); i++) {
		...
		// LOG("TK PathingState::InitBlocks: blockStates.peNodeOffsets %d now %d looking up %d", i,
		// blockStates.peNodeOffsets[md->pathType].size(), blockIdx);
		...
		// 		, blockStates.peNodeOffsets[md->pathType][blockIdx].x,
		// blockStates.peNodeOffsets[md->pathType][blockIdx].y 		, blockPos.x, blockPos.y);
	}
}
 ...
 */
int2 PathingState::FindBlockPosOffset(const MoveDef& moveDef,
    unsigned int blockX,
    unsigned int blockZ,
    int threadNum) const
{
...
void PathingState::CalcVertexPathCosts(const MoveDef& moveDef, int2 block, unsigned int threadNum)
{
	...
	const uint8_t nodeLinksObsoleteFlags = blockStates.nodeLinksObsoleteFlags[idx] & (moveDef.allowDirectionalPathing) ?
	                                           PATH_DIRECTIONS_MASK :
	                                           PATH_DIRECTIONS_HALF_MASK;
...
void PathingState::CalcVertexPathCost(const MoveDef& moveDef,
    int2 parentBlockPos,
    ...
    unsigned int threadNum)
{
	...
	const unsigned int vertexCostIdx =
	    moveDef.pathType * mapBlockCount * PATH_DIRECTION_VERTICES + parentBlockIdx * PATH_DIRECTION_VERTICES + pathDir;
	...
	const bool strtBlocked =
	    ((CMoveMath::IsBlocked(moveDef, startPos, nullptr, threadNum) & CMoveMath::BLOCK_STRUCTURE) != 0);
	...
	IPath::SearchResult result =
	    pathFinders[threadNum]->GetPath(moveDef, pfDef, nullptr, startPos, path, MAX_SEARCHED_NODES_PF >> 2);
...
bool PathingState::ReadFile(const std::string& peFileName, const std::string& mapFileName)
{
	...
	LOG("[PathEstimator::%s] hash=%s file=\"%s\" (exists=%d)", __func__, hashHexString.c_str(), cacheFileName.c_str(),
	    FileSystem::FileExists(cacheFileName));
...
bool PathingState::WriteFile(const std::string& peFileName, const std::string& mapFileName)
{
	...
	zipFile file =
	    zipOpen(dataDirsAccess.LocateFile(cacheFileName, FileQueryFlags::WRITE).c_str(), APPEND_STATUS_CREATE);
	...
	for (int pathType = 0; pathType < moveDefHandler.GetNumMoveDefs(); ++pathType) {
		zipWriteInFileInZip(file, (const void*)&blockStates.peNodeOffsets[pathType][0],
		    blockStates.peNodeOffsets[pathType].size() * sizeof(short2));
	}
...
void PathingState::Update()
{
	...
	{
		const int progressiveUpdates =
		    std::ceil(updatedBlocks.size() * (1.f / (BLOCKS_TO_UPDATE << 2)) * modInfo.pfUpdateRateScale);
...
void PathingState::UpdateVertexPathCosts(int blocksToUpdate)
{
	...
	while (!updatedBlocks.empty()) {
		...
		for (unsigned int i = 0; i < numMoveDefs; i++) {
			...
			// LOG("TK PathingState::Update: moveDef = %d %p (%p)", consumedBlocks.size(), &consumedBlocks.back(),
			// consumedBlocks.back().moveDef);
		}
	...
	// FindOffset (threadsafe)
	{
		...
		auto updateOffset = [&](const int n) {
			...
			blockStates.peNodeOffsets[currBlockMD->pathType][blockN] =
			    FindBlockPosOffset(*currBlockMD, sb.blockPos.x, sb.blockPos.y, ThreadPool::GetThreadNum());
		};
...

	{
		...
		auto updateVertexPathCosts = [this, &updateCostBlockNum](int threadNum) {
			...
			while ((n = --updateCostBlockNum) >= 0) {
				// LOG("TK PathingState::Update: PROC moveDef = %d %p (%p)", n, &consumedBlocks[n],
				// consumedBlocks[n].moveDef);
...
std::uint32_t PathingState::CalcChecksum() const
{
...

	{
		...
		SNPRINTF(msgBuffer.data(), msgBuffer.size(), "[PE::%s][BLK_SIZE=%d][SHA_DATA=%s]", __func__, BLOCK_SIZE,
		    hexChars.data());
...
// CPathCache::CacheItem PathingState::GetCache(const int2 strtBlock, const int2 goalBlock, float goalRadius, int
// pathType, const bool synced) const
// {
...
void PathingState::AddCache(const IPath::Path* path,
...
void PathingState::AddPathForCurrentFrame(const IPath::Path* path,
    const IPath::SearchResult result,
    const int2 strtBlock,
    const int2 goalBlock,
    float goalRadius,
...
void PathingState::PromotePathForCurrentFrame(const IPath::Path* path,
    const IPath::SearchResult result,
