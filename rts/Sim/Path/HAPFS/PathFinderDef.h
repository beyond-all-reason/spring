...
public:
	...
	bool IsGoalBlocked(const MoveDef& moveDef,
	    const CMoveMath::BlockType& blockMask,
	    const CSolidObject* owner,
	    int threadNum) const;

	float Heuristic(uint32_t srcSquareX,
	    uint32_t srcSquareZ,
	    uint32_t tgtSquareX,
	    uint32_t tgtSquareZ,
...
public:
	CCircularSearchConstraint(const float3& start = ZeroVector,
...
public:
	...
	CRectangularSearchConstraint(const float3 startPos, const float3 goalPos, float sqRadius, uint32_t blockSize);

	bool WithinConstraints(uint32_t xSquare, uint32_t zSquare) const
	{
		if (startBlockRect.Inside(int2(xSquare, zSquare)))
			return true;
		if (goalBlockRect.Inside(int2(xSquare, zSquare)))
			return true;
