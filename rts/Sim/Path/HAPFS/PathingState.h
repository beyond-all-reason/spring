...
public:
	...
	void Init(std::vector<IPathFinder*> pathFinderlist,
	...
	struct SOffsetBlock {
		...
		SOffsetBlock(const float _cost, const int x, const int y)
		    : cost(_cost)
		    , offset(x, y)
		{
		}
	...
	int2 BlockIdxToPos(const unsigned idx) const
	{
		return int2(idx % mapDimensionsInBlocks.x, idx / mapDimensionsInBlocks.x);
	}
	...
	// const
	CPathCache::CacheItem
	GetCache(const int2 strtBlock, const int2 goalBlock, float goalRadius, int pathType, const bool synced) const
	{
	...
	void AddPathForCurrentFrame(const IPath::Path* path,
	    const IPath::SearchResult result,
	    const int2 strtBlock,
	    const int2 goalBlock,
	    float goalRadius,
	...
	void PromotePathForCurrentFrame(const IPath::Path* path,
	    const IPath::SearchResult result,
...
private:
	...
	struct SingleBlock {
		...
		SingleBlock(const int2& pos, const MoveDef* md)
		    : blockPos(pos)
		    , moveDef(md)
		{
		}
