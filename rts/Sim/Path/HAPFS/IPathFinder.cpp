    ...
    const unsigned int maxNodes)
{
	...
	// selectedUnitsHandler.selectedUnits.end()){ 	LOG("Starting deeper logging for query: start (%d, %d) -> (%d, %d)
	// (%d
	// -> %d) [%f:%d] [%d] = %d" 			, mStartBlock.x, mStartBlock.y 			, goalBlock.x, goalBlock.y ,
	// mStartBlockIdx, mGoalBlockIdx 			, pfDef.sqGoalRadius, moveDef.pathType 			, BLOCK_SIZE,
	// debugLoggingActive);
	// }
