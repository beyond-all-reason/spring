...
struct PathNodeStateBuffer {
	PathNodeStateBuffer()
	{
		...
		static_assert(PATHOPT_SIZE <= std::numeric_limits<std::uint8_t>::max(),
		    "nodeMask basic type too small to hold PATHOPT bitmask");
#endif
	...
	unsigned int GetMemFootPrint() const
	{
		...
		if (!peNodeOffsets.empty())
			memFootPrint +=
			    (peNodeOffsets.size() * (sizeof(std::vector<short2>) + peNodeOffsets[0].size() * sizeof(short2)));
	...
	const float* GetNodeExtraCosts(bool synced) const { return extraCostsOverlay[synced]; }
...
public:
	...
	PathVector(int, const value_type&)
	    : bufPos(-1)
	{
		abort();
	}

	PathVector(iterator, iterator)
	    : bufPos(-1)
	{
		abort();
	}
