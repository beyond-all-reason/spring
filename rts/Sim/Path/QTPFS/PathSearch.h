...
struct Iteration {
	...
	Iteration& operator=(const Iteration& i)
	{
	...
	{
		nodeIndices = std::move(i.nodeIndices);
		return *this;
	}
...
struct Execution {
	...
	size_t GetMemFootPrint() const { return iterations.size() * sizeof(decltype(iterations)::value_type); };
...
public:
	...
	void Initialize(NodeLayer* layer, const float3& sourcePoint, const float3& targetPoint, const CSolidObject* owner);
...
private:
	...
	int SmoothPathPoints(const INode* nn0,
	    const INode* nn1,
	    const float3& p0,
	    const float3& p1,
	    const float3& p2,
...
public:
	static const std::uint32_t GenerateVirtualNodeNumber(const QTPFS::NodeLayer& nodeLayer,
