...
float QTPFS::INode::GetDistance(const INode* n, unsigned int type) const
{
	...
	switch (type) {
	case NODE_DIST_EUCLIDEAN: {
		return (math::sqrt((dx * dx) + (dz * dz)));
	} break;
	case NODE_DIST_MANHATTAN: {
		return (math::fabs(dx) + math::fabs(dz));
	} break;
	}
...
unsigned int QTPFS::INode::GetRectangleRelation(const SRectangle& r) const
{
	...
	//
	if ((r.x1 >= xmin() && r.x2 < xmax()) && (r.z1 >= zmin() && r.z2 < zmax())) {
		return REL_RECT_INTERIOR_NODE;
	}
	if ((r.x1 >= xmax() || r.x2 < xmin()) || (r.z1 >= zmax() || r.z2 < zmin())) {
		return REL_RECT_EXTERIOR_NODE;
	}
	if ((r.x1 < xmin() && r.x2 >= xmax()) && (r.z1 < zmin() && r.z2 >= zmax())) {
		return REL_NODE_INTERIOR_RECT;
	}
...
float2 QTPFS::INode::GetNeighborEdgeTransitionPoint(const INode* ngb, const float3& pos, float alpha) const
{
	...
	const unsigned int minx = std::max(xmin(), ngb->xmin()), maxx = std::min(xmax(), ngb->xmax());
	const unsigned int minz = std::max(zmin(), ngb->zmin()), maxz = std::min(zmax(), ngb->zmax());
...
//     with xsize and/or zsize equal to 1 heightmap square
	...
	const float midx = minx * (1.0f - alpha) + maxx * (0.0f + alpha),
	            midz = minz * (1.0f - alpha) + maxz * (0.0f + alpha);
#endif

	switch (GetNeighborRelation(ngb)) {
	// corners
	case REL_NGB_EDGE_T | REL_NGB_EDGE_L: {
		p.x = xmin() * SQUARE_SIZE;
		p.y = zmin() * SQUARE_SIZE;
	} break;
	case REL_NGB_EDGE_T | REL_NGB_EDGE_R: {
		p.x = xmax() * SQUARE_SIZE;
		p.y = zmin() * SQUARE_SIZE;
	} break;
	case REL_NGB_EDGE_B | REL_NGB_EDGE_R: {
		p.x = xmax() * SQUARE_SIZE;
		p.y = zmax() * SQUARE_SIZE;
	} break;
	case REL_NGB_EDGE_B | REL_NGB_EDGE_L: {
		p.x = xmin() * SQUARE_SIZE;
		p.y = zmax() * SQUARE_SIZE;
	} break;
...
#define CAST static_cast<unsigned int>
	...
	case REL_NGB_EDGE_T: {
		p.x = std::clamp(CAST(pos.x / SQUARE_SIZE), minx, maxx) * SQUARE_SIZE;
		p.y = minz * SQUARE_SIZE;
	} break;
	case REL_NGB_EDGE_B: {
		p.x = std::clamp(CAST(pos.x / SQUARE_SIZE), minx, maxx) * SQUARE_SIZE;
		p.y = maxz * SQUARE_SIZE;
	} break;
	case REL_NGB_EDGE_R: {
		p.y = std::clamp(CAST(pos.z / SQUARE_SIZE), minz, maxz) * SQUARE_SIZE;
		p.x = maxx * SQUARE_SIZE;
	} break;
	case REL_NGB_EDGE_L: {
		p.y = std::clamp(CAST(pos.z / SQUARE_SIZE), minz, maxz) * SQUARE_SIZE;
		p.x = minx * SQUARE_SIZE;
	} break;
...
#undef CAST
	...
	// edges
	case REL_NGB_EDGE_T: {
		p.x = midx * SQUARE_SIZE;
		p.y = zmin() * SQUARE_SIZE;
	} break;
	case REL_NGB_EDGE_R: {
		p.x = xmax() * SQUARE_SIZE;
		p.y = midz * SQUARE_SIZE;
	} break;
	case REL_NGB_EDGE_B: {
		p.x = midx * SQUARE_SIZE;
		p.y = zmax() * SQUARE_SIZE;
...
void QTPFS::QTNode::Init(const QTNode* parent,
    unsigned int nn,
    unsigned int x1,
    unsigned int z1,
    unsigned int x2,
...
bool QTPFS::QTNode::CanSplit(unsigned int depth, bool forced) const
{
	...
	if (((xsize() >> 1) == 0) || ((zsize() >> 1) == 0))
		return false;
	if ((xsize() <= MIN_SIZE_X) && (zsize() <= MIN_SIZE_Z))
		return false;
#endif
...
bool QTPFS::QTNode::Split(NodeLayer& nl, unsigned int depth, bool forced)
{
	...
	if ((childIndices[NODE_IDX_TL] = nl.AllocPoolNode(
	         this, GetChildID(NODE_IDX_TL, nl.GetRootMask()), xmin(), zmin(), xmid(), zmid())) == -1u)
		return false;
	if ((childIndices[NODE_IDX_TR] = nl.AllocPoolNode(
	         this, GetChildID(NODE_IDX_TR, nl.GetRootMask()), xmid(), zmin(), xmax(), zmid())) == -1u)
		return false;
	if ((childIndices[NODE_IDX_BL] = nl.AllocPoolNode(
	         this, GetChildID(NODE_IDX_BL, nl.GetRootMask()), xmin(), zmid(), xmid(), zmax())) == -1u)
		return false;
	if ((childIndices[NODE_IDX_BR] = nl.AllocPoolNode(
	         this, GetChildID(NODE_IDX_BR, nl.GetRootMask()), xmid(), zmid(), xmax(), zmax())) == -1u)
		return false;
...
void QTPFS::QTNode::PreTesselate(NodeLayer& nl,
    const SRectangle& r,
    SRectangle& ur,
    unsigned int depth,
    const UpdateThreadData* threadData)
{
	...
	const bool cont =
	    (rel == REL_RECT_INTERIOR_NODE) || (((xsize() >> 1) > (cr.x2 - cr.x1)) && ((zsize() >> 1) > (cr.z2 - cr.z1)));
...
void QTPFS::QTNode::Tesselate(NodeLayer& nl,
    const SRectangle& r,
    unsigned int depth,
    const UpdateThreadData* threadData)
{
...
bool QTPFS::QTNode::UpdateMoveCost(const UpdateThreadData* threadData,
    NodeLayer& nl,
    const SRectangle& r,
    ...
    bool& needSplit)
{
	...
	moveCostAvg =
	    (speedModAvg <= 0.001f) ? QTPFS_POSITIVE_INFINITY : (nl.UseShortestPath() ? 1.f : (1.f / speedModAvg));
...
unsigned int QTPFS::QTNode::GetMaxNumNeighbors() const
{
	...
	if (xmax() < (mapDims.mapx - 1)) {
		n += zsize();
	} // count EDGE_R ngbs
	if (zmin() > (0)) {
		n += xsize();
	} // count EDGE_T ngbs
	if (zmax() < (mapDims.mapy - 1)) {
		n += xsize();
	} // count EDGE_B ngbs
...
void QTPFS::QTNode::Serialize(std::fstream& fStream,
    NodeLayer& nodeLayer,
...
bool QTPFS::QTNode::UpdateNeighborCache(NodeLayer& nodeLayer, UpdateThreadData& threadData)
{
	...
	auto allowExitLink = [this](INode* ngb) { return IsExitOnly() || !ngb->IsExitOnly(); };
	...
	if (maxNgbs > 0) {
		...
		if (RectIntersects(threadData.areaRelinkedInner)) {
			...
			for (int ni = neighbours.size(); ni-- > 0;) {
				...
				if (curNode->NodeDeactivated() || !curNode->IsLeaf() ||
		...
		if (xmin() > relinkArea.x1) {
			...
			for (unsigned int hmz = nodeArea.z1; hmz < nodeArea.z2;) {
				...
				// 	LOG("Linking x %d -> [%d] (%d,%d) [%d,%d]", ngb->GetIndex(), relinkNodeindex, hmx, hmz, (hmx -
				// r.x1), (hmz - r.z1));
				// }
				hmz = ngb->zmax();
		...
		if (xmax() < relinkArea.x2) {
			...
			for (unsigned int hmz = nodeArea.z1; hmz < nodeArea.z2;) {
				...
				// if (nodeLayer.GetNodelayer() == 2) {
				// 	LOG("Linking x %d -> [%d] (%d,%d) [%d,%d] = %d", ngb->GetIndex(), relinkNodeindex, hmx, hmz, (hmx -
				// r.x1), (hmz - r.z1), rWidth);
				// }
				hmz = ngb->zmax();
		...
		if (zmin() > relinkArea.z1) {
			...
			for (unsigned int hmx = nodeArea.x1; hmx < nodeArea.x2;) {
				...
				// if (nodeLayer.GetNodelayer() == 2) {
				// 	LOG("Linking z %d -> [%d] (%d,%d) [%d,%d] = %d", ngb->GetIndex(), relinkNodeindex, hmx, hmz, (hmx -
				// r.x1), (hmz - r.z1), rWidth);
				// }
				hmx = ngb->xmax();
		...
		if (zmax() < relinkArea.z2) {
			...
			for (unsigned int hmx = nodeArea.x1; hmx < nodeArea.x2;) {
				...
				// if (nodeLayer.GetNodelayer() == 2) {
				// 	LOG("Linking %d -> [%d] (%d,%d) [%d,%d]", ngb->GetIndex(), relinkNodeindex, hmx, hmz, (hmx - r.x1),
				// (hmz - r.z1));
				// }
				hmx = ngb->xmax();
		...
		for (int i = 0; i < newNeighbors; i++) {
			...
			for (unsigned int i = 0; i < QTPFS_MAX_NETPOINTS_PER_NODE_EDGE; i++) {
				newNeighbour.netpoints[i] =
				    (INode::GetNeighborEdgeTransitionPoint(ngb, {}, QTPFS_NETPOINT_EDGE_SPACING_SCALE * (i + 1)));
			}
