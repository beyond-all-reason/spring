...
QTPFS::PathManager::~PathManager() {
	...
	std::for_each(nodeLayersCoarseMapUpdateTrack.mapChangeTrackers.begin(), nodeLayersCoarseMapUpdateTrack.mapChangeTrackers.end(), clearTrackers);
	nodeLayersMapDamageTrack.mapChangeTrackers.clear();
	nodeLayersCoarseMapUpdateTrack.mapChangeTrackers.clear();
...
void QTPFS::PathManager::Load() {
	...
	nodeLayersCoarseMapUpdateTrack.cellSize = std::min(rootSize, 128);
	nodeLayersCoarseMapUpdateTrack.width = mapDims.mapx / nodeLayersCoarseMapUpdateTrack.cellSize;
	nodeLayersCoarseMapUpdateTrack.height = mapDims.mapy / nodeLayersCoarseMapUpdateTrack.cellSize;
	
	nodeLayersMapDamageTrack.mapChangeTrackers.clear();
	nodeLayersCoarseMapUpdateTrack.mapChangeTrackers.clear();
	nodeLayersMapDamageTrack.mapChangeTrackers.reserve(numMoveDefs);
	nodeLayersCoarseMapUpdateTrack.mapChangeTrackers.reserve(numMoveDefs);
	for (int i = 0; i < numMoveDefs; ++i) {
		...
		{
			MapChangeTrack newChangeTrack;
			newChangeTrack.damageMap.resize(nodeLayersCoarseMapUpdateTrack.width*nodeLayersCoarseMapUpdateTrack.height);
			nodeLayersCoarseMapUpdateTrack.mapChangeTrackers.emplace_back(newChangeTrack);
		}
...
bool QTPFS::PathManager::SetNodeExtraCost(unsigned int x, unsigned int z, float cost, bool synced) {
	if (!IsFinalized())
		return false;
	...
	if (ecv.empty())
		ecv.resize(mapDims.mapx * mapDims.mapy, 0.0f); // alloc on-demand

	ecv[(z*mapDims.mapx) + x] = cost;

	return true;
}

bool QTPFS::PathManager::SetNodeExtraCosts(const float* costs, unsigned int sizex, unsigned int sizez, bool synced) {
	if (!IsFinalized())
		return false;
...
float QTPFS::PathManager::GetNodeExtraCost(unsigned int x, unsigned int z, bool synced) const {
	if (!IsFinalized())
		return 0.0f;
	...
	const int2 dsf = {mapDims.mapx / er[synced].x, mapDims.mapy / er[synced].y};

	const float* eco = nullptr;
	const float* ecd = nullptr;

	if ((eco = extraCostsOverlay[synced]) != nullptr)
		return eco[ (z / dsf.y) * er[synced].x  +  (x / dsf.x) ];

	// null if vector is empty
	if ((ecd = extraCosts[synced].data()) != nullptr)
		return ecd[(z*mapDims.mapx) + x];

	return 0.0f;
}

float QTPFS::PathManager::GetNodeExtraCostFast(unsigned int x, unsigned int z, bool synced) const {
	const float* eco;
	if ((eco = extraCostsOverlay[synced]) != nullptr) {
		const int2 dsf = {mapDims.mapx / er[synced].x, mapDims.mapy / er[synced].y};
		return eco[ (z / dsf.y) * er[synced].x  +  (x / dsf.x) ];
...
const float* QTPFS::PathManager::GetNodeExtraCosts(bool synced) const {
	if (!IsFinalized())
		return nullptr;
