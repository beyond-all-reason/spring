...
QTPFS::PathManager::~PathManager()
{
	...
	std::for_each(pathTraces.begin(), pathTraces.end(),
	    [](std::pair<unsigned int, QTPFS::PathSearchTrace::Execution*>& t) { delete t.second; });
	...
	pathTraces.clear();
	std::for_each(nodeLayersMapDamageTrack.mapChangeTrackers.begin(), nodeLayersMapDamageTrack.mapChangeTrackers.end(),
	    clearTrackers);
...
void QTPFS::PathManager::InitStatic()
{
	...
	// memory-related errors to occur.
	{
		...
		if (view.size() > 0) {
			LOG("%s: SharedPathChain is unexpectedly greater than 0.", __func__);
		}
	}
	{
		auto view = registry.view<PartialSharedPathChain>();
		if (view.size() > 0) {
			LOG("%s: PartialSharedPathChain is unexpectedly greater than 0.", __func__);
		}
	}
	{
		auto view = registry.view<IPath>();
		if (view.size() > 0) {
			LOG("%s: IPath is unexpectedly greater than 0.", __func__);
		}
	}
	...
	// prior to being used.
	{
		...
		if (view.size() > 0) {
			LOG("%s: PathIsTemp is unexpectedly greater than 0.", __func__);
		}
	}
	{
		auto view = registry.view<PathIsDirty>();
		if (view.size() > 0) {
			LOG("%s: PathIsDirty is unexpectedly greater than 0.", __func__);
		}
	}
	{
		auto view = registry.view<PathSpeedModInfoSystemComponent>();
		if (view.size() > 0) {
			LOG("%s: PathSpeedModInfoSystemComponent is unexpectedly greater than 0.", __func__);
		}
	}
	{
		auto view = registry.view<PathSearchRef>();
		if (view.size() > 0) {
			LOG("%s: PathSearchRef is unexpectedly greater than 0.", __func__);
		}
	}
}
...
void QTPFS::PathManager::InitNodeLayersThreaded(const SRectangle& rect)
{
	...
	for_mt(0, nodeLayers.size(), [this, &loadMsg, &rect](const int layerNum) {
		...
		std::for_each(rootRects.begin(), rootRects.end(),
		    [this, layerNum, currentThread](auto& rect) { UpdateNodeLayer(layerNum, rect, currentThread); });
	});
...
void QTPFS::PathManager::InitRootSize(const SRectangle& r)
{
	...
	if (rootSize == QTPFS_BAD_ROOT_NODE_SIZE)
		LOG("%s: Warning! Map width and height highest common factor is smaller than QTPFS is designed to handle.",
		    __func__);
...
void QTPFS::PathManager::UpdateNodeLayer(unsigned int layerNum, const SRectangle& rect, int currentThread)
{
	...
	if (rect.x1 == 0 && rect.x2 == 0) {
		...
		if (nlMapDmgTracker.damageQueue.size() == 0) {
			return;
		}
		...
		r = SRectangle(blockIdxX, blockIdxY, blockIdxX + DAMAGE_MAP_BLOCK_SIZE, blockIdxY + DAMAGE_MAP_BLOCK_SIZE);
	}
	...
	// LOG("%s: [%d] needTesselation=%d, wantTesselation=%d", __func__, layerNum, (int)needTesselation,
	// (int)wantTesselation);
...
void QTPFS::PathManager::TerrainChange(unsigned int x1,
    unsigned int z1,
    unsigned int x2,
    unsigned int z2,
    unsigned int type)
...
void QTPFS::PathManager::MapChanged(int x1, int y1, int x2, int y2)
{
	...
	for (int i = 0; i < layers; ++i) {
		...
		const int2 min{std::max((x1 - xsizeh) / res, 0), std::max((y1 - zsizeh) / res, 0)};
		const int2 max{std::min((x2 + xsizeh) / res, (w - 1)), std::min((y2 + zsizeh) / res, (h - 1))};
	...
	}
	{
		...
		auto numBlocksToUpdate = [this](int layerNum) {
			...
			int updatedBlocks = nodeLayersMapDamageTrack.mapChangeTrackers[layerNum].damageQueue.size();
			{
				...
				const int progressiveUpdates =
				    std::ceil(updatedBlocks * (1.f / (BLOCKS_TO_UPDATE << 3)) * modInfo.pfUpdateRateScale);
		...
		for_mt(0, nodeLayers.size(), [this, &rect, &numBlocksToUpdate](const int index) {
			...
			for (int i = 0; i < blocksToUpdate; ++i) {
				UpdateNodeLayer(layerNum, rect, curThread);
			}
		});
...
bool QTPFS::PathManager::InitializeSearch(entt::entity searchEntity)
{
	...
	if (registry.valid(pathEntity)) {
		...
		// LOG("%s: search vhash %x%x", __func__, int(search->GetPartialSearchHash() >> 32),
		// int(search->GetPartialSearchHash() & 32)); LOG("%s: path vhash %x%x", __func__, int(path->GetVirtualHash() >>
		// 32), int(path->GetVirtualHash() & 32)); assert(search->GetPartialSearchHash() == path->GetVirtualHash());
		...
		if (path->IsSynced()) {
			...
			if (search->GetPartialSearchHash() != QTPFS::BAD_HASH) {
				...
				if (partialSharedPathsIt == partialSharedPaths.end()) {
					...
					linkedListHelper.InsertChain<PartialSharedPathChain>(
					    partialSharedPaths[path->GetVirtualHash()], pathEntity);
				}
			}
		}
...
bool QTPFS::PathManager::ExecuteSearch(PathSearch* search, NodeLayer& nodeLayer, unsigned int pathType)
{
	ZoneScoped;
			...
			if (sharedPathsIt != sharedPaths.end()) {
				...
				if (chainHeadEntity != pathEntity) {
					...
					if (pathIsCopyable) {
						...
						PartialSharedPathMap::const_iterator partialSharedPathsIt =
						    partialSharedPaths.find(path->GetVirtualHash());
						if (partialSharedPathsIt != partialSharedPaths.end()) {
							assert(path->GetVirtualHash() != QTPFS::BAD_HASH);
							partialChainHeadEntity = partialSharedPathsIt->second;

							// If this path is the head of a partial path, we need to make sure it isn't blocking the
							// head of the full path copy (which would cause a deadlock.)
							if (partialChainHeadEntity == pathEntity) {
...
unsigned int QTPFS::PathManager::QueueSearch(const CSolidObject* object,
...
unsigned int QTPFS::PathManager::RequeueSearch(IPath* oldPath,
    const bool allowRawSearch,
...
void QTPFS::PathManager::RemovePathFromShared(entt::entity entity)
{
	...
	if (!registry.all_of<SharedPathChain>(entity))
		return;
...
void QTPFS::PathManager::RemovePathFromPartialShared(entt::entity entity)
{
	RECOIL_DETAILED_TRACY_ZONE;
	// if (!registry.valid(entity)) return;
	if (!registry.all_of<PartialSharedPathChain>(entity))
		return;
...
unsigned int QTPFS::PathManager::RequestPath(CSolidObject* object,
    const MoveDef* moveDef,
    float3 sourcePoint,
    float3 targetPoint,
    float radius,
...
float3 QTPFS::PathManager::NextWayPoint(const CSolidObject* owner,
    unsigned int pathID,
    ...
    bool synced)
{
	...
	if (nextPointIndex == 1) {
		...
		for (unsigned int i = (livePath->GetNextPointIndex()); i < lastPointIndex; i++) {
			...
			if (v0.SqLength() < squareRadius) {
				nextPointIndex = i + 1;
				break;
			}
			if (v1.SqLength() < squareRadius) {
				nextPointIndex = i + 2;
				break;
			}
			if (v0.dot(v1) <= -0.f) {
...
void QTPFS::PathManager::GetPathWayPoints(unsigned int pathID,
