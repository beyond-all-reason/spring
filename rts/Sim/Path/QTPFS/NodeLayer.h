...
public:
	...
	unsigned int AllocPoolNode(const INode* parent,
	    unsigned int nn,
	    unsigned int x1,
	    unsigned int z1,
	    unsigned int x2,
	    unsigned int z2)
	{
		...
		if (idx >= maxNodesAlloced) {
			maxNodesAlloced = idx + 1;
		}
	...
	void DecreaseOpenNodeCounter()
	{
		assert(numOpenNodes > 0);
		numOpenNodes -= (numOpenNodes > 0);
	}

	void DecreaseClosedNodeCounter()
	{
		assert(numClosedNodes > 0);
		numClosedNodes -= (numClosedNodes > 0);
	}
	...
	int GetRootNodeCount() const { return numRootNodes; }

	int GetNodelayer() const { return layerNumber; }
