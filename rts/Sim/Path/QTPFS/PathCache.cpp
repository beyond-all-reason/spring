...
bool QTPFS::PathCache::MarkDeadPaths(const SRectangle& r, const NodeLayer& nodeLayer)
{
	...
	const uint32_t damagedNodeNumber =
	    PathSearch::GenerateVirtualNodeNumber(nodeLayer, rootNode, damageWidth, r.x1, r.z1, &damageDepth);
	...
	auto testNodeDamage = [/*&testPathIntersect,*/ &nodeLayer, damagedNodeNumber, damageDepthMask, damageWidth,
	                          &pathWasChecked, &r](const QTPFS::IPath::PathNodeData& node, const IPath* path, int i) {
	...
	for (auto entity: pathView) {
		...
		if (registry.any_of<PathIsTemp>(entity)) {
			continue;
		}

		IPath* path = &pathView.get<IPath>(entity);

		if (path->IsSynced() == false) {
			continue;
		}
		if (path->GetPathType() != pathType) {
			continue;
		}
		...
		if ((r.x2 * SQUARE_SIZE) < pathMins.x) {
			continue;
		}
		if ((r.z2 * SQUARE_SIZE) < pathMins.z) {
			continue;
		}
		if ((r.x1 * SQUARE_SIZE) > pathMaxs.x) {
			continue;
		}
		if ((r.z1 * SQUARE_SIZE) > pathMaxs.z) {
			continue;
		}
		...
		if (path->IsRawPath() || pathNodeList.size() == 0) {
			...
			const bool p0InRect = ((p0.x >= (r.x1 * SQUARE_SIZE) && p0.x < (r.x2 * SQUARE_SIZE)) &&
			                       (p0.z >= (r.z1 * SQUARE_SIZE) && p0.z < (r.z2 * SQUARE_SIZE)));
			const bool p1InRect = ((p1.x >= (r.x1 * SQUARE_SIZE) && p1.x < (r.x2 * SQUARE_SIZE)) &&
			...
			//     (p0 --> p0 - rm, p1 --> p1 - rm)
			const bool xRangeInRect = (p0.x >= (r.x1 * SQUARE_SIZE) && p1.x < (r.x2 * SQUARE_SIZE)),
			...
			const bool edgeCrossesRect = (xRangeExRect && zRangeInRect) || (xRangeInRect && zRangeExRect) ||
			...
			for (int i = pathNodeStart; i > pathGoodFromNodeId; --i) {
				...
				// If the path is shared, then we need to check bad nodes so that path sharing can be stopped if
				// required. Otherwise, bad nodes can be ignored.
				if (node.IsNodeBad()) {
					assert(pathWasClean);
				...
				for (unsigned int i = minIdx; i < maxIdx; i++) {
					...
					if (intersectsPath) {
						...
						if (!triggerImmediateRepath)
							autoRefreshOnNode =
							    i + 1; // trigger happens when waypoints are requested, which always grabs one ahead.
