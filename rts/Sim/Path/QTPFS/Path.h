...
struct IPath {
	struct PathNodeData {
		...
		int GetNeighborRelationStatus(const PathNodeData& other) const
		{
			...
			return (NEIGHBOUR_BOTTOM * bottom) + (NEIGHBOUR_RIGHT * right) +
			       (NEIGHBOUR_LONGITUDE_TOUCH * (left | right)) + (NEIGHBOUR_LATITUDE_TOUCH * (top | bottom));
		}
	};
	...
	void SetBoundingBox()
	{
		boundingBoxMins.x = 1e6f;
		boundingBoxMaxs.x = -1e6f;
		boundingBoxMins.z = 1e6f;
		boundingBoxMaxs.z = -1e6f;
	...
	void SetSourcePoint(const float3& p)
	{ /* checkPointInBounds(p); */
		assert(points.size() >= 2);
		points[0] = p;
	}

	void SetTargetPoint(const float3& p)
	{ /* checkPointInBounds(p); */
		assert(points.size() >= 2);
	...
	{
		assert(pathType < moveDefHandler.GetNumMoveDefs());
