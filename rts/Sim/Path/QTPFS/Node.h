...
public:
	...
	void PreTesselate(NodeLayer& nl,
	    const SRectangle& r,
	    SRectangle& ur,
	    unsigned int depth,
	...
	void
	Serialize(std::fstream& fStream, NodeLayer& nodeLayer, unsigned int* streamSize, unsigned int depth, bool readMode);
	...
	{
		return xmin() <= rect.x1 && zmin() <= rect.y1 && xmax() >= rect.x2 && zmax() >= rect.y2;
	}

	bool RectIntersects(const SRectangle& rect) const
	{
		return !(xmin() >= rect.x2 || xmax() <= rect.x1 || zmin() >= rect.z2 || zmax() <= rect.z1);
	}
	...
	const std::vector<NeighbourPoints>& GetNeighbours() const { return neighbours; }
...
private:
	bool UpdateMoveCost(const UpdateThreadData* threadData,
	    NodeLayer& nl,
	    const SRectangle& r,
	...
	bool UpdateExitOnly(NodeLayer& nl, bool& needSplit);
...
struct SearchNode {
	...
	void SetPathCosts(float g, float h)
	{
		fCost = g + h;
		gCost = g;
		hCost = h;
