...
// per thread
template<typename T> struct SparseData {
	...
	void Reserve(size_t denseSize) { denseData.reserve(denseSize); }
...
typedef std::priority_queue<SearchQueueNode, std::vector<SearchQueueNode>, ShouldMoveTowardsBottomOfPriorityQueue>
    SearchPriorityQueue;
...
struct SearchThreadData {
	...
	{
		ZoneScoped;
		for (int i = 0; i < SEARCH_DIRECTIONS; ++i) ResetQueue(i);
	}

	void ResetQueue(int i)
	{
		ZoneScoped;
		while (!openNodes[i].empty()) openNodes[i].pop();
	}
...
struct UpdateThreadData {
	...
	void InitUpdate(const SRectangle& area, const INode& topNode, const MoveDef& md, int newThreadId)
	{
		...
		areaRelinkedInner = SRectangle(topNode.xmin(), topNode.zmin(), topNode.xmax(), topNode.zmax());
		areaRelinked = SRectangle(topNode.xmin() - 1, topNode.zmin() - 1, topNode.xmax() + 1, topNode.zmax() + 1);
		areaMaxBlockBits = SRectangle(topNode.xmin() - md.xsizeh, topNode.zmin() - md.zsizeh,
		    topNode.xmax() + md.xsizeh, topNode.zmax() + md.zsizeh);
	...
	SRectangle MapToRectangle() { return SRectangle(0, 0, mapDims.mapx, mapDims.mapy); }
