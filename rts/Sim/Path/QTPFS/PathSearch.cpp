...
void QTPFS::PathSearch::InitializeThread(SearchThreadData* threadData)
{
	...
	auto getTgtNode = [this, &fwd, pathToRepair](bool doPathRepair) {
		if (doPathRepair) {
			...
			// firstCleanNode.nodeNumber 		, int(!firstCleanNode.IsNodeBad()) 		, firstCleanNode.xmin,
			// firstCleanNode.zmin, firstCleanNode.xmax, firstCleanNode.zmax 		, firstCleanNode.netPoint.x,
			// firstCleanNode.netPoint.y 		, pathToRepair->GetFirstNodeIdOfCleanPath() 		,
...
void QTPFS::PathSearch::LoadPartialPath(IPath* path)
{
	...
	uint32_t badNodeCount = 0;
	{
		...
		std::for_each(nodes.begin(), nodes.end(),
			...
			// , nn1->GetNodeNumber() 		, nn1->xmin(), nn1->zmin(), nn1->xmax(), nn1->zmax() 		,
...
void QTPFS::PathSearch::LoadRepairPath()
{
	...
	for (auto i = nodeCount - 1; i >= firstCleanNodeId; --i) {
		...
		// stepIndex=%d", __func__ 		, node.nodeId, node.nodeNumber, int(!node.IsNodeBad()) 		, node.xmin,
		// node.zmin, node.xmax, node.zmax 		, nn1->GetNodeNumber() 		, nn1->xmin(), nn1->zmin(), nn1->xmax(),
		// nn1->zmax() 		, node.netPoint.x / SQUARE_SIZE, node.netPoint.y / SQUARE_SIZE 		, prevNodeId 		,
		// stepIndex
		// 		);
		// }
		assert(nn1->xmin() == node.xmin);
...
bool QTPFS::PathSearch::ExecutePathSearch()
{
	...
	auto reverseTrace = [this](int dir) {
		...
		// __func__ 		, revCurNode->index, revCurNode->nodeNumber 		, int(!revCurNode->isNodeBad()) ,
		...
		while (revCurNode != nullptr) {
			...
			if (fwdSearchNodes.isSet(revCurNode->GetIndex()) == false) {
				...
				// revCurNode->index, revCurNode->nodeNumber 		, int(!revCurNode->isNodeBad()) 		,
				// revCurNode->xmin, revCurNode->zmin, revCurNode->xmax, revCurNode->zmax 		,
			...
			if (bwdSearchNodes.isSet(curSearchNode->GetIndex())) {
				...
				if (haveFullPath) {
					...
					if (curSearchNode->GetPrevNode() == nullptr) {
						...
						// bwd.tgtSearchNode->index, bwd.tgtSearchNode->nodeNumber 		,
						// int(!bwd.tgtSearchNode->isNodeBad()) 		, bwd.tgtSearchNode->xmin,
						// bwd.tgtSearchNode->zmin, bwd.tgtSearchNode->xmax,bwd.tgtSearchNode->zmax
						// 		);
						// }
			...
			if (fwdSearchNodes.isSet(curSearchNode->GetIndex())) {
				...
				if (haveFullPath) {
					...
					if (curSearchNode->GetPrevNode() == nullptr) {
						...
						// if ((searchID == 7340095 || searchID == 10485810) /*&& pathOwner != nullptr && pathOwner->id
						// == 30809 */){ 	LOG("%s: bwd.tgtPoint2 (%f, %f, %f)", __func__, bwd.tgtPoint.x,
						// bwd.tgtPoint.y, bwd.tgtPoint.z);
						// }
					}
					searchThreadData->ResetQueue(SearchThreadData::SEARCH_FORWARD);
...
void QTPFS::PathSearch::IterateNodes(unsigned int searchDir)
{
	...
	// int(!searchData.minSearchNode->isNodeBad()) 		, searchData.minSearchNode->xmin,
	// searchData.minSearchNode->zmin, searchData.minSearchNode->xmax,searchData.minSearchNode->zmax
	// 		);
	// }
...
void QTPFS::PathSearch::TracePath(IPath* path)
{
	...
	if (fwd.srcSearchNode->GetIndex() != bwd.srcSearchNode->GetIndex()) {
		...
		if (!haveFullPath) {
			...
			while (tmpNode != nullptr) {
				...
				// tmpNode->index, tmpNode->nodeNumber 		, int(!newPoint.isBad) 		, tmpNode->xmin, tmpNode->zmin,
				// tmpNode->xmax, tmpNode->zmax 		, tmpNode->selectedNetpoint.x, tmpNode->selectedNetpoint.y
				// 		);
				// }
			...
			// if ((searchID == 7340095 || searchID == 10485810) /*&& pathOwner != nullptr && pathOwner->id == 30809
			// */){ 	LOG("%s: bwd.tgtPoint is (%f, %f, %f)", __func__, bwd.tgtPoint.x, bwd.tgtPoint.y,
			// bwd.tgtPoint.z);
			// }
			...
			while (tmpNode != nullptr) {
				...
				// tmpNode->index, tmpNode->nodeNumber 		, int(!tmpNode->isNodeBad()) 		, tmpNode->xmin,
				// tmpNode->zmin, tmpNode->xmax, tmpNode->zmax 		, tmpNode->selectedNetpoint.x,
				// tmpNode->selectedNetpoint.y
				// 		);
				// }
		...
		while ((nextNode != nullptr) && (tmpNode != fwd.srcSearchNode)) {
			...
			// tmpNode->nodeNumber 		, int(!tmpNode->isNodeBad()) 		, tmpNode->xmin, tmpNode->zmin,
			// tmpNode->xmax, tmpNode->zmax 		, tmpNode->selectedNetpoint.x, tmpNode->selectedNetpoint.y
			// 		);
			// }
	...
	for (TracePoint& curPoint: points) {
		...
		if ((nodeId & ONLY_NODE_ID_MASK) == 0) {
			...
			// selectedUnitsHandler.selectedUnits.end()); 	if (printMoveInfo) { 		LOG("%s: setting point %d (%f,
			// %f, %f)",
