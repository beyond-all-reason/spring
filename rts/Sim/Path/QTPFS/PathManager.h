...
public:
	...
	unsigned int RequestPath(CSolidObject* object,
...
private:
	...
	typedef void (PathManager::*MemberFunc)(unsigned int threadNum, unsigned int numThreads, const SRectangle& rect);
	...
	unsigned int QueueSearch(const CSolidObject* object,
	    const MoveDef* moveDef,
...
public:
	unsigned int
	RequeueSearch(IPath* oldPath, const bool allowRawSearch, const bool allowPartialSearch, const bool allowRepair);

private:
	bool ExecuteSearch(PathSearch* search, NodeLayer& nodeLayer, unsigned int pathType);
