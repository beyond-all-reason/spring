...
CPathCache::CPathCache(int blocksX, int blocksZ)
	...
	, numCacheMisses(0)
	, numHashCollisions(0)
	, maxCacheQueueSize(MAX_CACHE_QUEUE_SIZE)
{
...
bool CPathCache::AddPath(
	...
	if (cacheQue.size() > maxCacheQueueSize)
		RemoveFrontQueItem();
...
CPathCache::CacheItem& CPathCache::AddPathReturnItem(
	const IPath::Path* path,
	const IPath::SearchResult result,
	const int2 strtBlock,
	const int2 goalBlock,
	...
	// register any hash collisions
	if (iter != cachedPaths.end()){
		numHashCollisions += HashCollision(iter->second, strtBlock, goalBlock, goalRadius, pathType);
		return dummyCacheItem;
	}

	cachedPaths[hash] = CacheItem{result, *path, strtBlock, goalBlock, goalRadius, pathType};

	const int lifeTime = GAME_SPEED * MAX_PATH_LIFETIME_SECS;
	cacheQue.push_back({gs->frameNum + lifeTime, hash});
	maxCacheSize = std::max<std::uint64_t>(maxCacheSize, cacheQue.size());

	return cachedPaths[hash];
}

const CPathCache::CacheItem& CPathCache::GetPath(
	const int2 strtBlock,
	const int2 goalBlock,
	float goalRadius,
	int pathType
...
const CPathCache::CacheItem& CPathCache::GetCachedPath(
	...
	const std::uint64_t hash = GetHash(strtBlock, goalBlock, goalRadius, pathType);
	const auto iter = cachedPaths.find(hash);
	auto returnCache = dummyCacheItem;

	bool cacheHit = (iter != cachedPaths.end())
				&& ((iter->second).strtBlock == strtBlock) 
				&& ((iter->second).goalBlock == goalBlock)
				&& ((iter->second).pathType  == pathType );

	if (!cacheHit) {
		// Precache this request, so that the unithandler can group requests and reduce number of
		// actioned pathing requests.
		++numCacheMisses;
		returnCache = AddPathReturnItem(&dummyPathItem, IPath::Precached, strtBlock, goalBlock, goalRadius, pathType);
	}
	else {
		++numCacheHits;
		returnCache = (iter->second);
	}

	return returnCache;
}

bool CPathCache::UpdatePath(
	const IPath::Path* path,
	const IPath::SearchResult result,
	const int2 strtBlock,
	const int2 goalBlock,
...
void CPathCache::SetMinimumPathCacheSize(size_t newMinimumSize)
{
	if (newMinimumSize > maxCacheQueueSize) {
		cachedPaths.reserve(newMinimumSize);
		maxCacheQueueSize = newMinimumSize;
	}
}
