...
void CProjectileHandler::Kill()
{
...

	{
		...
		for (CProjectile* p: projectiles[true]) projMemPool.free(p);

		projectiles[true].clear();
	}

	{
		for (CProjectile* p: projectiles[false]) projMemPool.free(p);

		projectiles[false].clear();
	}

	{
		for (CGroundFlash* gf: groundFlashes) projMemPool.free(gf);
...
template<bool synced> void CProjectileHandler::UpdateProjectilesImpl()
{
...
template<class T> static void UPDATE_PTR_CONTAINER(T& cont)
{
	if (cont.empty())
		return;
...
template<class T> static void UPDATE_REF_CONTAINER(T& cont)
{
	if (cont.empty())
		return;
...
void CProjectileHandler::CheckUnitCollisions(CProjectile* p,
...
void CProjectileHandler::CheckFeatureCollisions(CProjectile* p,
...
void CProjectileHandler::CheckShieldCollisions(CProjectile* p,
    ...
    const float3 ppos1)
{
	...
	for (CPlasmaRepulser* repulser: tempRepulsers) {
		...
		if (!CCollisionHandler::DetectHit(
		        repulser->owner, &repulser->collisionVolume, CMatrix44f{cvpos}, rppos0, ppos1, &cq))
			continue;
...
void CProjectileHandler::AddFlyingPiece(int modelType,
...
void CProjectileHandler::AddNanoParticle(const float3 startPos,
    const float3 endPos,
    ...
    bool highPriority)
{
	...
	if (!inverse) {
		projMemPool.alloc<CNanoProjectile>(
		    startPos, dif * 3.0f, int(len / 3.0f), colors[globalRendering->teamNanospray]);
	}
	else {
		projMemPool.alloc<CNanoProjectile>(
		    startPos + dif * len, -dif * 3.0f, int(len / 3.0f), colors[globalRendering->teamNanospray]);
	}
}
