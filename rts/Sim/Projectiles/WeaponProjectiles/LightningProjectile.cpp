...
CR_REG_METADATA(CLightningProjectile,
    (CR_SETFLAG(CF_Synced), CR_MEMBER(color), CR_MEMBER(displacements1), CR_MEMBER(displacements2)))
...
void CLightningProjectile::Update()
{
	...
	if (--ttl <= 0) {
		...
		explGenHandler.GenExplosion(cegID, startPos + ((targetPos - startPos) / ttl), (targetPos - startPos), 0.0f,
		    displacements1[0], 0.0f, owner(), nullptr);
	}
...
void CLightningProjectile::Draw()
{
	...
	uint8_t col[4]{
	    (uint8_t)(color.x * 255), (uint8_t)(color.y * 255), (uint8_t)(color.z * 255),
	    1 // intensity*255;
	};
	...
	for (auto* displacement: displacements) {
		...
		for (size_t d = 1; d < displacements_size - 1; ++d) {
			...
			AddWeaponEffectsQuad<1>({tempPosO + (dir1 * (displacement[d] + WDV.thickness)), WDV.texture1->xstart,
			                            WDV.texture1->ystart, col},
			    {tempPos + (dir1 * (displacement[d + 1] + WDV.thickness)), WDV.texture1->xend, WDV.texture1->ystart,
			        col},
...
{
	RECOIL_DETAILED_TRACY_ZONE;
	const SColor lcolor = SColor{color[0], color[1], color[2]};
