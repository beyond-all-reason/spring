    ...
    , targetPos(params.end)
{
...

	{
		...
		validTextures = {false, IsValidTexture(weaponDef->visuals.texture1),
		    IsValidTexture(weaponDef->visuals.texture2), IsValidTexture(weaponDef->visuals.texture3),
	...
	if (ownerID != -1u && weaponNum != -1u) {
		...
		const CWeapon* weapon =
		    (owner != nullptr && weaponNum < owner->weapons.size()) ? owner->weapons[weaponNum] : nullptr;
...
void CWeaponProjectile::Explode(CUnit* hitUnit, CFeature* hitFeature, float3 impactPos, float3 impactDir)
{
	const DamageArray& damageArray = damages->GetDynamicDamages(startPos, impactPos);
	const CExplosionParams params = {.pos = impactPos,
...
void CWeaponProjectile::UpdateGroundBounce()
{
	...
	if (!bounced) {
		...
		const float waterDist = (weaponDef->waterBounce) ?
		                            CGround::LinePlaneCol(pos, dir, speed.w, CGround::GetWaterLevel(pos.x, pos.z)) :
		                            -1.0f;
		const float bounceDist = std::min(mix(groundDist, speed.w * 10000.0f, groundDist < 0.0f),
		    mix(waterDist, speed.w * 10000.0f, waterDist < 0.0f));
	...
	{
		// FIXME: ignoreWater?
		const float3 bounceNormal =
		    mix(CGround::GetNormal(bounceHitPos.x, bounceHitPos.z), UpVector, (bounceHitPos.y <= 0.0f));
		...
		explGenHandler.GenExplosion(
		    weaponDef->bounceExplosionGeneratorID, bounceHitPos, bounceNormal, speed.w, 1.0f, 1.0f, owner(), nullptr);
