...
CLargeBeamLaserProjectile::CLargeBeamLaserProjectile(const ProjectileParams& params)
...
void CLargeBeamLaserProjectile::Draw()
{
	...
	const float numBeamTiles =
	    std::floor(((std::max(beamTileMinDst, beamTileMaxDst) - beamTileMinDst) / tilelength) + 0.5f);
	...
	if (validTextures[1]) {
		...
		if (beamTileMinDst > beamLength) {
			...
			AddWeaponEffectsQuad<1>({pos1 - (xdir * beamEdgeSize), tex.xstart, tex.ystart, edgeColStart},
			...
			AddWeaponEffectsQuad<1>({pos1 - (xdir * beamCoreSize), tex.xstart, tex.ystart, coreColStart},
	...
	if (validTextures[2]) {
		AddWeaponEffectsQuad<2>({pos2 - (xdir * beamEdgeSize), WT2->xstart, WT2->ystart, edgeColStart},
		...
		AddWeaponEffectsQuad<2>({pos2 - (xdir * beamCoreSize), WT2->xstart, WT2->ystart, coreColStart},
	...
	if (validTextures[3]) {
		...
		AddWeaponEffectsQuad<3>({pos1 + (ydir * muzzleEdgeSize), WT3->xstart, WT3->ystart, edgeColor},
		...
		AddWeaponEffectsQuad<3>({pos1 + (ydir * muzzleCoreSize), WT3->xstart, WT3->ystart, coreColor},
	...
	if (validTextures[4]) {
		...
		AddWeaponEffectsQuad<4>({pos1 - (camera->GetRight() * flareEdgeSize) - (camera->GetUp() * flareEdgeSize),
		                            WT4->xstart, WT4->ystart, edgeColStart},
		    {pos1 + (camera->GetRight() * flareEdgeSize) - (camera->GetUp() * flareEdgeSize), WT4->xend, WT4->ystart,
		        edgeColStart},
		    {pos1 + (camera->GetRight() * flareEdgeSize) + (camera->GetUp() * flareEdgeSize), WT4->xend, WT4->yend,
		        edgeColStart},
		    {pos1 - (camera->GetRight() * flareEdgeSize) + (camera->GetUp() * flareEdgeSize), WT4->xstart, WT4->yend,
		        edgeColStart});

		AddWeaponEffectsQuad<4>({pos1 - (camera->GetRight() * flareCoreSize) - (camera->GetUp() * flareCoreSize),
		                            WT4->xstart, WT4->ystart, coreColStart},
		    {pos1 + (camera->GetRight() * flareCoreSize) - (camera->GetUp() * flareCoreSize), WT4->xend, WT4->ystart,
		        coreColStart},
		    {pos1 + (camera->GetRight() * flareCoreSize) + (camera->GetUp() * flareCoreSize), WT4->xend, WT4->yend,
		        coreColStart},
		    {pos1 - (camera->GetRight() * flareCoreSize) + (camera->GetUp() * flareCoreSize), WT4->xstart, WT4->yend,
		        coreColStart});
	}
