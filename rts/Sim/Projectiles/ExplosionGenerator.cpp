...
void CExplosionGeneratorHandler::ReloadGenerators(const std::string& tag)
{
	...
	if (!tag.empty()) {
		...
		if (it == expGenHashIdentMap.end()) {
			LOG_L(L_WARNING, "[%s] no CEG named \"%s\" (forgot the \"%s\" prefix?)", __func__, tag.c_str(),
			    CEG_PREFIX_STRING);
			return;
		}
...
bool CExplosionGeneratorHandler::GenExplosion(unsigned int expGenID,
    const float3& pos,
    const float3& dir,
    float damage,
    float radius,
...
bool CStdExplosionGenerator::Explosion(const float3& pos,
    const float3& dir,
    float damage,
    float radius,
    float gfxMod,
    ...
    bool withMutex)
{
	...
	if (projectileHandler.GetParticleSaturation() < 1.0f) {
		...
		for (int a = 0, n = smokeDamage * 0.6f; a < n; ++a) {
			const float3 speed((-0.1f + guRNG.NextFloat() * 0.2f), (0.1f + guRNG.NextFloat() * 0.3f) * smokeDamageISQRT,
			...
			projMemPool.alloc<CSmokeProjectile2>(
			    owner, pos, pos + dir, speed, time * mult, smokeDamageSQRT * 4.0f, 0.4f, 0.6f);
		}
		...
		if (groundExplosion) {
			...
			for (int a = 0, n = std::min(20.0f, damage * 0.8f); a < n; ++a) {
				const float3 explSpeed = float3((0.5f - guRNG.NextFloat()) * 1.5f, 1.7f + guRNG.NextFloat() * 1.6f,
				    (0.5f - guRNG.NextFloat()) * 1.5f);

				const float3 npos(pos.x - (0.5f - guRNG.NextFloat()) * (radius * 0.6f), pos.y - 2.0f - damage * 0.2f,
				    pos.z - (0.5f - guRNG.NextFloat()) * (radius * 0.6f));

				projMemPool.alloc<CDirtProjectile>(owner, npos, explSpeed * explSpeedMod, 90.0f + damage * 2.0f,
				    2.0f + sqrtDmg * 1.5f, 0.4f, 0.999f, color);
			}
		}
		...
		if (!airExplosion && !uwExplosion && waterExplosion) {
			...
			for (int a = 0, n = std::min(40.0f, damage * 0.8f); a < n; ++a) {
				const float3 speed((0.5f - guRNG.NextFloat()) * 0.2f, (a * 0.1f + guRNG.NextFloat() * 0.8f),
				    (0.5f - guRNG.NextFloat()) * 0.2f);
				const float3 npos(pos.x - (0.5f - guRNG.NextFloat()) * (radius * 0.2f), pos.y - 2.0f - sqrtDmg * 2.0f,
				    pos.z - (0.5f - guRNG.NextFloat()) * (radius * 0.2f));

				projMemPool.alloc<CDirtProjectile>(owner, npos, speed * (0.7f + std::min(30.0f, damage) / GAME_SPEED),
				    90.0f + damage * 2.0f, 2.0f + sqrtDmg * 2.0f, 0.3f, 0.99f, color);
			}
		}
		if (damage >= 20.0f && !uwExplosion && !airExplosion) {
			...
			for (int a = 0, n = (guRNG.NextInt(6)) + 3 + int(damage * 0.04f); a < n; ++a) {
				const float3 explSpeed = (altitude < 4.0f) ?
				                             float3((0.5f - guRNG.NextFloat()) * 2.0f, 1.8f + guRNG.NextFloat() * 1.8f,
				                                 (0.5f - guRNG.NextFloat()) * 2.0f) :
				                             float3(guRNG.NextVector() * 2);

				const float3 npos(pos.x - (0.5f - guRNG.NextFloat()) * (radius * 1), pos.y,
		...
		if (uwExplosion) {
			for (int a = 0, n = (damage * 0.7f); a < n; ++a) {
				projMemPool.alloc<CBubbleProjectile>(owner, pos + guRNG.NextVector() * radius * 0.5f,
				    guRNG.NextVector() * 0.2f + float3(0.0f, 0.2f, 0.0f), damage * 2.0f + guRNG.NextFloat() * damage,
				    1.0f + guRNG.NextFloat() * 2.0f, 0.02f, 0.5f + guRNG.NextFloat() * 0.3f);
			}
		}
		if (waterExplosion && !uwExplosion && !airExplosion) {
			for (int a = 0, n = (damage * 0.5f); a < n; ++a) {
				projMemPool.alloc<CWakeProjectile>(owner, pos + guRNG.NextVector() * radius * 0.2f,
				    guRNG.NextVector() * radius * 0.003f, sqrtDmg * 4.0f, damage * 0.03f,
				    0.3f + guRNG.NextFloat() * 0.2f, 0.8f / (sqrtDmg * 3 + 50 + guRNG.NextFloat() * 90.0f), 1);
			}
		}
		if (radius > 10.0f && damage > 4.0f) {
			...
			for (int a = 0, n = int(sqrtDmg) + 8; a < n; ++a) {
				...
				projMemPool.alloc<CExploSpikeProjectile>(owner, pos + explSpeed,
				    explSpeed * (0.9f + guRNG.NextFloat() * 0.4f), radius * 0.1f, radius * 0.1f, 0.6f,
				    0.8f / (8.0f + sqrtDmg));
			}
		}
	}
	...
	if (radius > 40.0f && damage > 12.0f) {
		CSpherePartProjectile::CreateSphere(owner, 5.0f + int(sqrtDmg * 0.7f), std::min(0.7f, damage * 0.02f),
		    (8.0f + damage * 2.5f) / (9.0f + sqrtDmg * 0.7f) * 0.5f, pos);
	}

	return true;
}

void CCustomExplosionGenerator::ExecuteExplosionCode(const char* code,
    float damage,
    char* instance,
    int spawnIndex,
    const float3& dir)
{
	...
	for (;;) {
		...
		case OP_STOREI: {
			std::uint8_t size = *(std::uint8_t*)code;
			code++;
			std::uint16_t offset = *(std::uint16_t*)code;
			code += 2;
			switch (size) {
			case 1: {
				*(std::int8_t*)(instance + offset) = (int)val;
			} break;
			case 2: {
				*(std::int16_t*)(instance + offset) = (int)val;
			} break;
			case 4: {
				*(std::int32_t*)(instance + offset) = (int)val;
			} break;
			case 8: {
				*(std::int64_t*)(instance + offset) = (int)val;
			} break;
			default: { /*no op*/
			} break;
			}
...
void CCustomExplosionGenerator::ParseExplosionCode(CCustomExplosionGenerator::ProjectileSpawnInfo* psi,
    const string& script,
    ...
    string& code)
{
	...
	if (isFloat) {
		...
		default: {
			throw content_error("[CCEG::ParseExplosionCode] incompatible float size \"" + IntToString(memberInfo.size) +
			                    "\" (" + script + ")");
		} break;
		}
	}
		...
		default: {
			throw content_error("[CCEG::ParseExplosionCode] incompatible integer size \"" +
			                    IntToString(memberInfo.size) + "\" (" + script + ")");
		} break;
		}
	}
	...
	for (size_t p = 0, len = script.length(); p < len;) {
		...
		if (c == 'i')
			...
			LOG_L(L_WARNING, "[CCEG::%s] unknown op-code \"%c\" in \"%s\" at index " _STPF_ "", __func__, c,
			    script.c_str(), p);
			continue;
		}
...
bool CCustomExplosionGenerator::Load(CExplosionGeneratorHandler* handler, const char* tag)
{
	...
	for (unsigned int n = 0; n < spawns.size(); n++) {
		...
		const string& className =
		    handler->GetProjectileClasses().ResolveAlias(spawnTable.GetString("class", spawnName));
		...
		for (const auto& propIt: props) {
			SExpGenSpawnableMemberInfo memberInfo = {0, 0, 0, STRING_HASH(std::move(StringToLower(propIt.first))),
			    SExpGenSpawnableMemberInfo::TYPE_INT, nullptr};

			if (CExpGenSpawnable::GetSpawnableMemberInfo(className, memberInfo)) {
				ParseExplosionCode(&psi, propIt.second, memberInfo, code);
			}
			else {
				LOG_L(L_WARNING, "[CCEG::%s] unknown field %s::%s in spawn-table \"%s\" for CEG \"%s\"", __func__, tag,
				    propIt.first.c_str(), spawnName.c_str(), className.c_str());
			}
		}
...
bool CCustomExplosionGenerator::Explosion(const float3& pos,
    const float3& dir,
    float damage,
    float radius,
    float gfxMod,
    ...
    bool withMutex)
{
	...
	if (expGenParams.useDefaultExplosions)
		return (explGenHandler.GenExplosion(
		    CExplosionGeneratorHandler::EXPGEN_ID_STANDARD, pos, dir, damage, radius, gfxMod, owner, hit));
