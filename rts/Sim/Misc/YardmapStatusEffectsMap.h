...
struct ObjectCollisionMapHelper {
	...
	uint8_t GetExitOnlyFlags() const { return YardmapStatusEffectsMap::EXIT_ONLY; }

	bool IsExitOnlyAt(int x, int z) const { return yardmapStatusEffectsMap.AreAllFlagsSet(x, z, GetExitOnlyFlags()); }

	void SetExitOnlyAt(int x, int z) const { yardmapStatusEffectsMap.SetFlags(x, z, GetExitOnlyFlags()); }

	void ClearExitOnlyAt(int x, int z) const { yardmapStatusEffectsMap.ClearFlags(x, z, GetExitOnlyFlags()); }

	uint8_t GetBlockBuildingFlags() const { return YardmapStatusEffectsMap::BLOCK_BUILDING; }
	...
	void SetBlockBuildingAt(int x, int z) const { yardmapStatusEffectsMap.SetFlags(x, z, GetBlockBuildingFlags()); }

	void ClearBlockBuildingAt(int x, int z) const { yardmapStatusEffectsMap.ClearFlags(x, z, GetBlockBuildingFlags()); }
};
