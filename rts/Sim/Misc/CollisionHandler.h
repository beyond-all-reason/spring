...
public:
	...
	void Transform(const CMatrix44f& m)
	{
		...
		if (b0 == CQ_POINT_ON_RAY) {
			p0 = m.Mul(p0);
		}
		if (b1 == CQ_POINT_ON_RAY) {
			p1 = m.Mul(p1);
		}
	}

	void Reset(const CollisionQuery* cq = nullptr) { *this = (cq != nullptr) ? *cq : CollisionQuery{}; }
	...
	float GetHitPosDist(const float3& pos, const float3& dir) const
	{
		return (std::max(0.0f, dir.dot(GetHitPos() - pos * (1 - InsideHit()))));
	}

	float GetIngressPosDist(const float3& pos, const float3& dir) const
	{
		return (std::max(0.0f, dir.dot(GetIngressPos() - pos)));
	}

	float GetEgressPosDist(const float3& pos, const float3& dir) const
	{
		return (std::max(0.0f, dir.dot(GetEgressPos() - pos)));
	}
...
public:
	...
	static bool DetectHit(const CSolidObject* o,
	    const CMatrix44f& m,
	    const float3 p0,
	    const float3 p1,
	...
	static bool DetectHit(const CSolidObject* o,
	    ...
	    CollisionQuery* cq = nullptr,
	    bool forceTrace = false);
	static bool MouseHit(const CSolidObject* o,
	    const CMatrix44f& m,
	    const float3& p0,
	    const float3& p1,
	    const CollisionVolume* v,
...
private:
	...
	static bool
	Collision(const CSolidObject* o, const CollisionVolume* v, const CMatrix44f& m, const float3 p, CollisionQuery* cq);
	// HITTEST_CONT helpers for DetectHit
	static bool Intersect(const CSolidObject* o,
	    const CollisionVolume* v,
	    const CMatrix44f& m,
	    const float3 p0,
	    const float3 p1,
...
private:
	 ...
	 */
	static bool
	Intersect(const CollisionVolume* v, const CMatrix44f& m, const float3& p0, const float3& p1, CollisionQuery* cq);
	static bool IntersectPieceTree(const CSolidObject* o,
	    const CMatrix44f& m,
	    const float3& p0,
	    const float3& p1,
	    CollisionQuery* cq);
	static bool IntersectPiecesHelper(const CSolidObject* o,
	    const CMatrix44f& m,
	    const float3& p0,
	    const float3& p1,
	    CollisionQuery* cqp);
