...
public:
	...
	CollisionVolume(const char cvTypeChar, const char cvAxisChar, const float3& cvScales, const float3& cvOffsets);
	...
	{
		if (DefaultToFootPrint()) {
			InitBox(float3(params.z, params.y, params.w));
			return true;
		}
		if (DefaultToSphere()) {
			InitSphere(params.x);
	...
	void InitShape(const float3& scales, const float3& offsets, const int vType, const int tType, const int pAxis);
	...
	bool HasCustomProp(float r) const
	{
		return (axisOffsets.SqLength() >= 1.0f || math::fabs(volumeBoundingRadius - r) >= 1.0f);
	}
...
private:
	float GetPointSurfaceDistance(const CSolidObject* obj,
...
private:
	...
	int8_t volumeAxes[3] = {COLVOL_AXIS_Z, COLVOL_AXIS_X, COLVOL_AXIS_Y};
	...
	bool defaultToFootPrint =
	    false; /// if true, volume becomes a box with dimensions equal to a SolidObject's {x,z}size
	bool defaultToPieceTree =
	    false; /// if true, volume is owned by a unit but defaults to piece-volume tree for hit-testing
};
