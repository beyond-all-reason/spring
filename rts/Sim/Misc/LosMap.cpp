...
template<typename F> void MidpointCircleAlgo(int radius, const F& func)
{
	int x = radius;
	int y = 0;
...
template<typename F> void MidpointCircleAlgoPerLine(int radius, const F& func)
{
	RECOIL_DETAILED_TRACY_ZONE;
	int x = radius;
	int y = 0;
...
public:
	...
	size_t GetLosTableRaySize(size_t losSize, size_t rayIndex) { return losTables[losSize][rayIndex].size(); }

	size_t GetLosTableSize(size_t losSize) { return losTables[losSize].size(); }
...
std::vector<int2> CLosTableHelper::GetCircleSurface(const int radius)
{
	...
	MidpointCircleAlgo(radius, [&](int x, int y) {
		...
		// second makes sure that only (0,radius) or (radius, 0) is generated (the other one is generated by mirroring
		// later)
		if (y != x && y != 0)
...
void CLosTableHelper::Debug(const LosTable& losRays, const std::vector<int2>& points, int radius)
{
	...
	auto setpixel = [&](int2 p, char value = 1) { image[p.y * (2 * radius + 1) + p.x] = value; };
...
void CLosMap::AddRaycast(SLosInstance* instance, int amount)
{
	...
	const bool visibleInstanceSquares =
	    (instance->allyteam >= 0 && (instance->allyteam == gu->myAllyTeam || gu->spectatingFullView));
...
inline void CastLos(float* prvAngle,
    float* maxAngle,
    const int2& off,
...
void CLosMap::UnsafeLosAdd(SLosInstance* li) const
{
	...
	for (size_t i = 0; i < numRays; ++i) {
		...
		for (size_t n = 0; n < numSquares; n++) {
			...
			CastLos(&prvAngles[2], &maxAngles[2], int2(square.y, -square.x), losRaySquares, raycastAngles, radius,
			    threadNum);
			CastLos(&prvAngles[3], &maxAngles[3], int2(-square.y, square.x), losRaySquares, raycastAngles, radius,
			    threadNum);
		}
	}
	...
	if (safeRect.Inside(pos)) {
		...
		for (size_t i = 0; i < numRays; ++i) {
			...
			for (size_t n = 0; n < numSquares; n++) {
				...
				if (safeRect.Inside(pos + int2(square.y, -square.x)))
					CastLos(&prvAngles[2], &maxAngles[2], int2(square.y, -square.x), losRaySquares, raycastAngles,
					    radius, threadNum);

				if (safeRect.Inside(pos + int2(-square.y, square.x)))
					CastLos(&prvAngles[3], &maxAngles[3], int2(-square.y, square.x), losRaySquares, raycastAngles,
					    radius, threadNum);
			}
		}
	}
