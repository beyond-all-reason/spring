...
public:
	...
	virtual bool IsMovingTowards(const float3& pos, float, bool) const
	{
		return (pos == goalPos && progressState == Active);
	}

	virtual bool IsAtGoalPos(const float3& pos, float radius) const
	{
		return (pos.SqDistance2D(goalPos) < (radius * radius));
	}
	...
	float CalcScriptMoveRate(float speed, float nsteps) const
	{
		return std::clamp(math::floor((speed / maxSpeed) * nsteps), 0.0f, nsteps - 1.0f);
	}
...
protected:
	...
	bool useWantedSpeed[2] = {true,
	    true}; // if false, SelUnitsAI will not (re)set wanted-speed for {[0] := individual, [1] := formation} orders
};
