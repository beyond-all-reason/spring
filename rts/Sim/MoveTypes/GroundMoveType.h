...
public:
	...
	// Update the unit's movement according to obsctacle avoidance and path following decisions from
	// UpdateTraversalPlan(). This is should be MT safe.
	...
	void TriggerSkipWayPoint() { earlyCurrWayPoint.y = -2.0f; }
...
private:
	...
	bool StartSkidding(const float3& vel, const float3& dir) const
	{
		return ((SignedSquare(vel.dot(dir)) + 0.01f) < (vel.SqLength() * sqSkidSpeedMult));
	}

	bool StopSkidding(const float3& vel, const float3& dir) const
	{
		return ((SignedSquare(vel.dot(dir)) + 0.01f) >= (vel.SqLength() * sqSkidSpeedMult));
	}
	...
	bool HandleStaticObjectCollision(CUnit* collider,
	...
	void HandleUnitCollisions(CUnit* collider,
	    const float3& colliderParams,
	    const UnitDef* colliderUD,
	    const MoveDef* colliderMD,
	    int curThread);
	float3 CalculatePushVector(const float3& colliderParams,
	    const float2& collideeParams,
	    const bool allowUCO,
	    const float4& separationVect,
	    CUnit* collider,
	    CUnit* collidee);
	void HandleFeatureCollisions(CUnit* collider,
	    const float3& colliderParams,
	    const UnitDef* colliderUD,
	    const MoveDef* colliderMD,
	    int curThread);
...
private:
	...
	short limitSpeedForTurning =
	    0; /// if set, take extra care to prevent overshooting while turning for the next N waypoints.
	...
	bool useRawMovement =
	    false; /// if true, move towards goal without invoking PFS (unrelated to MoveDef::allowRawMovement)
