...
struct CheckCollisionQuery {
	...
	{
		UpdateElevationForPos({int(pos.x / SQUARE_SIZE), int(pos.z / SQUARE_SIZE)});
	};
	...
	CSolidObject::PhysicalState physicalState =
	    CSolidObject::PhysicalState(CSolidObject::PhysicalState::PSTATE_BIT_ONGROUND);
...
public:
	...
	IsBlocked(const MoveDef& moveDef, const float3& pos, const CSolidObject* collider, int thread);
	static inline BlockType
	IsBlocked(const MoveDef& moveDef, int xSquare, int zSquare, const CSolidObject* collider, int thread);
	static BlockType IsBlockedNoSpeedModCheck(const MoveDef& moveDef,
	    int xSquare,
	    int zSquare,
	    const CSolidObject* collider,
	    int thread);
	static BlockType IsBlockedNoSpeedModCheckDiff(const MoveDef& moveDef,
	    int2 prevSqr,
	    int2 newSqr,
	    const CSolidObject* collider,
	    int thread = 0);
	static inline BlockType
	IsBlockedStructure(const MoveDef& moveDef, int xSquare, int zSquare, const CSolidObject* collider, int thread);
	...
	static BlockType
	SquareIsBlocked(const MoveDef& moveDef, int xSquare, int zSquare, MoveTypes::CheckCollisionQuery* collider);

	static BlockType
	SquareIsBlocked(const MoveDef& moveDef, const float3& pos, MoveTypes::CheckCollisionQuery* collider)
	{
		return (SquareIsBlocked(moveDef, pos.x / SQUARE_SIZE, pos.z / SQUARE_SIZE, collider));
	}

	static BlockType RangeIsBlocked(int xmin,
	    int xmax,
	    int zmin,
	    int zmax,
	    const MoveTypes::CheckCollisionQuery* collider,
	    int thread = 0);
	static BlockType RangeIsBlockedTempNum(int xmin,
	    int xmax,
	    int zmin,
	    int zmax,
	    const MoveTypes::CheckCollisionQuery* collider,
	...
	static BlockType RangeIsBlockedSt(int xmin,
	    int xmax,
	    int zmin,
	    int zmax,
	    const MoveTypes::CheckCollisionQuery* collider,
	    int magicNumber);
	static BlockType RangeIsBlockedMt(int xmin,
	    int xmax,
	    int zmin,
	    int zmax,
	    const MoveTypes::CheckCollisionQuery* collider,
	...
	static BlockType RangeIsBlockedHashedSt(int xmin,
	    int xmax,
	    int zmin,
	    int zmax,
	    const MoveTypes::CheckCollisionQuery* collider,
	    int magicNumber);
	static BlockType RangeIsBlockedHashedMt(int xmin,
	    int xmax,
	    int zmin,
	    int zmax,
	    const MoveTypes::CheckCollisionQuery* collider,
	...
	static void FloodFillRangeIsBlocked(const MoveDef& moveDef,
	    const CSolidObject* collider,
...
inline CMoveMath::BlockType
CMoveMath::IsBlocked(const MoveDef& moveDef, int xSquare, int zSquare, const CSolidObject* collider, int thread)
{
...
inline CMoveMath::BlockType
CMoveMath::IsBlocked(const MoveDef& moveDef, const float3& pos, const CSolidObject* collider, int thread)
{
	return (IsBlocked(moveDef, pos.x / SQUARE_SIZE, pos.z / SQUARE_SIZE, collider, thread));
}

inline CMoveMath::BlockType CMoveMath::IsBlockedStructure(const MoveDef& moveDef,
    int xSquare,
    int zSquare,
    const CSolidObject* collider,
    int thread)
