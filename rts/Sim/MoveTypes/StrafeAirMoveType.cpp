...
static float GetRudderDeflection(const CUnit* owner,
    ...
    bool isAttacking)
{
	...
	rudder += (1.0f * minHeightMult * ((goalDotRight < 0.0f) ? -0.01f : 0.01f) *
	           (math::fabs(rudder) < 0.01f && goalDotFront < 0.0f));

	return rudder;
}

static float GetAileronDeflection(const CUnit* owner,
    const CUnit* /*collidee*/,
    const float3& pos,
    const float4& spd,
    const SyncedFloat3& rightdir,
    ...
    bool isAttacking)
{
	...
	if (isAttacking) {
		const float minPredictedHeight =
		    pos.y + spd.y * 60.0f * math::fabs(frontdir.y) + std::min(0.0f, updir.y * 1.0f) * (GAME_SPEED * 5);
		...
		aileron += (1.0f * (1.0f - minSpeedMult) * (rightdir.y > 0.0f) *
		            (rightdir.y > maxAileronSpeedf || frontdir.y < -0.7f));
		aileron +=
		    (1.0f * (1.0f - minSpeedMult) * (aileron == 0.0f) * (rightdir.y > 0.0f) * (rightdir.y / maxAileronSpeedf));
		...
		aileron -= (1.0f * minSpeedMult * (aileron == 0.0f) * (1.0f - relBankMult) *
		            (rightdir.y < 0.0f && goalBankDif < 0.0f));
		aileron += (1.0f * minSpeedMult * (aileron == 0.0f) * (1.0f - relBankMult) *
		            (rightdir.y > 0.0f && goalBankDif > 0.0f));
...
static float GetElevatorDeflection(const CUnit* owner,
    const CUnit* collidee,
    const float3& pos,
    const float4& spd,
    const SyncedFloat3& rightdir,
    ...
    bool isAttacking)
{
	...
	if (isAttacking) {
		...
		elevator += (1.0f * speedMult * (goalDotY > maxElevatorSpeedf) * (collidee == owner) * (1.0f - zdirMult));
		elevator += (1.0f * speedMult * (elevator == 0.0f) * (goalDotY / maxElevatorSpeedf) * (collidee == owner) *
		             (1.0f - zdirMult));
...
#endif
		{
			...
			const float posHeight = CGround::GetHeightAboveWater(pos.x + spd.x * 40.0f, pos.z + spd.z * 40.0f);
			const float difHeight =
			    std::max(groundHeight, posHeight) + wantedHeight - pos.y - (frontdir.y * spd.w * 20.0f);
			...
			elevator += (1.0f * (elevator == 0.0f) * (spd.w > 0.8f) * (difHeight / maxElevatorSpeedf) *
			             (absFrontDirY < maxPitch));
		}
...
static float3 GetControlSurfaceAngles(const CUnit* owner,
    const CUnit* collidee,
    const float3& pos,
    const float4& spd,
    const SyncedFloat3& rightdir,
    ...
    bool isAttacking)
{
	...
	ctrlAngles.x = (yprInputLocks.x != 0.0f) ?
	                   GetRudderDeflection(owner, collidee, pos, spd, rightdir, updir, frontdir, goalDir, groundHeight,
	                       wantedHeight, maxCtrlAngles.x, maxBodyAngles.x, goalDotRight, goalDotFront, avoidCollision,
	                       isAttacking) :
	                   0.0f;
	ctrlAngles.y = (yprInputLocks.y != 0.0f) ?
	                   GetElevatorDeflection(owner, collidee, pos, spd, rightdir, updir, frontdir, goalDir,
	                       groundHeight, wantedHeight, maxCtrlAngles.y, maxBodyAngles.y, goalDotRight, goalDotFront,
	                       avoidCollision, isAttacking) :
	                   0.0f;
	ctrlAngles.z = (yprInputLocks.z != 0.0f) ?
	                   GetAileronDeflection(owner, collidee, pos, spd, rightdir, updir, frontdir, goalDir, groundHeight,
	                       wantedHeight, maxCtrlAngles.z, maxBodyAngles.z, goalDotRight, goalDotFront, avoidCollision,
	                       isAttacking) :
	                   0.0f;
...
static int SelectLoopBackManeuver(const SyncedFloat3& frontdir,
    const SyncedFloat3& rightdir,
    const float4& spd,
    float turnRadius,
    float groundDist)
    ...
    : AAirMoveType(owner)
{
	...
	wantedHeight =
	    (owner->unitDef->wantedHeight * 1.5f) + ((gsRNG.NextFloat() - 0.3f) * 15.0f * (isFighter ? 2.0f : 1.0f));
...
bool CStrafeAirMoveType::Update()
{
	...
	case AIRCRAFT_FLYING: {
		...
		const bool keepAttacking = ((owner->curTarget.type == Target_Unit && !owner->curTarget.unit->isDead) ||
		...
		*/
		{
			if (isAttacking && keepAttacking) {
				...
				case Target_Unit: {
					SetGoal(owner->curTarget.unit->pos);
				} break;
				case Target_Pos: {
					SetGoal(owner->curTarget.groundPos);
				} break;
				...
				if (maneuverState != MANEUVER_FLY_STRAIGHT) {
					...
					if (UpdateFlying(wantedHeight, 1.0f) && !goalInFront && loopbackAttack) {
						...
						if ((maneuverState = SelectLoopBackManeuver(
						         frontdir, rightdir, lastSpd, turnRadius, altitude)) == MANEUVER_IMMELMAN_INV)
							maneuverSubState = 0;
					}
				}
			}
	...
	case AIRCRAFT_LANDED: UpdateLanded(); break;
	case AIRCRAFT_LANDING: UpdateLanding(); break;
	...
	case AIRCRAFT_TAKEOFF: UpdateTakeOff(); break;
	default: break;
	}

	if (lastSpd == ZeroVector && owner->speed != ZeroVector) {
			...
			for (CUnit* unit: *qfQuery.units) {
				...
				if (unit->immobile) {
					...
					if (modInfo.allowUnitCollisionDamage) {
						owner->DoDamage(
						    DamageArray(damage), ZeroVector, nullptr, -CSolidObject::DAMAGE_COLLISION_OBJECT, -1);
						unit->DoDamage(
						    DamageArray(damage), ZeroVector, nullptr, -CSolidObject::DAMAGE_COLLISION_OBJECT, -1);
					}
...
void CStrafeAirMoveType::UpdateAttack()
{
	...
	const float3 goalDir = (goalDist > 0.0f) ? (goalPos - pos) / goalDist : ZeroVector;
...

	{
		...
		const float3 prvCtrlAngles[2] = {
		    {lastRudderPos[0], lastElevatorPos[0], lastAileronPos[0]},
		    {lastRudderPos[1], lastElevatorPos[1], lastAileronPos[1]}
        };
		const float3& curCtrlAngles = GetControlSurfaceAngles(owner, lastCollidee, pos, spd, rightdir, updir, frontdir,
		    goalDir, OnesVector, maxBodyAngles, maxCtrlAngles, prvCtrlAngles, gHeightAW, wantedHeight, goalDotRight,
		    goalDotFront, false && collisionState == COLLISION_DIRECT, true);
		...
		const float thrustLim =
		    ((attackee == nullptr) || attackee->unitDef->IsGroundUnit()) ? 1.0f : std::min(1.0f, rangeLim + angleLim);
...
bool CStrafeAirMoveType::UpdateFlying(float wantedHeight, float wantedThrottle)
{
	...
	const bool allowUnlockYawRoll =
	    (goalDist2D >= TurnRadius(turnRadius, spd.w) || goalVec.dot(owner->frontdir) > 0.0f);
	...
	const float nearGoal = ((goalDist2D < turnRadius * 0.5f && goalDir2D.dot(frontdir) < 0.7f) ||
	                        (goalDist2D < turnRadius && goalDir2D.dot(frontdir) < -0.1f));
	const float turnFlip =
	    ((!owner->UnderFirstPersonControl() || owner->fpsControlPlayer->fpsController.mouse2) * -2.0f + 1.0f);
	...
	const float3& curCtrlAngles = GetControlSurfaceAngles(owner, lastCollidee, pos, spd, rightdir, updir, frontdir,
	    goalDir2D, yprInputLocks, maxBodyAngles, maxCtrlAngles, prvCtrlAngles, groundHeight, wantedHeight, goalDotRight,
	    goalDotFront, false && collisionState == COLLISION_DIRECT, false);
...
void CStrafeAirMoveType::UpdateAirPhysics(const float4& controlInputs, const float3& thrustVector)
{
	...
	owner->SetVelocity(
	    spd + UpVector * (mapInfo->map.gravity * myGravity) * int((owner->midPos.y - owner->radius) > groundHeight));
	...
	owner->Move(
	    UpVector * (std::clamp(pos.y, groundHeight, readMap->GetCurrMaxHeight() + owner->unitDef->wantedHeight * 5.0f) -
	                   pos.y),
	    true);
...
void CStrafeAirMoveType::SetState(AAirMoveType::AircraftState newState)
{
	...
	switch (aircraftState) {
	case AIRCRAFT_CRASHING: owner->SetPhysicalStateBit(CSolidObject::PSTATE_BIT_CRASHING); break;
...
{
	RECOIL_DETAILED_TRACY_ZONE;
	if (((landPos.y = CGround::GetHeightReal(landPos)) < CGround::GetWaterLevel(landPos.x, landPos.z)) &&
	    ((mapInfo->water.damage > 0.0f) || !(floatOnWater || canSubmerge)))
		return -OnesVector;
