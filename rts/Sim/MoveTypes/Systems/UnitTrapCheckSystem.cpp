...
void SystemInit() { auto& comp = Sim::systemGlobals.CreateSystemComponent<YardmapTrapCheckSystemComponent>(); }
...
void TagUnitsThatMayBeStuck(std::vector<CUnit*>& curList, const CSolidObject* collidee, int curThread)
{
	...
	for (const CUnit* unit: *qfQuery.units) {
		...
		if ((squareX - moveDef->xsizeh) > xmax)
			continue;
		if ((squareZ - moveDef->zsizeh) > zmax)
			continue;
		if ((squareX + moveDef->xsizeh) < xmin)
			continue;
		if ((squareZ + moveDef->zsizeh) < zmin)
			continue;
