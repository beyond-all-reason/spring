...
CR_REG_METADATA(CCommandQueue, (CR_MEMBER(queue), CR_MEMBER(queueType), CR_MEMBER(tagCounter)))
...
bool CCommandAI::IsAttackCapable() const { return (owner->unitDef->CanAttack()); }
...
static inline bool IsCommandInMap(const Command& c)
{
	...
	switch (c.GetID()) {
	case CMD_CAPTURE: {
		pos = c.GetPos(1 * (c.GetNumParams() == 5));
	} break;
	case CMD_RECLAIM: {
		pos = c.GetPos(1 * (c.GetNumParams() == 5));
	} break;
...
bool CCommandAI::AllowedCommand(const Command& c, bool fromSynced)
{
	...
	case CMD_ATTACK: {
		...
		if (c.GetNumParams() == 1) {
			...
			const bool isGroundTargeted = (c.GetNumParams() > 3 && c.GetParam(3) == 0) || c.GetNumParams() == 3;
	...
	case CMD_REPAIR: {
		...
		if (repairee != nullptr && !repairee->pos.IsInBounds())
			return false;
		if (repairee != nullptr &&
		    ((repairee->beingBuilt && !ud->canAssist) || (!repairee->beingBuilt && !ud->canRepair)))
			return false;
	} break;
	}
	...
	if (cmdID == CMD_REPEAT &&
	    (npOrder || !ud->canRepeat || ((int)c.GetParam(0) % 2) != (int)c.GetParam(0) /* only 0 or 1 allowed */))
		return false;

	if (cmdID == CMD_TRAJECTORY && (npOrder || ud->highTrajectoryType < 2))
		return false;

	if (cmdID == CMD_ONOFF && (npOrder || !ud->onoffable || owner->beingBuilt ||
	                              ((int)c.GetParam(0) % 2) != (int)c.GetParam(0) /* only 0 or 1 allowed */))
		return false;

	if (cmdID == CMD_CLOAK &&
	    (npOrder || !ud->canCloak || ((int)c.GetParam(0) % 2) != (int)c.GetParam(0) /* only 0 or 1 allowed */))
		return false;
		...
		if ((cmdID == cmd2ID) || (cmdID < 0 && cmd2ID < 0) || attackAndFight) {
			if (c.GetNumParams() == 1) {
				...
				if (cmdID < 0) {
					...
					if (math::fabs(bc1.pos.x - bc2.pos.x) * 2 <=
					        std::max(bc1.GetXSize(), bc2.GetXSize()) * SQUARE_SIZE &&
					    math::fabs(bc1.pos.z - bc2.pos.z) * 2 <=
					        std::max(bc1.GetZSize(), bc2.GetZSize()) * SQUARE_SIZE) {
						return ci;
					}
				}
				else {
...
void CCommandAI::UpdateStockpileIcon()
{
	...
	for (unsigned int n = 0; n < possibleCommands.size(); n++) {
		...
		c.name = IntToString(stockpileWeapon->numStockpiled) + "/" +
...
bool CCommandAI::HasMoreMoveCommands(bool skipFirstCmd) const
{
	...
	const auto iter =
	    std::find_if(commandQue.begin() + int(skipFirstCmd && !commandQue.empty()), commandQue.end(), pred);
...
void CCommandAI::StopAttackingTargetIf(const std::function<bool(const CUnit*)>& pred)
{
	...
	const auto hasTarget = [&](const Command& c) {
		return (c.GetNumParams() == 1 && (c.GetID() == CMD_FIGHT || c.GetID() == CMD_ATTACK));
	};
