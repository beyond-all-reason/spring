...
static constexpr int CMDTYPE_ICON_UNIT_OR_MAP =
    13; // expect 1 parameters in return (unitid) or 3 parameters in return (mappos)
static constexpr int CMDTYPE_ICON_FRONT =
    14; // expect 3 or 6 parameters in return (middle of front and right side of front if a front was defined)
static constexpr int CMDTYPE_ICON_UNIT_OR_AREA =
    16; // expect 1 parameter in return (unitid) or 4 parameters in return (mappos+radius)
static constexpr int CMDTYPE_NEXT = 17; // used with CMD_INTERNAL
static constexpr int CMDTYPE_PREV = 18; // used with CMD_INTERNAL
static constexpr int CMDTYPE_ICON_UNIT_FEATURE_OR_AREA =
    19; // expect 1 parameter in return (unitid or featureid+unitHandler.MaxUnits() (id>unitHandler.MaxUnits()=feature))
...
public:
	...
	Command(const Command& c) { *this = c; }
	...
	bool IsObjectCommand(int& cpos) const
	{
		...
		case CMD_ATTACK:
		case CMD_FIGHT:
		case CMD_MANUALFIRE: cpos = 0; return (1 <= numParams && numParams < 3);
		case CMD_GUARD:
		case CMD_LOAD_ONTO: cpos = 0; return (numParams >= 1);
		case CMD_CAPTURE:
		case CMD_LOAD_UNITS:
		case CMD_RECLAIM:
		case CMD_REPAIR:
		case CMD_RESURRECT: cpos = 0; return (1 <= numParams && numParams < 4);
		case CMD_UNLOAD_UNIT: cpos = 3; return (numParams >= 4);
		case CMD_INSERT: {
			if (numParams < 3)
				return false;
	...
	bool IsMoveCommand() const
	{
		...
		case CMD_UNLOAD_UNITS: return true;
	...
	bool IsAreaCommand() const
	{
		...
		case CMD_RESURRECT:
			...
			return (numParams == 4);
		case CMD_UNLOAD_UNITS: return (numParams == 5);
		case CMD_AREA_ATTACK: return true;
