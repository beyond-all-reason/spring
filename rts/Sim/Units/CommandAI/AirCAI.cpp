...
void CAirCAI::GiveCommandReal(const Command& c, bool fromSynced)
{
	...
	if (c.GetID() == CMD_MOVE && c.GetNumParams() >= 3 &&
	    (c.GetParam(0) < 0.0f || c.GetParam(2) < 0.0f || c.GetParam(0) > (mapDims.mapx * SQUARE_SIZE) ||
...
void CAirCAI::SlowUpdate()
{
	...
	switch (c.GetID()) {
	case CMD_WAIT: {
		if ((myPlane->aircraftState == AAirMoveType::AIRCRAFT_FLYING) && myPlane->autoLand &&
...
bool CAirCAI::AirAutoGenerateTarget(AAirMoveType* myPlane)
{
	...
	if (ownerDef->IsFighterAirUnit()) {
		tgt = CGameHelper::GetClosestEnemyAircraft(
		    nullptr, owner->pos + owner->speed * 10.0f, searchRadius, owner->allyteam);
	}
	else {
		tgt =
		    CGameHelper::GetClosestValidTarget(owner->pos + owner->speed * 20.0f, searchRadius, owner->allyteam, this);
	}
...
void CAirCAI::ExecuteMove(Command& c)
{
	...
	const CStrafeAirMoveType* airMT =
	    (!owner->UsingScriptMoveType()) ? static_cast<const CStrafeAirMoveType*>(myPlane) : nullptr;
	...
	if ((owner->pos - cmdPos).SqLength2D() < sqRadius ||
	    (owner->pos + owner->speed * 8.0f - cmdPos).SqLength2D() < Square(127.0f))
		StopMoveAndFinishCommand();
}
...
void CAirCAI::ExecuteFight(Command& c)
{
	...
	if (c.GetNumParams() < 3) {
		LOG_L(L_ERROR, "[AirCAI::%s][f=%d][id=%d][#c.params=%d min=3]", __func__, gs->frameNum, owner->id,
		    c.GetNumParams());
		return;
	}
	...
	if ((owner->fireState >= FIRESTATE_FIREATWILL) && (owner->moveState != MOVESTATE_HOLDPOS) &&
	    (owner->maxRange > 0.0f) && ownerDef->canAttack) {
		CUnit* enemy = nullptr;
		...
		if (IsValidTarget(enemy, nullptr) &&
		    (owner->moveState != MOVESTATE_MANEUVER || LinePointDist(commandPos1, commandPos2, enemy->pos) < 1000)) {
...

		{
			...
			if ((enemy = CGameHelper::GetClosestValidTarget(pos, 500.0f * owner->moveState, owner->allyteam, this)) !=
			    nullptr) {
...
void CAirCAI::ExecuteGuard(Command& c)
{
	...
	const bool pushAttackCommand = (owner->maxRange > 0.0f) && owner->unitDef->canAttack &&
