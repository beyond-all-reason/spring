...
void CMissileLauncher::FireImpl(const bool scriptCall)
{
	...
	params.ttl =
	    (ttl == 0) ? math::ceil(std::max(targetDist, range) / projectileSpeed + 25 * weaponDef->selfExplode) : ttl;
...
bool CMissileLauncher::HaveFreeLineOfFire(const float3 srcPos, const float3 tgtPos, const SWeaponTarget& trg) const
{
	...
	if (globalRendering->drawDebugTraceRay) {
		for (int i = 1; i < 9; i++) {
			geometricObjects->SetColor(
			    geometricObjects->AddLine(srcPos + targetVec * mdist[i - 1] + UpVector * mheight[i - 1],
			        srcPos + targetVec * mdist[i] + UpVector * mheight[i], 3, 0, GAME_SPEED),
			    1.0f, 0.0f, 0.0f, 1.0f);
		}
	}
