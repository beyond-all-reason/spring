...
public:
	...
	{
		return (std::find(incomingProjectileIDs.begin(), incomingProjectileIDs.end(), projID) !=
...
public:
	/// test if the weapon is able to attack an enemy/mapspot just by its properties (no range check, no FreeLineOfFire
	/// check, ...)
...
public:
	...
	static float GetLiveRange2D(const CWeapon* w, const WeaponDef* wd, float modHeightDiff, float modProjGravity)
	{
...
protected:
	...
	virtual float GetPredictedImpactTime(
	    float3 p) const; //< how long time we predict it take for a projectile to reach target
...
public:
	...
	float maxForwardAngleDif; // for onlyForward/!turret weapons, max. angle between owner->frontdir and (targetPos -
	                          // owner->pos) (derived from UnitDefWeapon::maxAngleDif)
	float maxMainDirAngleDif; // for !onlyForward/turret weapons, max. angle from <mainDir> the weapon can aim (derived
	                          // from WeaponDef::tolerance)

	float heightBoostFactor; // controls cannon range height boost. default: -1 -- automatically calculate a more or
	                         // less sane value
