...
bool CCannon::HaveFreeLineOfFire(const float3 srcPos, const float3 tgtPos, const SWeaponTarget& trg) const
{
	...
	// linear parabolic coefficient is the ratio of vertical velocity to horizontal velocity, with slight adjustment due
	// to acceleration being applied in discrete steps. quadratic parabolic coefficient is the ratio of gravity to
	// (horizontal velocity)^2
	const float projectileSpeedHorizontal =
	    std::max(0.001f, projectileSpeed * launchDir.Length2D()); // ensure projectileSpeedHorizontal cannot be zero
	const float projectileSpeedVertical = projectileSpeed * launchDir.y;
	const float linCoeff = (projectileSpeedVertical + (gravity * 0.5f)) /
	                       projectileSpeedHorizontal; //(gravity * 0.5f) is factor due to discrete acceleration steps
	...
	const float groundDist =
	    ((avoidFlags & Collision::NOGROUND) == 0) ?
	...
	return (!TraceRay::TestTrajectoryCone(
	    srcPos, targetVec, xzTargetDist, linCoeff, qdrCoeff, angleSpread, owner->allyteam, avoidFlags, owner));
}
...
void CCannon::FireImpl(const bool scriptCall)
{
	...
	float3 launchDir = (targetVec.SqLength() > 4.0f) ?
	                       GetWantedDir(targetVec) :
	                       targetVec; // prevent vertical aim when emit-sfx firing the weapon
	...
	const int predict = math::ceil((sqSpeed2D == 0.0f) ? (-2.0f * projectileSpeed * launchDir.y / gravity) :
...
float3 CCannon::GetWantedDir(const float3& targetVec)
{
	...
	if (math::fabs(tgtDif.x) < (SQUARE_SIZE / 4.0f) && math::fabs(tgtDif.y) < (SQUARE_SIZE / 4.0f) &&
...
float3 CCannon::CalcWantedDir(const float3& targetVec) const
{
	...
	if (Dsq == 0.0f) {
		...
		if (Dsq < 1e12f && math::fabs(dy) < 1e6f) {
			...
			if (root1 >= 0.0f) {
				const float root2 =
				    2.0f * DFsq * Dsq * (vsq + g * dy + (highTrajectory ? -1.0f : 1.0f) * math::sqrt(root1));
				if (root2 >= 0.0f) {
