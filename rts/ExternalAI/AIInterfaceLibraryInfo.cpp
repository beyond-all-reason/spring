...
CAIInterfaceLibraryInfo::CAIInterfaceLibraryInfo(const CAIInterfaceLibraryInfo& interfaceInfo) = default;

CAIInterfaceLibraryInfo::CAIInterfaceLibraryInfo(const std::string& interfaceInfoFile)
{
...
size_t CAIInterfaceLibraryInfo::size() const { return keys.size(); }
...
const std::string& CAIInterfaceLibraryInfo::GetDataDir() const { return GetInfo(AI_INTERFACE_PROPERTY_DATA_DIR); }
...
const std::string& CAIInterfaceLibraryInfo::GetShortName() const { return GetInfo(AI_INTERFACE_PROPERTY_SHORT_NAME); }

const std::string& CAIInterfaceLibraryInfo::GetVersion() const { return GetInfo(AI_INTERFACE_PROPERTY_VERSION); }

const std::string& CAIInterfaceLibraryInfo::GetName() const { return GetInfo(AI_INTERFACE_PROPERTY_NAME); }
...
const std::string& CAIInterfaceLibraryInfo::GetURL() const { return GetInfo(AI_INTERFACE_PROPERTY_URL); }
...
const std::string& CAIInterfaceLibraryInfo::GetInfo(const std::string& key) const
{
	...
	if (!found) {
		LOG_L(L_WARNING, "AI Interface property '%s' could not be found.", key.c_str());
		return DEFAULT_VALUE;
	}
	else {
...
void CAIInterfaceLibraryInfo::SetName(const std::string& name) { SetInfo(AI_INTERFACE_PROPERTY_NAME, name); }
...
void CAIInterfaceLibraryInfo::SetURL(const std::string& url) { SetInfo(AI_INTERFACE_PROPERTY_URL, url); }

bool CAIInterfaceLibraryInfo::SetInfo(const std::string& key, const std::string& value, const std::string& description)
{
	...
	if (keyLower == snKey || keyLower == vKey) {
		if (value.find_first_of(BAD_CHARS) != std::string::npos) {
			LOG_L(L_WARNING,
			    "AI interface property (%s or %s)\n"
			    "contains illegal characters (%s).",
			    AI_INTERFACE_PROPERTY_SHORT_NAME, AI_INTERFACE_PROPERTY_VERSION, BAD_CHARS);
			return false;
		}
	}
