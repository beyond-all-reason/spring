...
HAPFSPathDrawer::HAPFSPathDrawer()
    : IPathDrawer()
{
...
void HAPFSPathDrawer::UpdateExtraTexture(int extraTex, int starty, int endy, int offset, unsigned char* texMem) const
{
		...
		if (useCurrentBuildOrder) {
			for (int ty = starty; ty < endy; ++ty) {
				for (int tx = 0; tx < mapDims.hmapx; ++tx) {
					const float3 pos(
					    tx * (SQUARE_SIZE << 1) + SQUARE_SIZE, 0.0f, ty * (SQUARE_SIZE << 1) + SQUARE_SIZE);
					...
					if (!losHandler->InLos(pos, gu->myAllyTeam)) {
						...
						const UnitDef* ud =
						    unitDefHandler->GetUnitDefByID(-guihandler->commands[guihandler->inCommand].id);
			...
			if (md != nullptr) {
				...
				for (int ty = starty; ty < endy; ++ty) {
					for (int tx = 0; tx < mapDims.hmapx; ++tx) {
						...
						if (los || losSqr) {
							if (CMoveMath::IsBlocked(*md, sqx, sqy, nullptr, thread) & CMoveMath::BLOCK_STRUCTURE) {
								scale -= 0.25f;
							}
							if (CMoveMath::IsBlocked(*md, sqx + 1, sqy, nullptr, thread) & CMoveMath::BLOCK_STRUCTURE) {
								scale -= 0.25f;
							}
							if (CMoveMath::IsBlocked(*md, sqx, sqy + 1, nullptr, thread) & CMoveMath::BLOCK_STRUCTURE) {
								scale -= 0.25f;
							}
							if (CMoveMath::IsBlocked(*md, sqx + 1, sqy + 1, nullptr, thread) &
	...
	case CLegacyInfoTextureHandler::drawPathHeat: {
		...
		for (int ty = starty; ty < endy; ++ty) {
			for (int tx = 0; tx < mapDims.hmapx; ++tx) {
				...
				texMem[texIdx + CLegacyInfoTextureHandler::COLOR_R] =
				    std::clamp(8 * phm->GetHeatValue(tx << 1, ty << 1), 32, 255);
	...
	case CLegacyInfoTextureHandler::drawPathCost: {
		...
		const unsigned int medResBlockSize = pm->GetMedResPE()->GetBlockSize(),
		                   medResBlocksX = pm->GetMedResPE()->GetNumBlocks().x;
		const unsigned int lowResBlockSize = pm->GetLowResPE()->GetBlockSize(),
		                   lowResBlocksX = pm->GetLowResPE()->GetNumBlocks().x;
		...
		for (int ty = starty; ty < endy; ++ty) {
			for (int tx = 0; tx < mapDims.hmapx; ++tx) {
				...
				if (std::isinf(gCost[0])) {
					gCost[0] = gCostMax[0];
				}
				if (std::isinf(gCost[1])) {
					gCost[1] = gCostMax[1];
				}
				if (std::isinf(gCost[2])) {
					gCost[2] = gCostMax[2];
				}
		...
		for (int z = 0; z < peNumBlocks.y; z++) {
			for (int x = 0; x < peNumBlocks.x; x++) {
				...
				for (int dir = 0; dir < PATH_DIRECTION_VERTICES; dir++) {
					...
					if (obz < 0)
						continue;
					if (obx >= peNumBlocks.x)
						continue;
					if (obz >= peNumBlocks.y)
						continue;

					const int obBlockNr = obz * peNumBlocks.x + obx;
					const int vertexNr = vertexBaseNr + blockNr * PATH_DIRECTION_VERTICES +
					                     GetBlockVertexOffset(*md, dir, peNumBlocks.x);
