...
void RenderBuffer::InitStatic()
{
	...
	RenderBuffer::typedRenderBuffers = {std::make_unique<TypedRenderBuffer<VA_TYPE_0>>(1 << 16, 1 << 17),
...
void RenderBuffer::KillStatic()
{
	...
	for (const auto& trb: typedRenderBuffers) {
		...
		LOG_L(L_INFO,
		    "[RenderBuffer::%s] Type = %s max size/init. capacity/curr. capacity: VBO = {%u, %u, %u}, EBO = {%u, %u, "
		    "%u}",
		    __func__, trb->GetBufferName(), static_cast<uint32_t>(S[0]), static_cast<uint32_t>(C0[0]),
		    static_cast<uint32_t>(Cx[0]), static_cast<uint32_t>(S[1]), static_cast<uint32_t>(C0[1]),
