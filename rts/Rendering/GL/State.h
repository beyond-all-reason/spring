...
/*
    ...
    It represents GL state, that was last applied by GL::SubState objects, or any other object that performs state
   change via this interface.
    ...
    Setting back to Unknown after the last SubState object enforces fetch from GL every time the first SubState object
   is created, but makes it possible to insert SubState object in any part of the code without serious restrictions to
   the whole prior code.

    User must make sure that ordinary GL commands that are not tracked do not collide with SubState object's
...
public:
	...
	inline StateAttribute()
	    : value(Unknown, GLParamsType()) {};
...
template<> class StateAttribute<&glPolygonMode, GL_POLYGON_MODE> {
private:
	using StatusType = bool;
	static constexpr StatusType Unknown = false;
	static constexpr StatusType WasUnknown = false;
...
public:
	...
	template<class Type,
	    std::enable_if_t<std::is_convertible_v<std::remove_reference_t<Type>, AttributeValueType>, bool> = true>
...
extern std::tuple<ATTRIBUTE(PolygonMode),
    ...
    ATTRIBUTE(ClipDistance)<1>, ATTRIBUTE(ClipDistance)<2>, ATTRIBUTE(ClipDistance)<3>, ATTRIBUTE(ClipDistance)<4>,
    ATTRIBUTE(ClipDistance)<5>, ATTRIBUTE(ClipDistance)<6>, ATTRIBUTE(ClipDistance)<7>, ATTRIBUTE(ScissorTest),
    ATTRIBUTE(Scissor), ATTRIBUTE(DepthTest), ATTRIBUTE(DepthFunc), ATTRIBUTE(DepthRangef), ATTRIBUTE(DepthClamp),
    ATTRIBUTE(DepthMask), ATTRIBUTE(StencilTest), ATTRIBUTE(StencilFunc), ATTRIBUTE(StencilMask), ATTRIBUTE(StencilOp),
    ATTRIBUTE(ClearStencil), ATTRIBUTE(AlphaTest), ATTRIBUTE(AlphaFunc), ATTRIBUTE(Blending), ATTRIBUTE(BlendFunc),
    ATTRIBUTE(BlendEquation), ATTRIBUTE(BlendEquationSeparate), ATTRIBUTE(BlendColor), ATTRIBUTE(ColorMask),
    ATTRIBUTE(ClearColor), ATTRIBUTE(PrimitiveRestart), ATTRIBUTE(PrimitiveRestartIndex), ATTRIBUTE(Multisampling),
    ATTRIBUTE(SampleShading), ATTRIBUTE(MinSampleShading), ATTRIBUTE(AlphaToCoverage), ATTRIBUTE(AlphaToOne),
    ATTRIBUTE(CubemapSeamless), ATTRIBUTE(PointSize), ATTRIBUTE(FrameBufferSRBG) > Attributes;
}; // namespace State
