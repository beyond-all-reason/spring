...
public:
	...
	template<typename T> static TypedRenderBuffer<T>& GetTypedRenderBuffer();
...
public:
	static Shader::IProgramObject& GetShader()
	{
		...
		vertSrc = fmt::sprintf(vertSrc, vsHeader, vsInputs, varyingsVS, vsAssignment, vsPosVertex);

		const std::string fragOutput = GetFragOutput();
		fragSrc = fmt::sprintf(fragSrc, fsHeader, varyingsFS, fragOutput);
...
private:
	...
	static void GetShaderHeaders(std::string& vsHeader, std::string& fsHeader)
	{
		if (globalRendering->supportExplicitAttribLoc) {
			vsHeader = fmt::format(
			    "{}{}{}", "#version 150 compatibility", nl, "#extension GL_ARB_explicit_attrib_location : require");
		}
		else {
	...
	static void GetAttributesStrings(std::string& vsInputs,
	    std::string& varyings,
	    std::string& vsAssignment,
	    std::string& vsPosVertex)
	{
		...
		for (const AttributeDef& ad: T::attributeDefs) {
			if (globalRendering->supportExplicitAttribLoc)
				vsInputsVec.emplace_back(fmt::format(vsInputsFmtYLoc, fmt::arg("indx", ad.index),
				    fmt::arg("type", TypeToString(ad)), fmt::arg("name", ad.name)));
			else
				vsInputsVec.emplace_back(
				    fmt::format(vsInputsFmtNLoc, fmt::arg("type", TypeToString(ad)), fmt::arg("name", ad.name)));

			if (ad.index == 0)
				continue;

			varyingsVec.emplace_back(
			    fmt::format(varyingsFmt, fmt::arg("type", TypeToString(ad)), fmt::arg("name", ad.name)));

			vsAssignmentVec.emplace_back(fmt::format(vsAssignmentFmt, fmt::arg("name", ad.name)));
		}
...
template<> inline const std::string RenderBufferShader<VA_TYPE_0>::GetFragOutput() { return "\toutColor = vec4(1.0);"; }

template<> inline const std::string RenderBufferShader<VA_TYPE_C>::GetFragOutput() { return "\toutColor = vcolor;"; }

template<> inline const std::string RenderBufferShader<VA_TYPE_N>::GetFragOutput() { return "\toutColor = vec4(1.0);"; }

template<> inline const std::string RenderBufferShader<VA_TYPE_T>::GetFragOutput()
{
	return "\toutColor = texture(tex, vuv);";
}

template<> inline const std::string RenderBufferShader<VA_TYPE_T4>::GetFragOutput()
{
	return "\toutColor = texture(tex, vuv.xy);";
}

template<> inline const std::string RenderBufferShader<VA_TYPE_TN>::GetFragOutput()
{
	return "\toutColor = texture(tex, vuv);";
}

template<> inline const std::string RenderBufferShader<VA_TYPE_TC>::GetFragOutput()
{
	return "\toutColor = vcolor * texture(tex, vuv);";
}

template<> inline const std::string RenderBufferShader<VA_TYPE_PROJ>::GetFragOutput()
{
	assert(false); // change tex type to sampler2darray
	return "\toutColor = vcolor * texture(tex, vuvw);";
}

template<> inline const std::string RenderBufferShader<VA_TYPE_TNT>::GetFragOutput()
{
	return "\toutColor = texture(tex, vuv);";
}

template<> inline const std::string RenderBufferShader<VA_TYPE_2D0>::GetFragOutput()
{
	return "\toutColor = vec4(1.0);";
}

template<> inline const std::string RenderBufferShader<VA_TYPE_2DC>::GetFragOutput() { return "\toutColor = vcolor;"; }

template<> inline const std::string RenderBufferShader<VA_TYPE_2DT>::GetFragOutput()
{
	return "\toutColor = texture(tex, vuv);";
}

template<> inline const std::string RenderBufferShader<VA_TYPE_2DTC>::GetFragOutput()
{
	return "\toutColor = vcolor * texture(tex, vuv);";
}

template<typename T> class TypedRenderBuffer : public RenderBuffer {
public:
	using VertType = T;
	...
	TypedRenderBuffer(size_t vertCount0_,
	    size_t elemCount0_,
	    IStreamBufferConcept::Types bufferType_ = bufferTypeDefault,
	    bool optimizeForStreaming_ = true)
	...
	const char* GetBufferName() const override { return vboTypeName; }

	std::array<size_t, 2> GetBuffersCapacity() const override { return std::array{verts.capacity(), indcs.capacity()}; }
	...
	template<typename Iterator> void AddVertices(Iterator begin, Iterator end)
	{
		if (readOnly)
			return;
	...
	void AddVertices(std::initializer_list<VertType>&& vertices) { AddVertices(vertices.begin(), vertices.end()); }

	void AddVertices(const std::vector<VertType>& vertices) { AddVertices(vertices.begin(), vertices.end()); }
	...
	template<typename Iterator> void AddIndices(Iterator begin, Iterator end, int32_t vertBias = 0)
	{
		if (readOnly)
			return;
	...
	template<std::size_t N> void AddQuadTriangles(const VertType (&vs)[N])
	{
		static_assert(N == 4);
	...
	{
		AddQuadTrianglesImpl(tl, tr, br, bl);
	}

	void AddQuadTriangles(const VertType& tl, const VertType& tr, const VertType& br, const VertType& bl)
	{
		AddQuadTrianglesImpl(std::move(tl), std::move(tr), std::move(br), std::move(bl));
	}
	...
	template<std::size_t N> void AddQuadLines(const VertType (&vs)[N])
	{
		static_assert(N == 4);
	...
	void AddQuadLines(const VertType& tl, const VertType& tr, const VertType& br, const VertType& bl)
	{
		AddQuadLinesImpl(std::move(tl), std::move(tr), std::move(br), std::move(bl));
	}

	// render with DrawElements(GL_TRIANGLES)
	template<std::size_t N> void MakeQuadsTriangles(const VertType (&vs)[N], int xDiv, int yDiv)
	{
		static_assert(N == 4);
	...
	{
		MakeQuadsTrianglesImpl(tl, tr, br, bl, xDiv, yDiv);
	}

	void MakeQuadsTriangles(const VertType& tl,
	    const VertType& tr,
	    const VertType& br,
	    const VertType& bl,
	    int xDiv,
	    int yDiv)
	{
		MakeQuadsTrianglesImpl(std::move(tl), std::move(tr), std::move(br), std::move(bl), xDiv, yDiv);
	}

	// render with DrawArrays(GL_LINE_LOOP)
	template<std::size_t N> void MakeQuadsLines(const VertType (&vs)[N], int xDiv, int yDiv)
	{
		static_assert(N == 4);
		MakeQuadsLinesImpl(vs[0], vs[1], vs[2], vs[3], xDiv, yDiv);
	}

	void MakeQuadsLines(VertType&& tl, VertType&& tr, VertType&& br, VertType&& bl, int xDiv, int yDiv)
	{
		MakeQuadsLinesImpl(tl, tr, br, bl, xDiv, yDiv);
	}

	void
	MakeQuadsLines(const VertType& tl, const VertType& tr, const VertType& br, const VertType& bl, int xDiv, int yDiv)
	{
		MakeQuadsLinesImpl(std::move(tl), std::move(tr), std::move(br), std::move(bl), xDiv, yDiv);
	}
	...
	bool ShouldSubmit() const { return ShouldSubmit(false) || ShouldSubmit(true); }
	...
	{
		assert((indcs.size() - eboUploadIndex == 0) && (verts.size() - vboUploadIndex == 0) &&
		       (indcs.size() - eboStartIndex == 0) && (verts.size() - vboStartIndex == 0));
	}

	static Shader::IProgramObject& GetShader() { return shader.GetShader(); }

private:
	template<typename TT = VertType,
...
template<typename T> inline void TypedRenderBuffer<T>::UploadVBO()
{
...
template<typename T> inline void TypedRenderBuffer<T>::UploadEBO()
{
...
template<typename T> inline void TypedRenderBuffer<T>::AssertBoundShader() const
{
...
template<typename T> inline void TypedRenderBuffer<T>::DrawArrays(uint32_t mode, bool rewind)
{
...
template<typename T> inline void TypedRenderBuffer<T>::DrawElements(uint32_t mode, bool rewind)
{
	AssertBoundShader();
...
#endif
	vao.Bind();
	glDrawElements(mode, static_cast<GLsizei>(indcsCount), GL_UNSIGNED_INT,
	    BUFFER_OFFSET(uint32_t, ebo->BufferElemOffset() + eboStartIndex));
	vao.Unbind();
#undef BUFFER_OFFSET
...
template<typename T> inline void TypedRenderBuffer<T>::DropCurrent()
{
	if (readOnly)
		return;
...
template<typename T> inline TypedRenderBuffer<T> TypedRenderBuffer<T>::CopyCurrent(bool readOnly) const
{
...
template<typename T> inline void TypedRenderBuffer<T>::CondInit()
{
	if (vao.GetIdRaw() > 0)
		return;
...
template<typename T> inline void TypedRenderBuffer<T>::InitVAO() const
{
	assert(vbo);
...
#define GET_TYPED_RENDER_BUFFER(T, idx)                                             \
	template<> inline TypedRenderBuffer<T>& RenderBuffer::GetTypedRenderBuffer<T>() \
	{                                                                               \
