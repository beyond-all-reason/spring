...
public:
	...
	/**
	 * @param target can be either GL_ARRAY_BUFFER, GL_ELEMENT_ARRAY_BUFFER, GL_PIXEL_PACK_BUFFER,
	 * GL_PIXEL_UNPACK_BUFFER or GL_UNIFORM_BUFFER_EXT
	...
	bool BindBufferRange(GLuint index, GLuint offset, GLsizeiptr size) const
	{
		return BindBufferRangeImpl(curBoundTarget, index, vboId, offset, size);
	}

	bool BindBufferRange(GLenum target, GLuint index, GLuint offset, GLsizeiptr size) const
	{
		return BindBufferRangeImpl(target, index, vboId, offset, size);
	};

	bool UnbindBufferRange(GLuint index) const { return BindBufferRangeImpl(curBoundTarget, index, 0u, 0u, bufSize); };

	bool UnbindBufferRange(GLuint index, GLuint offset, GLsizeiptr size) const
	{
		return BindBufferRangeImpl(curBoundTarget, index, 0u, offset, size);
	};

	bool UnbindBufferRange(GLenum target, GLuint index, GLuint offset, GLsizeiptr size) const
	{
		return BindBufferRangeImpl(target, index, 0u, offset, size);
	};

	/**
	 * @param usage can be either GL_STREAM_DRAW, GL_STREAM_READ, GL_STREAM_COPY, GL_STATIC_DRAW, GL_STATIC_READ,
	...
	template<typename TData> void New(const std::vector<TData>& data, GLenum newUsage = GL_STATIC_DRAW)
	{
		New(sizeof(TData) * data.size(), newUsage, data.data());
	};
	...
	GLubyte* MapBuffer(const std::vector<TData>& data, GLintptr elemOffset = 0, GLbitfield access = GL_WRITE_ONLY)
	{
		return MapBuffer(sizeof(TData) * elemOffset, sizeof(TData) * data.size(), access);
	};
	...
	template<typename TData> void SetBufferSubData(const std::vector<TData>& data, GLintptr elemOffset = 0)
	{
		SetBufferSubData(sizeof(TData) * elemOffset, sizeof(TData) * data.size(), data.data());
	}
	...
	GLuint GetIdRaw() const { return vboId; };

	GLenum GetCurrTarget() const { return curBoundTarget; }
...
private:
	struct BoundBufferRangeIndex {
		BoundBufferRangeIndex()
		    : target{0u}
		    , index{0u} {};
		BoundBufferRangeIndex(GLenum target, GLuint index)
		    : target{target}
		    , index{index} {};

		bool operator==(const BoundBufferRangeIndex& rhs) const { return target == rhs.target && index == rhs.index; };
	...
	struct BoundBufferRangeData {
		BoundBufferRangeData()
		    : offset{~0u}
		    , size{0} {};
		BoundBufferRangeData(GLuint offset, GLsizeiptr size)
		    : offset{offset}
		    , size{size} {};

		bool operator==(const BoundBufferRangeData& rhs) const { return offset == rhs.offset && size == rhs.size; };
