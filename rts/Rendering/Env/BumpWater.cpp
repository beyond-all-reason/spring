...
static void
GLSLDefineConst4f(string& str, const string& name, const float x, const float y, const float z, const float w)
{
...
static GLuint
LoadTexture(const string& filename, const float anisotropy = 0.0f, int* sizeX = nullptr, int* sizeY = nullptr)
{
...
void CBumpWater::InitResources(bool loadShader)
{
	...
	if (shoreWaves) {
...

		{
			...
			blurShader->AttachShaderObject(
			    shaderHandler->CreateShaderObject("GLSL/BumpWaterCoastBlurVS.glsl", "", GL_VERTEX_SHADER));
			blurShader->AttachShaderObject(
			    shaderHandler->CreateShaderObject("GLSL/BumpWaterCoastBlurFS.glsl", "", GL_FRAGMENT_SHADER));
	...
	string definitions;
	if (reflection > 0)
		definitions += "#define opt_reflection\n";
	if (refraction > 0)
		definitions += "#define opt_refraction\n";
	if (shoreWaves)
		definitions += "#define opt_shorewaves\n";
	if (depthCopy)
		definitions += "#define opt_depth\n";
	if (blurRefl)
		definitions += "#define opt_blurreflection\n";
	if (endlessOcean)
		definitions += "#define opt_endlessocean\n";

	GLSLDefineConstf3(
	    definitions, "MapMid", float3(mapDims.mapx * SQUARE_SIZE * 0.5f, 0.0f, mapDims.mapy * SQUARE_SIZE * 0.5f));
	GLSLDefineConstf2(
	    definitions, "ScreenInverse", 1.0f / globalRendering->viewSizeX, 1.0f / globalRendering->viewSizeY);
...

	{
		...
		const float scaleX =
		    (mapX > mapZ) ? (mapDims.mapy >> 6) / 16.0f * (float)mapX / mapZ : (mapDims.mapx >> 6) / 16.0f;
		const float scaleZ =
		    (mapX > mapZ) ? (mapDims.mapy >> 6) / 16.0f : (mapDims.mapx >> 6) / 16.0f * (float)mapZ / mapX;
	...
	// LOAD SHADERS
	{
		...
		waterShader->AttachShaderObject(
		    shaderHandler->CreateShaderObject("GLSL/BumpWaterVS.glsl", definitions, GL_VERTEX_SHADER));
		waterShader->AttachShaderObject(
		    shaderHandler->CreateShaderObject("GLSL/BumpWaterFS.glsl", definitions, GL_FRAGMENT_SHADER));
...
void CBumpWater::UploadCoastline(const bool forceFull)
{
	...
	const float* heightMap =
	    (!gs->PreSimFrame()) ? readMap->GetCornerHeightMapUnsynced() : readMap->GetCornerHeightMapSynced();
...
void CBumpWater::UpdateCoastmap(const bool initialize)
{
	...
	for (const CoastAtlasRect& r: coastmapAtlasRects) {
		rbt4.AddQuadTriangles(
		    {
		        {r.x1, r.y1, 0.0f},
                {r.tx1, r.ty1, 0.0f, 0.0f}
        },
		    {{r.x1, r.y2, 0.0f}, {r.tx1, r.ty2, 0.0f, 1.0f}}, {{r.x2, r.y2, 0.0f}, {r.tx2, r.ty2, 1.0f, 1.0f}},
		...
		for (int i = 0; i < 5; ++i) {
			...
			for (const CoastAtlasRect& r: coastmapAtlasRects) {
				...
				rbt4.AddQuadTriangles(
				    ...
				    {{r.tx1, r.ty2, 0.0f}, {r.x1, r.y2, 0.0f, 1.0f}}, {{r.tx2, r.ty2, 0.0f}, {r.x2, r.y2, 1.0f, 1.0f}},
...
void CBumpWater::UpdateDynWaves(const bool initialize)
{
	...
	for (unsigned char y = 0; y < tiles; ++y) {
		for (unsigned char x = 0; x < tiles; ++x) {
			...
			rb2tc.AddQuadTriangles({(x + 0) * tilesize, (y + 0) * tilesize, (tx + 0) * tilesize, (ty + 0) * tilesize},
...
void CBumpWater::Draw()
{
	...
	if (refraction == 1) {
		...
		glBindTexture(target, refractTexture);
		glCopyTexSubImage2D(target, 0, 0, 0, globalRendering->viewPosX, globalRendering->viewPosY,
	...
	glBindTexture(GL_TEXTURE_2D, caustTextures[causticTexNum]);
	...
	glBindTexture(GL_TEXTURE_2D, waveRandTexture);
