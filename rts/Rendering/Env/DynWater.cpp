...
void CDynWater::InitResources(bool loadShader)
{
	...
	for (int y = 0; y < 64; ++y) {
		for (int x = 0; x < 64; ++x) {
			...
			temp[(y * 64 + x) * 4 + 2] =
			    std::cos(x * math::TWOPI / 64.0f) + ((x < 32) ? (16 - x) : (x - 48)) / 16.0f * 0.3f;
			temp[(y * 64 + x) * 4 + 3] = 0;
		}
	}
	...
	if (!foam.LoadGrayscale(waterRendering->foamTexture))
		LOG_L(L_WARNING, "[%s] could not load grayscale foam-texture %s\n", __func__,
		    waterRendering->foamTexture.c_str());
...
void CDynWater::FreeResources()
{
	...
	const auto DeleteProgram = [](GLuint& proID) {
		if (proID > 0) {
			glSafeDeleteProgram(proID);
			proID = 0;
		}
	};
	...
	for (auto& rbt: rawBumpTexture) DeleteTexture(rbt);
...
void CDynWater::Draw()
{
	...
	glProgramEnvParameter4fARB(
	    GL_VERTEX_PROGRAM_ARB, 13, -(camPosBig.x - WH_SIZE) / WF_SIZE, -(camPosBig.z - WH_SIZE) / WF_SIZE, 0, 0);
	glProgramEnvParameter4fARB(
	    GL_VERTEX_PROGRAM_ARB, 14, 1.0f / (mapDims.mapxp1 * SQUARE_SIZE), 1.0f / (mapDims.mapyp1 * SQUARE_SIZE), 0, 0);
	// glProgramEnvParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 0, 1.0f/4096.0f, 1.0f/4096.0f, 0, 0);
	glProgramEnvParameter4fARB(
	    GL_FRAGMENT_PROGRAM_ARB, 1, camera->GetPos().x, camera->GetPos().y, camera->GetPos().z, 0);
	...
	glProgramEnvParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 10, sunLighting->groundDiffuseColor.x,
	    sunLighting->groundDiffuseColor.y, sunLighting->groundDiffuseColor.z, 0);
	glProgramEnvParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 11, sunLighting->groundAmbientColor.x,
	    sunLighting->groundAmbientColor.y, sunLighting->groundAmbientColor.z, 0);
...
void CDynWater::Update()
{
	...
	camPosBig.x = std::floor(std::max((float)WH_SIZE,
	                             std::min((float)mapDims.mapx * SQUARE_SIZE - WH_SIZE, (float)camera->GetPos().x)) /
	                         (W_SIZE * 16)) *
	              (W_SIZE * 16);
	camPosBig.z = std::floor(std::max((float)WH_SIZE,
	                             std::min((float)mapDims.mapy * SQUARE_SIZE - WH_SIZE, (float)camera->GetPos().z)) /
	                         (W_SIZE * 16)) *
	              (W_SIZE * 16);
...
void CDynWater::DrawWaves()
{
	...
	glProgramEnvParameter4fARB(GL_VERTEX_PROGRAM_ARB, 12, float(WF_SIZE) / (mapDims.mapxp1 * SQUARE_SIZE),
	    float(WF_SIZE) / (mapDims.mapyp1 * SQUARE_SIZE), 0, 0);
	glProgramEnvParameter4fARB(GL_VERTEX_PROGRAM_ARB, 13, (camPosBig.x - WH_SIZE) / (mapDims.pwr2mapx * SQUARE_SIZE),
	    (camPosBig.z - WH_SIZE) / (mapDims.pwr2mapy * SQUARE_SIZE), 0, 0);
	glProgramEnvParameter4fARB(GL_VERTEX_PROGRAM_ARB, 14, dx / WF_SIZE, dy / WF_SIZE, 0, 0);
	glProgramEnvParameter4fARB(
	    GL_VERTEX_PROGRAM_ARB, 15, (camPosBig.x - WH_SIZE) / WF_SIZE * 4, (camPosBig.x - WH_SIZE) / WF_SIZE * 4, 0, 0);
	...
	// update normals pass
	glBegin(GL_QUADS);
	glTexCoord2f(start, start);
	glVertexf3(ZeroVector);
	glTexCoord2f(start, end);
	glVertexf3(UpVector);
	glTexCoord2f(end, end);
	glVertexf3(XYVector);
	glTexCoord2f(end, start);
	glVertexf3(RgtVector);
	glEnd();
...
void CDynWater::DrawHeightTex()
{
	...
	glBegin(GL_QUADS);
	glTexCoord2f(startx, startz);
	glVertex3f(startv, startv, 0);
	glTexCoord2f(startx, endz);
	glVertex3f(startv, endv, 0);
	glTexCoord2f(endx, endz);
	glVertex3f(endv, endv, 0);
	glTexCoord2f(endx, startz);
	glVertex3f(endv, startv, 0);
	glEnd();
...
static inline void DrawVertexAQ(int x, int y) { va->AddVertexQ0(x * WSQUARE_SIZE, 0, y * WSQUARE_SIZE); }
...
void CDynWater::DrawWaterSurface()
{
	...
	camPosBig2.x =
	    std::floor(std::max((float)WH_SIZE, std::min((float)mapDims.mapx * SQUARE_SIZE - WH_SIZE, cam->GetPos().x)) /
	               (W_SIZE * 16)) *
	    (W_SIZE * 16);
	camPosBig2.z =
	    std::floor(std::max((float)WH_SIZE, std::min((float)mapDims.mapy * SQUARE_SIZE - WH_SIZE, cam->GetPos().z)) /
	               (W_SIZE * 16)) *
	    (W_SIZE * 16);
	...
	for (int lod = 1; lod < (2 << 5); lod *= 2) {
		...
		for (int y = ystart; y < yend; y += lod) {
			...
			for (int x = xs; x < xe; x += lod) {
				...
				if ((lod == 1) || (x > (cx + vrhlod)) || (x < (cx - vrhlod)) || (y > (cy + vrhlod)) ||
				    (y < (cy - vrhlod))) { // normal terrain
					if (!inStrip) {
						DrawVertexAQ(x, y);
						DrawVertexAQ(x, ylod);
						inStrip = true;
...
void CDynWater::DrawDetailNormalTex()
{
	...
	glProgramEnvParameter4fARB(
	    GL_VERTEX_PROGRAM_ARB, 10, 5, 0, 0, 1.0f / 120); // controls the position and speed of the waves
	...
	glProgramEnvParameter4fARB(
	    GL_FRAGMENT_PROGRAM_ARB, 0, 0.2f * swh, 0.0f * swh, 0.7f * lwh, 0); // controls the height of the waves
...
void CDynWater::AddShipWakes()
{
	...
	glProgramEnvParameter4fARB(
	    GL_VERTEX_PROGRAM_ARB, 11, -(oldCamPosBig.x - WH_SIZE) / WF_SIZE, -(oldCamPosBig.z - WH_SIZE) / WF_SIZE, 0, 0);
...
void CDynWater::AddExplosions()
{
	...
	for (std::vector<Explosion>::iterator ei = explosions.begin(); ei != explosions.end(); ++ei) {
		...
		if ((std::fabs(pos.x - camPosBig.x) > (WH_SIZE - 50)) || (std::fabs(pos.z - camPosBig.z) > (WH_SIZE - 50))) {
			continue;
		}
		float inv = 1.01f;
		if (pos.y < 0) {
