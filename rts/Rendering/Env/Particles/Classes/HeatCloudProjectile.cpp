...
void CHeatCloudProjectile::Serialize(creg::ISerializer* s)
{
	...
	if (!s->IsWriting())
		texture = name.empty() ? projectileDrawer->heatcloudtex : projectileDrawer->textureAtlas->GetTexturePtr(name);
}
...
void CHeatCloudProjectile::Draw()
{
	...
	std::array<float3, 4> bounds = {-ri * drawsize - up * drawsize, ri * drawsize - up * drawsize,
	    ri * drawsize + up * drawsize, -ri * drawsize + up * drawsize};

	if (math::fabs(rotVal) > 0.01f) {
		float3::rotate<false>(rotVal, camera->GetForward(), bounds);
	}
	AddEffectsQuad({drawPos + bounds[0], texture->xstart, texture->ystart, col},
...
int CHeatCloudProjectile::GetProjectilesCount() const { return 1; }
