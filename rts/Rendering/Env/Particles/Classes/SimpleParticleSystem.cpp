...
void CSimpleParticleSystem::Draw()
{
	...
	const auto DoParticleDraw = [this](const float3& xdir, const float3& ydir, const float3& zdir, const Particle* p) {
		...
		std::array<float3, 4> bounds = {-ydir * size - xdir * size, -ydir * size + xdir * size,
		    ydir * size + xdir * size, ydir * size - xdir * size};

		if (math::fabs(p->rotVal) > 0.01f) {
			float3::rotate<false>(p->rotVal, zdir, bounds);
		}
		AddEffectsQuad({pDrawPos + bounds[0], texture->xstart, texture->ystart, color},
...
void CSimpleParticleSystem::Init(const CUnit* owner, const float3& offset)
{
	...
	for (auto& p: particles) {
		...
		p.pos = offset;
		p.speed = ((up * emitMul.y) * fastmath::cos(ay) -
		              ((right * emitMul.x) * fastmath::cos(az) - (forward * emitMul.z) * fastmath::sin(az)) *
		                  fastmath::sin(ay)) *
		          (particleSpeed + (guRNG.NextFloat() * particleSpeedSpread));
...
int CSimpleParticleSystem::GetProjectilesCount() const { return numParticles; }
