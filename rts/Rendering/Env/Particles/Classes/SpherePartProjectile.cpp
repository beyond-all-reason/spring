    ...
    , ttl(ttl)
{
	...
	texx = projectileDrawer->sphereparttex->xstart +
	       (projectileDrawer->sphereparttex->xend - projectileDrawer->sphereparttex->xstart) * 0.5f;
	texy = projectileDrawer->sphereparttex->ystart +
	       (projectileDrawer->sphereparttex->yend - projectileDrawer->sphereparttex->ystart) * 0.5f;
}
...
void CSpherePartProjectile::Draw()
{
	...
	for (int y = 0; y < 4; ++y) {
		for (int x = 0; x < 4; ++x) {
			float alpha = baseAlpha * (1.0f - std::min(1.0f, float(age + globalRendering->timeOffset) / (float)ttl)) *
			...
			alpha = baseAlpha * (1.0f - std::min(1.0f, (float)(age + globalRendering->timeOffset) / (float)ttl)) *
			        (1 - std::fabs(y + 1 + ybase - 8.0f) / 8.0f * 1.0f);
			...
			AddEffectsQuad({centerPos + vectors[y * 5 + x] * interSize, texx, texy, col0},
...
int CSpherePartProjectile::GetProjectilesCount() const { return 4 * 4; }

void CSpherePartProjectile::CreateSphere(const CUnit* owner,
    int ttl,
    float alpha,
    float expansionSpeed,
    float3 pos,
...
int CSpherePartSpawner::GetProjectilesCount() const { return 0; }
