...
void CDirtProjectile::Serialize(creg::ISerializer* s)
{
	...
	if (!s->IsWriting())
		texture = name.empty() ? projectileDrawer->randdotstex : projectileDrawer->textureAtlas->GetTexturePtr(name);
}
...
void CDirtProjectile::Draw()
{
	...
	AddEffectsQuad({drawPos - camera->GetRight() * interSize - camera->GetUp() * interSize * partAbove, texx,
	                   texture->ystart, col},
