...
CExploSpikeProjectile::CExploSpikeProjectile(CUnit* owner,
    const float3& pos,
    const float3& spd,
    float length,
    float width,
...
#define let projectileDrawer->laserendtex
	AddEffectsQuad({drawPos - l - w, let->xstart, let->ystart, col}, {drawPos + l - w, let->xend, let->ystart, col},
	    {drawPos + l + w, let->xend, let->yend, col}, {drawPos - l + w, let->xstart, let->yend, col});
#undef let
}
