...
static const bool GrassSort(const CGrassDrawer::GrassStruct* a, const CGrassDrawer::GrassStruct* b)
{
	RECOIL_DETAILED_TRACY_ZONE;
	const float distA =
	    GetGrassBlockCamDist((a->posX + 0.5f) * grassBlockSize, (a->posZ + 0.5f) * grassBlockSize, true);
	const float distB =
	    GetGrassBlockCamDist((b->posX + 0.5f) * grassBlockSize, (b->posZ + 0.5f) * grassBlockSize, true);
	return (distA > distB);
}
    ...
    , updateVisibility(false)
{
	...
	// load grass density from map
	{
		...
		if (grassbm.width != mapDims.mapx / grassSquareSize || grassbm.height != mapDims.mapy / grassSquareSize) {
			char b[128];
			SNPRINTF(b, sizeof(b), "grass-map has wrong size (%dx%d, should be %dx%d)\n", grassbm.width, grassbm.height,
			    mapDims.mapx / grassSquareSize, mapDims.mapy / grassSquareSize);
...
#define sh shaderHandler
	...
	static const std::string shaderNames[GRASS_PROGRAM_LAST] = {
	    "grassNearAdvShader", "grassDistAdvShader", "grassShadGenShader"};
	...
	for (int i = 0; i < GRASS_PROGRAM_LAST; i++) {
		...
		grassShaders[i]->AttachShaderObject(
		    sh->CreateShaderObject("GLSL/GrassVertProg.glsl", shaderDefines[i], GL_VERTEX_SHADER));
		grassShaders[i]->AttachShaderObject(
		    sh->CreateShaderObject("GLSL/GrassFragProg.glsl", shaderDefines[i], GL_FRAGMENT_SHADER));
		grassShaders[i]->Link();

		grassShaders[i]->Enable();
		grassShaders[i]->SetUniform(
		    "mapSizePO2", 1.0f / (mapDims.pwr2mapx * SQUARE_SIZE), 1.0f / (mapDims.pwr2mapy * SQUARE_SIZE));
		grassShaders[i]->SetUniform(
		    "mapSize", 1.0f / (mapDims.mapx * SQUARE_SIZE), 1.0f / (mapDims.mapy * SQUARE_SIZE));
...
void CGrassDrawer::DrawBillboard(const int x, const int y, const float dist, VA_TYPE_TN* va_tn)
{
	...
	for (int a = 0; a < numTurfs; a++) {
		...
		const float3 pos(
		    p.x, CGround::GetHeightReal(p.x, p.y, false) - CGround::GetSlope(p.x, p.y, false) * 30.0f, p.y);
...
void CGrassDrawer::DrawFarBillboards(const std::vector<GrassStruct*>& inviewFarGrass)
{
	...
	if (updateBillboards) {
		...
		for_mt(0, inviewFarGrass.size(), [&](const int i) {
			...
			const float distSq =
			    GetGrassBlockCamDist((g.posX + 0.5f) * grassBlockSize, (g.posZ + 0.5f) * grassBlockSize, true);

			if (distSq == g.lastDist)
				return;

			const bool inAlphaRange1 =
			    (distSq < Square(maxDetailedDist + 128.0f * 1.5f)) || (distSq > Square(maxGrassDist - 128.0f));
			const bool inAlphaRange2 =
			    (g.lastDist < Square(maxDetailedDist + 128.0f * 1.5f)) || (g.lastDist > Square(maxGrassDist - 128.0f));
...
void CGrassDrawer::Update()
{
	...
	for (const GrassStruct& gs: grass) {
		if ((gs.lastSeen != lastVisibilityUpdate) && (gs.lastSeen < globalRendering->drawFrame - 50) &&
		    gs.lastFar != 0) {
...
void CGrassDrawer::CreateGrassDispList(int listNum)
{
	...
	for (int a = 0; a < strawPerTurf; ++a) {
		...
		const float maxAng =
		    mapInfo->grass.bladeAngle * std::max(grng.NextFloat(), 1.0f - smoothstep(0.0f, 1.0f, lngRnd));
		...
		for (float h = 0.0f; h < 1.0f; h += (1.0f / numSections)) {
			...
			va->AddVertexTN(
			    basePos + edgePosL, xtexCoord - (1.0f / 32) * h + (1.0f / 16), h, (n - sideVect * 0.04f).ANormalize());
		}
...
void CGrassDrawer::CreateFarTex()
{
	...
	glBlitFramebufferEXT(
	    0, 0, texSizeX * sizeMod, texSizeY * sizeMod, 0, 0, texSizeX, texSizeY, GL_COLOR_BUFFER_BIT, GL_LINEAR);
