 ...
 */
static inline unsigned char smoothStep(int x, int y, int a) { return (unsigned char)((x * (1.0f - a)) + (y * a)); }
...
CInMapDrawView::~CInMapDrawView() { glDeleteTextures(1, &texture); }
...
void InMapDraw_QuadDrawer::DrawPoint(const CInMapDrawModel::MapPoint* point) const
{
	...
	rbp->AddQuadTriangles({pos1 - dir1 * size, 0.25f, 0, color}, {pos1 + dir1 * size, 0.25f, 1, color},
	    {pos1 + dir1 * size + dir2 * size, 0.00f, 1, color}, {pos1 - dir1 * size + dir2 * size, 0.00f, 0, color});
	rbp->AddQuadTriangles({pos1 - dir1 * size, 0.75f, 0, color}, {pos1 + dir1 * size, 0.75f, 1, color},
	    {pos2 + dir1 * size, 0.75f, 1, color}, {pos2 - dir1 * size, 0.75f, 0, color});
	rbp->AddQuadTriangles({pos2 - dir1 * size, 0.25f, 0, color}, {pos2 + dir1 * size, 0.25f, 1, color},
	    {pos2 + dir1 * size - dir2 * size, 0.00f, 1, color}, {pos2 - dir1 * size - dir2 * size, 0.00f, 0, color});
...
void CInMapDrawView::Draw()
{
...

	{
		...
		if (globalRendering->amdHacks) {
			...
			glLineWidth(1.0f);
			rbl.DrawArrays(
			    GL_LINES, true); // width greater than 2 causes GUI flicker on ATI hardware as of driver version 9.3
		}
		else {
