...
public:
	...
	inline std::string
	Wrap(const std::string& text, float fontSize, float maxWidth, float maxHeight = MAX_HEIGHT_DEFAULT);
...
private:
	...
	struct word {
		...
		unsigned int pos; //! position in the original text (needed for remerging colorcodes after wrapping; in
		                  //! printable chars (linebreaks and space don't count))
	};
	...
	spring::u8string
	Wrap(const spring::u8string& text, float fontSize, float maxWidth, float maxHeight = MAX_HEIGHT_DEFAULT);
};
