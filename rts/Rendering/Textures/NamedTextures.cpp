...
static void GenInsertTex(const std::string& texName,
...
static bool Load(const std::string& texName, unsigned int texID, bool genInsert)
{
	...
	if (bitmap.compressed) {
		...
		if (globalRendering->amdHacks && nearest) {
			bitmap =
			    bitmap.CreateRescaled(std::bit_ceil<uint32_t>(bitmap.xsize), std::bit_ceil<uint32_t>(bitmap.ysize));
		}
	...
	case GL_TEXTURE_2D_ARRAY: {
		texInfo.texType = GL_TEXTURE_2D_ARRAY;
	} break;
	case GL_TEXTURE_3D: {
		texInfo.texType = GL_TEXTURE_3D;
	} break;
	case GL_TEXTURE_CUBE_MAP: {
		texInfo.texType = GL_TEXTURE_CUBE_MAP;
	} break;
	default: {
		texInfo.texType = GL_TEXTURE_2D;
	} break;
	}
#endif
