...
CONFIG(bool, AllowDrawModelPostDeferredEvents)
    .defaultValue(true)
    .description("Enable Draw{Units,Features}PostDeferred Lua callins.");
CONFIG(bool, AllowMultiSampledFrameBuffers)
    .defaultValue(false)
...
static constexpr DECL_ARRAY(LuaMatShader::Pass, shaderPasses, 2) = {
    LuaMatShader::LUASHADER_PASS_FWD, LuaMatShader::LUASHADER_PASS_DFR};
...
{
	SetupOpaqueUnitDrawState(modelType, deferredPass);
}

static void ResetOpaqueFeatureDrawState(unsigned int modelType, bool deferredPass)
{
	ResetOpaqueUnitDrawState(modelType, deferredPass);
}
...
{
	SetupAlphaUnitDrawState(modelType, deferredPass);
}

static void ResetAlphaFeatureDrawState(unsigned int modelType, bool deferredPass)
{
	ResetAlphaUnitDrawState(modelType, deferredPass);
}
...
{
	SetupShadowUnitDrawState(modelType, deferredPass);
}

static void ResetShadowFeatureDrawState(unsigned int modelType, bool deferredPass)
{
	ResetShadowUnitDrawState(modelType, deferredPass);
}
...
static const void SetObjectUniformsNop(const CSolidObject* o, const LuaMaterial* m, int objectType, bool deferredPass)
{
} // no-op

static const void SetObjectUniformsLua(const CSolidObject* o, const LuaMaterial* m, int objectType, bool deferredPass)
{
	m->ExecuteInstanceUniforms(o->id, objectType, deferredPass);
}

static const void SetObjectUniformsDef(const CSolidObject* o, const LuaMaterial* m, int objectType, bool deferredPass)
{
} // no-op
...
void LuaObjectDrawer::Update(bool init)
{
	...
	if ((drawDeferredEnabled = geomBuffer->Valid())) {
		...
		notifyEventFlags[LUAOBJ_UNIT] =
		    !unitDrawer->DrawForward() || configHandler->GetBool("AllowDrawModelPostDeferredEvents");
		bufferClearFlags[LUAOBJ_UNIT] = unitDrawer->DrawDeferred();
		notifyEventFlags[LUAOBJ_FEATURE] =
		    !featureDrawer->DrawForward() || configHandler->GetBool("AllowDrawModelPostDeferredEvents");
...
void LuaObjectDrawer::DrawMaterialBin(const LuaMatBin* currBin,
...
void LuaObjectDrawer::DrawBinObject(const CSolidObject* obj,
    LuaObjType objType,
