...
bool UniformConstants::Supported()
{
	static bool supported =
	    VBO::IsSupported(GL_UNIFORM_BUFFER) && GLAD_GL_ARB_shading_language_420pack; // UBO && UBO layout(binding=x)
	return supported;
}
...
#ifndef HEADLESS
		LOG_L(L_ERROR,
		    "[UniformConstants::%s] Important OpenGL extensions are not supported by the system\n  "
		    "GLAD_GL_ARB_uniform_buffer_object = %d\n  GLAD_GL_ARB_shading_language_420pack = %d",
		    __func__, GLAD_GL_ARB_uniform_buffer_object, GLAD_GL_ARB_shading_language_420pack);
#endif
		return;
	}
...
void UniformConstants::UpdateMatricesImpl(UniformMatricesBuffer* updateBuffer)
{
	...
	updateBuffer->cameraBillboardView =
	    updateBuffer->cameraView *
	    camPlayer->GetBillBoardMatrix(); // GetBillBoardMatrix() is supposed to be multiplied by the view Matrix
...
void UniformConstants::UpdateParamsImpl(UniformParamsBuffer* updateBuffer)
{
	...
	updateBuffer->renderCaps = globalRendering->supportClipSpaceControl << 0;

	updateBuffer->timeInfo = float4{(float)gs->frameNum, spring_tomsecs(globalRendering->grTime) * 0.001f,
	    (gs->GetLuaSimFrame() + globalRendering->timeOffset) / GAME_SPEED,
	    globalRendering
	        ->timeOffset}; // gameFrame, drawSeconds, interpolated(unsynced)GameSeconds(synced), frameTimeOffset
	updateBuffer->viewGeometry = float4{(float)globalRendering->viewSizeX, (float)globalRendering->viewSizeY,
	    (float)globalRendering->viewPosX, (float)globalRendering->viewPosY}; // vsx, vsy, vpx, vpy
	updateBuffer->mapSize =
	    float4{(float)mapDims.mapx, (float)mapDims.mapy, (float)mapDims.pwr2mapx, (float)mapDims.pwr2mapy} *
	    (float)SQUARE_SIZE; // xz, xzPO2
	updateBuffer->mapHeight = float4{readMap->GetCurrMinHeight(), readMap->GetCurrMaxHeight(),
	    readMap->GetInitMinHeight(), readMap->GetInitMaxHeight()};

	const auto& sky = ISky::GetSky();
	float4 fogColor = (sky != nullptr) ? float4{sky->fogColor.x, sky->fogColor.y, sky->fogColor.z, 1.0f} :
	                                     float4{0.7f, 0.7f, 0.8f, 1.0f};
	updateBuffer->fogColor = fogColor;

	const auto camPlayer = CCameraHandler::GetCamera(CCamera::CAMTYPE_PLAYER);
	float4 fogParams = (sky != nullptr) ? float4{sky->fogStart * camPlayer->GetFarPlaneDist(),
	                                          sky->fogEnd * camPlayer->GetFarPlaneDist(), 0.0f, 0.0f} :
	                                      float4{0.1f * CGlobalRendering::MAX_VIEW_RANGE,
	                                          1.0f * CGlobalRendering::MAX_VIEW_RANGE, 0.0f, 0.0f};
	fogParams.w = 1.0f / (fogParams.y - fogParams.x);
	updateBuffer->fogParams = fogParams;

	updateBuffer->sunDir =
	    (sky != nullptr) ? sky->GetLight()->GetLightDir() : float4(/*map default*/ 0.0f, 0.447214f, 0.894427f, 1.0f);
	...
	updateBuffer->mouseScreenPos = float2{static_cast<float>(mouse->lastx),
	    static_cast<float>(globalRendering->viewPosY + globalRendering->viewSizeY - mouse->lasty - 1)};

	updateBuffer->mouseStatus =
	    (mouse->buttons[SDL_BUTTON_LEFT].pressed << 0 | mouse->buttons[SDL_BUTTON_MIDDLE].pressed << 1 |
	        mouse->buttons[SDL_BUTTON_RIGHT].pressed << 2 | mouse->offscreen << 3 | mouse->mmbScroll << 4 |
	        mouse->locked << 5);
...

	{
		...
		const float traceDist =
		    TraceRay::GuiTraceRay(camPos, pxlDir, rawRange, nullptr, unit, feature, true, false, true);
	...
	for (int teamID = 0; teamID < teamHandler.ActiveTeams(); ++teamID) {
		...
		updateBuffer->teamColor[teamID] =
		    float4{team->color[0] / 255.0f, team->color[1] / 255.0f, team->color[2] / 255.0f, team->color[3] / 255.0f};
	}
}
