...
bool CIconHandler::LoadIcons(const std::string& filename)
{
	...
	dummyIconData[SAFETY_DATA_IDX] = {};
	dummyIconData[DEFAULT_DATA_IDX] = {
	    "default", GetDefaultTexture(), 1.0f, 1.0f, false, false, DEFAULT_TEX_SIZE_X, DEFAULT_TEX_SIZE_Y};

	for (const std::string& iconName: iconNames) {
		const LuaTable iconTable = iconTypes.SubTable(iconName);

		AddIcon(iconName, iconTable.GetString("bitmap", ""), iconTable.GetFloat("size", 1.0f),
		    iconTable.GetFloat("distance", 1.0f), iconTable.GetBool("radiusAdjust", false));
	}
...
bool CIconHandler::AddIcon(const std::string& iconName,
    ...
    bool radAdj)
{
	...
	try {
		...
		LOG_L(L_DEBUG, "[IconHandler::%s] exception \"%s\" adding icon \"%s\" with texture \"%s\"", __func__, ex.what(),
		    iconName.c_str(), texName.c_str());
		return false;
	}
...
CIcon::CIcon(unsigned int idx) { iconHandler.GetIconDataMut(dataIdx = idx)->Ref(); }

CIcon::CIcon(const CIcon& icon) { iconHandler.GetIconDataMut(dataIdx = icon.dataIdx)->Ref(); }
...
CIconData::CIconData(const std::string& _name,
    unsigned int _texID,
    float _size,
    float _distance,
    bool radAdj,
...
void CIconData::BindTexture() const { glBindTexture(GL_TEXTURE_2D, texID); }
