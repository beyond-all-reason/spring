...
static void
CopyShaderState_Uniforms(GLuint newProgID, GLuint oldProgID, Shader::IProgramObject::UniformStates* uniformStates)
{
...
#define HANDLE_MATTYPE(type, size, ftype, internalTypeName)                                          \
	case type##size: {                                                                               \
		...
		default: LOG_L(L_WARNING, "Unknown GLSL uniform \"%s\" has unknown vartype \"%X\"", name, type);
		}
	}
...
static void CopyShaderState_ShaderStorage(GLuint newProgID, GLuint oldProgID)
{
	...
	for (int i = 0; i < numUniformBlocks; ++i) {
		...
		glGetProgramResourceiv(oldProgID, GL_SHADER_STORAGE_BLOCK, oldLoc, 1, props, 1, nullptr, &value);
