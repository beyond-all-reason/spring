...
static void
ParseUniformsTable(Shader::IProgramObject* program, const LuaTable* root, const std::string& fieldName, int type)
{
	...
	for (const std::string& key: keys) {
		...
		case UNIFORM_TYPE_FLOAT_MATRIX: {
			if (subTable.GetType(key) == LuaTable::TABLE) {
				...
				default:
					LOG_L(L_ERROR, "%s-shader uniformMatrix error: only square matrices supported right now: %s",
					    program->GetName().c_str(), program->GetLog().c_str());
				}
			}
		} break;
		};
...
static void CreateShaderObject(Shader::IProgramObject* program,
...
static void ParseShaderTable(const LuaTable* root, const std::string key, std::stringstream& data)
{
	RECOIL_DETAILED_TRACY_ZONE;
