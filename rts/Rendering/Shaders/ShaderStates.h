...
#define RC(p) (((p).first).c_str())
		...
		for (const auto& p: bitFlags) {
			if (PV(p))
				ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "#define %s\n", RC(p));
		}
		for (const auto& p: intFlags) {
			ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "#define %s %d\n", RC(p), PV(p));
		}
		for (const auto& p: fltFlags) {
			ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "#define %s %f\n", RC(p), PV(p));
		}
...
#undef PV
	...
	template<typename R> bool Get(const char* key, R& val)
	{
