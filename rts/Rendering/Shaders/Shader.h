...
public:
	NullShaderObject(unsigned int shType, const std::string& shSrcFile)
	    : IShaderObject(shType, shSrcFile)
	{
	}
};
...
public:
	...
	typedef std::unique_ptr<CompiledShaderObject, std::function<void(CompiledShaderObject* so)>>
	    CompiledShaderObjectUniquePtr;
...
struct ShaderEnabledToken {
	...
	ShaderEnabledToken& operator=(ShaderEnabledToken&& rhs) noexcept
	{
		std::swap(prog, rhs.prog);
		return *this;
	}
...
public:
	...
	template<typename VAT> void BindAttribLocations();

	void SetLogReporting(bool b, bool shObjects = true);

	[[nodiscard]] ShaderEnabledToken EnableScoped() { return ShaderEnabledToken(this); }
	...
	{
		for (auto so: shaderObjs) so->SetReloadComplete();
	}

	bool IsReloadRequested() const
	{
		bool reloadRequested = false;
		for (auto so: shaderObjs) reloadRequested |= so->IsReloadRequested();
...
public:
	...
	template<typename TV> inline void SetUniform(const char* name, TV v0, TV v1)
	{
		SetUniform(GetUniformState(name), v0, v1);
	}

	template<typename TV> inline void SetUniform(const char* name, TV v0, TV v1, TV v2)
	{
		SetUniform(GetUniformState(name), v0, v1, v2);
	}

	template<typename TV> inline void SetUniform(const char* name, TV v0, TV v1, TV v2, TV v3)
	{
		SetUniform(GetUniformState(name), v0, v1, v2, v3);
	}

	template<typename TV> inline void SetUniform2v(const char* name, const TV* v)
	{
		SetUniform2v(GetUniformState(name), 1, v);
	}

	template<typename TV> inline void SetUniform3v(const char* name, const TV* v)
	{
		SetUniform3v(GetUniformState(name), 1, v);
	}

	template<typename TV> inline void SetUniform4v(const char* name, const TV* v)
	{
		SetUniform4v(GetUniformState(name), 1, v);
	}

	/// variants with count param
	template<typename TV> inline void SetUniform1v(const char* name, const GLsizei count, const TV* v)
	{
		SetUniform1v(GetUniformState(name), count, v);
	}

	template<typename TV> inline void SetUniform2v(const char* name, const GLsizei count, const TV* v)
	{
		SetUniform2v(GetUniformState(name), count, v);
	}

	template<typename TV> inline void SetUniform3v(const char* name, const GLsizei count, const TV* v)
	{
		SetUniform3v(GetUniformState(name), count, v);
	}

	template<typename TV> inline void SetUniform4v(const char* name, const GLsizei count, const TV* v)
	{
		SetUniform4v(GetUniformState(name), count, v);
	}

	template<typename TV> inline void SetUniformMatrix2x2(const char* name, bool transp, const TV* v)
	{
		SetUniformMatrix2x2(GetUniformState(name), transp, v);
	}

	template<typename TV> inline void SetUniformMatrix3x3(const char* name, bool transp, const TV* v)
	{
		SetUniformMatrix3x3(GetUniformState(name), transp, v);
	}

	template<typename TV> inline void SetUniformMatrix4x4(const char* name, bool transp, const TV* v)
	{
		SetUniformMatrix4x4(GetUniformState(name), transp, v);
	}
	...
	} //< only needed for ARB, for GLSL uniforms of vertex & frag shader are accessed in the same space
...
protected:
	...
	virtual void SetUniform(UniformState* uState, int v0, int v1, int v2)
	{
		SetUniform3i(uState->GetLocation(), v0, v1, v2);
	}

	virtual void SetUniform(UniformState* uState, float v0, float v1, float v2)
	{
		SetUniform3f(uState->GetLocation(), v0, v1, v2);
	}

	virtual void SetUniform(UniformState* uState, int v0, int v1, int v2, int v3)
	{
		SetUniform4i(uState->GetLocation(), v0, v1, v2, v3);
	}

	virtual void SetUniform(UniformState* uState, float v0, float v1, float v2, float v3)
	{
		SetUniform4f(uState->GetLocation(), v0, v1, v2, v3);
	}

	virtual void SetUniform1v(UniformState* uState, GLsizei count, const int* v)
	{
		SetUniform1iv(uState->GetLocation(), count, v);
	}

	virtual void SetUniform1v(UniformState* uState, GLsizei count, const float* v)
	{
		SetUniform1fv(uState->GetLocation(), count, v);
	}

	virtual void SetUniform2v(UniformState* uState, GLsizei count, const int* v)
	{
		SetUniform2iv(uState->GetLocation(), count, v);
	}

	virtual void SetUniform2v(UniformState* uState, GLsizei count, const float* v)
	{
		SetUniform2fv(uState->GetLocation(), count, v);
	}

	virtual void SetUniform3v(UniformState* uState, GLsizei count, const int* v)
	{
		SetUniform3iv(uState->GetLocation(), count, v);
	}

	virtual void SetUniform3v(UniformState* uState, GLsizei count, const float* v)
	{
		SetUniform3fv(uState->GetLocation(), count, v);
	}

	virtual void SetUniform4v(UniformState* uState, GLsizei count, const int* v)
	{
		SetUniform4iv(uState->GetLocation(), count, v);
	}

	virtual void SetUniform4v(UniformState* uState, GLsizei count, const float* v)
	{
		SetUniform4fv(uState->GetLocation(), count, v);
	}

	virtual void SetUniformMatrix2x2(UniformState* uState, bool transp, const float* m)
	{
		SetUniformMatrix2fv(uState->GetLocation(), transp, m);
	}

	virtual void SetUniformMatrix3x3(UniformState* uState, bool transp, const float* m)
	{
		SetUniformMatrix3fv(uState->GetLocation(), transp, m);
	}

	virtual void SetUniformMatrix4x4(UniformState* uState, bool transp, const float* m)
	{
		SetUniformMatrix4fv(uState->GetLocation(), transp, m);
	}
...
public:
	NullProgramObject(const std::string& poName)
...
template<typename VAT> inline void IProgramObject::BindAttribLocations()
{
