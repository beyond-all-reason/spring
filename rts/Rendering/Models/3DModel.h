...
struct SVertexData {
	...
	{
		assert(bi.size() == 4);
		boneIDsLow = {static_cast<uint8_t>((bi[0].first) & 0xFF), static_cast<uint8_t>((bi[1].first) & 0xFF),
		    static_cast<uint8_t>((bi[2].first) & 0xFF), static_cast<uint8_t>((bi[3].first) & 0xFF)};

		boneWeights = {(static_cast<uint8_t>(math::round(bi[0].second * 255.0f))),
		...
		boneIDsHigh = {static_cast<uint8_t>((bi[0].first >> 8) & 0xFF), static_cast<uint8_t>((bi[1].first >> 8) & 0xFF),
		    static_cast<uint8_t>((bi[2].first >> 8) & 0xFF), static_cast<uint8_t>((bi[3].first >> 8) & 0xFF)};
	}
};
...
struct S3DModel {
	...
	S3DModel& operator=(S3DModel&& m) noexcept
	{
		...
		for (auto po: pieceObjects) po->SetParentModel(this);
	...
	S3DModelPiece* GetPiece(size_t i) const
	{
		assert(i < pieceObjects.size());
...
/**
 * LocalModel
 * Instance of S3DModel. Container for the geometric properties & piece visibility status of the agent's instance of a
 * 3d model.
 */
...
struct LocalModelPiece {
	...
	CMatrix44f CalcPieceSpaceMatrixRaw(const float3& p, const float3& r, const float3& s) const
	{
	...
	const CMatrix44f& GetPieceSpaceMatrix() const
	{
		if (dirty)
...
struct LocalModel {
	...
	const LocalModelPiece* GetPiece(unsigned int i) const
	{
		assert(HasPiece(i));
		return &pieces[i];
	}

	LocalModelPiece* GetPiece(unsigned int i)
	{
		assert(HasPiece(i));
		return &pieces[i];
	}
