...
struct SIndexAndCount {
	...
	uint64_t operator()(const SIndexAndCount& o) const { return ((uint64_t)o.index << 32 | o.count); }
...
public:
	...
	bool SubmitImmediately(const S3DModel* model,
	    uint8_t teamID,
	    uint8_t drawFlags,
...
private:
	...
	bool SubmitImmediatelyImpl(const TObj* obj,
	    uint32_t indexStart,
	    uint32_t indexCount,
	    uint8_t teamID,
	    uint8_t drawFlags,
	...
	template<typename TObj>
	bool
	AddToSubmissionImpl(const TObj* obj, uint32_t indexStart, uint32_t indexCount, uint8_t teamID, uint8_t drawFlags);
