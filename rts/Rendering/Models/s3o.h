...
struct Piece {
	...
	int vertexTable; ///< file offset to vertice table, vertice table is made up of dwords indicating vertices for this
	                 ///< piece, to indicate end of a triangle strip use 0xffffffff
...
struct S3OHeader {
	...
	float midx; ///< these give the offset from origin(which is supposed to lay in the ground plane) to the middle of
	            ///< the unit collision sphere
	float midy;
	float midz;
