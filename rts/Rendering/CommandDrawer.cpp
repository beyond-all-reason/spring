...
void CommandDrawer::Draw(const CCommandAI* cai, int queueDrawDepth) const
{
	...
	if ((dynamic_cast<const CAirCAI*>(cai)) != nullptr) {
		DrawAirCAICommands(static_cast<const CAirCAI*>(cai), queueDrawDepth);
		return;
	}
	if ((dynamic_cast<const CBuilderCAI*>(cai)) != nullptr) {
		DrawBuilderCAICommands(static_cast<const CBuilderCAI*>(cai), queueDrawDepth);
		return;
	}
	if ((dynamic_cast<const CFactoryCAI*>(cai)) != nullptr) {
		DrawFactoryCAICommands(static_cast<const CFactoryCAI*>(cai), queueDrawDepth);
		return;
	}
	if ((dynamic_cast<const CMobileCAI*>(cai)) != nullptr) {
		DrawMobileCAICommands(static_cast<const CMobileCAI*>(cai), queueDrawDepth);
		return;
	}
...
void CommandDrawer::DrawBuilderCAICommands(const CBuilderCAI* cai, int queueDrawDepth) const
{
	...
	for (auto ci = commandQue.begin(); ci != commandQue.end(); ++ci) {
		...
		case CMD_RECLAIM:
		case CMD_RESURRECT: {
			const float* color = (cmdID == CMD_RECLAIM) ? cmdColors.reclaim : cmdColors.resurrect;
...
void CommandDrawer::DrawWaitIcon(const Command& cmd) const { waitCommandsAI.AddIcon(cmd, lineDrawer.GetLastPos()); }
