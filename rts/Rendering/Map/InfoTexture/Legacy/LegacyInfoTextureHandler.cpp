...
CLegacyInfoTextureHandler::~CLegacyInfoTextureHandler() { glDeleteTextures(NUM_INFOTEXTURES, &infoTextureIDs[0]); }

bool CLegacyInfoTextureHandler::IsEnabled() const { return (drawMode != drawNormal); }
...
GLuint CLegacyInfoTextureHandler::GetCurrentInfoTexture() const { return infoTextureIDs[drawMode]; }
...
bool CLegacyInfoTextureHandler::UpdateExtraTexture(BaseGroundDrawMode texDrawMode)
{
	...
	if (updateTextureState < extraTextureUpdateRate) {
		...
		if (highResInfoTexWanted) {
			...
			infoTexMem =
			    reinterpret_cast<unsigned char*>(infoTexPBO.MapBuffer(offset, (endy - starty) * mapDims.pwr2mapx * 4));
		}
		else {
			...
			infoTexMem =
			    reinterpret_cast<unsigned char*>(infoTexPBO.MapBuffer(offset, (endy - starty) * pwr2mapx_half * 4));
		}
			...
			for (int y = starty; y < endy; ++y) {
				...
				for (int x = 0; x < mapDims.hmapx; ++x) {
					...
					if (myLos[alx + (aly * losHandler->los.size.x)]) {
						infoTexMem[a + COLOR_R] = (unsigned char)std::min(
						    255.0f, 900.0f * fastmath::apxsqrt(fastmath::apxsqrt(extractDepthMap[y_hmapx + x])));
					}
					else {
		...
		case drawLos: {
			...
			for (int y = starty; y < endy; ++y) {
				for (int x = 0; x < endx; ++x) {
					...
					const int inJam = (totalLos == 255) ?
					                      InterpolateLos(myJammer, losHandler->jammer.size, jammerMipLevel, 255, pos) :
					                      0;
		...
		if (highResInfoTexWanted) {
		    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, starty, mapDims.pwr2mapx, (endy-starty), GL_BGRA, GL_UNSIGNED_BYTE,
		infoTexPBO.GetPtr(offset)); } else { glTexSubImage2D(GL_TEXTURE_2D, 0, 0, starty, mapDims.pwr2mapx>>1,
		(endy-starty), GL_BGRA, GL_UNSIGNED_BYTE, infoTexPBO.GetPtr(offset));
		}
		*/
	...
	if (updateTextureState == extraTextureUpdateRate) {
		...
		if (highResInfoTexWanted) {
			glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mapDims.pwr2mapx, mapDims.pwr2mapy, GL_BGRA, GL_UNSIGNED_BYTE,
			    infoTexPBO.GetPtr());
		}
		else {
			glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mapDims.pwr2mapx >> 1, mapDims.pwr2mapy >> 1, GL_BGRA,
			    GL_UNSIGNED_BYTE, infoTexPBO.GetPtr());
		}
