...
void HUDDrawer::DrawModel(const CUnit* unit)
{
	...
	if (unit->moveType->UseHeading()) {
		...
		CMatrix44f m(ZeroVector, float3(camera->GetRight().x, camera->GetUp().x, camera->GetDir().x),
...
void HUDDrawer::DrawCameraDirectionArrow(const CUnit* unit)
{
	...
	if (unit->moveType->UseHeading()) {
		...
		glRotatef(
		    GetHeadingFromVector(camera->GetDir().x, camera->GetDir().z) * 180.0f / 32768 + 180, 0.0f, 0.0f, 1.0f);
...
void HUDDrawer::DrawWeaponStates(const CUnit* unit)
{
	...
	font->glFormat(
	    -0.9f, 0.35f, 1.0f, FONT_SCALE | FONT_NORM, "Health: %.0f / %.0f", (float)unit->health, (float)unit->maxHealth);
			...
			if (!wd->isShield) {
				...
				if (wd->stockpile && !w->numStockpiled) {
					if (w->numStockpileQued) {
						...
						font->glFormat(-0.9f, yPos, fontSize, FONT_SCALE | FONT_NORM, "%s: Stockpiling (%i%%)",
						    wd->description.c_str(), int(100.0f * w->buildPercent + 0.5f));
					}
					else {
						glColor4f(0.8f, 0.2f, 0.2f, 0.8f);
						font->glFormat(
						    -0.9f, yPos, fontSize, FONT_SCALE | FONT_NORM, "%s: No ammo", wd->description.c_str());
					}
				}
				else if (w->reloadStatus > gs->frameNum) {
					glColor4f(0.8f, 0.2f, 0.2f, 0.8f);
					font->glFormat(-0.9f, yPos, fontSize, FONT_SCALE | FONT_NORM, "%s: Reloading (%i%%)",
					    wd->description.c_str(),
					    100 - int(100.0f * (w->reloadStatus - gs->frameNum) / int(w->reloadTime / unit->reloadSpeed) +
					              0.5f));
				}
				else if (!w->angleGood) {
					glColor4f(0.6f, 0.6f, 0.2f, 0.8f);
					font->glFormat(
					    -0.9f, yPos, fontSize, FONT_SCALE | FONT_NORM, "%s: Aiming", wd->description.c_str());
				}
				else {
					glColor4f(0.2f, 0.8f, 0.2f, 0.8f);
...
void HUDDrawer::DrawTargetReticle(const CUnit* unit)
{
	...
	for (unsigned int a = 0; a < unit->weapons.size(); ++a) {
		...
		switch (a) {
		case 0: glColor4f(0.0f, 1.0f, 0.0f, 0.7f); break;
		case 1: glColor4f(1.0f, 0.0f, 0.0f, 0.7f); break;
		default: glColor4f(0.0f, 0.0f, 1.0f, 0.7f);
		}
		...
		if (w->HaveTarget()) {
			...
			for (int b = 0; b <= 80; ++b) {
				glVertexf3(pos + (v2 * fastmath::sin(b * math::TWOPI / 80) + v3 * fastmath::cos(b * math::TWOPI / 80)) *
				                     radius);
			}
			glEnd();
			...
			if (!w->onlyForward) {
				...
				for (int b = 0; b <= 80; ++b) {
					glVertexf3(
					    pos +
					    (v2 * fastmath::sin(b * math::TWOPI / 80) + v3 * fastmath::cos(b * math::TWOPI / 80)) * radius);
				}
				glEnd();
			}
			...
			if (!w->onlyForward) {
				...
				glVertexf3(
				    pos + (v2 * fastmath::sin(math::PI * 0.25f) + v3 * fastmath::cos(math::PI * 0.25f)) * radius);
				glVertexf3(
				    pos + (v2 * fastmath::sin(math::PI * 1.25f) + v3 * fastmath::cos(math::PI * 1.25f)) * radius);
...
void HUDDrawer::Draw(const CUnit* unit)
{
	...
	auto state = GL::SubState(DepthTest(GL_FALSE), Blending(GL_TRUE), BlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA));
