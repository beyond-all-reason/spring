...
QTPFSPathDrawer::QTPFSPathDrawer() { pm = dynamic_cast<QTPFS::PathManager*>(pathManager); }
...
void QTPFSPathDrawer::DrawNodes(const MoveDef* md,
...
void QTPFSPathDrawer::GetVisibleNodes(const QTPFS::QTNode* nt,
...
void QTPFSPathDrawer::DrawSearchExecution(unsigned int pathType,
...
void QTPFSPathDrawer::DrawSearchIteration(unsigned int pathType,
    ...
    TypedRenderBuffer<VA_TYPE_C>& rb) const
{
...

	{
		...
		for (size_t i = 1, n = nodeIndices.size(); i < n; i++) {
			...
			DrawNode(
			    pushedNode = static_cast<const QTPFS::QTNode*>(nodeLayer.GetNode(hmx, hmz)), rb, &NODE_COLORS[2][0]);
		}
...
void QTPFSPathDrawer::DrawNode(const QTPFS::QTNode* node,
    TypedRenderBuffer<VA_TYPE_C>& rb,
    const unsigned char* color) const
{
	...
	rb.AddQuadTriangles({v0, color}, {v1, color}, {v2, color}, {v3, color});
}

void QTPFSPathDrawer::DrawNodeW(const MoveDef* md,
...
void QTPFSPathDrawer::DrawNodeLink(const QTPFS::QTNode* pushedNode,
...
#define zmidw(n) (n->zmid() * SQUARE_SIZE)
	...
	const float3 v0 = {xmidw(pushedNode) * 1.0f,
	    CGround::GetHeightReal(xmidw(pushedNode), zmidw(pushedNode), false) + 4.0f, zmidw(pushedNode) * 1.0f};
	const float3 v1 = {xmidw(poppedNode) * 1.0f,
	    CGround::GetHeightReal(xmidw(poppedNode), zmidw(poppedNode), false) + 4.0f, zmidw(poppedNode) * 1.0f};
		...
		if (md != nullptr) {
			...
			for (int ty = starty; ty < endy; ++ty) {
				for (int tx = 0; tx < mapDims.hmapx; ++tx) {
					...
					if (los || losSqr) {
						if (CMoveMath::IsBlocked(*md, sqx, sqz) & CMoveMath::BLOCK_STRUCTURE) {
							scale -= 0.25f;
						}
						if (CMoveMath::IsBlocked(*md, sqx + 1, sqz) & CMoveMath::BLOCK_STRUCTURE) {
							scale -= 0.25f;
						}
						if (CMoveMath::IsBlocked(*md, sqx, sqz + 1) & CMoveMath::BLOCK_STRUCTURE) {
							scale -= 0.25f;
						}
						if (CMoveMath::IsBlocked(*md, sqx + 1, sqz + 1) & CMoveMath::BLOCK_STRUCTURE) {
							scale -= 0.25f;
						}
					}
