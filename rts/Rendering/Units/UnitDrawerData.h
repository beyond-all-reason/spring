...
public:
	...
	{
		return eventName == "RenderUnitPreCreated" || eventName == "RenderUnitCreated" ||
		       eventName == "RenderUnitDestroyed" || eventName == "UnitEnteredRadar" || eventName == "UnitEnteredLos" ||
		       eventName == "UnitLeftRadar" || eventName == "UnitLeftLos" || eventName == "PlayerChanged";
	}
...
public:
	...
	const std::vector<TempDrawUnit>& GetTempOpaqueDrawUnits(int modelType) const
	{
		return savedData.tempOpaqueUnits[modelType];
	}

	const std::vector<TempDrawUnit>& GetTempAlphaDrawUnits(int modelType) const
	{
		return savedData.tempAlphaUnits[modelType];
	}
	...
	const spring::unsynced_map<icon::CIconData*,
	    std::pair<std::vector<const CUnit*>, std::vector<const GhostSolidObject*>>>&
	GetUnitsByIcon() const
	{
		return unitsByIcon;
	}
...
private:
	...
	spring::unsynced_map<icon::CIconData*, std::pair<std::vector<const CUnit*>, std::vector<const GhostSolidObject*>>>
	    unitsByIcon;
