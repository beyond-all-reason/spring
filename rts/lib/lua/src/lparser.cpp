...
static void check_match(LexState* ls, int what, int who, int where)
{
	if (!testnext(ls, what)) {
		if (where == ls->linenumber)
			...
			luaX_syntaxerror(ls, luaO_pushfstring(ls->L, LUA_QS " expected (to close " LUA_QS " at line %d)",
...
static void constructor(LexState* ls, expdesc* t)
{
	...
	do {
		...
		case '[': { /* constructor_item -> recfield */ recfield(ls, &cc); break;
		}
		default: { /* constructor_part -> listfield */ listfield(ls, &cc); break;
		}
		}
...
static void parlist(LexState* ls)
{
	...
	if (ls->t.token != ')') { /* is `parlist' not empty? */
		do {
			switch (ls->t.token) {
			case TK_NAME: { /* param -> NAME */ new_localvar(ls, str_checkname(ls), nparams++); break;
			}
			case TK_DOTS: { /* param -> `...' */ luaX_next(ls);
...
static int statement(LexState* ls)
{
	...
	switch (ls->t.token) {
	case TK_IF: { /* stat -> ifstat */ ifstat(ls, line); return 0;
	}
	case TK_WHILE: { /* stat -> whilestat */ whilestat(ls, line); return 0;
	}
	...
	case TK_FOR: { /* stat -> forstat */ forstat(ls, line); return 0;
	}
	case TK_REPEAT: { /* stat -> repeatstat */ repeatstat(ls, line); return 0;
	}
	case TK_FUNCTION: {
	...
	case TK_RETURN: { /* stat -> retstat */ retstat(ls); return 1; /* must be last statement */
	}
