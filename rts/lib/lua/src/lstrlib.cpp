...
init: /* using goto's to optimize tail recursion */
	...
	case ')': { /* end capture */ return end_capture(ms, s, p + 1);
	}
	case L_ESC: {
		switch (*(p + 1)) {
	...
	case '\0': { /* end of pattern */ return s; /* match succeeded */
	}
	case '$': {
	...
	dflt: {                               /* it is a pattern item */
		...
		case '*': { /* 0 or more repetitions */ return max_expand(ms, s, p, ep);
		}
		case '+': { /* 1 or more repetitions */ return (m ? max_expand(ms, s + 1, p, ep) : NULL);
		}
		case '-': { /* 0 or more repetitions (minimum) */ return min_expand(ms, s, p, ep);
		}
		default: {
			if (!m)
				return NULL;
