...
public:
	...
	explicit strand(const Executor1& e,
	    constraint_t<
	        conditional_t<!is_same<Executor1, strand>::value, is_convertible<Executor1, Executor>, false_type>::value> =
	        0)
	    : executor_(e)
	    , impl_(strand::create_implementation(executor_))
	{
	...
	inner_executor_type get_inner_executor() const noexcept { return executor_; }
	...
	constraint_t<can_query<const Executor&, Property>::value,
	    conditional_t<is_convertible<Property, execution::blocking_t>::value,
	        execution::blocking_t,
	        query_result_t<const Executor&, Property>>>
	query(const Property& p) const noexcept(is_nothrow_query<const Executor&, Property>::value)
	{
		return this->query_helper(is_convertible<Property, execution::blocking_t>(), p);
	}
	 ...
	 */
	template<typename Property>
	constraint_t<can_require<const Executor&, Property>::value &&
	...
	{
		return strand<decay_t<require_result_t<const Executor&, Property>>>(asio::require(executor_, p), impl_);
	}
	 ...
	 */
	template<typename Property>
	constraint_t<can_prefer<const Executor&, Property>::value &&
	                 !is_convertible<Property, execution::blocking_t::always_t>::value,
	...
	{
		return strand<decay_t<prefer_result_t<const Executor&, Property>>>(asio::prefer(executor_, p), impl_);
	}

#if !defined(ASIO_NO_TS_EXECUTORS)
	/// Obtain the underlying execution context.
	execution_context& context() const noexcept { return executor_.context(); }

	/// Inform the strand that it has some outstanding work to do.
	/**
	 * The strand delegates this call to its underlying executor.
	 */
	void on_work_started() const noexcept { executor_.on_work_started(); }

	/// Inform the strand that some work is no longer outstanding.
	/**
	 * The strand delegates this call to its underlying executor.
	 */
	void on_work_finished() const noexcept { executor_.on_work_finished(); }
	...
	constraint_t<traits::execute_member<const Executor&, Function>::is_valid, void> execute(Function&& f) const
	{
		detail::strand_executor_service::execute(impl_, executor_, static_cast<Function&&>(f));
	}
	...
	{
		detail::strand_executor_service::dispatch(impl_, executor_, static_cast<Function&&>(f), a);
	}
	 ...
	 * internal storage needed for function invocation.
	 */
	template<typename Function, typename Allocator> void post(Function&& f, const Allocator& a) const
	{
		detail::strand_executor_service::post(impl_, executor_, static_cast<Function&&>(f), a);
	}
	 ...
	 * internal storage needed for function invocation.
	 */
	template<typename Function, typename Allocator> void defer(Function&& f, const Allocator& a) const
	{
		detail::strand_executor_service::defer(impl_, executor_, static_cast<Function&&>(f), a);
	}
#endif // !defined(ASIO_NO_TS_EXECUTORS)
	 ...
	 * state.
	 */
	friend bool operator==(const strand& a, const strand& b) noexcept { return a.impl_ == b.impl_; }
...
private:
	...
	static implementation_type create_implementation(const InnerExecutor& ex,
	    constraint_t<can_query<InnerExecutor, execution::context_t>::value> = 0)
	{
		return use_service<detail::strand_executor_service>(asio::query(ex, execution::context))
	...
	static implementation_type create_implementation(const InnerExecutor& ex,
	    constraint_t<!can_query<InnerExecutor, execution::context_t>::value> = 0)
	{
		return use_service<detail::strand_executor_service>(ex.context()).create_implementation();
	}
	...
	template<typename Property>
	query_result_t<const Executor&, Property> query_helper(false_type, const Property& property) const
	{
		return asio::query(executor_, property);
	}

	template<typename Property> execution::blocking_t query_helper(true_type, const Property& property) const
	{
		execution::blocking_t result = asio::query(executor_, property);
		return result == execution::blocking.always ? execution::blocking.possibly : result;
	}
...
inline strand<typename ExecutionContext::executor_type> make_strand(ExecutionContext& ctx,
...
template<typename Executor> struct equality_comparable<strand<Executor>> {
...
struct execute_member<strand<Executor>,
    Function,
    enable_if_t<traits::execute_member<const Executor&, Function>::is_valid>> {
	static constexpr bool is_valid = true;
...
struct query_member<strand<Executor>, Property, enable_if_t<can_query<const Executor&, Property>::value>> {
	static constexpr bool is_valid = true;
	static constexpr bool is_noexcept = is_nothrow_query<Executor, Property>::value;
	typedef conditional_t<is_convertible<Property, execution::blocking_t>::value,
	    execution::blocking_t,
	    query_result_t<Executor, Property>>
	    result_type;
};
...
template<typename Executor, typename Property>
struct require_member<strand<Executor>,
    Property,
...
template<typename Executor, typename Property>
struct prefer_member<strand<Executor>,
    Property,
