...
public:
	...
	/// Construct a polymorphic wrapper for the specified executor.
	template<typename Executor> executor(std::nothrow_t, Executor e) noexcept;

	/// Allocator-aware constructor to create a polymorphic wrapper for the
	/// specified executor.
	template<typename Executor, typename Allocator> executor(allocator_arg_t, const Allocator& a, Executor e);
	...
	/// executor.
	template<typename Executor> executor& operator=(Executor&& e) noexcept
	{
	...
	execution_context& context() const noexcept { return get_impl()->context(); }

	/// Inform the executor that it has some outstanding work to do.
	void on_work_started() const noexcept { get_impl()->on_work_started(); }

	/// Inform the executor that some work is no longer outstanding.
	void on_work_finished() const noexcept { get_impl()->on_work_finished(); }
	...
	operator unspecified_bool_type() const noexcept { return impl_ ? &executor::unspecified_bool_true : 0; }
	...
	const std::type_info& target_type() const noexcept { return impl_ ? impl_->target_type() : typeid(void); }
#else  // !defined(ASIO_NO_TYPEID) || defined(GENERATING_DOCUMENTATION)
	const void* target_type() const noexcept { return impl_ ? impl_->target_type() : 0; }
	...
	friend bool operator!=(const executor& a, const executor& b) noexcept { return !(a == b); }
...
private:
	...
	template<typename T> static type_id_result_type type_id()
	{
#if !defined(ASIO_NO_TYPEID)
		return typeid(T);
#else  // !defined(ASIO_NO_TYPEID)
	...
	protected:
		impl_base(bool fast_dispatch)
		    : fast_dispatch_(fast_dispatch)
		{
		}
	...
	impl_base* clone() const noexcept { return impl_ ? impl_->clone() : 0; }
