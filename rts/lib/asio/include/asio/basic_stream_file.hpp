...
public:
	...
	{
		this->impl_.get_service().set_is_stream(this->impl_.get_implementation(), true);
	}
	 ...
	 */
	basic_stream_file(const executor_type& ex, const char* path, file_base::flags open_flags)
	    : basic_file<Executor>(ex)
	{
		asio::error_code ec;
		this->impl_.get_service().set_is_stream(this->impl_.get_implementation(), true);
		this->impl_.get_service().open(this->impl_.get_implementation(), path, open_flags, ec);
	 ...
	 * @throws asio::system_error Thrown on failure.
	 */
	basic_stream_file(const executor_type& ex, const std::string& path, file_base::flags open_flags)
	    : basic_file<Executor>(ex)
	{
		asio::error_code ec;
		this->impl_.get_service().set_is_stream(this->impl_.get_implementation(), true);
		this->impl_.get_service().open(this->impl_.get_implementation(), path.c_str(), open_flags, ec);
		asio::detail::throw_error(ec, "open");
	}
	 ...
	 * @throws asio::system_error Thrown on failure.
	 */
	basic_stream_file(const executor_type& ex, const native_handle_type& native_file)
	...
	{
		asio::error_code ec;
		uint64_t n = this->impl_.get_service().seek(this->impl_.get_implementation(), offset, whence, ec);
	...
	{
		return this->impl_.get_service().seek(this->impl_.get_implementation(), offset, whence, ec);
	}
	...
	{
		asio::error_code ec;
		std::size_t s = this->impl_.get_service().write_some(this->impl_.get_implementation(), buffers, ec);
	...
	{
		return this->impl_.get_service().write_some(this->impl_.get_implementation(), buffers, ec);
	}
	...
	{
		asio::error_code ec;
		std::size_t s = this->impl_.get_service().read_some(this->impl_.get_implementation(), buffers, ec);
	...
	{
		return this->impl_.get_service().read_some(this->impl_.get_implementation(), buffers, ec);
	}
	...
	public:
		...
		void operator()(WriteHandler&& handler, const ConstBufferSequence& buffers) const
		{
			...
			self_->impl_.get_service().async_write_some(
			    self_->impl_.get_implementation(), buffers, handler2.value, self_->impl_.get_executor());
		}
