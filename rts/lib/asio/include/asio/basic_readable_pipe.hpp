...
public:
	...
	typedef detail::win_iocp_handle_service::native_handle_type native_handle_type;
#elif defined(ASIO_HAS_IO_URING_AS_DEFAULT)
	 ...
	 */
	basic_readable_pipe(const executor_type& ex, const native_handle_type& native_pipe)
	    : impl_(0, ex)
	{
		asio::error_code ec;
	...
	template<typename Executor1> friend class basic_readable_pipe;
	 ...
	 */
	ASIO_SYNC_OP_VOID assign(const native_handle_type& native_pipe, asio::error_code& ec)
	{
		impl_.get_service().assign(impl_.get_implementation(), native_pipe, ec);
