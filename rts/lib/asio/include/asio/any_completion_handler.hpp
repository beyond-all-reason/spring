...
public:
	...
	cancellation_slot get_cancellation_slot() const noexcept { return cancel_state_.slot(); }

private:
	cancellation_state cancel_state_;
};

template<typename Handler> class any_completion_handler_impl : public any_completion_handler_impl_base {
public:
	...
	struct uninit_deleter {
		typename std::allocator_traits<associated_allocator_t<Handler,
		    asio::recycling_allocator<void>>>::template rebind_alloc<any_completion_handler_impl>
		    alloc;
	...
	template<typename S, typename H> static any_completion_handler_impl* create(S&& slot, H&& h)
	{
		...
		any_completion_handler_impl* ptr =
		    new (uninit_ptr.get()) any_completion_handler_impl(static_cast<S&&>(slot), static_cast<H&&>(h));
	...
	any_completion_executor executor(const any_completion_executor& candidate) const noexcept
	{
		return any_completion_executor(std::nothrow, (get_associated_executor)(handler_, candidate));
	}

	any_completion_executor immediate_executor(const any_io_executor& candidate) const noexcept
	{
		return any_completion_executor(std::nothrow, (get_associated_immediate_executor)(handler_, candidate));
	}

	void* allocate(std::size_t size, std::size_t align) const
	{
		typename std::allocator_traits<
		    associated_allocator_t<Handler, asio::recycling_allocator<void>>>::template rebind_alloc<unsigned char>
		    alloc((get_associated_allocator)(handler_, asio::recycling_allocator<void>()));

		std::size_t space = size + align - 1;
		unsigned char* base = std::allocator_traits<decltype(alloc)>::allocate(alloc, space + sizeof(std::ptrdiff_t));
	...
	void deallocate(void* p, std::size_t size, std::size_t align) const
	{
		if (p) {
			...
			std::allocator_traits<decltype(alloc)>::deallocate(alloc, base, size + align - 1 + sizeof(std::ptrdiff_t));
		}
	}
...
template<typename Signature> class any_completion_handler_call_fn;

template<typename R, typename... Args> class any_completion_handler_call_fn<R(Args...)> {
public:
	...
	template<typename Handler> static void impl(any_completion_handler_impl_base* impl, Args... args)
	{
		static_cast<any_completion_handler_impl<Handler>*>(impl)->call(static_cast<Args&&>(args)...);
	}
...
template<typename... Signatures> class any_completion_handler_call_fns;

template<typename Signature>
class any_completion_handler_call_fns<Signature> : public any_completion_handler_call_fn<Signature> {
public:
...
class any_completion_handler_call_fns<Signature, Signatures...>
    : public any_completion_handler_call_fn<Signature>, public any_completion_handler_call_fns<Signatures...> {
public:
...
public:
	...
	void destroy(any_completion_handler_impl_base* impl) const { destroy_fn_(impl); }

	template<typename Handler> static void impl(any_completion_handler_impl_base* impl)
	{
...
public:
	using type = any_completion_executor (*)(any_completion_handler_impl_base*, const any_io_executor&);
	...
	any_completion_executor immediate_executor(any_completion_handler_impl_base* impl,
	...
	template<typename Handler>
	static any_completion_executor impl(any_completion_handler_impl_base* impl, const any_io_executor& candidate)
	{
		return static_cast<any_completion_handler_impl<Handler>*>(impl)->immediate_executor(candidate);
	}
...
class any_completion_handler_fn_table : private any_completion_handler_destroy_fn,
...
template<typename Handler, typename... Signatures> struct any_completion_handler_fn_table_instance {
	static constexpr any_completion_handler_fn_table<Signatures...> value =
	    any_completion_handler_fn_table<Signatures...>(&any_completion_handler_destroy_fn::impl<Handler>,
...
template<typename T, typename... Signatures> class any_completion_handler_allocator {
private:
	...
	constexpr any_completion_handler_allocator(int, const any_completion_handler<Signatures...>& h) noexcept
...
public:
	...
	void deallocate(T* p, std::size_t n) const { fn_table_->deallocate(impl_, p, sizeof(T) * n, alignof(T)); }
};
...
private:
	...
	template<typename, typename> friend struct associated_immediate_executor;
...
public:
	...
	any_completion_handler(H&& h, constraint_t<!is_same<decay_t<H>, any_completion_handler>::value> = 0)
	    : fn_table_(&detail::any_completion_handler_fn_table_instance<Handler, Signatures...>::value)
	    , impl_(detail::any_completion_handler_impl<Handler>::create((get_associated_cancellation_slot)(h),
	          static_cast<H&&>(h)))
	{
	}
	...
	{
		any_completion_handler(static_cast<any_completion_handler&&>(other)).swap(*this);
		return *this;
	}
	...
	allocator_type get_allocator() const noexcept { return allocator_type(0, *this); }
	...
	auto operator()(Args&&... args) -> decltype(fn_table_->call(impl_, static_cast<Args&&>(args)...))
	{
	...
	/// Equality operator.
	friend constexpr bool operator==(const any_completion_handler& a, nullptr_t) noexcept { return a.impl_ == nullptr; }

	/// Equality operator.
	friend constexpr bool operator==(nullptr_t, const any_completion_handler& b) noexcept { return nullptr == b.impl_; }
...
struct associated_executor<any_completion_handler<Signatures...>, Candidate> {
	    ...
	    const Candidate& candidate = Candidate()) noexcept
	{
		...
		return handler.fn_table_ ? handler.fn_table_->executor(handler.impl_, any_candidate) : any_candidate;
	}
};

template<typename... Signatures, typename Candidate>
struct associated_immediate_executor<any_completion_handler<Signatures...>, Candidate> {
	using type = any_completion_executor;
	    ...
	    const Candidate& candidate = Candidate()) noexcept
	{
		...
		return handler.fn_table_ ? handler.fn_table_->immediate_executor(handler.impl_, any_candidate) : any_candidate;
	}
};
