...
public:
	 ...
	 */
	explicit basic_streambuf(std::size_t maximum_size = (std::numeric_limits<std::size_t>::max)(),
	...
	{
		return asio::buffer(asio::const_buffer(gptr(), (pptr() - gptr()) * sizeof(char_type)));
	}
...
private:
	...
	{
		return std::min<std::size_t>(std::max<std::size_t>(512, sb.buffer_.capacity() - sb.size()),
...
public:
	/// The type used to represent the input sequence as a list of buffers.
	typedef typename basic_streambuf<Allocator>::const_buffers_type const_buffers_type;

	/// The type used to represent the output sequence as a list of buffers.
	typedef typename basic_streambuf<Allocator>::mutable_buffers_type mutable_buffers_type;
