...
public:
	 ...
	 */
	basic_random_access_file(const executor_type& ex, const char* path, file_base::flags open_flags)
	 ...
	 * @throws asio::system_error Thrown on failure.
	 */
	basic_random_access_file(const executor_type& ex, const std::string& path, file_base::flags open_flags)
	    : basic_file<Executor>(ex, path, open_flags)
	{
	}
	 ...
	 * @throws asio::system_error Thrown on failure.
	 */
	basic_random_access_file(const executor_type& ex, const native_handle_type& native_file)
	...
	{
		asio::error_code ec;
		std::size_t s = this->impl_.get_service().write_some_at(this->impl_.get_implementation(), offset, buffers, ec);
	 ...
	 */
	template<typename ConstBufferSequence>
	std::size_t write_some_at(uint64_t offset, const ConstBufferSequence& buffers, asio::error_code& ec)
	{
		return this->impl_.get_service().write_some_at(this->impl_.get_implementation(), offset, buffers, ec);
	}
	...
	auto async_write_some_at(uint64_t offset,
	    ...
	    -> decltype(async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
	        declval<initiate_async_write_some_at>(),
	        token,
	        offset,
	        buffers))
	{
	...
	{
		asio::error_code ec;
		std::size_t s = this->impl_.get_service().read_some_at(this->impl_.get_implementation(), offset, buffers, ec);
	 ...
	 */
	template<typename MutableBufferSequence>
	std::size_t read_some_at(uint64_t offset, const MutableBufferSequence& buffers, asio::error_code& ec)
	{
		return this->impl_.get_service().read_some_at(this->impl_.get_implementation(), offset, buffers, ec);
	}
	...
	auto async_read_some_at(uint64_t offset,
	    ...
	    -> decltype(async_initiate<ReadToken, void(asio::error_code, std::size_t)>(
	        declval<initiate_async_read_some_at>(),
	        token,
	        offset,
	        buffers))
	{
...
private:
	...
	basic_random_access_file& operator=(const basic_random_access_file&) = delete;
	...
	public:
		...
		void operator()(WriteHandler&& handler, uint64_t offset, const ConstBufferSequence& buffers) const
		{
			...
			self_->impl_.get_service().async_write_some_at(
			    self_->impl_.get_implementation(), offset, buffers, handler2.value, self_->impl_.get_executor());
		}
	...
	public:
		...
		void operator()(ReadHandler&& handler, uint64_t offset, const MutableBufferSequence& buffers) const
		{
			// If you get an error on the following line it means that your handler
