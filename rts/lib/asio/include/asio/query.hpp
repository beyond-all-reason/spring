 ...
 */
template<typename T, typename Property> struct can_query : integral_constant<bool, automatically_determined> {};
 ...
 */
template<typename T, typename Property> struct is_nothrow_query : integral_constant<bool, automatically_determined> {};
 ...
 */
template<typename T, typename Property> struct query_result {
...
struct call_traits<Impl,
    ...
    enable_if_t<static_query<T, Property>::is_valid>> : static_query<T, Property> {
...
struct call_traits<Impl,
    ...
    enable_if_t<query_member<typename Impl::template proxy<T>::type, Property>::is_valid>>
...
struct call_traits<Impl,
    ...
    enable_if_t<!static_query<T, Property>::is_valid>,
    enable_if_t<!query_member<typename Impl::template proxy<T>::type, Property>::is_valid>,
    enable_if_t<query_free<T, Property>::is_valid>> : query_free<T, Property> {
...
struct impl {
	template<typename T> struct proxy {
		...
		struct type {
			template<typename P>
			auto query(P&& p) noexcept(noexcept(declval<conditional_t<true, T, P>>().query(static_cast<P&&>(p))))
	...
	ASIO_NODISCARD constexpr enable_if_t<call_traits<impl, T, void(Property)>::overload == static_value,
	    typename call_traits<impl, T, void(Property)>::result_type>
	operator()(T&&, Property&&) const noexcept(call_traits<impl, T, void(Property)>::is_noexcept)
	{
...
static constexpr const asio_query_fn::impl& query = asio_query_fn::static_instance<>::instance;
...
struct can_query : integral_constant<bool,
                       asio_query_fn::call_traits<query_t, T, void(Property)>::overload != asio_query_fn::ill_formed> {
};

#if defined(ASIO_HAS_VARIABLE_TEMPLATES)

template<typename T, typename Property> constexpr bool can_query_v = can_query<T, Property>::value;

#endif // defined(ASIO_HAS_VARIABLE_TEMPLATES)

template<typename T, typename Property>
struct is_nothrow_query : integral_constant<bool, asio_query_fn::call_traits<query_t, T, void(Property)>::is_noexcept> {
};

#if defined(ASIO_HAS_VARIABLE_TEMPLATES)

template<typename T, typename Property> constexpr bool is_nothrow_query_v = is_nothrow_query<T, Property>::value;

#endif // defined(ASIO_HAS_VARIABLE_TEMPLATES)

template<typename T, typename Property> struct query_result {
	typedef typename asio_query_fn::call_traits<query_t, T, void(Property)>::result_type type;
};

template<typename T, typename Property> using query_result_t = typename query_result<T, Property>::type;
