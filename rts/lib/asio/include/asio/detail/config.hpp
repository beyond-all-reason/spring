...
#elif defined(_MSC_VER) && (defined(__INTELLISENSE__) || (!defined(__MWERKS__) && !defined(__EDG_VERSION__)))
...
#define ASIO_STATIC_CONSTEXPR(type, assignment) static constexpr type assignment
...
#else // (__GNUC__ >= 8)
#define ASIO_STATIC_CONSTEXPR_DEFAULT_INIT(type, name) static const type name
#endif // (__GNUC__ >= 8)
...
#if (_LIBCPP_STD_VER > 14) && defined(_LIBCPP_HAS_ALIGNED_ALLOC) && !defined(_LIBCPP_MSVCRT) && !defined(__MINGW32__)
...
#if defined(ASIO_HAS_STD_CHRONO) || defined(ASIO_HAS_BOOST_CHRONO)
...
#if defined(ASIO_HAS_BOOST_SOURCE_LOCATION)
#define ASIO_SOURCE_LOCATION_PARAM , const boost::source_location& loc
#define ASIO_SOURCE_LOCATION_DEFAULTED_PARAM , const boost::source_location& loc = BOOST_CURRENT_LOCATION
...
#if (WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP) || WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_TV_TITLE)) && \
...
// for threads support.
    ...
    defined(__OpenBSD__) || defined(__linux__) || defined(__HAIKU__)
...
#if defined(ASIO_HAS_IOCP) || !defined(ASIO_WINDOWS) && !defined(ASIO_WINDOWS_RUNTIME) && !defined(__CYGWIN__)
...
#elif defined(__GNUC__) && !defined(__MINGW32__) && !defined(linux) && !defined(__linux) && !defined(__linux__)
#define ASIO_HAS_THREADS 1
...
#elif ((__clang_major__ == 3) && (__clang_minor__ >= 6)) || (__clang_major__ > 3)
#define ASIO_UNUSED_TYPEDEF __attribute__((__unused__))
