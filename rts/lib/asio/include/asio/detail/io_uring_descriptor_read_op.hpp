...
template<typename MutableBufferSequence> class io_uring_descriptor_read_op_base : public io_uring_operation {
public:
	io_uring_descriptor_read_op_base(const asio::error_code& success_ec,
	    ...
	    : io_uring_operation(success_ec,
	          ...
	          &io_uring_descriptor_read_op_base::do_perform,
	          complete_func)
	...
	{
		ASIO_ASSUME(base != 0);
		io_uring_descriptor_read_op_base* o(static_cast<io_uring_descriptor_read_op_base*>(base));
		...
		if ((o->state_ & descriptor_ops::internal_non_blocking) != 0) {
			...
			::io_uring_prep_readv(sqe, o->descriptor_, o->bufs_.buffers(), o->bufs_.count(), -1);
		}
	}
	...
	static bool do_perform(io_uring_operation* base, bool after_completion)
	{
		...
		if ((o->state_ & descriptor_ops::internal_non_blocking) != 0) {
			if (o->bufs_.is_single_buffer) {
				return descriptor_ops::non_blocking_read1(o->descriptor_, o->bufs_.first(o->buffers_).data(),
...
class io_uring_descriptor_read_op : public io_uring_descriptor_read_op_base<MutableBufferSequence> {
public:
	...
	io_uring_descriptor_read_op(const asio::error_code& success_ec,
	    ...
	    : io_uring_descriptor_read_op_base<MutableBufferSequence>(success_ec,
	          ...
	          state,
	          buffers,
	          &io_uring_descriptor_read_op::do_complete)
	...
	do_complete(void* owner, operation* base, const asio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
	{
		...
		ASIO_ASSUME(base != 0);
		io_uring_descriptor_read_op* o(static_cast<io_uring_descriptor_read_op*>(base));
