...
enum {
	max_signal_number = NSIG
};
#else
enum {
	max_signal_number = 128
};
#endif
...
class signal_set_service : public execution_context_service_base<signal_set_service> {
public:
	...
	add(implementation_type& impl, int signal_number, signal_set_base::flags_t f, asio::error_code& ec);
	...
	ASIO_DECL void cancel_ops_by_key(implementation_type& impl, void* cancellation_key);
	...
	void async_wait(implementation_type& impl, Handler& handler, const IoExecutor& io_ex)
	{
		...
		if (slot.is_connected()) {
			p.p->cancellation_key_ = &slot.template emplace<signal_op_cancellation>(this, &impl);
		}

		ASIO_HANDLER_CREATION((scheduler_.context(), *p.p, "signal_set", &impl, 0, "async_wait"));
