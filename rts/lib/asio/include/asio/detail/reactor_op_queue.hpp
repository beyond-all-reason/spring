...
template<typename Descriptor> class reactor_op_queue : private noncopyable {
public:
	...
	{
		std::pair<iterator, bool> entry = operations_.insert(value_type(descriptor, mapped_type()));
	...
	// restarted.
	bool
	cancel_operations(iterator i, op_queue<operation>& ops, const asio::error_code& ec = asio::error::operation_aborted)
	{
	...
	// demultiplexing function may need to be interrupted and restarted.
	bool cancel_operations_by_key(iterator i,
	    op_queue<operation>& ops,
	...
	// operations were cancelled, in which case the reactor's event
	// demultiplexing function may need to be interrupted and restarted.
	bool cancel_operations_by_key(Descriptor descriptor,
	    op_queue<operation>& ops,
	    void* cancellation_key,
	    const asio::error_code& ec = asio::error::operation_aborted)
	{
		return this->cancel_operations_by_key(operations_.find(descriptor), ops, cancellation_key, ec);
	}
	...
	bool has_operation(Descriptor descriptor) const { return operations_.find(descriptor) != operations_.end(); }
