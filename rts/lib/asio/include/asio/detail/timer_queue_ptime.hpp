...
template<> class timer_queue<time_traits<boost::posix_time::ptime>> : public timer_queue_base {
public:
	// The time type.
	...
	// Per-timer data.
	typedef timer_queue<forwarding_posix_time_traits>::per_timer_data per_timer_data;
	...
	ASIO_DECL bool enqueue_timer(const time_type& time, per_timer_data& timer, wait_op* op);
	...
	ASIO_DECL std::size_t cancel_timer(per_timer_data& timer,
	    op_queue<operation>& ops,
	    std::size_t max_cancelled = (std::numeric_limits<std::size_t>::max)());

	// Cancel and dequeue operations for the given timer and key.
	ASIO_DECL void cancel_timer_by_key(per_timer_data* timer, op_queue<operation>& ops, void* cancellation_key);

	// Move operations from one timer to another, empty timer.
	ASIO_DECL void move_timer(per_timer_data& target, per_timer_data& source);
