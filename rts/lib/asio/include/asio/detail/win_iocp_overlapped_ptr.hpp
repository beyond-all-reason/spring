...
class win_iocp_overlapped_ptr : private noncopyable {
public:
	...
	explicit win_iocp_overlapped_ptr(const Executor& ex, Handler&& handler)
	    : ptr_(0)
	    , iocp_service_(0)
	{
	...
	// object.
	template<typename Executor, typename Handler> void reset(const Executor& ex, Handler handler)
	{
		...
		ASIO_HANDLER_CREATION((ex.context(), *p.p, "iocp_service", iocp_service, 0, "overlapped"));
	...
	OVERLAPPED* get() { return ptr_; }

	// Get the contained OVERLAPPED object.
	const OVERLAPPED* get() const { return ptr_; }
...
private:
	...
	static win_iocp_io_context* get_iocp_service(const Executor& ex,
	    enable_if_t<can_query<const Executor&, execution::context_t>::value>* = 0)
	{
		return &use_service<win_iocp_io_context>(asio::query(ex, execution::context));
	}
	...
	static win_iocp_io_context* get_iocp_service(const io_context::executor_type& ex)
	{
