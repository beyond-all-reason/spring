...
template<typename MutableBufferSequence> class io_uring_socket_recv_op_base : public io_uring_operation {
public:
	io_uring_socket_recv_op_base(const asio::error_code& success_ec,
	    ...
	    : io_uring_operation(success_ec,
	          ...
	          &io_uring_socket_recv_op_base::do_perform,
	          complete_func)
	    , socket_(socket)
	    , state_(state)
	    , buffers_(buffers)
	...
	static void do_prepare(io_uring_operation* base, ::io_uring_sqe* sqe)
	{
		...
		if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
			...
			::io_uring_prep_read_fixed(sqe, o->socket_, o->bufs_.buffers()->iov_base, o->bufs_.buffers()->iov_len, 0,
		...
		if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
			...
			if (after_completion || !except_op) {
				if (o->bufs_.is_single_buffer) {
					return socket_ops::non_blocking_recv1(o->socket_, o->bufs_.first(o->buffers_).data(),
					    o->bufs_.first(o->buffers_).size(), o->flags_, (o->state_ & socket_ops::stream_oriented) != 0,
					    o->ec_, o->bytes_transferred_);
				}
				else {
					return socket_ops::non_blocking_recv(o->socket_, o->bufs_.buffers(), o->bufs_.count(), o->flags_,
					    (o->state_ & socket_ops::stream_oriented) != 0, o->ec_, o->bytes_transferred_);
				}
			}
		}
...
class io_uring_socket_recv_op : public io_uring_socket_recv_op_base<MutableBufferSequence> {
public:
	...
	io_uring_socket_recv_op(const asio::error_code& success_ec,
	    ...
	    : io_uring_socket_recv_op_base<MutableBufferSequence>(success_ec,
	          ...
	          buffers,
	          flags,
	          &io_uring_socket_recv_op::do_complete)
	...
	do_complete(void* owner, operation* base, const asio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
	{
		...
		ASIO_ASSUME(base != 0);
		io_uring_socket_recv_op* o(static_cast<io_uring_socket_recv_op*>(base));
