...
public:
	...
	public:
		...
		ASIO_DECL explicit location(const char* file, int line, const char* func);
	...
	ASIO_DECL static void creation(execution_context& context,
	    tracked_handler& h,
	...
	ASIO_DECL static void
	reactor_registration(execution_context& context, uintmax_t native_handle, uintmax_t registration);

	// Record that a descriptor has been deregistered from the reactor.
	ASIO_DECL static void
	reactor_deregistration(execution_context& context, uintmax_t native_handle, uintmax_t registration);

	// Record a reactor-based operation that is associated with a handler.
	ASIO_DECL static void reactor_events(execution_context& context, uintmax_t registration, unsigned events);

	// Record a reactor-based operation that is associated with a handler.
	ASIO_DECL static void reactor_operation(const tracked_handler& h, const char* op_name, const asio::error_code& ec);

	// Record a reactor-based operation that is associated with a handler.
	ASIO_DECL static void reactor_operation(const tracked_handler& h,
	    const char* op_name,
...
#define ASIO_HANDLER_LOCATION(args) asio::detail::handler_tracking::location tracked_location args
...
#define ASIO_HANDLER_REACTOR_REGISTRATION(args) asio::detail::handler_tracking::reactor_registration args

#define ASIO_HANDLER_REACTOR_DEREGISTRATION(args) asio::detail::handler_tracking::reactor_deregistration args
...
#define ASIO_HANDLER_REACTOR_EVENTS(args) asio::detail::handler_tracking::reactor_events args

#define ASIO_HANDLER_REACTOR_OPERATION(args) asio::detail::handler_tracking::reactor_operation args
