...
template<typename MutableBufferSequence> class io_uring_socket_recvmsg_op_base : public io_uring_operation {
public:
	io_uring_socket_recvmsg_op_base(const asio::error_code& success_ec,
	    ...
	    : io_uring_operation(success_ec,
	          ...
	          &io_uring_socket_recvmsg_op_base::do_perform,
	          complete_func)
	    , socket_(socket)
	    , state_(state)
	    , buffers_(buffers)
	...
	{
		ASIO_ASSUME(base != 0);
		io_uring_socket_recvmsg_op_base* o(static_cast<io_uring_socket_recvmsg_op_base*>(base));
	...
	static bool do_perform(io_uring_operation* base, bool after_completion)
	{
		...
		if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
			...
			if (after_completion || !except_op) {
				return socket_ops::non_blocking_recvmsg(o->socket_, o->bufs_.buffers(), o->bufs_.count(), o->in_flags_,
...
class io_uring_socket_recvmsg_op : public io_uring_socket_recvmsg_op_base<MutableBufferSequence> {
public:
	...
	do_complete(void* owner, operation* base, const asio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
	{
		...
		ASIO_ASSUME(base != 0);
		io_uring_socket_recvmsg_op* o(static_cast<io_uring_socket_recvmsg_op*>(base));
