...
class posix_serial_port_service : public execution_context_service_base<posix_serial_port_service> {
public:
#if defined(ASIO_HAS_IO_URING_AS_DEFAULT)
	...
	void construct(implementation_type& impl) { descriptor_service_.construct(impl); }
	...
	void
	move_assign(implementation_type& impl, posix_serial_port_service& other_service, implementation_type& other_impl)
	{
		descriptor_service_.move_assign(impl, other_service.descriptor_service_, other_impl);
	}

	// Destroy a serial port implementation.
	void destroy(implementation_type& impl) { descriptor_service_.destroy(impl); }
	...
	bool is_open(const implementation_type& impl) const { return descriptor_service_.is_open(impl); }
	...
	native_handle_type native_handle(implementation_type& impl) { return descriptor_service_.native_handle(impl); }
	...
	{
		return do_set_option(impl, &posix_serial_port_service::store_option<SettableSerialPortOption>, &option, ec);
	}
	...
	{
		return do_get_option(impl, &posix_serial_port_service::load_option<GettableSerialPortOption>, &option, ec);
	}
...
private:
	...
	typedef asio::error_code (*store_function_type)(const void*, termios&, asio::error_code&);

	// Helper function template to store a serial port option.
	template<typename SettableSerialPortOption>
	static asio::error_code store_option(const void* option, termios& storage, asio::error_code& ec)
	{
	...
	typedef asio::error_code (*load_function_type)(void*, const termios&, asio::error_code&);

	// Helper function template to load a serial port option.
	template<typename GettableSerialPortOption>
	static asio::error_code load_option(void* option, const termios& storage, asio::error_code& ec)
	{
