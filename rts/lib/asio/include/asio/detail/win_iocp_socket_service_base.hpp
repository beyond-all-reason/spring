...
public:
	...
	void async_wait(base_implementation_type& impl, socket_base::wait_type w, Handler& handler, const IoExecutor& io_ex)
	{
		...
		ASIO_HANDLER_CREATION((context_, *p.p, "socket", &impl, impl.socket_, "async_wait"));
		...
		int op_type = -1;
		switch (w) {
		case socket_base::wait_read: op_type = start_null_buffers_receive_op(impl, 0, p.p, iocp_op); break;
	    ...
	    const IoExecutor& io_ex)
	{
		...
		ASIO_HANDLER_CREATION((context_, *p.p, "socket", &impl, impl.socket_, "async_send"));
		...
		start_send_op(impl, bufs.buffers(), bufs.count(), flags,
		    (impl.state_ & socket_ops::stream_oriented) != 0 && bufs.all_empty(), o);
		p.v = p.p = 0;
	}
	    ...
	    const IoExecutor& io_ex)
	{
		...
		ASIO_HANDLER_CREATION((context_, *p.p, "socket", &impl, impl.socket_, "async_send(null_buffers)"));
	    ...
	    const IoExecutor& io_ex)
	{
		...
		// Allocate and construct an operation to wrap the handler.
		typedef win_iocp_socket_recv_op<MutableBufferSequence, Handler, IoExecutor> op;
		typename op::ptr p = {asio::detail::addressof(handler), op::ptr::allocate(handler), 0};
		operation* o = p.p = new (p.v) op(impl.state_, impl.cancel_token_, buffers, handler, io_ex);

		ASIO_HANDLER_CREATION((context_, *p.p, "socket", &impl, impl.socket_, "async_receive"));
	    ...
	    const IoExecutor& io_ex)
	{
		...
		ASIO_HANDLER_CREATION((context_, *p.p, "socket", &impl, impl.socket_, "async_receive(null_buffers)"));
	    ...
	    const IoExecutor& io_ex)
	{
		...
		// Allocate and construct an operation to wrap the handler.
		typedef win_iocp_socket_recvmsg_op<MutableBufferSequence, Handler, IoExecutor> op;
		typename op::ptr p = {asio::detail::addressof(handler), op::ptr::allocate(handler), 0};
		operation* o = p.p = new (p.v) op(impl.cancel_token_, buffers, out_flags, handler, io_ex);

		ASIO_HANDLER_CREATION((context_, *p.p, "socket", &impl, impl.socket_, "async_receive_with_flags"));
	    ...
	    const IoExecutor& io_ex)
	{
		...
		ASIO_HANDLER_CREATION(
		    (context_, *p.p, "socket", &impl, impl.socket_, "async_receive_with_flags(null_buffers)"));
...
protected:
	...
	do_open(base_implementation_type& impl, int family, int type, int protocol, asio::error_code& ec);

	// Assign a native socket to a socket implementation.
	ASIO_DECL asio::error_code
	do_assign(base_implementation_type& impl, int type, socket_type native_socket, asio::error_code& ec);
	...
	ASIO_DECL int start_null_buffers_receive_op(base_implementation_type& impl,
	    socket_base::message_flags flags,
	    reactor_op* op,
	    operation* iocp_op);
	...
	ASIO_DECL void start_reactor_op(base_implementation_type& impl, int op_type, reactor_op* op);
	...
	ASIO_DECL void update_cancellation_thread_id(base_implementation_type& impl);
	...
	typedef BOOL(PASCAL* connect_ex_fn)(SOCKET, const socket_addr_type*, int, void*, DWORD, DWORD*, OVERLAPPED*);
	...
	ASIO_DECL connect_ex_fn get_connect_ex(base_implementation_type& impl, int type);
	...
	ASIO_DECL void* interlocked_compare_exchange_pointer(void** dest, void* exch, void* cmp);
	...
	public:
		...
		static void
		do_complete(void* owner, operation* base, const asio::error_code& result_ec, std::size_t bytes_transferred)
		{
	...
	public:
		...
		long* get_cancel_requested() { return &cancel_requested_; }
	...
	public:
		...
		void use_reactor(select_reactor* r, select_reactor::per_descriptor_data* p, int o)
		{
			reactor_ = r;
			reactor_data_ = p;
			op_type_ = o;
		...
		void operator()(cancellation_type_t type)
		{
			if (!!(type & (cancellation_type::terminal | cancellation_type::partial | cancellation_type::total))) {
				if (reactor_) {
					reactor_->cancel_ops_by_key(socket_, *reactor_data_, op_type_, this);
				}
				else {
#if defined(_WIN32_WINNT) && (_WIN32_WINNT >= 0x0600)
					HANDLE sock_as_handle = reinterpret_cast<HANDLE>(socket_);
