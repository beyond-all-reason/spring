...
template<typename Buffers> struct prepared_buffers_max {
	enum {
		value = buffer_sequence_adapter_base::max_buffers
	};
};

template<typename Elem, std::size_t N> struct prepared_buffers_max<boost::array<Elem, N>> {
	enum {
		value = N
	};
};

template<typename Elem, std::size_t N> struct prepared_buffers_max<std::array<Elem, N>> {
	enum {
		value = N
	};
};
...
template<typename Buffer, std::size_t MaxBuffers> struct prepared_buffers {
	typedef Buffer value_type;
	typedef const Buffer* const_iterator;

	enum {
		max_buffers = MaxBuffers < 16 ? MaxBuffers : 16
	};
...
template<typename Buffer, typename Buffers, typename Buffer_Iterator> class consuming_buffers {
public:
	typedef prepared_buffers<Buffer, prepared_buffers_max<Buffers>::value> prepared_buffers_type;
	...
	std::size_t total_consumed() const { return total_consumed_; }
...
template<typename Buffer> class consuming_single_buffer {
public:
	// Construct to represent the entire list of buffers.
	...
	// Determine if we are at the end of the buffers.
	bool empty() const { return total_consumed_ >= buffer_.size(); }

	// Get the buffer for a single transfer, with a size.
	Buffer prepare(std::size_t max_size) { return asio::buffer(buffer_ + total_consumed_, max_size); }

	// Consume the specified number of bytes from the buffers.
	void consume(std::size_t size) { total_consumed_ += size; }
...
template<>
class consuming_buffers<mutable_buffer, mutable_buffers_1, const mutable_buffer*>
    : public consuming_single_buffer<ASIO_MUTABLE_BUFFER> {
public:
...
template<>
class consuming_buffers<mutable_buffer, mutable_registered_buffer, const mutable_buffer*>
...
template<>
class consuming_buffers<const_buffer, mutable_registered_buffer, const mutable_buffer*>
    : public consuming_single_buffer<mutable_registered_buffer> {
public:
	explicit consuming_buffers(const mutable_registered_buffer& buffer)
	    : consuming_single_buffer<mutable_registered_buffer>(buffer)
...
template<>
class consuming_buffers<const_buffer, const_registered_buffer, const const_buffer*>
...
class consuming_buffers<Buffer, boost::array<Elem, 2>, typename boost::array<Elem, 2>::const_iterator> {
public:
	// Construct to represent the entire list of buffers.
	...
	// Determine if we are at the end of the buffers.
	bool empty() const { return total_consumed_ >= Buffer(buffers_[0]).size() + Buffer(buffers_[1]).size(); }
	...
	boost::array<Buffer, 2> prepare(std::size_t max_size)
	{
		...
		result[1] = asio::buffer(result[1] + (total_consumed_ < buffer0_size ? 0 : total_consumed_ - buffer0_size),
...
template<typename Buffer, typename Elem>
class consuming_buffers<Buffer, std::array<Elem, 2>, typename std::array<Elem, 2>::const_iterator> {
public:
	// Construct to represent the entire list of buffers.
...
class consuming_buffers<Buffer, null_buffers, const mutable_buffer*> : public asio::null_buffers {
public:
	...
	null_buffers prepare(std::size_t) { return null_buffers(); }
	...
	std::size_t total_consumed() const { return 0; }
};
