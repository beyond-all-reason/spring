...
public:
	...
	template<typename Lock> void unlock_and_signal_one_for_destruction(Lock& lock)
	{
		ASIO_ASSERT(lock.locked());
		state_ |= 1;
		bool have_waiters = (state_ > 1);
    ...
    (defined(__ANDROID__) && (__ANDROID_API__ < 21) && defined(HAVE_PTHREAD_COND_TIMEDWAIT_RELATIVE))
			...
			::pthread_cond_timedwait_relative_np(&cond_, &lock.mutex().mutex_, &ts); // Ignore EINVAL.
