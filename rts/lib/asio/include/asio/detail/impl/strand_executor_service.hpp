...
template<typename F, typename Allocator> class strand_executor_service::allocator_binder {
public:
...
class strand_executor_service::invoker<Executor, enable_if_t<execution::is_executor<Executor>::value>> {
public:
	...
	struct on_invoker_exit {
		...
		~on_invoker_exit()
		{
			if (push_waiting_to_ready(this_->impl_)) {
				...
				asio::prefer(asio::require(static_cast<executor_type&&>(ex), execution::blocking.never),
    ...
    enable_if_t<can_query<Executor, execution::allocator_t<void>>::value>*)
{
	return strand_executor_service::do_execute(
	    impl, ex, static_cast<Function&&>(function), asio::query(ex, execution::allocator));
}
...
inline void strand_executor_service::execute(const implementation_type& impl,
    ...
    enable_if_t<!can_query<Executor, execution::allocator_t<void>>::value>*)
{
	return strand_executor_service::do_execute(impl, ex, static_cast<Function&&>(function), std::allocator<void>());
}
    ...
    const Allocator& a)
{
	...
	if (asio::query(ex, execution::blocking) != execution::blocking.never && running_in_this_thread(impl)) {
	...
	ASIO_HANDLER_CREATION((impl->service_->context(), *p.p, "strand_executor", impl.get(), 0, "execute"));
    ...
    const Allocator& a)
{
	...
	ASIO_HANDLER_CREATION((impl->service_->context(), *p.p, "strand_executor", impl.get(), 0, "dispatch"));

	// Add the function to the strand and schedule the strand if required.
	bool first = enqueue(impl, p.p);
	p.v = p.p = 0;
	if (first) {
		asio::dispatch(ex, allocator_binder<invoker<Executor>, Allocator>(invoker<Executor>(impl, ex), a));
	}
}
    ...
    const Allocator& a)
{
	...
	ASIO_HANDLER_CREATION((impl->service_->context(), *p.p, "strand_executor", impl.get(), 0, "post"));

	// Add the function to the strand and schedule the strand if required.
	bool first = enqueue(impl, p.p);
	p.v = p.p = 0;
	if (first) {
		asio::post(ex, allocator_binder<invoker<Executor>, Allocator>(invoker<Executor>(impl, ex), a));
	}
}
    ...
    const Allocator& a)
{
	...
	ASIO_HANDLER_CREATION((impl->service_->context(), *p.p, "strand_executor", impl.get(), 0, "defer"));

	// Add the function to the strand and schedule the strand if required.
	bool first = enqueue(impl, p.p);
	p.v = p.p = 0;
	if (first) {
		asio::defer(ex, allocator_binder<invoker<Executor>, Allocator>(invoker<Executor>(impl, ex), a));
	}
}
