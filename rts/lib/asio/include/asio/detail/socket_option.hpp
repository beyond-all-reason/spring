...
public:
	...
	template<typename Protocol> int level(const Protocol&) const { return Level; }
	...
	template<typename Protocol> void resize(const Protocol&, std::size_t s)
	{
		...
		switch (s) {
		case sizeof(char): value_ = *reinterpret_cast<char*>(&value_) ? 1 : 0; break;
		case sizeof(value_): break;
		default: {
...
public:
	...
	int timeout ASIO_PREVENT_MACRO_SUBSTITUTION() const { return static_cast<int>(value_.l_linger); }
	...
	template<typename Protocol> detail::linger_type* data(const Protocol&) { return &value_; }

	// Get the address of the linger data.
	template<typename Protocol> const detail::linger_type* data(const Protocol&) const { return &value_; }
