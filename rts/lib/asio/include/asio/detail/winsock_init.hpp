...
protected:
	...
	ASIO_DECL static void startup(data& d, unsigned char major, unsigned char minor);
...
template<int Major = 2, int Minor = 2> class winsock_init : private winsock_init_base {
public:
	...
	class manual {
	public:
		manual() { manual_startup(data_); }

		manual(const manual&) { manual_startup(data_); }

		~manual() { manual_cleanup(data_); }
	};
...
template<int Major, int Minor> winsock_init_base::data winsock_init<Major, Minor>::data_;
