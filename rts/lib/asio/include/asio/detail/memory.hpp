...
template<typename T> inline T* to_address(T* p) { return p; }

template<typename T> inline const T* to_address(const T* p) { return p; }

template<typename T> inline volatile T* to_address(volatile T* p) { return p; }

template<typename T> inline const volatile T* to_address(const volatile T* p) { return p; }
...
#define ASIO_USES_ALLOCATOR(t)                                                       \
	namespace std {                                                                  \
	template<typename Allocator> struct uses_allocator<t, Allocator> : true_type {}; \
	}                                                                                \
	/**/
#define ASIO_REBIND_ALLOC(alloc, t) typename std::allocator_traits<alloc>::template rebind_alloc<t>
