...
class deadline_timer_service : public execution_context_service_base<deadline_timer_service<Time_Traits>> {
public:
	// The time type.
	...
	struct implementation_type : private asio::detail::noncopyable {
		time_type expiry;
	...
	std::size_t cancel(implementation_type& impl, asio::error_code& ec)
	{
		...
		ASIO_HANDLER_OPERATION((scheduler_.context(), "deadline_timer", &impl, 0, "cancel"));
	...
	std::size_t cancel_one(implementation_type& impl, asio::error_code& ec)
	{
		...
		ASIO_HANDLER_OPERATION((scheduler_.context(), "deadline_timer", &impl, 0, "cancel_one"));
	...
	time_type expiry(const implementation_type& impl) const { return impl.expiry; }
	...
	{
		return expires_at(impl, Time_Traits::add(Time_Traits::now(), expiry_time), ec);
	}
	...
	void wait(implementation_type& impl, asio::error_code& ec)
	{
		...
		while (Time_Traits::less_than(now, impl.expiry) && !ec) {
			this->do_wait(Time_Traits::to_posix_duration(Time_Traits::subtract(impl.expiry, now)), ec);
			now = Time_Traits::now();
		}
	}
	...
	void async_wait(implementation_type& impl, Handler& handler, const IoExecutor& io_ex)
	{
		...
		if (slot.is_connected()) {
			p.p->cancellation_key_ = &slot.template emplace<op_cancellation>(this, &impl.timer_data);
		}

		impl.might_have_pending_waits = true;

		ASIO_HANDLER_CREATION((scheduler_.context(), *p.p, "deadline_timer", &impl, 0, "async_wait"));
...
private:
	...
	template<typename Duration> void do_wait(const Duration& timeout, asio::error_code& ec)
	{
#if defined(ASIO_WINDOWS_RUNTIME)
		std::this_thread::sleep_for(
		    std::chrono::seconds(timeout.total_seconds()) + std::chrono::microseconds(timeout.total_microseconds()));
		ec = asio::error_code();
	...
	public:
		op_cancellation(deadline_timer_service* s, typename timer_queue<Time_Traits>::per_timer_data* p)
		    : service_(s)
		    , timer_data_(p)
		{
		}
		...
		void operator()(cancellation_type_t type)
		{
			if (!!(type & (cancellation_type::terminal | cancellation_type::partial | cancellation_type::total))) {
				service_->scheduler_.cancel_timer_by_key(service_->timer_queue_, timer_data_, this);
			}
		}
