...
public:
	io_uring_socket_recvfrom_op_base(const asio::error_code& success_ec,
	    ...
	    : io_uring_operation(success_ec,
	          ...
	          &io_uring_socket_recvfrom_op_base::do_perform,
	          complete_func)
	    , socket_(socket)
	    , state_(state)
	    , buffers_(buffers)
	    ...
	    , msghdr_()
	{
		...
		msghdr_.msg_name = static_cast<sockaddr*>(static_cast<void*>(sender_endpoint_.data()));
	...
	{
		ASIO_ASSUME(base != 0);
		io_uring_socket_recvfrom_op_base* o(static_cast<io_uring_socket_recvfrom_op_base*>(base));
		...
		if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
			...
			if (after_completion || !except_op) {
				...
				if (o->bufs_.is_single_buffer) {
					result = socket_ops::non_blocking_recvfrom1(o->socket_, o->bufs_.first(o->buffers_).data(),
					    o->bufs_.first(o->buffers_).size(), o->flags_, o->sender_endpoint_.data(), &addr_len, o->ec_,
					    o->bytes_transferred_);
				}
				else {
					result = socket_ops::non_blocking_recvfrom(o->socket_, o->bufs_.buffers(), o->bufs_.count(),
					    o->flags_, o->sender_endpoint_.data(), &addr_len, o->ec_, o->bytes_transferred_);
				}
				if (result && !o->ec_)
...
class io_uring_socket_recvfrom_op : public io_uring_socket_recvfrom_op_base<MutableBufferSequence, Endpoint> {
public:
	...
	io_uring_socket_recvfrom_op(const asio::error_code& success_ec,
	    ...
	    : io_uring_socket_recvfrom_op_base<MutableBufferSequence, Endpoint>(success_ec,
	          socket,
	          state,
	          buffers,
	          endpoint,
	...
	do_complete(void* owner, operation* base, const asio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
	{
		...
		ASIO_ASSUME(base != 0);
		io_uring_socket_recvfrom_op* o(static_cast<io_uring_socket_recvfrom_op*>(base));
