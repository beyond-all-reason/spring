...
public:
	io_uring_socket_sendto_op_base(const asio::error_code& success_ec,
	    ...
	    : io_uring_operation(success_ec,
	          ...
	          &io_uring_socket_sendto_op_base::do_perform,
	          complete_func)
	    , socket_(socket)
	    , state_(state)
	    , buffers_(buffers)
	    ...
	    , msghdr_()
	{
		...
		msghdr_.msg_name = static_cast<sockaddr*>(static_cast<void*>(destination_.data()));
	...
	{
		ASIO_ASSUME(base != 0);
		io_uring_socket_sendto_op_base* o(static_cast<io_uring_socket_sendto_op_base*>(base));
	...
	static bool do_perform(io_uring_operation* base, bool after_completion)
	{
		...
		if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
			if (o->bufs_.is_single_buffer) {
				return socket_ops::non_blocking_sendto1(o->socket_, o->bufs_.first(o->buffers_).data(),
				    o->bufs_.first(o->buffers_).size(), o->flags_, o->destination_.data(), o->destination_.size(),
				    o->ec_, o->bytes_transferred_);
			}
			else {
				return socket_ops::non_blocking_sendto(o->socket_, o->bufs_.buffers(), o->bufs_.count(), o->flags_,
...
class io_uring_socket_sendto_op : public io_uring_socket_sendto_op_base<ConstBufferSequence, Endpoint> {
public:
	...
	io_uring_socket_sendto_op(const asio::error_code& success_ec,
	    ...
	    : io_uring_socket_sendto_op_base<ConstBufferSequence, Endpoint>(success_ec,
	          socket,
	          state,
	          buffers,
	          endpoint,
	...
	do_complete(void* owner, operation* base, const asio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
	{
		...
		ASIO_ASSUME(base != 0);
		io_uring_socket_sendto_op* o(static_cast<io_uring_socket_sendto_op*>(base));
