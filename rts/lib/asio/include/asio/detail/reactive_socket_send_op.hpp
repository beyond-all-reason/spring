...
template<typename ConstBufferSequence> class reactive_socket_send_op_base : public reactor_op {
public:
	reactive_socket_send_op_base(const asio::error_code& success_ec,
	    ...
	    : reactor_op(success_ec, &reactive_socket_send_op_base::do_perform, complete_func)
	    , socket_(socket)
	    , state_(state)
	    , buffers_(buffers)
	    , flags_(flags)
	...
	static status do_perform(reactor_op* base)
	{
		...
		if (bufs_type::is_single_buffer) {
			result = socket_ops::non_blocking_send1(o->socket_, bufs_type::first(o->buffers_).data(),
			             bufs_type::first(o->buffers_).size(), o->flags_, o->ec_, o->bytes_transferred_) ?
			             done :
			             not_done;
			...
			result = socket_ops::non_blocking_send(
			             o->socket_, bufs.buffers(), bufs.count(), o->flags_, o->ec_, o->bytes_transferred_) ?
			             done :
			             not_done;
...
public:
	...
	reactive_socket_send_op(const asio::error_code& success_ec,
	    ...
	    : reactive_socket_send_op_base<ConstBufferSequence>(success_ec,
	          socket,
	          state,
	          buffers,
	          flags,
