...
template<> struct composed_work_guard<system_executor> {
public:
	...
	executor_type get_executor() const noexcept { return system_executor(); }
};

#if !defined(ASIO_NO_TS_EXECUTORS)

template<typename Executor>
struct composed_work_guard<Executor, enable_if_t<!execution::is_executor<Executor>::value>>
...
inline composed_io_executors<void()> make_composed_io_executors() { return composed_io_executors<void()>(); }
...
template<typename Head> inline composed_io_executors<void(Head)> make_composed_io_executors(const Head& head)
{
	return composed_io_executors<void(Head)>(head);
}

template<typename Head, typename... Tail> struct composed_io_executors<void(Head, Tail...)> {
	explicit composed_io_executors(const Head& head, const Tail&... tail) noexcept
	    : head_(head)
	    , tail_(tail...)
	{
	}
...
inline composed_io_executors<void(Head, Tail...)> make_composed_io_executors(const Head& head, const Tail&... tail)
{
...
template<> struct composed_work<void()> {
	...
	void reset() { head_.reset(); }

	typedef system_executor head_type;
	composed_work_guard<system_executor> head_;
};

template<typename Head> struct composed_work<void(Head)> {
...
template<typename Head, typename... Tail> struct composed_work<void(Head, Tail...)> {
...
inline typename IoObject::executor_type get_composed_io_executor(IoObject& io_object,
    enable_if_t<!is_executor<IoObject>::value>* = 0,
    enable_if_t<!execution::is_executor<IoObject>::value>* = 0)
{
...
inline const Executor& get_composed_io_executor(const Executor& ex,
    enable_if_t<is_executor<Executor>::value || execution::is_executor<Executor>::value>* = 0)
{
	return ex;
}
