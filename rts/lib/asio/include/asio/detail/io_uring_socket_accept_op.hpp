...
template<typename Socket, typename Protocol> class io_uring_socket_accept_op_base : public io_uring_operation {
public:
	io_uring_socket_accept_op_base(const asio::error_code& success_ec,
	    ...
	    : io_uring_operation(success_ec,
	          ...
	          &io_uring_socket_accept_op_base::do_perform,
	          complete_func)
	    , socket_(socket)
	    , state_(state)
	    , peer_(peer)
	...
	{
		ASIO_ASSUME(base != 0);
		io_uring_socket_accept_op_base* o(static_cast<io_uring_socket_accept_op_base*>(base));

		if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
			::io_uring_prep_poll_add(sqe, o->socket_, POLLIN);
		}
		else {
			::io_uring_prep_accept(sqe, o->socket_, o->peer_endpoint_ ? o->peer_endpoint_->data() : 0,
	...
	static bool do_perform(io_uring_operation* base, bool after_completion)
	{
		...
		if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
			...
			bool result = socket_ops::non_blocking_accept(o->socket_, o->state_,
			    o->peer_endpoint_ ? o->peer_endpoint_->data() : 0, o->peer_endpoint_ ? &addrlen : 0, o->ec_,
			    new_socket);
...
template<typename Socket, typename Protocol, typename Handler, typename IoExecutor>
class io_uring_socket_accept_op : public io_uring_socket_accept_op_base<Socket, Protocol> {
public:
	...
	io_uring_socket_accept_op(const asio::error_code& success_ec,
	    ...
	    : io_uring_socket_accept_op_base<Socket, Protocol>(success_ec,
	          socket,
	          state,
	          peer,
	          protocol,
	...
	static void
	do_complete(void* owner, operation* base, const asio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
	{
		...
		handler_work<Handler, IoExecutor> w(static_cast<handler_work<Handler, IoExecutor>&&>(o->work_));
...
template<typename Protocol, typename PeerIoExecutor, typename Handler, typename IoExecutor>
class io_uring_socket_move_accept_op
    : private Protocol::socket::template rebind_executor<PeerIoExecutor>::other,
      public io_uring_socket_accept_op_base<typename Protocol::socket::template rebind_executor<PeerIoExecutor>::other,
          Protocol> {
public:
	...
	io_uring_socket_move_accept_op(const asio::error_code& success_ec,
	    ...
	    , io_uring_socket_accept_op_base<peer_socket_type, Protocol>(success_ec,
	          socket,
	          state,
	          *this,
	          protocol,
	...
	do_complete(void* owner, operation* base, const asio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
	{
		// Take ownership of the handler object.
		ASIO_ASSUME(base != 0);
		io_uring_socket_move_accept_op* o(static_cast<io_uring_socket_move_accept_op*>(base));
		ptr p = {asio::detail::addressof(o->handler_), o, o};
		...
		// to ensure that any owning sub-object remains valid until after we have
		// deallocated the memory here.
		detail::move_binder2<Handler, asio::error_code, peer_socket_type> handler(
		    0, static_cast<Handler&&>(o->handler_), o->ec_, static_cast<peer_socket_type&&>(*o));
		p.h = asio::detail::addressof(handler.handler_);
		p.reset();
...
private:
	typedef typename Protocol::socket::template rebind_executor<PeerIoExecutor>::other peer_socket_type;
