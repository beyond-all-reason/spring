...
template<typename ConstBufferSequence> class io_uring_socket_send_op_base : public io_uring_operation {
public:
	io_uring_socket_send_op_base(const asio::error_code& success_ec,
	    ...
	    : io_uring_operation(success_ec,
	          ...
	          &io_uring_socket_send_op_base::do_perform,
	          complete_func)
	    , socket_(socket)
	    , state_(state)
	    , buffers_(buffers)
	...
	{
		ASIO_ASSUME(base != 0);
		io_uring_socket_send_op_base* o(static_cast<io_uring_socket_send_op_base*>(base));

		if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
			::io_uring_prep_poll_add(sqe, o->socket_, POLLOUT);
		}
		else if (o->bufs_.is_single_buffer && o->bufs_.is_registered_buffer && o->flags_ == 0) {
			::io_uring_prep_write_fixed(sqe, o->socket_, o->bufs_.buffers()->iov_base, o->bufs_.buffers()->iov_len, 0,
			    o->bufs_.registered_id().native_handle());
		}
		else {
	...
	static bool do_perform(io_uring_operation* base, bool after_completion)
	{
		...
		if ((o->state_ & socket_ops::internal_non_blocking) != 0) {
			if (o->bufs_.is_single_buffer) {
				return socket_ops::non_blocking_send1(o->socket_, o->bufs_.first(o->buffers_).data(),
				    o->bufs_.first(o->buffers_).size(), o->flags_, o->ec_, o->bytes_transferred_);
			}
			else {
				return socket_ops::non_blocking_send(
				    o->socket_, o->bufs_.buffers(), o->bufs_.count(), o->flags_, o->ec_, o->bytes_transferred_);
			}
		}
...
class io_uring_socket_send_op : public io_uring_socket_send_op_base<ConstBufferSequence> {
public:
	...
	io_uring_socket_send_op(const asio::error_code& success_ec,
	    ...
	    : io_uring_socket_send_op_base<ConstBufferSequence>(success_ec,
	          ...
	          buffers,
	          flags,
	          &io_uring_socket_send_op::do_complete)
	...
	do_complete(void* owner, operation* base, const asio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
	{
		...
		ASIO_ASSUME(base != 0);
		io_uring_socket_send_op* o(static_cast<io_uring_socket_send_op*>(base));
