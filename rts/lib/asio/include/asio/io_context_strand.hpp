...
public:
	...
	explicit strand(asio::io_context& io_context)
	    : service_(asio::use_service<asio::detail::strand_service>(io_context))
	{
	...
	asio::io_context& context() const noexcept { return service_.get_io_context(); }

	/// Inform the strand that it has some outstanding work to do.
	/**
	 * The strand delegates this call to its underlying io_context.
	 */
	void on_work_started() const noexcept { context().get_executor().on_work_started(); }

	/// Inform the strand that some work is no longer outstanding.
	/**
	 * The strand delegates this call to its underlying io_context.
	 */
	void on_work_finished() const noexcept { context().get_executor().on_work_finished(); }
	...
	{
		return detail::wrapped_handler<io_context::strand, Handler, detail::is_continuation_if_running>(*this, handler);
	}
#endif // !defined(ASIO_NO_DEPRECATED)
	 ...
	 * returns @c false.
	 */
	bool running_in_this_thread() const noexcept { return service_.running_in_this_thread(impl_); }
...
private:
	...
	struct initiate_dispatch {
		template<typename LegacyCompletionHandler>
		void operator()(LegacyCompletionHandler&& handler, strand* self) const
		{
