...
#if defined(ASIO_HAS_BOOST_DATE_TIME) && defined(ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
...
template<typename Protocol, typename Clock = chrono::steady_clock, typename WaitTraits = wait_traits<Clock>>
...
public:
	 ...
	 * pointer otherwise.
	 */
	template<typename... T> basic_socket_streambuf* connect(T... x)
	{
		init_buffers();
	...
	basic_socket<Protocol>& socket() { return *this; }
	 ...
	 * buffer.
	 */
	const asio::error_code& puberror() const { return error(); }
	 ...
	 * time.
	 */
	time_point expires_at() const { return expiry_time_; }
#endif // !defined(ASIO_NO_DEPRECATED)
	 ...
	 */
	void expires_at(const time_point& expiry_time) { expiry_time_ = expiry_time; }
	 ...
	 */
	duration expires_from_now() const { return traits_helper::subtract(expires_at(), traits_helper::now()); }
...
protected:
	int_type underflow()
	{
		...
		for (;;) {
			...
			detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer> bufs(
			    asio::buffer(get_buffer_) + putback_max);
			detail::signed_size_type bytes =
			    detail::socket_ops::recv(socket().native_handle(), bufs.buffers(), bufs.count(), 0, ec_);

			// Check if operation succeeded.
			if (bytes > 0) {
				setg(&get_buffer_[0], &get_buffer_[0] + putback_max, &get_buffer_[0] + putback_max + bytes);
			...
			// Operation failed.
			if (ec_ != asio::error::would_block && ec_ != asio::error::try_again)
				return traits_type::eof();

			// Wait for socket to become ready.
			if (detail::socket_ops::poll_read(socket().native_handle(), 0, timeout(), ec_) < 0)
				return traits_type::eof();
		}
	...
	int_type overflow(int_type c)
	{
		...
		if (put_buffer_.empty()) {
			...
			output_buffer = asio::buffer(pbase(), (pptr() - pbase()) * sizeof(char_type));
		}
		...
		while (output_buffer.size() > 0) {
			...
			if (!socket().native_non_blocking())
				socket().native_non_blocking(true, ec_);
			detail::buffer_sequence_adapter<const_buffer, const_buffer> bufs(output_buffer);
			detail::signed_size_type bytes =
			    detail::socket_ops::send(socket().native_handle(), bufs.buffers(), bufs.count(), 0, ec_);
			...
			// Wait for socket to become ready.
			if (detail::socket_ops::poll_write(socket().native_handle(), 0, timeout(), ec_) < 0)
				return traits_type::eof();
		}
...
private:
	...
	int timeout() const
	{
		int64_t msec = traits_helper::to_posix_duration(traits_helper::subtract(expiry_time_, traits_helper::now()))
		                   .total_milliseconds();
	...
	template<typename EndpointSequence> void connect_to_endpoints(const EndpointSequence& endpoints)
	{
		this->connect_to_endpoints(endpoints.begin(), endpoints.end());
	}

	template<typename EndpointIterator> void connect_to_endpoints(EndpointIterator begin, EndpointIterator end)
	{
#if defined(ASIO_WINDOWS_RUNTIME)
		ec_ = asio::error::operation_not_supported;
#else  // defined(ASIO_WINDOWS_RUNTIME)
		...
		for (EndpointIterator i = begin; i != end; ++i) {
			...
			if (!socket().native_non_blocking())
				socket().native_non_blocking(true, ec_);
			detail::socket_ops::connect(socket().native_handle(), ep.data(), ep.size(), ec_);
			...
			// Operation failed.
			if (ec_ != asio::error::in_progress && ec_ != asio::error::would_block)
				continue;

			// Wait for socket to become ready.
			if (detail::socket_ops::poll_connect(socket().native_handle(), timeout(), ec_) < 0)
				continue;

			// Get the error code from the connect operation.
			int connect_error = 0;
			size_t connect_error_len = sizeof(connect_error);
			if (detail::socket_ops::getsockopt(socket().native_handle(), 0, SOL_SOCKET, SO_ERROR, &connect_error,
			        &connect_error_len, ec_) == detail::socket_error_retval)
				return;

			// Check the result of the connect operation.
			ec_ = asio::error_code(connect_error, asio::error::get_system_category());
			if (!ec_)
				return;
		}
#endif // defined(ASIO_WINDOWS_RUNTIME)
