...
template<cancellation_type_t Mask> struct cancellation_filter {
	...
	cancellation_type_t operator()(cancellation_type_t type) const noexcept { return type & Mask; }
};
...
typedef cancellation_filter<cancellation_type::terminal | cancellation_type::partial | cancellation_type::total>
    enable_total_cancellation;

#else // defined(GENERATING_DOCUMENTATION)

typedef cancellation_filter<static_cast<cancellation_type_t>(
    static_cast<unsigned int>(cancellation_type::terminal) | static_cast<unsigned int>(cancellation_type::partial))>
    enable_partial_cancellation;

typedef cancellation_filter<static_cast<cancellation_type_t>(static_cast<unsigned int>(cancellation_type::terminal) |
...
public:
	...
	constexpr cancellation_state(CancellationSlot slot, Filter filter)
	    : impl_(slot.is_connected() ? &slot.template emplace<impl<Filter, Filter>>(filter, filter) : 0)
	{
	}
	...
	constexpr cancellation_state(CancellationSlot slot, InFilter in_filter, OutFilter out_filter)
	    : impl_(slot.is_connected() ?
	                &slot.template emplace<impl<InFilter, OutFilter>>(static_cast<InFilter&&>(in_filter),
	 ...
	 */
	constexpr cancellation_slot slot() const noexcept { return impl_ ? impl_->signal_.slot() : cancellation_slot(); }

	/// Returns the cancellation types that have been triggered.
	cancellation_type_t cancelled() const noexcept { return impl_ ? impl_->cancelled_ : cancellation_type_t(); }

	/// Clears the specified cancellation types, if they have been triggered.
	void clear(cancellation_type_t mask = cancellation_type::all) noexcept
	{
		if (impl_)
...
private:
	...
	template<typename InFilter = enable_terminal_cancellation, typename OutFilter = InFilter> struct impl : impl_base {
		impl()
		    : in_filter_()
		    , out_filter_()
		{
