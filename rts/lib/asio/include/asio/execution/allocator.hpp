...
template<typename ProtoAllocator> struct allocator_t {
	 ...
	 */
	template<typename OtherAllocator> allocator_t < OtherAllocator operator()(const OtherAllocator& a);
};
...
template<typename ProtoAllocator> struct allocator_t {
	...
	struct query_static_constexpr_member
	    : traits::query_static_constexpr_member<typename static_proxy<T>::type, allocator_t> {};
	...
	constexpr ProtoAllocator value() const { return a_; }
...
#if defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT) && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)
template<typename ProtoAllocator> template<typename E, typename T> const T allocator_t<ProtoAllocator>::static_query_v;
#endif // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
       //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)
...
template<> struct allocator_t<void> {
	...
	constexpr allocator_t<OtherProtoAllocator> operator()(const OtherProtoAllocator& a) const
	{
...
#if defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT) && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)
template<typename E, typename T> const T allocator_t<void>::static_query_v;
#endif // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
       //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)
...
struct static_query<T,
    execution::allocator_t<ProtoAllocator>,
    enable_if_t<execution::allocator_t<ProtoAllocator>::template query_static_constexpr_member<T>::is_valid>> {
	static constexpr bool is_valid = true;
	static constexpr bool is_noexcept = true;

	typedef typename execution::allocator_t<ProtoAllocator>::template query_static_constexpr_member<T>::result_type
	    result_type;

	static constexpr result_type value()
	{
		return execution::allocator_t<ProtoAllocator>::template query_static_constexpr_member<T>::value();
	}
};
