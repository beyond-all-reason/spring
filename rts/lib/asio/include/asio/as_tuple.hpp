...
public:
	 ...
	 * location.
	 */
	constexpr as_tuple_t(default_constructor_tag = default_constructor_tag(), CompletionToken token = CompletionToken())
	...
	/// Constructor.
	template<typename T>
	constexpr explicit as_tuple_t(T&& completion_token)
...
ASIO_NODISCARD inline constexpr as_tuple_t<decay_t<CompletionToken>> as_tuple(CompletionToken&& completion_token)
{
	return as_tuple_t<decay_t<CompletionToken>>(static_cast<CompletionToken&&>(completion_token));
}
