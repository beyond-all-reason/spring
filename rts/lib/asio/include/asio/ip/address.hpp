...
public:
	...
	ASIO_DECL address& operator=(const asio::ip::address_v4& ipv4_address) noexcept;

	/// Assign from an IPv6 address.
	ASIO_DECL address& operator=(const asio::ip::address_v6& ipv6_address) noexcept;

	/// Get whether the address is an IP version 4 address.
	bool is_v4() const noexcept { return type_ == ipv4; }

	/// Get whether the address is an IP version 6 address.
	bool is_v6() const noexcept { return type_ == ipv6; }
	...
	ASIO_DECL friend bool operator==(const address& a1, const address& a2) noexcept;

	/// Compare two addresses for inequality.
	friend bool operator!=(const address& a1, const address& a2) noexcept { return !(a1 == a2); }

	/// Compare addresses for ordering.
	ASIO_DECL friend bool operator<(const address& a1, const address& a2) noexcept;

	/// Compare addresses for ordering.
	friend bool operator>(const address& a1, const address& a2) noexcept { return a2 < a1; }
 ...
 * @relates address
 */
ASIO_DECL address make_address(const char* str, asio::error_code& ec) noexcept;
 ...
 * @relates address
 */
ASIO_DECL address make_address(const std::string& str, asio::error_code& ec) noexcept;
 ...
 * @relates address
 */
ASIO_DECL address make_address(string_view str, asio::error_code& ec) noexcept;
...
template<> struct hash<asio::ip::address> {
	std::size_t operator()(const asio::ip::address& addr) const noexcept
	{
		return addr.is_v4() ? std::hash<asio::ip::address_v4>()(addr.to_v4()) :
