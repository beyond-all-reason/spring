...
public:
	...
	typedef detail::win_iocp_serial_port_service::native_handle_type native_handle_type;
#else
	typedef detail::posix_serial_port_service::native_handle_type native_handle_type;
#endif
	...
	{
		asio::error_code ec;
		impl_.get_service().assign(impl_.get_implementation(), native_serial_port, ec);
	...
	ASIO_SYNC_OP_VOID set_option(const SettableSerialPortOption& option, asio::error_code& ec)
	{
		impl_.get_service().set_option(impl_.get_implementation(), option, ec);
		ASIO_SYNC_OP_VOID_RETURN(ec);
	}
	 ...
	 * asio::serial_port_base::character_size
	 */
	template<typename GettableSerialPortOption> void get_option(GettableSerialPortOption& option) const
	{
		asio::error_code ec;
	...
	ASIO_SYNC_OP_VOID get_option(GettableSerialPortOption& option, asio::error_code& ec) const
	{
		impl_.get_service().get_option(impl_.get_implementation(), option, ec);
		ASIO_SYNC_OP_VOID_RETURN(ec);
	}
	...
	{
		asio::error_code ec;
		std::size_t s = impl_.get_service().write_some(impl_.get_implementation(), buffers, ec);
	...
	{
		return impl_.get_service().write_some(impl_.get_implementation(), buffers, ec);
	}
	...
	{
		asio::error_code ec;
		std::size_t s = impl_.get_service().read_some(impl_.get_implementation(), buffers, ec);
	...
	{
		return impl_.get_service().read_some(impl_.get_implementation(), buffers, ec);
	}
