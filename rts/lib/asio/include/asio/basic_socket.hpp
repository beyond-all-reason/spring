...
template<typename Protocol, typename Executor = any_io_executor> class basic_socket;
...
public:
	 ...
	 */
	template<typename ExecutionContext>
	basic_socket(ExecutionContext& context,
	...
	{
		asio::error_code ec;
		impl_.get_service().assign(impl_.get_implementation(), protocol, native_socket, ec);
	...
	template<typename Protocol1, typename Executor1> friend class basic_socket;
	 ...
	 */
	ASIO_SYNC_OP_VOID
	assign(const protocol_type& protocol, const native_handle_type& native_socket, asio::error_code& ec)
	{
		impl_.get_service().assign(impl_.get_implementation(), protocol, native_socket, ec);
		ASIO_SYNC_OP_VOID_RETURN(ec);
	}
	 ...
	 * mark.
	 */
	bool at_mark(asio::error_code& ec) const { return impl_.get_service().at_mark(impl_.get_implementation(), ec); }
	...
	{
		asio::error_code ec;
		std::size_t s = impl_.get_service().available(impl_.get_implementation(), ec);
	...
	void connect(const endpoint_type& peer_endpoint)
	{
		asio::error_code ec;
		if (!is_open()) {
			impl_.get_service().open(impl_.get_implementation(), peer_endpoint.protocol(), ec);
	 ...
	 */
	template<ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code)) ConnectToken = default_completion_token_t<executor_type>>
	auto async_connect(const endpoint_type& peer_endpoint,
	    ConnectToken&& token = default_completion_token_t<executor_type>())
	    -> decltype(async_initiate<ConnectToken, void(asio::error_code)>(declval<initiate_async_connect>(),
	        token,
	        peer_endpoint,
	...
	{
		asio::error_code ec;
		endpoint_type ep = impl_.get_service().remote_endpoint(impl_.get_implementation(), ec);
...
private:
	...
	public:
		...
		void
		operator()(ConnectHandler&& handler, const endpoint_type& peer_endpoint, const asio::error_code& open_ec) const
		{
			...
			if (open_ec) {
				asio::post(self_->impl_.get_executor(),
				    asio::detail::bind_handler(static_cast<ConnectHandler&&>(handler), open_ec));
			}
			else {
				detail::non_const_lvalue<ConnectHandler> handler2(handler);
				self_->impl_.get_service().async_connect(
				    self_->impl_.get_implementation(), peer_endpoint, handler2.value, self_->impl_.get_executor());
			}
		}
