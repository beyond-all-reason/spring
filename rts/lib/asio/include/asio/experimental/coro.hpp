...
struct coro {
	...
	~coro()
	{
		if (coro_ != nullptr) {
			...
			auto handle = detail::coroutine_handle<promise_type>::from_promise(*coro_);
			if (handle)
	...
	{
		return async_initiate<CompletionToken, typename traits::completion_handler>(initiate_async_resume(this), token);
	}
	...
	{
		return detail::coro_with_arg<std::decay_t<T>, coro>{std::forward<T>(ip), *this};
	}
...
private:
	...
	explicit coro(promise_type* const cr)
	    : coro_(cr)
	{
	}
