...
class basic_concurrent_channel
    ...
    : public detail::
          channel_send_functions<basic_concurrent_channel<Executor, Traits, Signatures...>, Executor, Signatures...>
...
private:
	...
	typedef typename service_type::template implementation_type<Traits, Signatures...>::payload_type payload_type;

	template<typename... PayloadSignatures, ASIO_COMPLETION_TOKEN_FOR(PayloadSignatures...) CompletionToken>
	auto do_async_receive(detail::channel_payload<PayloadSignatures...>*, CompletionToken&& token)
	    -> decltype(async_initiate<CompletionToken, PayloadSignatures...>(declval<initiate_async_receive>(), token))
	{
		return async_initiate<CompletionToken, PayloadSignatures...>(initiate_async_receive(this), token);
	}
...
public:
	 ...
	 * in the channel.
	 */
	basic_concurrent_channel(const executor_type& ex, std::size_t max_buffer_size = 0)
	    : service_(&asio::use_service<service_type>(basic_concurrent_channel::get_context(ex)))
	    , impl_()
	    , executor_(ex)
	{
	...
	template<typename, typename, typename...> friend class basic_concurrent_channel;
	...
	basic_concurrent_channel(basic_concurrent_channel<Executor1, Traits, Signatures...>&& other,
	...
	/// Destructor.
	~basic_concurrent_channel() { service_->destroy(impl_); }
	 ...
	 */
	template<typename... Args> std::size_t try_send_n(std::size_t count, Args&&... args);
	...
	template<typename CompletionToken ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
	auto async_receive(CompletionToken&& token ASIO_DEFAULT_COMPLETION_TOKEN(Executor))
#if !defined(GENERATING_DOCUMENTATION)
	    -> decltype(this->do_async_receive(static_cast<payload_type*>(0), static_cast<CompletionToken&&>(token)))
#endif // !defined(GENERATING_DOCUMENTATION)
	{
		return this->do_async_receive(static_cast<payload_type*>(0), static_cast<CompletionToken&&>(token));
	}

private:
	// Disallow copying and assignment.
	basic_concurrent_channel(const basic_concurrent_channel&) = delete;
	basic_concurrent_channel& operator=(const basic_concurrent_channel&) = delete;

	template<typename, typename, typename...> friend class detail::channel_send_functions;

	// Helper function to get an executor's context.
	template<typename T>
	static execution_context& get_context(const T& t, enable_if_t<execution::is_executor<T>::value>* = 0)
	{
	...
	public:
		...
		template<typename SendHandler> void operator()(SendHandler&& handler, payload_type&& payload) const
		{
			asio::detail::non_const_lvalue<SendHandler> handler2(handler);
			self_->service_->async_send(
			    self_->impl_, static_cast<payload_type&&>(payload), handler2.value, self_->get_executor());
		}
	...
	public:
		...
		template<typename ReceiveHandler> void operator()(ReceiveHandler&& handler) const
		{
			asio::detail::non_const_lvalue<ReceiveHandler> handler2(handler);
			self_->service_->async_receive(self_->impl_, handler2.value, self_->get_executor());
		}
	...
	typename service_type::template implementation_type<Traits, Signatures...> impl_;
