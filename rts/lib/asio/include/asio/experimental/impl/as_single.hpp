...
template<typename Handler> class as_single_handler {
public:
	...
	template<typename Arg> void operator()(Arg&& arg) { static_cast<Handler&&>(handler_)(static_cast<Arg&&>(arg)); }
...
template<typename Handler> inline bool asio_handler_is_continuation(as_single_handler<Handler>* this_handler)
{
	return asio_handler_cont_helpers::is_continuation(this_handler->handler_);
}

template<typename Signature> struct as_single_signature {
	typedef Signature type;
};

template<typename R> struct as_single_signature<R()> {
	typedef R type();
};

template<typename R, typename Arg> struct as_single_signature<R(Arg)> {
	typedef R type(Arg);
};

template<typename R, typename... Args> struct as_single_signature<R(Args...)> {
...
struct async_result<experimental::as_single_t<CompletionToken>, Signature> {
	template<typename Initiation> struct init_wrapper {
		...
		template<typename Handler, typename... Args> void operator()(Handler&& handler, Args&&... args)
		{
			static_cast<Initiation&&>(initiation_)(
			    experimental::detail::as_single_handler<decay_t<Handler>>(static_cast<Handler&&>(handler)),
	...
	static auto initiate(Initiation&& initiation, RawCompletionToken&& token, Args&&... args)
	    -> decltype(async_initiate<CompletionToken,
	        ...
	        static_cast<Args&&>(args)...))
	{
		return async_initiate<CompletionToken, typename experimental::detail::as_single_signature<Signature>::type>(
...
struct associator<Associator, experimental::detail::as_single_handler<Handler>, DefaultCandidate>
	...
	static auto get(const experimental::detail::as_single_handler<Handler>& h, const DefaultCandidate& c) noexcept
