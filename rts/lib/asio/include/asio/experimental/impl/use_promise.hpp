...
template<typename Signature, typename Executor, typename Allocator> struct promise_handler;
...
struct async_result<experimental::use_promise_t<Allocator>, R(Args...)> {
	...
	static auto initiate(Initiation initiation, experimental::use_promise_t<Allocator> up, InitArgs... args)
	    -> experimental::promise<void(decay_t<Args>...), asio::associated_executor_t<Initiation>, Allocator>
	{
		using handler_type = experimental::detail::promise_handler<void(decay_t<Args>...),
