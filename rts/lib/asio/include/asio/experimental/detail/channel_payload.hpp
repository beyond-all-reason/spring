...
template<typename... Signatures> class channel_payload;

template<typename R> class channel_payload<R()> {
public:
	explicit channel_payload(channel_message<R()>) {}

	template<typename Handler> void receive(Handler& handler) { static_cast<Handler&&>(handler)(); }
};

template<typename Signature> class channel_payload<Signature> {
public:
	...
	template<typename Handler> void receive(Handler& handler) { message_.receive(handler); }
...
public:
	...
	{
		std::visit([&](auto& message) { message.receive(handler); }, message_);
	}
...
template<typename R1, typename R2> class channel_payload<R1(), R2(asio::error_code)> {
public:
...
template<typename Sig1, typename Sig2> class channel_payload<Sig1, Sig2> {
public:
	    ...
	    : index_(other.index_)
	{
		switch (index_) {
		case 1:
			new (&storage_.message_1_) message_1_type(static_cast<message_1_type&&>(other.storage_.message_1_));
			break;
		case 2:
			new (&storage_.message_2_) message_2_type(static_cast<message_2_type&&>(other.storage_.message_2_));
			break;
		default: break;
		}
	}
	...
	{
		switch (index_) {
		case 1: storage_.message_1_.~message_1_type(); break;
		case 2: storage_.message_2_.~message_2_type(); break;
		default: break;
		}
	}
	...
	{
		switch (index_) {
		case 1: storage_.message_1_.receive(handler); break;
		case 2: storage_.message_2_.receive(handler); break;
		default: break;
		}
	}
