...
template<typename Payload> class channel_receive : public channel_operation {
public:
	void immediate(Payload payload) { func_(this, immediate_op, &payload); }

	void post(Payload payload) { func_(this, post_op, &payload); }

	void dispatch(Payload payload) { func_(this, dispatch_op, &payload); }
...
public:
	...
	static void do_action(channel_operation* base, channel_operation::action a, void* v)
	{
		...
		if (a != channel_operation::destroy_op) {
			...
			channel_handler<Payload, Handler> handler(static_cast<Payload&&>(*payload), o->handler_);
