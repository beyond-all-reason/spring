 ...
 */
template<typename CompletionToken> class as_single_t {
public:
	 ...
	 * location.
	 */
	constexpr as_single_t(default_constructor_tag = default_constructor_tag(),
	...
	/// Constructor.
	template<typename T>
	constexpr explicit as_single_t(T&& completion_token)
...
ASIO_NODISCARD inline constexpr as_single_t<decay_t<CompletionToken>> as_single(CompletionToken&& completion_token)
{
	return as_single_t<decay_t<CompletionToken>>(static_cast<CompletionToken&&>(completion_token));
}
