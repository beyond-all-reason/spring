...
public:
	 ...
	 * opened.
	 */
	explicit basic_file(const executor_type& ex, const char* path, file_base::flags open_flags)
	    : impl_(0, ex)
	{
		asio::error_code ec;
	 ...
	 * opened.
	 */
	explicit basic_file(const executor_type& ex, const std::string& path, file_base::flags open_flags)
	    : impl_(0, ex)
	{
		asio::error_code ec;
		impl_.get_service().open(impl_.get_implementation(), path.c_str(), open_flags, ec);
		asio::detail::throw_error(ec, "open");
	}
	 ...
	 */
	template<typename ExecutionContext>
	basic_file(ExecutionContext& context,
	 ...
	 */
	template<typename Executor1>
	constraint_t<is_convertible<Executor1, Executor>::value, basic_file&> operator=(basic_file<Executor1>&& other)
	{
	 ...
	 * @endcode
	 */
	ASIO_SYNC_OP_VOID open(const char* path, file_base::flags open_flags, asio::error_code& ec)
	{
		impl_.get_service().open(impl_.get_implementation(), path, open_flags, ec);
	 ...
	 * @endcode
	 */
	ASIO_SYNC_OP_VOID open(const std::string& path, file_base::flags open_flags, asio::error_code& ec)
	{
		impl_.get_service().open(impl_.get_implementation(), path.c_str(), open_flags, ec);
		ASIO_SYNC_OP_VOID_RETURN(ec);
	}
	 ...
	 * @param ec Set to indicate what error occurred, if any.
	 */
	ASIO_SYNC_OP_VOID assign(const native_handle_type& native_file, asio::error_code& ec)
	{
		impl_.get_service().assign(impl_.get_implementation(), native_file, ec);
		ASIO_SYNC_OP_VOID_RETURN(ec);
	}
	 ...
	 * @param ec Set to indicate what error occurred, if any.
	 */
	uint64_t size(asio::error_code& ec) const { return impl_.get_service().size(impl_.get_implementation(), ec); }
