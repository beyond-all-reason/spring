...
public:
...
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	mutable_buffer(void* data, std::size_t size, asio::detail::function<void()> debug_check)
	    : data_(data)
	    , size_(size)
	...
	const asio::detail::function<void()>& get_debug_check() const { return debug_check_; }
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
...
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	mutable_buffers_1(void* data, std::size_t size, asio::detail::function<void()> debug_check)
...
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	const_buffer(const void* data, std::size_t size, asio::detail::function<void()> debug_check)
	    : data_(data)
	    , size_(size)
	    , debug_check_(debug_check)
	{
...
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	const_buffers_1(const void* data, std::size_t size, asio::detail::function<void()> debug_check)
...
public:
	...
	/// Get a random-access iterator for one past the last element.
	const_iterator end() const noexcept { return &buf_; }
...
inline const mutable_buffer* buffer_sequence_begin(const MutableBuffer& b,
    constraint_t<is_convertible<const MutableBuffer*, const mutable_buffer*>::value> = 0) noexcept
{
...
inline const const_buffer* buffer_sequence_begin(const ConstBuffer& b,
    constraint_t<is_convertible<const ConstBuffer*, const const_buffer*>::value> = 0) noexcept
{
...
inline auto buffer_sequence_begin(C& c,
    constraint_t<!is_convertible<const C*, const mutable_buffer*>::value &&
                 !is_convertible<const C*, const const_buffer*>::value> = 0) noexcept -> decltype(c.begin())
{
	return c.begin();
}
...
inline auto buffer_sequence_end(C& c,
    constraint_t<!is_convertible<const C*, const mutable_buffer*>::value &&
                 !is_convertible<const C*, const const_buffer*>::value> = 0) noexcept -> decltype(c.end())
{
	return c.end();
}
...
struct buffer_sequence_cardinality : conditional_t<is_same<BufferSequence, mutable_buffer>::value
#if !defined(ASIO_NO_DEPRECATED)
...
template<typename Iterator> inline std::size_t buffer_size(one_buffer, Iterator begin, Iterator) noexcept
{
	return const_buffer(*begin).size();
}

template<typename Iterator> inline std::size_t buffer_size(multiple_buffers, Iterator begin, Iterator end) noexcept
{
 ...
 */
template<typename BufferSequence> inline std::size_t buffer_size(const BufferSequence& b) noexcept
{
	return detail::buffer_size(detail::buffer_sequence_cardinality<BufferSequence>(), asio::buffer_sequence_begin(b),
...
template<typename PointerToPodType> inline PointerToPodType buffer_cast(const mutable_buffer& b) noexcept
{
	return static_cast<PointerToPodType>(b.data());
}

/// Cast a non-modifiable buffer to a specified pointer to POD type.
template<typename PointerToPodType> inline PointerToPodType buffer_cast(const const_buffer& b) noexcept
{
	return static_cast<PointerToPodType>(b.data());
}
 ...
 * @relates mutable_buffer
 */
inline mutable_buffer operator+(std::size_t n, const mutable_buffer& b) noexcept { return b + n; }
 ...
 * @relates const_buffer
 */
inline const_buffer operator+(std::size_t n, const const_buffer& b) noexcept { return b + n; }

#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
namespace detail {

template<typename Iterator> class buffer_debug_check {
public:
 ...
 */
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(const mutable_buffer& b) noexcept { return ASIO_MUTABLE_BUFFER(b); }
 ...
 */
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(const mutable_buffer& b, std::size_t max_size_in_bytes) noexcept
{
	return ASIO_MUTABLE_BUFFER(mutable_buffer(b.data(), b.size() < max_size_in_bytes ? b.size() : max_size_in_bytes
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	    ,
	    b.get_debug_check()
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
 ...
 */
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const const_buffer& b) noexcept { return ASIO_CONST_BUFFER(b); }
 ...
 *     min(b.size(), max_size_in_bytes)); @endcode
 */
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const const_buffer& b, std::size_t max_size_in_bytes) noexcept
{
	return ASIO_CONST_BUFFER(b.data(), b.size() < max_size_in_bytes ? b.size() : max_size_in_bytes
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	    ,
	    b.get_debug_check()
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
 ...
 */
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(void* data, std::size_t size_in_bytes) noexcept
{
 ...
 */
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const void* data, std::size_t size_in_bytes) noexcept
{
 ...
 */
template<typename PodType, std::size_t N> ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(PodType (&data)[N]) noexcept
{
...
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(PodType (&data)[N], std::size_t max_size_in_bytes) noexcept
{
	return ASIO_MUTABLE_BUFFER(data, N * sizeof(PodType) < max_size_in_bytes ? N * sizeof(PodType) : max_size_in_bytes);
}
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const PodType (&data)[N]) noexcept
{
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const PodType (&data)[N], std::size_t max_size_in_bytes) noexcept
{
	return ASIO_CONST_BUFFER(data, N * sizeof(PodType) < max_size_in_bytes ? N * sizeof(PodType) : max_size_in_bytes);
}
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(boost::array<PodType, N>& data) noexcept
{
	return ASIO_MUTABLE_BUFFER(data.c_array(), data.size() * sizeof(PodType));
}
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(boost::array<PodType, N>& data, std::size_t max_size_in_bytes) noexcept
{
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(boost::array<const PodType, N>& data) noexcept
{
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(boost::array<const PodType, N>& data,
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const boost::array<PodType, N>& data) noexcept
{
	return ASIO_CONST_BUFFER(data.data(), data.size() * sizeof(PodType));
}
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const boost::array<PodType, N>& data,
    std::size_t max_size_in_bytes) noexcept
{
	return ASIO_CONST_BUFFER(data.data(),
	    data.size() * sizeof(PodType) < max_size_in_bytes ? data.size() * sizeof(PodType) : max_size_in_bytes);
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(std::array<PodType, N>& data) noexcept
{
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(std::array<PodType, N>& data, std::size_t max_size_in_bytes) noexcept
{
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(std::array<const PodType, N>& data) noexcept
{
	return ASIO_CONST_BUFFER(data.data(), data.size() * sizeof(PodType));
}
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(std::array<const PodType, N>& data,
    std::size_t max_size_in_bytes) noexcept
{
	return ASIO_CONST_BUFFER(data.data(),
	    data.size() * sizeof(PodType) < max_size_in_bytes ? data.size() * sizeof(PodType) : max_size_in_bytes);
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const std::array<PodType, N>& data) noexcept
{
	return ASIO_CONST_BUFFER(data.data(), data.size() * sizeof(PodType));
}
 ...
 */
template<typename PodType, std::size_t N>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const std::array<PodType, N>& data,
    std::size_t max_size_in_bytes) noexcept
{
	return ASIO_CONST_BUFFER(data.data(),
	    data.size() * sizeof(PodType) < max_size_in_bytes ? data.size() * sizeof(PodType) : max_size_in_bytes);
...
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(std::vector<PodType, Allocator>& data) noexcept
{
	return ASIO_MUTABLE_BUFFER(data.size() ? &data[0] : 0, data.size() * sizeof(PodType)
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	                                                           ,
	    detail::buffer_debug_check<typename std::vector<PodType, Allocator>::iterator>(data.begin())
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
	);
}
 ...
 */
template<typename PodType, typename Allocator>
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(std::vector<PodType, Allocator>& data,
    std::size_t max_size_in_bytes) noexcept
{
 ...
 */
template<typename PodType, typename Allocator>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const std::vector<PodType, Allocator>& data) noexcept
{
	return ASIO_CONST_BUFFER(data.size() ? &data[0] : 0, data.size() * sizeof(PodType)
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	                                                         ,
	    detail::buffer_debug_check<typename std::vector<PodType, Allocator>::const_iterator>(data.begin())
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
	);
}
 ...
 */
template<typename PodType, typename Allocator>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const std::vector<PodType, Allocator>& data,
    std::size_t max_size_in_bytes) noexcept
{
...
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(std::basic_string<Elem, Traits, Allocator>& data) noexcept
{
	return ASIO_MUTABLE_BUFFER(data.size() ? &data[0] : 0, data.size() * sizeof(Elem)
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	                                                           ,
	    detail::buffer_debug_check<typename std::basic_string<Elem, Traits, Allocator>::iterator>(data.begin())
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
	);
}
 ...
 */
template<typename Elem, typename Traits, typename Allocator>
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(std::basic_string<Elem, Traits, Allocator>& data,
    std::size_t max_size_in_bytes) noexcept
{
	return ASIO_MUTABLE_BUFFER(data.size() ? &data[0] : 0,
 ...
 */
template<typename Elem, typename Traits, typename Allocator>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const std::basic_string<Elem, Traits, Allocator>& data) noexcept
{
	return ASIO_CONST_BUFFER(data.data(), data.size() * sizeof(Elem)
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	                                          ,
	    detail::buffer_debug_check<typename std::basic_string<Elem, Traits, Allocator>::const_iterator>(data.begin())
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
	);
}
 ...
 */
template<typename Elem, typename Traits, typename Allocator>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const std::basic_string<Elem, Traits, Allocator>& data,
    std::size_t max_size_in_bytes) noexcept
{
	return ASIO_CONST_BUFFER(
	    data.data(), data.size() * sizeof(Elem) < max_size_in_bytes ? data.size() * sizeof(Elem) : max_size_in_bytes
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	    ,
	    detail::buffer_debug_check<typename std::basic_string<Elem, Traits, Allocator>::const_iterator>(data.begin())
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
...
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(basic_string_view<Elem, Traits> data) noexcept
{
	return ASIO_CONST_BUFFER(data.size() ? &data[0] : 0, data.size() * sizeof(Elem)
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
	                                                         ,
	    detail::buffer_debug_check<typename basic_string_view<Elem, Traits>::iterator>(data.begin())
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
	);
}
 ...
 */
template<typename Elem, typename Traits>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(basic_string_view<Elem, Traits> data,
    std::size_t max_size_in_bytes) noexcept
{
	return ASIO_CONST_BUFFER(data.size() ? &data[0] : 0,
	    data.size() * sizeof(Elem) < max_size_in_bytes ? data.size() * sizeof(Elem) : max_size_in_bytes
 ...
 */
template<typename T>
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(T& data,
    ...
    constraint_t<!is_const<remove_reference_t<typename std::iterator_traits<typename T::iterator>::reference>>::value,
        defaulted_constraint> = defaulted_constraint()) noexcept
{
	return ASIO_MUTABLE_BUFFER(
	    data.size() ? detail::to_address(data.begin()) : 0, data.size() * sizeof(typename T::value_type));
}
...
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(T& data,
    ...
    constraint_t<!is_const<remove_reference_t<typename std::iterator_traits<typename T::iterator>::reference>>::value,
        defaulted_constraint> = defaulted_constraint()) noexcept
{
	return ASIO_MUTABLE_BUFFER(data.size() ? detail::to_address(data.begin()) : 0,
 ...
 */
template<typename T>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(T& data,
    constraint_t<is_contiguous_iterator<typename T::iterator>::value, defaulted_constraint> = defaulted_constraint(),
    constraint_t<!is_convertible<T, const_buffer>::value, defaulted_constraint> = defaulted_constraint(),
    constraint_t<!is_convertible<T, mutable_buffer>::value, defaulted_constraint> = defaulted_constraint(),
    constraint_t<is_const<remove_reference_t<typename std::iterator_traits<typename T::iterator>::reference>>::value,
...
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(T& data,
    ...
    constraint_t<is_const<remove_reference_t<typename std::iterator_traits<typename T::iterator>::reference>>::value,
        defaulted_constraint> = defaulted_constraint()) noexcept
{
	return ASIO_CONST_BUFFER(data.size() ? detail::to_address(data.begin()) : 0,
	    data.size() * sizeof(typename T::value_type) < max_size_in_bytes ?
	        data.size() * sizeof(typename T::value_type) :
	        max_size_in_bytes);
}
 ...
 */
template<typename T>
ASIO_NODISCARD inline ASIO_CONST_BUFFER buffer(const T& data,
    constraint_t<is_contiguous_iterator<typename T::const_iterator>::value, defaulted_constraint> =
        defaulted_constraint(),
    constraint_t<!is_convertible<T, const_buffer>::value, defaulted_constraint> = defaulted_constraint(),
    constraint_t<!is_convertible<T, mutable_buffer>::value, defaulted_constraint> = defaulted_constraint()) noexcept
{
	return ASIO_CONST_BUFFER(
	    data.size() ? detail::to_address(data.begin()) : 0, data.size() * sizeof(typename T::value_type));
}
 ...
 */
template<typename Elem, typename Traits, typename Allocator> class dynamic_string_buffer {
public:
	 ...
	 * max_size(). Otherwise returns max_size().
	 */
	std::size_t capacity() const noexcept { return (std::min)(string_.capacity(), max_size()); }
	 ...
	 */
	const_buffers_type data() const noexcept { return const_buffers_type(asio::buffer(string_, size_)); }
#endif // !defined(ASIO_NO_DYNAMIC_BUFFER_V1)
	...
	{
		return mutable_buffers_type(asio::buffer(asio::buffer(string_, max_size_) + pos, n));
	}
	...
	{
		return const_buffers_type(asio::buffer(asio::buffer(string_, max_size_) + pos, n));
	}
	 ...
	 * is emptied.
	 */
	void shrink(std::size_t n) { string_.resize(n > size() ? 0 : size() - n); }
...
public:
	 ...
	 */
	std::size_t capacity() const noexcept { return (std::min)(vector_.capacity(), max_size()); }
	 ...
	 * sequence.
	 */
	const_buffers_type data() const noexcept { return const_buffers_type(asio::buffer(vector_, size_)); }
#endif // !defined(ASIO_NO_DYNAMIC_BUFFER_V1)
	...
	mutable_buffers_type data(std::size_t pos, std::size_t n) noexcept
	{
		return mutable_buffers_type(asio::buffer(asio::buffer(vector_, max_size_) + pos, n));
	}
	...
	const_buffers_type data(std::size_t pos, std::size_t n) const noexcept
	{
		return const_buffers_type(asio::buffer(asio::buffer(vector_, max_size_) + pos, n));
	}
	 ...
	 * is emptied.
	 */
	void shrink(std::size_t n) { vector_.resize(n > size() ? 0 : size() - n); }
 ...
 */
template<typename Elem, typename Traits, typename Allocator>
ASIO_NODISCARD inline dynamic_string_buffer<Elem, Traits, Allocator> dynamic_buffer(
 ...
 */
template<typename Elem, typename Traits, typename Allocator>
ASIO_NODISCARD inline dynamic_string_buffer<Elem, Traits, Allocator>
dynamic_buffer(std::basic_string<Elem, Traits, Allocator>& data, std::size_t max_size) noexcept
{
...
ASIO_NODISCARD inline dynamic_vector_buffer<Elem, Allocator> dynamic_buffer(std::vector<Elem, Allocator>& data) noexcept
{
 ...
 */
template<typename Elem, typename Allocator>
ASIO_NODISCARD inline dynamic_vector_buffer<Elem, Allocator> dynamic_buffer(std::vector<Elem, Allocator>& data,
...
{
	return (buffer_copy_1)(*target_begin, asio::buffer(*source_begin, max_bytes_to_copy));
}

template<typename TargetIterator, typename SourceIterator>
std::size_t buffer_copy(one_buffer,
    multiple_buffers,
    TargetIterator target_begin,
    TargetIterator,
    SourceIterator source_begin,
    ...
    std::size_t max_bytes_to_copy = (std::numeric_limits<std::size_t>::max)()) noexcept
{
	...
	for (mutable_buffer target_buffer(asio::buffer(*target_begin, max_bytes_to_copy));
    ...
    std::size_t max_bytes_to_copy = (std::numeric_limits<std::size_t>::max)()) noexcept
{
	...
	for (const_buffer source_buffer(asio::buffer(*source_begin, max_bytes_to_copy));
    ...
    std::size_t max_bytes_to_copy) noexcept
{
	...
	while (total_bytes_copied != max_bytes_to_copy && target_iter != target_end && source_iter != source_end) {
		...
		std::size_t bytes_copied =
		    (buffer_copy_1)(target_buffer, asio::buffer(source_buffer, max_bytes_to_copy - total_bytes_copied));
		total_bytes_copied += bytes_copied;
...
inline std::size_t buffer_copy(const MutableBufferSequence& target, const ConstBufferSequence& source) noexcept
{
	return detail::buffer_copy(detail::buffer_sequence_cardinality<MutableBufferSequence>(),
	    detail::buffer_sequence_cardinality<ConstBufferSequence>(), asio::buffer_sequence_begin(target),
	    asio::buffer_sequence_end(target), asio::buffer_sequence_begin(source), asio::buffer_sequence_end(source));
}
...
template<unsigned char... Bytes> struct bytes {};

// Literal processor that converts binary literals to an array of bytes.

template<typename Bytes, char... Chars> struct bin_literal;

template<unsigned char... Bytes> struct bin_literal<bytes<Bytes...>> {
	static const std::size_t size = sizeof...(Bytes);
	static const unsigned char data[sizeof...(Bytes)];
};

template<unsigned char... Bytes> const unsigned char bin_literal<bytes<Bytes...>>::data[sizeof...(Bytes)] = {Bytes...};
...
template<unsigned char... Bytes,
    ...
    char Bit0,
    char... Chars>
struct bin_literal<bytes<Bytes...>, Bit7, Bit6, Bit5, Bit4, Bit3, Bit2, Bit1, Bit0, Chars...>
    : bin_literal<bytes<Bytes...,
                      static_cast<unsigned char>((Bit7 == '1' ? 0x80 : 0) | (Bit6 == '1' ? 0x40 : 0) |
                                                 (Bit5 == '1' ? 0x20 : 0) | (Bit4 == '1' ? 0x10 : 0) |
                                                 (Bit3 == '1' ? 0x08 : 0) | (Bit2 == '1' ? 0x04 : 0) |
                                                 (Bit1 == '1' ? 0x02 : 0) | (Bit0 == '1' ? 0x01 : 0))>,
          Chars...> {};

template<unsigned char... Bytes, char... Chars> struct bin_literal<bytes<Bytes...>, Chars...> {
	static_assert(sizeof...(Chars) == 0, "number of digits in a binary buffer literal must be a multiple of 8");
...
template<typename Bytes, char... Chars> struct hex_literal;

template<unsigned char... Bytes> struct hex_literal<bytes<Bytes...>> {
	static const std::size_t size = sizeof...(Bytes);
	static const unsigned char data[sizeof...(Bytes)];
};

template<unsigned char... Bytes> const unsigned char hex_literal<bytes<Bytes...>>::data[sizeof...(Bytes)] = {Bytes...};
    ...
    : hex_literal<
          bytes<Bytes...,
              static_cast<unsigned char>(
                  Lo >= 'A' && Lo <= 'F' ? Lo - 'A' + 10 : (Lo >= 'a' && Lo <= 'f' ? Lo - 'a' + 10 : Lo - '0')) |
                  ((static_cast<unsigned char>(
                       Hi >= 'A' && Hi <= 'F' ? Hi - 'A' + 10 : (Hi >= 'a' && Hi <= 'f' ? Hi - 'a' + 10 : Hi - '0')))
                      << 4)>,
          Chars...> {};

template<unsigned char... Bytes, char Char> struct hex_literal<bytes<Bytes...>, Char> {
	static_assert(!Char, "a hexadecimal buffer literal must have an even number of digits");

	static const std::size_t size = 0;
	static const unsigned char data[1];
};

template<unsigned char... Bytes, char Char> const unsigned char hex_literal<bytes<Bytes...>, Char>::data[1] = {};
...
template<template<typename, char...> class Literal, typename Clean, char... Raw> struct remove_separators;

template<template<typename, char...> class Literal, char... Clean, char... Raw>
struct remove_separators<Literal, chars<Clean...>, '\'', Raw...> : remove_separators<Literal, chars<Clean...>, Raw...> {
};

template<template<typename, char...> class Literal, char... Clean, char C, char... Raw>
struct remove_separators<Literal, chars<Clean...>, C, Raw...> : remove_separators<Literal, chars<Clean..., C>, Raw...> {
};

template<template<typename, char...> class Literal, char... Clean>
struct remove_separators<Literal, chars<Clean...>> : Literal<bytes<>, Clean...> {};

// Helper template to determine the literal type based on the prefix.

template<char... Chars> struct literal;

template<char... Chars> struct literal<'0', 'b', Chars...> : remove_separators<bin_literal, chars<>, Chars...> {};

template<char... Chars> struct literal<'0', 'B', Chars...> : remove_separators<bin_literal, chars<>, Chars...> {};

template<char... Chars> struct literal<'0', 'x', Chars...> : remove_separators<hex_literal, chars<>, Chars...> {};
...
inline ASIO_CONST_BUFFER operator""_buf(const char* data, std::size_t n) { return ASIO_CONST_BUFFER(data, n); }

/// Literal operator for creating const_buffer objects from unbounded binary or
/// hexadecimal integer literals.
template<char... Chars> inline ASIO_CONST_BUFFER operator""_buf()
{
	return ASIO_CONST_BUFFER(+detail::literal<Chars...>::data, detail::literal<Chars...>::size);
}
