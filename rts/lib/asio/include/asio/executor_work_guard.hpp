...
template<typename Executor, typename = void, typename = void> class executor_work_guard;
...
template<typename Executor> class executor_work_guard<Executor, enable_if_t<is_executor<Executor>::value>> {
public:
	typedef Executor executor_type;
...
class executor_work_guard<Executor,
    enable_if_t<!is_executor<Executor>::value>,
...
public:
	...
	explicit executor_work_guard(const executor_type& e) noexcept
	    ...
	    , owns_(true)
	{
		new (&work_) work_type(asio::prefer(executor_, execution::outstanding_work.tracked));
	}
	    ...
	    , owns_(other.owns_)
	{
		if (owns_) {
			new (&work_)
			    work_type(static_cast<work_type&&>(*static_cast<work_type*>(static_cast<void*>(&other.work_))));
			other.owns_ = false;
		}
	}
...
private:
	...
	typedef decay_t<prefer_result_t<const executor_type&, execution::outstanding_work_t::tracked_t>> work_type;
 ...
 */
template<typename Executor>
ASIO_NODISCARD inline executor_work_guard<Executor> make_work_guard(const Executor& ex,
...
ASIO_NODISCARD inline executor_work_guard<typename ExecutionContext::executor_type>
make_work_guard(ExecutionContext& ctx, constraint_t<is_convertible<ExecutionContext&, execution_context&>::value> = 0)
{
 ...
 */
template<typename T>
ASIO_NODISCARD inline executor_work_guard<typename constraint_t<
    !is_executor<T>::value && !execution::is_executor<T>::value && !is_convertible<T&, execution_context&>::value,
    associated_executor<T>>::type>
make_work_guard(const T& t)
{
	return executor_work_guard<associated_executor_t<T>>(associated_executor<T>::get(t));
}
...
ASIO_NODISCARD inline executor_work_guard<associated_executor_t<T, Executor>> make_work_guard(const T& t,
    const Executor& ex,
    constraint_t<is_executor<Executor>::value || execution::is_executor<Executor>::value> = 0)
{
	return executor_work_guard<associated_executor_t<T, Executor>>(associated_executor<T, Executor>::get(t, ex));
}
...
ASIO_NODISCARD inline executor_work_guard<associated_executor_t<T, typename ExecutionContext::executor_type>>
make_work_guard(const T& t,
...
{
	return executor_work_guard<associated_executor_t<T, typename ExecutionContext::executor_type>>(
