 ...
 */
template<typename Stream> class buffered_read_stream : private noncopyable {
public:
	...
	auto async_write_some(const ConstBufferSequence& buffers,
	    ...
	    -> decltype(declval<conditional_t<true, Stream&, WriteHandler>>().async_write_some(buffers,
	        static_cast<WriteHandler&&>(handler)))
	{
		return next_layer_.async_write_some(buffers, static_cast<WriteHandler&&>(handler));
	}
	...
	std::size_t in_avail() { return storage_.size(); }
...
private:
	...
	template<typename MutableBufferSequence> std::size_t copy(const MutableBufferSequence& buffers)
	{
		std::size_t bytes_copied = asio::buffer_copy(buffers, storage_.data(), storage_.size());
	...
	/// copied.
	template<typename MutableBufferSequence> std::size_t peek_copy(const MutableBufferSequence& buffers)
	{
