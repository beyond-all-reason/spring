 ...
 */
template<typename Stream> class stream : public stream_base, private noncopyable {
private:
...
public:
	 ...
	 */
	executor_type get_executor() noexcept { return next_layer_.lowest_layer().get_executor(); }
	...
	{
		core_.engine_.set_verify_callback(new detail::verify_callback<VerifyCallback>(callback), ec);
		ASIO_SYNC_OP_VOID_RETURN(ec);
	}
	...
	ASIO_SYNC_OP_VOID handshake(handshake_type type, const ConstBufferSequence& buffers, asio::error_code& ec)
	{
		detail::io(next_layer_, core_, detail::buffered_handshake_op<ConstBufferSequence>(type, buffers), ec);
		ASIO_SYNC_OP_VOID_RETURN(ec);
	}
	...
	auto async_handshake(handshake_type type, HandshakeToken&& token = default_completion_token_t<executor_type>())
	    -> decltype(async_initiate<HandshakeToken, void(asio::error_code)>(declval<initiate_async_handshake>(),
	        token,
	        type))
	{
		return async_initiate<HandshakeToken, void(asio::error_code)>(initiate_async_handshake(this), token, type);
	}
	...
	template<typename ConstBufferSequence,
	    ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code, std::size_t))
	        BufferedHandshakeToken = default_completion_token_t<executor_type>>
	auto async_handshake(handshake_type type,
	    ...
	    -> decltype(async_initiate<BufferedHandshakeToken, void(asio::error_code, std::size_t)>(
	        declval<initiate_async_buffered_handshake>(),
	        token,
	        type,
	        buffers))
	{
		return async_initiate<BufferedHandshakeToken, void(asio::error_code, std::size_t)>(
	...
	auto async_shutdown(ShutdownToken&& token = default_completion_token_t<executor_type>())
	    -> decltype(async_initiate<ShutdownToken, void(asio::error_code)>(declval<initiate_async_shutdown>(), token))
	{
		return async_initiate<ShutdownToken, void(asio::error_code)>(initiate_async_shutdown(this), token);
	}
	...
	auto async_write_some(const ConstBufferSequence& buffers,
	    ...
	    -> decltype(async_initiate<WriteToken, void(asio::error_code, std::size_t)>(
	        declval<initiate_async_write_some>(),
	        token,
	        buffers))
	{
	...
	auto async_read_some(const MutableBufferSequence& buffers,
	    ...
	    -> decltype(async_initiate<ReadToken, void(asio::error_code, std::size_t)>(declval<initiate_async_read_some>(),
	        token,
	        buffers))
	{
...
private:
	...
	public:
		...
		template<typename HandshakeHandler> void operator()(HandshakeHandler&& handler, handshake_type type) const
		{
			...
			detail::async_io(self_->next_layer_, self_->core_, detail::handshake_op(type), handler2.value);
		}
	...
	public:
		...
		void
		operator()(BufferedHandshakeHandler&& handler, handshake_type type, const ConstBufferSequence& buffers) const
		{
			...
			asio::detail::non_const_lvalue<BufferedHandshakeHandler> handler2(handler);
			detail::async_io(self_->next_layer_, self_->core_,
			    detail::buffered_handshake_op<ConstBufferSequence>(type, buffers), handler2.value);
		}
	...
	public:
		...
		template<typename ShutdownHandler> void operator()(ShutdownHandler&& handler) const
		{
			// If you get an error on the following line it means that your handler
			...
			detail::async_io(self_->next_layer_, self_->core_, detail::shutdown_op(), handler2.value);
		}
	...
	public:
		...
		void operator()(WriteHandler&& handler, const ConstBufferSequence& buffers) const
		{
			...
			detail::async_io(
			    self_->next_layer_, self_->core_, detail::write_op<ConstBufferSequence>(buffers), handler2.value);
		}
	...
	public:
		...
		void operator()(ReadHandler&& handler, const MutableBufferSequence& buffers) const
		{
			...
			detail::async_io(
			    self_->next_layer_, self_->core_, detail::read_op<MutableBufferSequence>(buffers), handler2.value);
		}
