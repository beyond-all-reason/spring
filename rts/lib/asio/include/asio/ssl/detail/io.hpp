	...
	do switch (op(core.engine_, ec, bytes_transferred)) {
		case engine::want_input_and_retry:
			...
			if (core.input_.size() == 0) {
				core.input_ = asio::buffer(core.input_buffer_, next_layer.read_some(core.input_buffer_, io_ec));
				if (!ec)
					ec = io_ec;
			}
...
public:
	...
	io_op(io_op&& other)
	    : asio::detail::base_from_cancellation_state<Handler>(
	          static_cast<asio::detail::base_from_cancellation_state<Handler>&&>(other))
	    , next_layer_(other.next_layer_)
	    , core_(other.core_)
			...
			do {
				...
				case engine::want_input_and_retry:
					...
					if (core_.expiry(core_.pending_read_) == core_.neg_infin()) {
						...
						ASIO_HANDLER_LOCATION((__FILE__, __LINE__, Operation::tracking_name()));

						// Start reading some data from the underlying transport.
						next_layer_.async_read_some(asio::buffer(core_.input_buffer_), static_cast<io_op&&>(*this));
					}
					else {
						ASIO_HANDLER_LOCATION((__FILE__, __LINE__, Operation::tracking_name()));
				...
				case engine::want_output:
					...
					if (core_.expiry(core_.pending_write_) == core_.neg_infin()) {
						...
						asio::async_write(
						    next_layer_, core_.engine_.get_output(core_.output_buffer_), static_cast<io_op&&>(*this));
					}
					else {
						ASIO_HANDLER_LOCATION((__FILE__, __LINE__, Operation::tracking_name()));
				...
				default:
					...
					if (start) {
						...
						next_layer_.async_read_some(asio::buffer(core_.input_buffer_, 0), static_cast<io_op&&>(*this));
...
struct associator<Associator, ssl::detail::io_op<Stream, Operation, Handler>, DefaultCandidate>
	...
	static auto get(const ssl::detail::io_op<Stream, Operation, Handler>& h, const DefaultCandidate& c) noexcept
