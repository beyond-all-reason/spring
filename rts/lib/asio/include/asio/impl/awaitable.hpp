...
public:
	...
	{
		return asio::detail::thread_info_base::allocate(asio::detail::thread_info_base::awaitable_frame_tag(),
		    asio::detail::thread_context::top_of_thread_call_stack(), size);
	}

	void operator delete(void* pointer, std::size_t size)
	{
		asio::detail::thread_info_base::deallocate(asio::detail::thread_info_base::awaitable_frame_tag(),
		    asio::detail::thread_context::top_of_thread_call_stack(), pointer, size);
	}
	...
	void set_error(const asio::error_code& ec) { this->set_except(std::make_exception_ptr(asio::system_error(ec))); }
	...
	void clear_cancellation_slot() { this->attached_thread_->entry_point()->cancellation_state_.slot().clear(); }
	...
	)
	{
		...
		return awaitable_async_op<completion_signature_of_t<Op>, decay_t<Op>, Executor>{std::forward<Op>(op), this
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
#if defined(ASIO_HAS_SOURCE_LOCATION)
		    ,
		    location
	...
	auto await_transform(this_coro::executor_t) noexcept
	{
		struct result {
			...
			auto await_resume() const noexcept { return this_->attached_thread_->get_executor(); }
		};
	...
	auto await_transform(this_coro::cancellation_state_t) noexcept
	{
		struct result {
			...
			auto await_resume() const noexcept { return this_->attached_thread_->get_cancellation_state(); }
		};
	...
	auto await_transform(this_coro::reset_cancellation_state_0_t) noexcept
	{
		struct result {
			...
			auto await_resume() const { return this_->attached_thread_->reset_cancellation_state(); }
		};
	...
	auto await_transform(this_coro::throw_if_cancelled_0_t) noexcept
	{
		struct result {
			...
			auto await_resume() { return this_->attached_thread_->throw_if_cancelled(); }
		};
	...
	auto await_transform(this_coro::throw_if_cancelled_1_t throw_if_cancelled) noexcept
	{
		struct result {
			...
			auto await_resume() { this_->attached_thread_->throw_if_cancelled(value_); }
		};
	...
	auto await_transform(Function f,
	    enable_if_t<is_convertible<result_of_t<Function(awaitable_frame_base*)>, awaitable_thread<Executor>*>::value>* =
	        nullptr)
	{
		struct result {
			Function function_;
	...
	void attach_thread(awaitable_thread<Executor>* handler) noexcept { attached_thread_ = handler; }
...
template<typename T, typename Executor> class awaitable_frame : public awaitable_frame_base<Executor> {
public:
	...
	T get()
	{
		...
		return std::move(*std::launder(static_cast<T*>(static_cast<void*>(result_))));
	}
...
template<typename Executor> class awaitable_frame<void, Executor> : public awaitable_frame_base<Executor> {
public:
...
class awaitable_frame<awaitable_thread_entry_point, Executor> : public awaitable_frame_base<Executor> {
public:
	awaitable_frame()
	    : top_of_stack_(0)
	    , has_executor_(false)
...
public:
	...
	awaitable_thread(awaitable<awaitable_thread_entry_point, Executor> p,
	    const Executor& ex,
	    cancellation_slot parent_cancel_slot,
	...
	~awaitable_thread()
	{
		if (bottom_of_stack_.valid()) {
			...
			(post)(bottom_frame->u_.executor_, [a = std::move(bottom_of_stack_)]() mutable {
				(void)awaitable<awaitable_thread_entry_point, Executor>(std::move(a));
			});
		}
	}

	awaitable_frame<awaitable_thread_entry_point, Executor>* entry_point() { return bottom_of_stack_.frame_; }

	executor_type get_executor() const noexcept { return bottom_of_stack_.frame_->u_.executor_; }

	cancellation_state get_cancellation_state() const noexcept { return bottom_of_stack_.frame_->cancellation_state_; }
	...
	{
		bottom_of_stack_.frame_->cancellation_state_ =
		    cancellation_state(bottom_of_stack_.frame_->parent_cancellation_slot_, static_cast<Filter&&>(filter));
	}
	...
	{
		bottom_of_stack_.frame_->cancellation_state_ =
		    cancellation_state(bottom_of_stack_.frame_->parent_cancellation_slot_, static_cast<InFilter&&>(in_filter),
		        static_cast<OutFilter&&>(out_filter));
	}

	bool throw_if_cancelled() const { return bottom_of_stack_.frame_->throw_if_cancelled_; }

	void throw_if_cancelled(bool value) { bottom_of_stack_.frame_->throw_if_cancelled_ = value; }
...
protected:
	...
	void pump()
	{
		do bottom_of_stack_.frame_->top_of_stack_->resume();
		while (bottom_of_stack_.frame_ && bottom_of_stack_.frame_->top_of_stack_);

		if (bottom_of_stack_.frame_) {
			awaitable<awaitable_thread_entry_point, Executor> a(std::move(bottom_of_stack_));
...
template<typename Signature, typename Executor> class awaitable_async_op_handler;

template<typename R, typename Executor>
class awaitable_async_op_handler<R(), Executor> : public awaitable_thread<Executor> {
public:
	struct result_type {};

	awaitable_async_op_handler(awaitable_thread<Executor>* h, result_type&)
...
template<typename R, typename Executor>
class awaitable_async_op_handler<R(asio::error_code), Executor> : public awaitable_thread<Executor> {
public:
	...
	static void resume(result_type& result) { throw_error(*result); }
...
template<typename R, typename Executor>
class awaitable_async_op_handler<R(std::exception_ptr), Executor> : public awaitable_thread<Executor> {
public:
...
class awaitable_async_op_handler<R(T), Executor> : public awaitable_thread<Executor> {
public:
	...
	static T resume(result_type& result) { return std::move(*result); }
...
template<typename R, typename T, typename Executor>
class awaitable_async_op_handler<R(asio::error_code, T), Executor> : public awaitable_thread<Executor> {
public:
	struct result_type {
		asio::error_code* ec_;
		T* value_;
...
template<typename R, typename T, typename Executor>
class awaitable_async_op_handler<R(std::exception_ptr, T), Executor> : public awaitable_thread<Executor> {
public:
	struct result_type {
...
class awaitable_async_op_handler<R(Ts...), Executor> : public awaitable_thread<Executor> {
public:
	...
	static std::tuple<Ts...> resume(result_type& result) { return std::move(*result); }
...
template<typename R, typename... Ts, typename Executor>
class awaitable_async_op_handler<R(asio::error_code, Ts...), Executor> : public awaitable_thread<Executor> {
public:
	struct result_type {
		asio::error_code* ec_;
		std::tuple<Ts...>* value_;
...
template<typename R, typename... Ts, typename Executor>
class awaitable_async_op_handler<R(std::exception_ptr, Ts...), Executor> : public awaitable_thread<Executor> {
public:
	struct result_type {
		std::exception_ptr* ex_;
		std::tuple<Ts...>* value_;
	...
	template<typename... Args> void operator()(std::exception_ptr ex, Args&&... args)
	{
		result_.ex_ = &ex;
		std::tuple<Ts...> value(std::forward<Args>(args)...);
		result_.value_ = &value;
...
template<typename Signature, typename Op, typename Executor> class awaitable_async_op {
public:
	typedef awaitable_async_op_handler<Signature, Executor> handler_type;

	awaitable_async_op(Op&& o,
	    awaitable_frame_base<Executor>* frame
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
#if defined(ASIO_HAS_SOURCE_LOCATION)
	    ,
	...
	{
		frame_->after_suspend([](void* arg) {
			awaitable_async_op* self = static_cast<awaitable_async_op*>(arg);
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
#if defined(ASIO_HAS_SOURCE_LOCATION)
			ASIO_HANDLER_LOCATION(
			    (self->location_.file_name(), self->location_.line(), self->location_.function_name()));
#endif // defined(ASIO_HAS_SOURCE_LOCATION)
#endif // defined(ASIO_ENABLE_HANDLER_TRACKING)
			std::forward<Op&&>(self->op_)(handler_type(self->frame_->detach_thread(), self->result_));
		}, this);
	}
