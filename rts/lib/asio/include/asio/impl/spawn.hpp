...
public:
	    ...
	    cancellation_state cancel_state = cancellation_state())
	{
		...
		callee_type callee(entry_point<decay_t<F>>(static_cast<F&&>(f), &spawned_thread), attributes);
	...
	static spawned_thread_base* spawn(F&& f,
	    ...
	    cancellation_state cancel_state = cancellation_state())
	{
		return spawn(static_cast<F&&>(f), boost::coroutines::attributes(), parent_cancel_slot, cancel_state);
	}
...
private:
	template<typename Function> class entry_point {
	public:
		template<typename F>
		entry_point(F&& f, spawned_coroutine_thread** spawned_thread_out)
...
public:
	    ...
	    cancellation_state cancel_state = cancellation_state())
	{
		...
		fiber_type callee(allocator_arg_t(), static_cast<StackAllocator&&>(stack_allocator),
		    entry_point<decay_t<F>>(static_cast<F&&>(f), &spawned_thread));
	...
	static spawned_thread_base* spawn(F&& f,
	    ...
	    cancellation_state cancel_state = cancellation_state())
	{
		return spawn(allocator_arg_t(), boost::context::fixedsize_stack(), static_cast<F&&>(f), parent_cancel_slot,
		    cancel_state);
	}
...
private:
	...
	public:
		template<typename F>
		entry_point(F&& f, spawned_fiber_thread** spawned_thread_out)
		    : function_(static_cast<F&&>(f))
		    , spawned_thread_out_(spawned_thread_out)
		{
		}
		...
		{
			Function function(static_cast<Function&&>(function_));
			spawned_fiber_thread spawned_thread(static_cast<fiber_type&&>(caller));
			*spawned_thread_out_ = &spawned_thread;
			spawned_thread_out_ = 0;
			spawned_thread.suspend();
#if !defined(ASIO_NO_EXCEPTIONS)
...
template<typename Executor> class spawn_handler_base {
public:
	...
	executor_type get_executor() const noexcept { return yield_.executor_; }

	cancellation_slot_type get_cancellation_slot() const noexcept { return spawned_thread_->get_cancellation_slot(); }
...
template<typename Executor, typename Signature> class spawn_handler;

template<typename Executor, typename R> class spawn_handler<Executor, R()> : public spawn_handler_base<Executor> {
public:
...
class spawn_handler<Executor, R(asio::error_code)> : public spawn_handler_base<Executor> {
public:
	typedef void return_type;
...
template<typename Executor, typename R>
class spawn_handler<Executor, R(exception_ptr)> : public spawn_handler_base<Executor> {
public:
	typedef void return_type;
...
class spawn_handler<Executor, R(T)> : public spawn_handler_base<Executor> {
public:
	...
	static return_type on_resume(result_type& result) { return static_cast<return_type&&>(*result); }
...
template<typename Executor, typename R, typename T>
class spawn_handler<Executor, R(asio::error_code, T)> : public spawn_handler_base<Executor> {
public:
	typedef T return_type;
...
template<typename Executor, typename R, typename T>
class spawn_handler<Executor, R(exception_ptr, T)> : public spawn_handler_base<Executor> {
public:
	typedef T return_type;
...
class spawn_handler<Executor, R(Ts...)> : public spawn_handler_base<Executor> {
public:
...
template<typename Executor, typename R, typename... Ts>
class spawn_handler<Executor, R(asio::error_code, Ts...)> : public spawn_handler_base<Executor> {
public:
	typedef std::tuple<Ts...> return_type;
	...
	template<typename... Args> void operator()(asio::error_code ec, Args&&... args)
	{
		return_type value(static_cast<Args&&>(args)...);
		if (this->yield_.ec_) {
			*this->yield_.ec_ = ec;
...
template<typename Executor, typename R, typename... Ts>
class spawn_handler<Executor, R(exception_ptr, Ts...)> : public spawn_handler_base<Executor> {
public:
	typedef std::tuple<Ts...> return_type;
	...
	template<typename... Args> void operator()(exception_ptr ex, Args&&... args)
	{
		return_type value(static_cast<Args&&>(args)...);
		result_.ex_ = &ex;
		result_.value_ = &value;
...
template<typename Executor, typename Signature> class async_result<basic_yield_context<Executor>, Signature> {
public:
	...
	static return_type initiate(Initiation&& init, const basic_yield_context<Executor>& yield, InitArgs&&... init_args)
	{
		typename handler_type::result_type result = typename handler_type::result_type();

		yield.spawned_thread_->suspend_with([&]() {
			static_cast<Initiation&&>(init)(handler_type(yield, result), static_cast<InitArgs&&>(init_args)...);
		});
	...
	template<typename Initiation, typename... InitArgs> struct suspend_with_helper {
		typename handler_type::result_type& result_;
		Initiation&& init_;
		const basic_yield_context<Executor>& yield_;
		std::tuple<InitArgs&&...> init_args_;

		template<std::size_t... I> void do_invoke(detail::index_sequence<I...>)
		{
			static_cast<Initiation&&>(init_)(
			    handler_type(yield_, result_), static_cast<InitArgs&&>(std::get<I>(init_args_))...);
		}

		void operator()() { this->do_invoke(detail::make_index_sequence<sizeof...(InitArgs)>()); }
	};
	...
	static return_type initiate(Initiation&& init, const basic_yield_context<Executor>& yield, InitArgs&&... init_args)
	{
		...
		yield.spawned_thread_->suspend_with(suspend_with_helper<Initiation, InitArgs...>{result,
		    static_cast<Initiation&&>(init), yield, std::tuple<InitArgs&&...>(static_cast<InitArgs&&>(init_args)...)});
...
template<typename Executor, typename Function, typename Handler> class spawn_entry_point {
public:
	template<typename F, typename H>
	spawn_entry_point(const Executor& ex, F&& f, H&& h)
	    : executor_(ex)
	    , function_(static_cast<F&&>(f))
	...
	void operator()(spawned_thread_base* spawned_thread)
	{
		...
		this->call(yield, void_type<result_of_t<Function(basic_yield_context<Executor>)>>());
	}
...
private:
	void call(const basic_yield_context<Executor>& yield, void_type<void>)
	{
...
#endif // !defined(ASIO_NO_EXCEPTIONS)
		{
			...
			if (!yield.spawned_thread_->has_context_switched())
				(post)(yield);
			detail::binder1<Handler, exception_ptr> handler(handler_, exception_ptr());
	...
	template<typename T> void call(const basic_yield_context<Executor>& yield, void_type<T>)
	{
#if !defined(ASIO_NO_EXCEPTIONS)
		try
#endif // !defined(ASIO_NO_EXCEPTIONS)
		{
			...
			if (!yield.spawned_thread_->has_context_switched())
				(post)(yield);
			detail::binder2<Handler, exception_ptr, T> handler(handler_, exception_ptr(), static_cast<T&&>(result));
			work_.complete(handler, handler.handler_);
		}
#if !defined(ASIO_NO_EXCEPTIONS)
#if defined(ASIO_HAS_BOOST_CONTEXT_FIBER)
...
struct spawn_cancellation_signal_emitter {
	...
	void operator()() { signal_->emit(type_); }
};

template<typename Handler, typename Executor, typename = void> class spawn_cancellation_handler {
public:
	...
	cancellation_slot slot() { return signal_.slot(); }
...
class spawn_cancellation_handler<Handler,
    Executor,
    enable_if_t<is_same<typename associated_executor<Handler, Executor>::asio_associated_executor_is_unspecialised,
        void>::value>> {
public:
	...
	void operator()(cancellation_type_t type) { signal_.emit(type); }

private:
	cancellation_signal signal_;
};

template<typename Executor> class initiate_spawn {
public:
	typedef Executor executor_type;
	...
	template<typename Handler, typename F> void operator()(Handler&& handler, F&& f) const
	{
		typedef decay_t<Handler> handler_type;
		typedef decay_t<F> function_type;
		typedef spawn_cancellation_handler<handler_type, Executor> cancel_handler_type;

		associated_cancellation_slot_t<handler_type> slot = asio::get_associated_cancellation_slot(handler);

		cancel_handler_type* cancel_handler =
		    slot.is_connected() ? &slot.template emplace<cancel_handler_type>(handler, executor_) : 0;

		cancellation_slot proxy_slot(cancel_handler ? cancel_handler->slot() : cancellation_slot());

		cancellation_state cancel_state(proxy_slot);

		(dispatch)(executor_, spawned_thread_resumer(default_spawned_thread_type::spawn(
		                          spawn_entry_point<Executor, function_type, handler_type>(
		                              executor_, static_cast<F&&>(f), static_cast<Handler&&>(handler)),
	...
	void operator()(Handler&& handler, allocator_arg_t, StackAllocator&& stack_allocator, F&& f) const
	{
		typedef decay_t<Handler> handler_type;
		typedef decay_t<F> function_type;
		typedef spawn_cancellation_handler<handler_type, Executor> cancel_handler_type;
		...
		(dispatch)(executor_, spawned_thread_resumer(spawned_fiber_thread::spawn(allocator_arg_t(),
...
template<typename Executor,
    typename F,
    ASIO_COMPLETION_TOKEN_FOR(typename detail::spawn_signature<result_of_t<F(basic_yield_context<Executor>)>>::type)
        CompletionToken>
inline auto spawn(const Executor& ex,
    F&& function,
    CompletionToken&& token,
#if defined(ASIO_HAS_BOOST_COROUTINE)
    constraint_t<!is_same<decay_t<CompletionToken>, boost::coroutines::attributes>::value>,
#endif // defined(ASIO_HAS_BOOST_COROUTINE)
    constraint_t<is_executor<Executor>::value || execution::is_executor<Executor>::value>)
    -> decltype(async_initiate<CompletionToken,
        typename detail::spawn_signature<result_of_t<F(basic_yield_context<Executor>)>>::type>(
        ...
        static_cast<F&&>(function)))
{
	return async_initiate<CompletionToken,
	    typename detail::spawn_signature<result_of_t<F(basic_yield_context<Executor>)>>::type>(
	    detail::initiate_spawn<Executor>(ex), token, static_cast<F&&>(function));
}

template<typename ExecutionContext,
    typename F,
    ASIO_COMPLETION_TOKEN_FOR(typename detail::spawn_signature<
        result_of_t<F(basic_yield_context<typename ExecutionContext::executor_type>)>>::type) CompletionToken>
inline auto spawn(ExecutionContext& ctx,
    F&& function,
    CompletionToken&& token,
#if defined(ASIO_HAS_BOOST_COROUTINE)
    constraint_t<!is_same<decay_t<CompletionToken>, boost::coroutines::attributes>::value>,
    ...
    -> decltype(async_initiate<CompletionToken,
        typename detail::spawn_signature<
            result_of_t<F(basic_yield_context<typename ExecutionContext::executor_type>)>>::type>(
        declval<detail::initiate_spawn<typename ExecutionContext::executor_type>>(),
        token,
        static_cast<F&&>(function)))
{
	return (spawn)(ctx.get_executor(), static_cast<F&&>(function), static_cast<CompletionToken&&>(token));
}
        ...
        static_cast<F&&>(function)))
{
	return async_initiate<CompletionToken,
	    typename detail::spawn_signature<result_of_t<F(basic_yield_context<Executor>)>>::type>(
	    detail::initiate_spawn<Executor>(ex), token, allocator_arg_t(), static_cast<StackAllocator&&>(stack_allocator),
	    static_cast<F&&>(function));
}
...
inline auto spawn(ExecutionContext& ctx,
    ...
    -> decltype(async_initiate<CompletionToken,
        ...
        static_cast<F&&>(function)))
{
	return (spawn)(ctx.get_executor(), allocator_arg_t(), static_cast<StackAllocator&&>(stack_allocator),
...
template<typename Executor, typename Function, typename Handler> class old_spawn_entry_point {
public:
	template<typename F, typename H>
...
template<typename Function> inline void spawn(Function&& function, const boost::coroutines::attributes& attributes)
{
	associated_executor_t<decay_t<Function>> ex((get_associated_executor)(function));
...
void spawn(Handler&& handler,
    Function&& function,
    const boost::coroutines::attributes& attributes,
    constraint_t<!is_executor<decay_t<Handler>>::value && !execution::is_executor<decay_t<Handler>>::value &&
                 !is_convertible<Handler&, execution_context&>::value>)
{
	...
	(dispatch)(ex, detail::spawned_thread_resumer(detail::spawned_coroutine_thread::spawn(
	                   detail::old_spawn_entry_point<executor_type, decay_t<Function>, void (*)()>(
	                       ex, static_cast<Function&&>(function), &detail::default_spawn_handler),
	                   attributes)));
}

template<typename Executor, typename Function>
void spawn(basic_yield_context<Executor> ctx, Function&& function, const boost::coroutines::attributes& attributes)
{
	(dispatch)(ctx.get_executor(),
	    detail::spawned_thread_resumer(detail::spawned_coroutine_thread::spawn(
	        detail::old_spawn_entry_point<Executor, decay_t<Function>, void (*)()>(
	            ctx.get_executor(), static_cast<Function&&>(function), &detail::default_spawn_handler),
	        attributes)));
}
...
inline void spawn(const Executor& ex,
    ...
    constraint_t<is_executor<Executor>::value || execution::is_executor<Executor>::value>)
{
	asio::spawn(asio::strand<Executor>(ex), static_cast<Function&&>(function), attributes);
}

template<typename Function, typename Executor>
inline void spawn(const strand<Executor>& ex, Function&& function, const boost::coroutines::attributes& attributes)
{
	asio::spawn(asio::bind_executor(ex, &detail::default_spawn_handler), static_cast<Function&&>(function), attributes);
}
...
inline void
spawn(const asio::io_context::strand& s, Function&& function, const boost::coroutines::attributes& attributes)
{
	asio::spawn(asio::bind_executor(s, &detail::default_spawn_handler), static_cast<Function&&>(function), attributes);
}
...
inline void spawn(ExecutionContext& ctx,
    ...
    constraint_t<is_convertible<ExecutionContext&, execution_context&>::value>)
{
	asio::spawn(ctx.get_executor(), static_cast<Function&&>(function), attributes);
}
