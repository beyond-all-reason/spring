...
{
	detail::scoped_ptr<Service> svc(new Service(e, static_cast<Args&&>(args)...));
...
template<typename Service> inline void add_service(execution_context& e, Service* svc)
{
	// Check that Service meets the necessary type requirements.
	(void)static_cast<execution_context::service*>(static_cast<Service*>(0));
...
inline execution_context& execution_context::service::context() { return owner_; }
