 ...
 */
template<typename Stream> class buffered_write_stream : private noncopyable {
public:
	...
	auto async_read_some(const MutableBufferSequence& buffers,
	    ...
	    -> decltype(declval<conditional_t<true, Stream&, ReadHandler>>().async_read_some(buffers,
	        static_cast<ReadHandler&&>(handler)))
	{
		return next_layer_.async_read_some(buffers, static_cast<ReadHandler&&>(handler));
	}
	...
	std::size_t in_avail() { return next_layer_.in_avail(); }

	/// Determine the amount of data that may be read without blocking.
	std::size_t in_avail(asio::error_code& ec) { return next_layer_.in_avail(ec); }

private:
	/// Copy data into the internal buffer from the specified source buffer.
	/// Returns the number of bytes copied.
	template<typename ConstBufferSequence> std::size_t copy(const ConstBufferSequence& buffers);
