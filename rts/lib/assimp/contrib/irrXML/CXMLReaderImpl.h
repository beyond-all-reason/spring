...
template<class char_type, class superclass> class CXMLReaderImpl : public IIrrXMLReader<char_type, superclass> {
public:
	//! Constructor
	...
	//! Destructor
	virtual ~CXMLReaderImpl() { delete[] TextData; }
	...
	virtual EXML_NODE getNodeType() const { return CurrentNodeType; }

	//! Returns attribute count of the current XML node.
	virtual int getAttributeCount() const { return Attributes.size(); }
	...
	int getAttributeValueAsInt(const char_type* name) const { return (int)getAttributeValueAsFloat(name); }

	//! Returns the value of an attribute as integer.
	int getAttributeValueAsInt(int idx) const { return (int)getAttributeValueAsFloat(idx); }
	...
	virtual const char_type* getNodeName() const { return NodeName.c_str(); }

	//! Returns data of the current node.
	virtual const char_type* getNodeData() const { return NodeName.c_str(); }

	//! Returns if an element is an empty element, like <foo />
	virtual bool isEmptyElement() const { return IsEmptyElement; }

	//! Returns format of the source xml file.
	virtual ETEXT_FORMAT getSourceFormat() const { return SourceFormat; }

	//! Returns format of the strings returned by the parser.
	virtual ETEXT_FORMAT getParserFormat() const { return TargetFormat; }
...
private:
	...
	void parseCurrentNode()
	{
		...
		switch (*P) {
		case L'/': parseClosingXMLElement(); break;
		case L'?': ignoreDefinition(); break;
		case L'!':
			if (!parseCDATA())
				parseComment();
			break;
		default: parseOpeningXMLElement(); break;
		}
	}
		...
		while (*P != L'>') {
			if (isWhiteSpace(*P))
				...
				if (*P != L'/') {
					...
					while (*P != attributeQuoteChar && *P) ++P;
					...
					SAttribute attr;
					attr.Name =
					    core::string<char_type>(attributeNameBegin, (int)(attributeNameEnd - attributeNameBegin));
	...
	void convertTextData(src_char_type* source, char* pointerToStore, int sizeWithoutHeader)
	{
		...
		if (sizeof(src_char_type) > 1 && isLittleEndian(TargetFormat) != isLittleEndian(SourceFormat))
		...
		if (sizeof(src_char_type) == sizeof(char_type)) {
			...
			for (int i = 0; i < sizeWithoutHeader; ++i) TextData[i] = char_type(source[i] & cc);
	...
	template<class src_char_type> void convertToLittleEndian(src_char_type* t)
	{
		if (sizeof(src_char_type) == 4) {
			...
			while (*t) {
				*t = ((*t & 0xff000000) >> 24) | ((*t & 0x00ff0000) >> 8) | ((*t & 0x0000ff00) << 8) |
				     ((*t & 0x000000ff) << 24);
				++t;
			}
		}
	...
	{
		return f == ETF_ASCII || f == ETF_UTF8 || f == ETF_UTF16_LE || f == ETF_UTF32_LE;
	}

	//! returns true if a character is whitespace
	inline bool isWhiteSpace(char_type c) { return (c == ' ' || c == '\t' || c == '\n' || c == '\r'); }
	...
	void storeTargetFormat()
	{
		...
		case 1: TargetFormat = ETF_UTF8; break;
		case 2: TargetFormat = ETF_UTF16_LE; break;
		case 4: TargetFormat = ETF_UTF32_LE; break;
		default: TargetFormat = ETF_ASCII; // should never happen.
		}
	}
