...
public:
	...
	void reallocate(u32 new_size)
	{
		...
		for (s32 i = 0; i < end; ++i) data[i] = old_data[i];
	...
	void push_front(const T& element)
	{
		...
		for (int i = (int)used; i > 0; --i) data[i] = data[i - 1];
	...
	void insert(const T& element, u32 index = 0)
	{
		...
		for (u32 i = used++; i > index; i--) data[i] = data[i - 1];
	...
	void set_free_when_destroyed(bool f) { free_when_destroyed = f; }
	...
	void operator=(const array<T>& other)
	{
		...
		for (u32 i = 0; i < other.used; ++i) data[i] = other.data[i];
	}
	...
	s32 binary_search(const T& element) { return binary_search(element, 0, used - 1); }
	...
	void erase(u32 index)
	{
		...
		for (u32 i = index + 1; i < used; ++i) data[i - 1] = data[i];
	...
	void erase(u32 index, s32 count)
	{
		...
		for (u32 i = index + count; i < used; ++i) data[i - count] = data[i];
