...
const float fast_atof_table[16] = { // we write [16] here instead of [] to work around a swig bug
    0.0f, 0.1f, 0.01f, 0.001f, 0.0001f, 0.00001f, 0.000001f, 0.0000001f, 0.00000001f, 0.000000001f, 0.0000000001f,
    0.00000000001f, 0.000000000001f, 0.0000000000001f, 0.00000000000001f, 0.000000000000001f};
	...
	while (running) {
		...
		if (new_value < value) {
			DefaultLogger::get()->warn(
			    std::string("Converting the string \"") + in + "\" into a value resulted in overflow.");
			return 0;
		}
...
// ------------------------------------------------------------------------------------
template<typename Real> inline const char* fast_atoreal_move(const char* c, Real& out, bool check_comma = true)
{
	Real f = 0;
