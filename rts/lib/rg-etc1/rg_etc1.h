...
// If preserve_alpha is true, the alpha channel of the destination pixels will not be overwritten. Otherwise, alpha will
// be set to 255.
...
// To pack large textures, it is highly recommended to call pack_etc1_block() in parallel, on different blocks, from
// multiple threads (particularly when using cHighQuality).
enum etc1_quality {
	cLowQuality,
	cMediumQuality,
	cHighQuality,
...
// 32-bit RGBA pixels must always be arranged as (R,G,B,A) (R first, A last) in memory, independent of platform
// endianness. A should always be 255. Returns squared error of result. This function is thread safe, and does not
// dynamically allocate any memory. pack_etc1_block() does not currently support "perceptual" colorspace metrics - it
// primarily optimizes for RGB RMSE.
