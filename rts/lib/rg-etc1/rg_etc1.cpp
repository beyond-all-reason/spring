...
//
// v1.04 - 5/15/14 - Fix signed vs. unsigned subtraction problem (noticed when compiled with gcc) in
// pack_etc1_block_init().
//         This issue would cause an assert when this func. was called in debug. (Note this module was developed/testing
//         with MSVC, I still need to test it throughly when compiled with gcc.)
//
...
public:
	...
	explicit inline color_quad_u8(parameter_t y, parameter_t alpha = component_traits::cMax) { set(y, alpha); }
	...
	inline color_quad_u8&
	set(parameter_t red, parameter_t green, parameter_t blue, parameter_t alpha = component_traits::cMax)
	{
	...
	{
		for (uint i = 0; i < cNumComps; i++) c[i] = static_cast<component_t>(rg_etc1::clamp<parameter_t>(c[i], l, h));
		return *this;
	}
	...
	{
		return rg_etc1::square(r - c.r) + rg_etc1::square(g - c.g) + rg_etc1::square(b - c.b) +
		       rg_etc1::square(a - c.a);
	}

	inline bool rgb_equals(const color_quad_u8& rhs) const { return (r == rhs.r) && (g == rhs.g) && (b == rhs.b); }

	inline bool operator==(const color_quad_u8& rhs) const { return m_u32 == rhs.m_u32; }

	color_quad_u8& operator+=(const color_quad_u8& other)
	{
		for (uint i = 0; i < 4; i++) c[i] = static_cast<component_t>(clamp(c[i] + other.c[i]));
		return *this;
	}
...
struct vec3F {
	...
	{
		for (uint i = 0; i < 3; i++) m_s[i] += other.m_s[i];
		return *this;
	}

	inline vec3F& operator*=(float s)
	{
		for (uint i = 0; i < 3; i++) m_s[i] *= s;
		return *this;
	}
};
...
// Given an ETC1 diff/inten_table/selector, and an 8-bit desired color, this table encodes the best packed_color in the
// low byte, and the abs error in the high byte.
static uint16 g_etc1_inverse_lookup[2 * 8 * 4][256]; // [diff/inten_table/selector][desired_color]

// g_color8_to_etc_block_config[color][table_index] = Supplies for each 8-bit color value a list of packed ETC1
// diff/intensity table/selectors/packed_colors that map to that color. To pack: diff | (inten << 1) | (selector << 4) |
// (packed_c << 8)
static const uint16 g_color8_to_etc_block_config_0_255[2][33] = {
    {0x0000, 0x0010, 0x0002, 0x0012, 0x0004, 0x0014, 0x0006, 0x0016, 0x0008, 0x0018, 0x000A, 0x001A, 0x000C, 0x001C,
     0x000E, 0x001E, 0x0001, 0x0011, 0x0003, 0x0013, 0x0005, 0x0015, 0x0007, 0x0017, 0x0009, 0x0019, 0x000B, 0x001B,
     0x000D, 0x001D, 0x000F, 0x001F, 0xFFFF},
    {0x0F20, 0x0F30, 0x0E32, 0x0F22, 0x0E34, 0x0F24, 0x0D36, 0x0F26, 0x0C38, 0x0E28, 0x0B3A, 0x0E2A, 0x093C, 0x0E2C,
     0x053E, 0x0D2E, 0x1E31, 0x1F21, 0x1D33, 0x1F23, 0x1C35, 0x1E25, 0x1A37, 0x1E27, 0x1839, 0x1D29, 0x163B, 0x1C2B,
     0x133D, 0x1B2D, 0x093F, 0x1A2F, 0xFFFF},
};
...
struct etc1_block {
	...
	inline void clear() { zero_this(this); }
	...
	inline bool get_flip_bit() const { return (m_bytes[3] & 1) != 0; }
	...
	inline bool get_diff_bit() const { return (m_bytes[3] & 2) != 0; }
	...
	static bool
	unpack_color5(color_quad_u8& result, uint16 packed_color5, uint16 packed_delta3, bool scaled, uint alpha = 255U);
	...
	static bool
	get_diff_subblock_colors(color_quad_u8* pDst, uint16 packed_color5, uint16 packed_delta3, uint table_idx);
...
struct etc1_solution_coordinates {
	...
	inline etc1_solution_coordinates(const etc1_solution_coordinates& other) { *this = other; }
...
private:
	...
	bool evaluate_solution(const etc1_solution_coordinates& coords,
	    potential_solution& trial_solution,
	    potential_solution* pBest_solution);
	bool evaluate_solution_fast(const etc1_solution_coordinates& coords,
	    potential_solution& trial_solution,
	    potential_solution* pBest_solution);
};
...
bool etc1_optimizer::compute()
{
	...
	// Scan through a subset of the 3D lattice centered around the avg block color trying each 3D (555 or 444) lattice
	// point as a potential block color. Each time a better solution is found try to refine the current solution's block
	// color based of the current selectors and intensity table index.
	for (int zdi = 0; zdi < scan_delta_size; zdi++) {
		...
		for (int ydi = 0; ydi < scan_delta_size; ydi++) {
			...
			for (int xdi = 0; xdi < scan_delta_size; xdi++) {
				...
				// Now we have the input block, the avg. color of the input pixels, a set of trial selector indices, and
				// the block color+intensity index. Now, for each component, attempt to refine the current solution by
				// solving a simple linear equation. For example, for 4 colors: The goal is: pixel0 -
				// (block_color+inten_table[selector0]) + pixel1 - (block_color+inten_table[selector1]) + pixel2 -
				// (block_color+inten_table[selector2]) + pixel3 - (block_color+inten_table[selector3]) = 0 Rearranging
				// this: (pixel0 + pixel1 + pixel2 + pixel3) - (block_color+inten_table[selector0]) -
				// (block_color+inten_table[selector1]) - (block_color+inten_table[selector2]) -
				// (block_color+inten_table[selector3]) = 0 (pixel0 + pixel1 + pixel2 + pixel3) - block_color -
				// inten_table[selector0] - block_color-inten_table[selector1] - block_color-inten_table[selector2] -
				// block_color-inten_table[selector3] = 0 (pixel0 + pixel1 + pixel2 + pixel3) - 4*block_color -
				// inten_table[selector0] - inten_table[selector1] - inten_table[selector2] - inten_table[selector3] = 0
				// (pixel0 + pixel1 + pixel2 + pixel3) - 4*block_color - (inten_table[selector0] +
				// inten_table[selector1] + inten_table[selector2] + inten_table[selector3]) = 0 (pixel0 + pixel1 +
				// pixel2 + pixel3)/4 - block_color - (inten_table[selector0] + inten_table[selector1] +
				// inten_table[selector2] + inten_table[selector3])/4 = 0 block_color = (pixel0 + pixel1 + pixel2 +
				// pixel3)/4 - (inten_table[selector0] + inten_table[selector1] + inten_table[selector2] +
				// inten_table[selector3])/4 So what this means: optimal_block_color = avg_input - avg_inten_delta So
				// the optimal block color can be computed by taking the average block color and subtracting the current
				// average of the intensity delta. Unfortunately, optimal_block_color must then be quantized to 555 or
				// 444 so it's not always possible to improve matters using this formula. Also, the above formula is for
				// unclamped intensity deltas. The actual implementation takes into account clamping.

				const uint max_refinement_trials =
				    (m_pParams->m_quality == cLowQuality) ? 2 : (((xd | yd | zd) == 0) ? 4 : 2);
				for (uint refinement_trial = 0; refinement_trial < max_refinement_trials; refinement_trial++) {
					...
					const int br1 = rg_etc1::clamp<int>(
					    static_cast<uint>((m_avg_color[0] - avg_delta_r_f) * m_limit / 255.0f + .5f), 0, m_limit);
					const int bg1 = rg_etc1::clamp<int>(
					    static_cast<uint>((m_avg_color[1] - avg_delta_g_f) * m_limit / 255.0f + .5f), 0, m_limit);
					const int bb1 = rg_etc1::clamp<int>(
					    static_cast<uint>((m_avg_color[2] - avg_delta_b_f) * m_limit / 255.0f + .5f), 0, m_limit);

					bool skip = false;

					if ((mbr == br1) && (mbg == bg1) && (mbb == bb1))
						skip = true;
					else if ((br1 == m_best_solution.m_coords.m_unscaled_color.r) &&
					         (bg1 == m_best_solution.m_coords.m_unscaled_color.g) &&
					         (bb1 == m_best_solution.m_coords.m_unscaled_color.b))
						skip = true;
...
#ifdef RG_ETC1_BUILD_DEBUG
	{
		...
		for (uint i = 0; i < n; i++) actual_error += pSrc_pixels[i].squared_distance_rgb(block_colors[pSelectors[i]]);
...
void etc1_optimizer::init(const params& p, results& r)
{
	...
	if (m_pParams->m_quality <= cMediumQuality) {
		m_pSorted_luma_indices =
		    indirect_radix_sort(n, m_sorted_luma[0], m_sorted_luma[1], m_luma, 0, sizeof(m_luma[0]), false);
		m_pSorted_luma = m_sorted_luma[0];
		if (m_pSorted_luma_indices == m_sorted_luma[0])
			m_pSorted_luma = m_sorted_luma[1];

		for (uint i = 0; i < n; i++) m_pSorted_luma[i] = m_luma[m_pSorted_luma_indices[i]];
	}
...
bool etc1_optimizer::evaluate_solution(const etc1_solution_coordinates& coords,
    potential_solution& trial_solution,
    potential_solution* pBest_solution)
{
	...
	for (uint inten_table = 0; inten_table < cETC1IntenModifierValues; inten_table++) {
		...
		for (uint c = 0; c < n; c++) {
			...
			uint best_error = rg_etc1::square(src_pixel.r - block_colors[0].r) +
			                  rg_etc1::square(src_pixel.g - block_colors[0].g) +
			                  rg_etc1::square(src_pixel.b - block_colors[0].b);

			uint trial_error = rg_etc1::square(src_pixel.r - block_colors[1].r) +
			                   rg_etc1::square(src_pixel.g - block_colors[1].g) +
			                   rg_etc1::square(src_pixel.b - block_colors[1].b);
			if (trial_error < best_error) {
				best_error = trial_error;
				best_selector_index = 1;
			}

			trial_error = rg_etc1::square(src_pixel.r - block_colors[2].r) +
			              rg_etc1::square(src_pixel.g - block_colors[2].g) +
			              rg_etc1::square(src_pixel.b - block_colors[2].b);
			if (trial_error < best_error) {
				best_error = trial_error;
				best_selector_index = 2;
			}

			trial_error = rg_etc1::square(src_pixel.r - block_colors[3].r) +
			              rg_etc1::square(src_pixel.g - block_colors[3].g) +
			              rg_etc1::square(src_pixel.b - block_colors[3].b);
			if (trial_error < best_error) {
				best_error = trial_error;
...
bool etc1_optimizer::evaluate_solution_fast(const etc1_solution_coordinates& coords,
    potential_solution& trial_solution,
    potential_solution* pBest_solution)
{
	if (m_pParams->m_constrain_against_base_color5) {
		...
		if ((rg_etc1::minimum(dr, dg, db) < cETC1ColorDeltaMin) ||
		    (rg_etc1::maximum(dr, dg, db) > cETC1ColorDeltaMax)) {
			trial_solution.m_valid = false;
			return false;
		}
	}
	...
	for (int inten_table = cETC1IntenModifierValues - 1; inten_table >= 0; --inten_table) {
		...
		// evaluate_solution_fast() enforces/assumesd a total ordering of the input colors along the intensity (1,1,1)
		// axis to more quickly classify the inputs to selectors. The inputs colors have been presorted along the
		// projection onto this axis, and ETC1 block colors are always ordered along the intensity axis, so this
		// classification is fast. 0   1   2   3
		//   01  12  23
		const uint block_inten_midpoints[3] = {
		    block_inten[0] + block_inten[1], block_inten[1] + block_inten[2], block_inten[2] + block_inten[3]};
		...
		if ((m_pSorted_luma[n - 1] * 2) < block_inten_midpoints[0]) {
			...
			for (uint c = 0; c < n; c++) total_error += block_colors[0].squared_distance_rgb(pSrc_pixels[c]);
		}
			...
			for (uint c = 0; c < n; c++) total_error += block_colors[3].squared_distance_rgb(pSrc_pixels[c]);
		}
		else {
		...
		for (uint inten = 0; inten < 8; inten++) {
			for (uint selector = 0; selector < 4; selector++) {
				...
				for (uint color = 0; color < 256; color++) {
					...
					g_etc1_inverse_lookup[inverse_table_index][color] =
					    static_cast<uint16>(best_packed_c | (best_error << 8));
				}
			}
		}
	}
...
// For random 888 inputs, MSE results are better than Erricson's ETC1 packer in "slow" mode ~9.5% of the time, is
// slightly worse only ~.01% of the time, and is equal the rest of the time.
static uint64 pack_etc1_block_solid_color(etc1_block& block, const uint8* pColor, etc1_pack_params& pack_params)
{
	...
	// For each possible 8-bit value, there is a precomputed list of diff/inten/selector configurations that allow that
	// 8-bit value to be encoded with no error.
	for (uint i = 0; i < 3; i++) {
		...
		for (int delta = -delta_range; delta <= delta_range; delta++) {
			...
			do {
				...
				const uint trial_error =
				    rg_etc1::square(c_plus_delta - pColor[i]) + rg_etc1::square(p1 >> 8) + rg_etc1::square(p2 >> 8);
				if (trial_error < best_error) {
					best_error = trial_error;
					best_x = x;
...
static uint pack_etc1_block_solid_color_constrained(etc1_optimizer::results& results,
    uint num_colors,
    const uint8* pColor,
...
unsigned int pack_etc1_block(void* pETC1_block, const unsigned int* pSrc_pixels_rgba, etc1_pack_params& pack_params)
{
		...
		for (uint use_color4 = 0; use_color4 < 2; use_color4++) {
			...
			for (subblock = 0; subblock < 2; subblock++) {
				...
				if ((params.m_quality >= cMediumQuality) && ((subblock) || (use_color4))) {
					...
					for (r = 7; r >= 1; --r)
						if (subblock_pixels[r].m_u32 != subblock_pixel0_u32)
							break;
					if (!r) {
						pack_etc1_block_solid_color_constrained(results[2], 8, &subblock_pixels[0].r, pack_params,
						    !use_color4, (subblock && !use_color4) ? &results[0].m_block_color_unscaled : NULL);
					}
				}
				...
				if (params.m_quality >= cMediumQuality) {
					// TODO: Fix fairly arbitrary/unrefined thresholds that control how far away to scan for potentially
					// better solutions.
	...
	RG_ETC1_ASSERT(best_use_color4 || (rg_etc1::minimum(dr, dg, db) >= cETC1ColorDeltaMin) &&
	                                      (rg_etc1::maximum(dr, dg, db) <= cETC1ColorDeltaMax));

	if (best_use_color4) {
		dst_block.m_bytes[0] = static_cast<uint8>(
		    best_results[1].m_block_color_unscaled.r | (best_results[0].m_block_color_unscaled.r << 4));
		dst_block.m_bytes[1] = static_cast<uint8>(
		    best_results[1].m_block_color_unscaled.g | (best_results[0].m_block_color_unscaled.g << 4));
		dst_block.m_bytes[2] = static_cast<uint8>(
		    best_results[1].m_block_color_unscaled.b | (best_results[0].m_block_color_unscaled.b << 4));
	}
	else {
		if (dr < 0)
			dr += 8;
	...
	dst_block.m_bytes[3] =
	    static_cast<uint8>((best_results[1].m_block_inten_table << 2) | (best_results[0].m_block_inten_table << 5) |
