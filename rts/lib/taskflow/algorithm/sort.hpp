...
void parallel_pdqsort(
  ...
  while (true) {
    ...
    rt.silent_async(
      [&rt, begin, pivot_pos, comp, bad_allowed, cutoff, leftmost] () mutable {
        parallel_pdqsort<Iter, Compare, Branchless>(
          rt, begin, pivot_pos, comp, cutoff, bad_allowed, leftmost
        );
      }
    );
    begin = pivot_pos + 1;
...
TF_FORCE_INLINE auto make_sort_task(B beg, E end, C cmp, size_t cf) {

  return [b=beg, e=end, cmp, cf] (Runtime& rt) mutable {
    ...
    >(rt, beg, end, cmp, cf, log2(end - beg));
...
TF_FORCE_INLINE auto make_sort_task(B beg, E end) {
  ...
  constexpr size_t cf = detail::parallel_sort_cutoff<value_type>();
  return make_sort_task(beg, end, std::less<value_type>{}, cf);
}
...
TF_FORCE_INLINE auto make_sort_task(B beg, E end, C cmp) {
  using value_type = std::decay_t<decltype(*std::declval<B>())>;
  constexpr size_t cf = detail::parallel_sort_cutoff<value_type>();
  return make_sort_task(beg, end, cmp, cf);
}
...
// Function: sort
template <typename B, typename E, typename C>
Task FlowBuilder::sort(B beg, E end, C cmp, size_t cf) {
    return emplace(make_sort_task(beg, end, cmp, cf));
}
