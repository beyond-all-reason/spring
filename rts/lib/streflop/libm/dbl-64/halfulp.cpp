...
/*                uroot.c                                               */
/*                                                                      */
/*Routine halfulp(StreflopDouble x, StreflopDouble y) computes x^y where result does    */
...
StreflopDouble __ieee754_sqrt(StreflopDouble x);
...
{
  mynumber v;
  StreflopDouble z,u,uu,j1,j2,j3,j4,j5;
  int4 k,l,m,n;
  if (y <= 0) {               /*if power is negative or zero */
    ...
    z = (StreflopDouble) k;
    return (z*y == -1075.0)?StreflopDouble(0.0): -StreflopDouble(10.0);
  }
                              /* if y > 0  */
  v.x() = y;
    if (v.i[LOW_HALF] != 0) return -10.0;
  ...
  if (((v.i[HIGH_HALF]&0x000fffff)|v.i[LOW_HALF]) == 0) {
    ...
    return (((StreflopDouble) k)*y == -1075.0)?StreflopDouble(0.0):-StreflopDouble(10.0);
  }
