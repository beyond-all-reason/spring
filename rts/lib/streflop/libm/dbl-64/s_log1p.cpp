...
#ifdef __STDC__
	StreflopDouble __log1p(StreflopDouble x)
#else
	StreflopDouble __log1p(x)
	StreflopDouble x;
#endif
{
	StreflopDouble hfsq,f,c,s,z,R,u,z2,z4,z6,R1,R2,R3,R4;
	int32_t k,hx,hu,ax;
	...
	if(k!=0) {
		...
		GET_HIGH_WORD(hu,u);
	        k  = (hu>>20)-1023;
	        c  = (k>0)? StreflopDouble(1.0)-(u-x):x-(u-StreflopDouble(1.0));/* correction term */
		c /= u;
	    } else {
		u  = x;
		GET_HIGH_WORD(hu,u);
