...
/*                                                                */
/* Ultimate asin/acos routines. Given an IEEE StreflopDouble machine      */
...
void __doasin(StreflopDouble x, StreflopDouble dx, StreflopDouble w[]);
void __dubsin(StreflopDouble x, StreflopDouble dx, StreflopDouble v[]);
void __dubcos(StreflopDouble x, StreflopDouble dx, StreflopDouble v[]);
void __docos(StreflopDouble x, StreflopDouble dx, StreflopDouble v[]);
StreflopDouble __sin32(StreflopDouble x, StreflopDouble res, StreflopDouble res1);
StreflopDouble __cos32(StreflopDouble x, StreflopDouble res, StreflopDouble res1);

/***************************************************************************/
/* An ultimate asin routine. Given an IEEE StreflopDouble machine number x         */
/* it computes the correctly rounded (to nearest) value of arcsin(x)       */
/***************************************************************************/
namespace streflop_libm {
StreflopDouble __ieee754_asin(StreflopDouble x){
  StreflopDouble x1,x2,xx,s1,s2,res1,p,t,res,r,cor,cc,y,c,z,w[2];
  mynumber u,v;
  int4 k,m,n;
#if 0
  int4 nn;
  ...
  if (k<0x3ff00000)  {
    z = 0.5*((m>0)?(StreflopDouble(1.0)-x):(StreflopDouble(1.0)+x));
    v.x()=z;
    k=v.i[HIGH_HALF];
...
StreflopDouble __ieee754_acos(StreflopDouble x)
{
  StreflopDouble x1,x2,xx,s1,s2,res1,p,t,res,r,cor,cc,y,c,z,w[2],eps;
#if 0
  StreflopDouble fc;
#endif
  mynumber u,v;
...
/*-------------------0.921875 <= |x| < 0.953125 ------------------*/
  ...
  if (k==0x3ff00000 && u.i[LOW_HALF]==0) return (m>0)?StreflopDouble(0.0):2.0*hp0.x();
  else
  if (k>0x7ff00000 || (k == 0x7ff00000 && u.i[LOW_HALF] != 0)) return x;
  else {
    u.i[HIGH_HALF]=0x7ff00000;
