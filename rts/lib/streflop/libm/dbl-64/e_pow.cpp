...
/* An ultimate power routine. Given two IEEE StreflopDouble machine numbers y,x    */
...
StreflopDouble __exp1(StreflopDouble x, StreflopDouble xx, StreflopDouble error);
static StreflopDouble log1(StreflopDouble x, StreflopDouble *delta, StreflopDouble *error);
static StreflopDouble my_log2(StreflopDouble x, StreflopDouble *delta, StreflopDouble *error);
StreflopDouble __slowpow(StreflopDouble x, StreflopDouble y,StreflopDouble z);
static StreflopDouble power1(StreflopDouble x, StreflopDouble y);
static int checkint(StreflopDouble x);

/***************************************************************************/
/* An ultimate power routine. Given two IEEE StreflopDouble machine numbers y,x    */
/* it computes the correctly rounded (to nearest) value of X^y.            */
/***************************************************************************/
StreflopDouble __ieee754_pow(StreflopDouble x, StreflopDouble y) {
  StreflopDouble z,a,aa,error, t,a1,a2,y1,y2;
#if 0
  StreflopDouble gor=1.0;
#endif
  mynumber u,v;
  int k;
  int4 qx,qy;
  ...
  if (x == 0) {
	...
	|| (v.i[HIGH_HALF] & 0x7fffffff) > 0x7ff00000)
      return y;
    if (ABS(y) > 1.0e20) return (y>0)?StreflopDouble(0.0):INF.x();
    k = checkint(y);
    if (k == -1)
      return y < 0 ? StreflopDouble(1.0)/x : x;
    else
      return y < 0 ? StreflopDouble(1.0)/ABS(x) : StreflopDouble(0.0);                               /* return 0 */
  }
  /* if x<0 */
  if (u.i[HIGH_HALF] < 0) {
    k = checkint(y);
    if (k==0) {
      if ((v.i[HIGH_HALF] & 0x7fffffff) == 0x7ff00000 && v.i[LOW_HALF] == 0) {
	if (x == -1.0) return 1.0;
	else if (x > -1.0) return v.i[HIGH_HALF] < 0 ? INF.x() : StreflopDouble(0.0);
	else return v.i[HIGH_HALF] < 0 ? StreflopDouble(0.0) : INF.x();
      }
      else if (u.i[HIGH_HALF] == 0xfff00000 && u.i[LOW_HALF] == 0)
	return y < 0 ? StreflopDouble(0.0) : INF.x();
  ...
  if (qy > 0x7ff00000 || (qy == 0x7ff00000 && v.i[LOW_HALF] != 0))
    return x == 1.0 ? StreflopDouble(1.0) : NaNQ.x();
                                                                 /*  if y<2^-0x7fe   */

  if (qx == 0x7ff00000)                              /* x= 2^-0x3ff */
    {if (y == 0) return NaNQ.x();
    return (y>0)?x:StreflopDouble(0.0); }

  if (qy > 0x45f00000 && qy < 0x7ff00000) {
    if (x == 1.0) return 1.0;
    if (y>0) return (x>1.0)?INF.x():StreflopDouble(0.0);
    if (y<0) return (x<1.0)?INF.x():StreflopDouble(0.0);
  }
...
/****************************************************************************/
/* Computing log(x) (x is left argument). The result is the returned StreflopDouble */
/* + the parameter delta.                                                   */
...
static StreflopDouble log1(StreflopDouble x, StreflopDouble *delta, StreflopDouble *error) {
  ...
  int n;
#endif
  StreflopDouble uu,vv,eps,nx,e,e1,e2,t,t1,t2,res,add=0;
#if 0
  StreflopDouble cor;
#endif
  mynumber u,v;
...
/* Computing log(x)(x is left argument).The result is return StreflopDouble + delta.*/
...
static StreflopDouble my_log2(StreflopDouble x, StreflopDouble *delta, StreflopDouble *error) {
  ...
  StreflopDouble cor;
#endif
  StreflopDouble ou1,ou2,lu1,lu2,ov,lv1,lv2,a,a1,a2;
  StreflopDouble y,yy,z,zz,j1,j2,j3,j4,j5,j6,j7,j8;
  mynumber u,v;
#ifdef BIG_ENDI
  mynumber
/**/ two52          = {{0x43300000, 0x00000000}}; /* 2**52         */
...
/**********************************************************************/
/* Routine receives a StreflopDouble x and checks if it is an integer. If not */
