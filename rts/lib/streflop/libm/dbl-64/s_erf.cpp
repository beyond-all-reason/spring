 ...
 * StreflopDouble erfc(StreflopDouble x)
 *			     x
 *		      2      |\
 *     erf(x)  =  ---------  | exp(-t*t)dt
 *	 	   sqrt(pi) \|
...
two =  2.00000000000000000000e+00, /* 0x40000000, 0x00000000 */
	/* c = (StreflopSimple)0.84506291151 */
...
#ifdef __STDC__
	StreflopDouble __erf(StreflopDouble x)
#else
	StreflopDouble __erf(x)
	StreflopDouble x;
#endif
{
	int32_t hx,ix,i;
	StreflopDouble R,S,P,Q,s,y,z,r;
	GET_HIGH_WORD(hx,x);
	ix = hx&0x7fffffff;
	if(ix>=0x7ff00000) {		/* erf(nan)=nan */
	    i = ((u_int32_t)hx>>31)<<1;
	    return (StreflopDouble)(1-i)+one/x;	/* erf(+-inf)=+-1 */
	}

	if(ix < 0x3feb0000) {		/* |x|<0.84375 */
	    StreflopDouble r1,r2,s1,s2,s3,z2,z4;
	...
	if(ix < 0x3ff40000) {		/* 0.84375 <= |x| < 1.25 */
	    StreflopDouble s2,s4,s6,P1,P2,P3,P4,Q1,Q2,Q3,Q4;
	    s = fabs(x)-one;
#ifdef DO_NOT_USE_THIS
	...
	if(ix< 0x4006DB6E) {	/* |x| < 1/0.35 */
...
#else
	    StreflopDouble R1,R2,R3,R4,S1,S2,S3,S4,s2,s4,s6,s8;
	    ...
	    S=one+s*(sb1+s*(sb2+s*(sb3+s*(sb4+s*(
				sb5+s*(sb6+s*sb7))))));
#else
	    StreflopDouble R1,R2,R3,S1,S2,S3,S4,s2,s4,s6;
...
#ifdef __STDC__
	StreflopDouble __erfc(StreflopDouble x)
#else
	StreflopDouble __erfc(x)
	StreflopDouble x;
#endif
{
	int32_t hx,ix;
	...
	if(ix>=0x7ff00000) {			/* erfc(nan)=nan */
						/* erfc(+-inf)=0,2 */
	    return (StreflopDouble)(((u_int32_t)hx>>31)<<1)+one/x;
	}
