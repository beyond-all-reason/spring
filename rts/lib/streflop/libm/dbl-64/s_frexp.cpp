 ...
 * for non-zero x
 *	x = frexp(arg,&exp);
 * return a StreflopDouble fp quantity x such that 0.5 <= |x| <1.0
...
#ifdef __STDC__
	StreflopDouble __frexp(StreflopDouble x, int *eptr)
#else
	StreflopDouble __frexp(x, eptr)
	StreflopDouble x; int *eptr;
#endif
{
	int32_t hx, ix, lx;
