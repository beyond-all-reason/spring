...
#ifdef __STDC__
	StreflopDouble __expm1(StreflopDouble x)
#else
	StreflopDouble __expm1(x)
	StreflopDouble x;
#endif
{
	StreflopDouble y,hi,lo,c,t,e,hxs,hfx,r1,h2,h4,R1,R2,R3;
	int32_t k,xsb;
	u_int32_t hx;
	...
	if(hx >= 0x4043687A) {			/* if |x|>=56*ln2 */
	    if(hx >= 0x40862E42) {		/* if |x|>=709.78... */
		    ...
		    if(((hx&0xfffff)|low)!=0)
		         return x+x; 	 /* NaN */
		    else return (xsb==0)? x:-StreflopDouble(1.0);/* exp(+-inf)={inf,-1} */
	        }
