...
#endif
    huge = 1e30f,
    one = 1.0f, invsqrtpi = 5.6418961287e-01f, /* 0x3f106ebb */
...
#else
Simple __ieee754_j1f(x) Simple x;
#endif
{
	...
	if (ix < 0x32000000) { /* |x|<2**-27 */
		if (huge + x > one)
			return (Simple)0.5f * x; /* inexact if x!=0 necessary */
	}
	z = x * x;
...
#else
Simple __ieee754_y1f(x) Simple x;
#endif
{
...
#ifdef __STDC__
static const Simple pr8[6] = {
    /* for x in [inf, 8]=1/[0,0.125f] */
#else
static Simple pr8[6] = {
    /* for x in [inf, 8]=1/[0,0.125f] */
#endif
...
#ifdef __STDC__
static const Simple pr5[6] = {
    /* for x in [8,4.5454f]=1/[0.125f,0.22001f] */
#else
static Simple pr5[6] = {
    /* for x in [8,4.5454f]=1/[0.125f,0.22001f] */
#endif
...
#else
static Simple pr3[6] = {
    /* for x in [4.547f,2.8571f]=1/[0.2199f,0.35001f] */
#endif
...
#ifdef __STDC__
static const Simple pr2[6] = {
    /* for x in [2.8570f,2]=1/[0.3499f,0.5f] */
#else
static Simple pr2[6] = {
    /* for x in [2.8570f,2]=1/[0.3499f,0.5f] */
#endif
...
#else
static Simple ponef(x) Simple x;
#endif
{
#ifdef __STDC__
	const Simple *p, *q;
...
#else
static Simple qonef(x) Simple x;
#endif
{
#ifdef __STDC__
	const Simple *p, *q;
