...
template<typename T, typename... Tail> const T& first(const T& value, const Tail&...) { return value; }
...
template<typename Char> struct text {
	...
	template<typename OutputIt, typename... Args> constexpr OutputIt format(OutputIt out, const Args&...) const
	{
...
template<typename T, int N, typename... Args> constexpr const T& get_arg_checked(const Args&... args)
{
...
template<typename Char, typename T, int N> struct field {
	...
	template<typename OutputIt, typename... Args> constexpr OutputIt format(OutputIt out, const Args&... args) const
	{
...
template<typename Char> struct is_compiled_format<runtime_named_field<Char>> : std::true_type {};
...
template<int ID, typename Char> constexpr auto parse_arg_id(const Char* begin, const Char* end)
{
...
template<typename T> struct field_type<T, enable_if_t<detail::is_named_arg<T>::value>> {
	...
	if constexpr (str[POS] == '{') {
		...
		if constexpr (str[POS + 1] == '{') {
			...
			if constexpr (arg_id_result.arg_id.kind == arg_id_kind::index) {
				...
				return parse_replacement_field_then_tail<get_type<arg_index, Args>, Args, arg_id_end_pos, arg_index,
				...
				if constexpr (arg_index >= 0) {
					constexpr auto next_id = ID != manual_indexing_id ? ID + 1 : manual_indexing_id;
					return parse_replacement_field_then_tail<decltype(get_type<arg_index, Args>::value), Args,
					    arg_id_end_pos, arg_index, next_id>(format_str);
				}
				else if constexpr (c == '}') {
