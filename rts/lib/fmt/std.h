...
template<typename Char> auto get_path_string(const std::filesystem::path& p) { return p.string<Char>(); }

template<typename Char> void write_escaped_path(basic_memory_buffer<Char>& quoted, const std::filesystem::path& p)
{
...
template<> inline void write_escaped_path<char>(memory_buffer& quoted, const std::filesystem::path& p)
{
...
template<typename Char> struct formatter<std::thread::id, Char> : basic_ostream_formatter<Char> {};
...
private:
	...
	template<class U> FMT_CONSTEXPR static void maybe_set_debug_format(U&, ...) {}
...
template<typename... Types> struct is_variant_like_<std::variant<Types...>> : std::true_type {};
...
template<typename Char> struct formatter<std::monostate, Char> {
	...
	template<typename FormatContext> auto format(const std::monostate&, FormatContext& ctx) const -> decltype(ctx.out())
	{
...
struct formatter<Variant,
	...
	template<typename FormatContext> auto format(const Variant& value, FormatContext& ctx) const -> decltype(ctx.out())
	{
		auto out = ctx.out();
...
template<typename T, typename Enable = void> struct has_flip : std::false_type {};

template<typename T> struct has_flip<T, void_t<decltype(std::declval<T>().flip())>> : std::true_type {};
...
template<typename C> struct is_bit_reference_like<std::__bit_const_reference<C>> {
