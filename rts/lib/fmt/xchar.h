...
template<typename... T> constexpr format_arg_store<wformat_context, T...> make_wformat_args(const T&... args)
{
	return {args...};
}
...
{
	return vformat_to(
	    out, loc, detail::to_string_view(format_str), fmt::make_format_args<buffer_context<Char>>(args...));
}
...
template<typename... T> void println(std::FILE* f, wformat_string<T...> fmt, T&&... args)
{
