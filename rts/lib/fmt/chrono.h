...
template<typename Duration> inline auto localtime(std::chrono::local_time<Duration> time) -> std::tm
{
...
template<typename OutputIt> auto write_padding(OutputIt out, pad_type pad, int width) -> OutputIt
{
...
template<typename OutputIt> auto write_padding(OutputIt out, pad_type pad) -> OutputIt
{
	if (pad != pad_type::none)
...
{
	static constexpr const char* full_name_list[] = {"January", "February", "March", "April", "May", "June", "July",
...
void write_fractional_seconds(OutputIt& out, Duration d, int precision = -1)
{
	...
	using subsecond_precision =
	    std::chrono::duration<typename std::common_type<typename Duration::rep, std::chrono::seconds::rep>::type,
...
void write_floating_seconds(memory_buffer& buf, Duration duration, int num_fractional_digits = -1)
{
	...
	format_to(std::back_inserter(buf), FMT_STRING("{:.{}f}"),
	    std::fmod(val * static_cast<rep>(Duration::period::num) / static_cast<rep>(Duration::period::den),
	        static_cast<rep>(60)),
	    num_fractional_digits);
}

template<typename OutputIt, typename Char, typename Duration = std::chrono::seconds> class tm_writer {
private:
	...
	template<typename T, FMT_ENABLE_IF(!has_member_data_tm_zone<T>::value)> void format_tz_name_impl(const T&)
	{
		format_localized('Z');
	}
...
template<typename T, FMT_ENABLE_IF(std::is_integral<T>::value&& has_isfinite<T>::value)> inline bool isfinite(T)
{
	return true;
}
...
struct formatter<std::chrono::time_point<std::chrono::system_clock, Duration>, Char> : formatter<std::tm, Char> {
	...
	template<typename FormatContext>
	auto format(std::chrono::time_point<std::chrono::system_clock, Duration> val, FormatContext& ctx) const
	    -> decltype(ctx.out())
	{
		using period = typename Duration::period;
		if (detail::const_check(
		        period::num != 1 || period::den != 1 || std::is_floating_point<typename Duration::rep>::value)) {
			const auto epoch = val.time_since_epoch();
			auto subsecs =
			    std::chrono::duration_cast<Duration>(epoch - std::chrono::duration_cast<std::chrono::seconds>(epoch));
			...
			return formatter<std::tm, Char>::do_format(
			    gmtime(std::chrono::time_point_cast<std::chrono::seconds>(val)), ctx, &subsecs);
		}
...
struct formatter<std::chrono::local_time<Duration>, Char> : formatter<std::tm, Char> {
	...
	auto format(std::chrono::local_time<Duration> val, FormatContext& ctx) const -> decltype(ctx.out())
	{
		...
		if (period::num != 1 || period::den != 1 || std::is_floating_point<typename Duration::rep>::value) {
			...
			return formatter<std::tm, Char>::do_format(
			    localtime(std::chrono::time_point_cast<std::chrono::seconds>(val)), ctx, &subsecs);
		}

		return formatter<std::tm, Char>::format(
		    localtime(std::chrono::time_point_cast<std::chrono::seconds>(val)), ctx);
	}
};
#endif
...
struct formatter<std::chrono::time_point<std::chrono::utc_clock, Duration>, Char>
	...
	template<typename FormatContext>
	auto format(std::chrono::time_point<std::chrono::utc_clock, Duration> val, FormatContext& ctx) const
	    -> decltype(ctx.out())
	{
