 ...
 * CHInt: just an optional optimization to reduce memory consumption when cache size is equal to or less than
 * 255,65535,4B-1,...
 */
template<typename LruKey, typename LruValue, typename CHInt = size_t> class LRUClockCache {
...
public:
	...
	inline const LruValue Get(const LruKey& key) noexcept { return AccessClock2Hand(key, nullptr); }

	// only syntactic difference
	inline const std::vector<LruValue> GetMultiple(const std::vector<LruKey>& key) const noexcept
	{
	...
	inline const LruValue GetThreadSafe(const LruKey& key) noexcept
	{
	...
	// then cache is available from RAM on next get/set access with same key
	inline void Set(const LruKey& key, const LruValue& val) noexcept { AccessClock2Hand(key, &val, 1); }

	// thread-safe but slower version of set()
	inline void SetThreadSafe(const LruKey& key, const LruValue& val) noexcept
	{
		std::lock_guard<std::mutex> lg(mut);
	...
	// CLOCK algorithm with 2 hand counters (1 for second chance for a cache slot to survive, 1 for eviction of cache
	// slot) opType=0: get opType=1: set
