...
static int meth_accept(lua_State* L)
{
	...
	if (err == IO_DONE) {
		...
		clnt->sock = sock;
		io_init(&clnt->io, (p_send)socket_send, (p_recv)socket_recv, (p_error)socket_ioerror, &clnt->sock);
...
static const char* unix_trybind(p_unix un, const char* path)
{
	...
	local.sun_len = sizeof(local.sun_family) + sizeof(local.sun_len) + len + 1;
...
static int global_create(lua_State* L)
{
	...
	if (err == IO_DONE) {
		...
		socket_setnonblocking(&sock);
		un->sock = sock;
		io_init(&un->io, (p_send)socket_send, (p_recv)socket_recv, (p_error)socket_ioerror, &un->sock);
