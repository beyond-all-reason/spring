...
template<> struct simd_batch_traits<batch<int16_t, 8>> {
...
template<> class batch<int16_t, 8> : public simd_batch<batch<int16_t, 8>> {
public:
	...
	template<class... Args, class Enable = detail::is_array_initializer_t<int16_t, 8, Args...>> batch(Args... args);
...
namespace detail {
inline int16x8_t init_from_bool(uint16x8_t a) { return vandq_s16(reinterpret_cast<int16x8_t>(a), vdupq_n_s16(1)); }
} // namespace detail
...
inline batch<int16_t, 8>& batch<int16_t, 8>::load_unaligned(const int16_t* src) { return load_aligned(src); }
...
inline batch<int16_t, 8>& batch<int16_t, 8>::load_unaligned(const uint16_t* src) { return load_aligned(src); }

inline void batch<int16_t, 8>::store_aligned(int16_t* dst) const { vst1q_s16(dst, this->m_value); }

inline void batch<int16_t, 8>::store_unaligned(int16_t* dst) const { store_aligned(dst); }
...
inline void batch<int16_t, 8>::store_unaligned(uint16_t* dst) const { store_aligned(dst); }

XSIMD_DEFINE_LOAD_STORE_INT16(int16_t, 8, 8)
XSIMD_DEFINE_LOAD_STORE_LONG(int16_t, 8, 8)

inline batch<int16_t, 8>::operator int16x8_t() const { return this->m_value; }

namespace detail {

template<> struct batch_kernel<int16_t, 8> : neon_int_kernel_base<batch<int16_t, 8>> {
	using batch_type = batch<int16_t, 8>;
	using value_type = int16_t;
	using batch_bool_type = batch_bool<int16_t, 8>;

	static batch_type neg(const batch_type& rhs) { return vnegq_s16(rhs); }

	static batch_type add(const batch_type& lhs, const batch_type& rhs) { return vaddq_s16(lhs, rhs); }

	static batch_type sub(const batch_type& lhs, const batch_type& rhs) { return vsubq_s16(lhs, rhs); }

	static batch_type sadd(const batch_type& lhs, const batch_type& rhs) { return vqaddq_s16(lhs, rhs); }

	static batch_type ssub(const batch_type& lhs, const batch_type& rhs) { return vqsubq_s16(lhs, rhs); }

	static batch_type mul(const batch_type& lhs, const batch_type& rhs) { return vmulq_s16(lhs, rhs); }

	static batch_type div(const batch_type& lhs, const batch_type& rhs)
	{
		return neon_detail::unroll_op<8, int16x8_t, int16_t>(
	...
	static batch_bool_type eq(const batch_type& lhs, const batch_type& rhs) { return vceqq_s16(lhs, rhs); }

	static batch_bool_type neq(const batch_type& lhs, const batch_type& rhs) { return !(lhs == rhs); }

	static batch_bool_type lt(const batch_type& lhs, const batch_type& rhs) { return vcltq_s16(lhs, rhs); }

	static batch_bool_type lte(const batch_type& lhs, const batch_type& rhs) { return vcleq_s16(lhs, rhs); }

	static batch_type bitwise_and(const batch_type& lhs, const batch_type& rhs) { return vandq_s16(lhs, rhs); }

	static batch_type bitwise_or(const batch_type& lhs, const batch_type& rhs) { return vorrq_s16(lhs, rhs); }

	static batch_type bitwise_xor(const batch_type& lhs, const batch_type& rhs) { return veorq_s16(lhs, rhs); }

	static batch_type bitwise_not(const batch_type& rhs) { return vmvnq_s16(rhs); }

	static batch_type bitwise_andnot(const batch_type& lhs, const batch_type& rhs) { return vbicq_s16(lhs, rhs); }

	static batch_type min(const batch_type& lhs, const batch_type& rhs) { return vminq_s16(lhs, rhs); }

	static batch_type max(const batch_type& lhs, const batch_type& rhs) { return vmaxq_s16(lhs, rhs); }

	static batch_type abs(const batch_type& rhs) { return vabsq_s16(rhs); }
...
{
	switch (n) {
	case 0: return lhs; XSIMD_REPEAT_16(vshlq_n_s16);
	default: break;
	}
...
{
	switch (n) {
	case 0: return lhs; XSIMD_REPEAT_16(vshrq_n_s16);
	default: break;
	}
	return batch<int16_t, 8>(int16_t(0));
}
...
inline batch<int16_t, 8> operator<<(const batch<int16_t, 8>& lhs, int32_t rhs) { return detail::shift_left(lhs, rhs); }

inline batch<int16_t, 8> operator>>(const batch<int16_t, 8>& lhs, int32_t rhs) { return detail::shift_right(lhs, rhs); }
