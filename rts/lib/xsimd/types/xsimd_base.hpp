...
namespace detail {
template<class T, std::size_t N> struct batch_kernel;
}

template<class X> struct simd_batch_inner_types {
	using batch_reference = X&;
	using const_batch_reference = const X&;
};

template<class T> using batch_type_t = typename T::batch_type;

namespace detail {
template<class X> struct get_real_batch_type {
	using batch_type = batch_type_t<X>;
};

template<class T, std::size_t N> struct get_real_batch_type<batch<std::complex<T>, N>> {
	using batch_type = typename batch<std::complex<T>, N>::real_batch;
};

#ifdef XSIMD_ENABLE_XTL_COMPLEX
template<class T, bool i3ec, std::size_t N> struct get_real_batch_type<batch<xtl::xcomplex<T, T, i3ec>, N>> {
	using batch_type = typename batch<xtl::xcomplex<T, T, i3ec>, N>::real_batch;
};
#endif
} // namespace detail

template<class T> using real_batch_type_t = typename detail::get_real_batch_type<typename T::batch_type>::batch_type;

namespace detail {
template<class X> struct is_simd_type : std::is_base_of<simd_base<X>, X> {};
} // namespace detail
 ...
 * @sa simd_batch_bool
 */
template<class X> class simd_batch : public simd_base<X> {
public:
	...
	template<class T> static X from_unaligned(T* src);

	template<class T> static X from_aligned(T* src);
...
template<class X> typename simd_batch_traits<X>::batch_bool_type operator!(const simd_base<X>& rhs);
...
template<class X> batch_type_t<X> fmin(const simd_base<X>& lhs, const simd_base<X>& rhs);

template<class X> batch_type_t<X> fmax(const simd_base<X>& lhs, const simd_base<X>& rhs);
...
template<class X> typename simd_batch_traits<X>::value_type hadd(const simd_base<X>& rhs);

template<class X> enable_if_simd_t<X> haddp(const X* row);

template<class X>
batch_type_t<X>
select(const typename simd_batch_traits<X>::batch_bool_type& cond, const simd_base<X>& a, const simd_base<X>& b);

template<class X> batch_type_t<X> zip_lo(const simd_base<X>& lhs, const simd_base<X>& rhs);

template<class X> batch_type_t<X> zip_hi(const simd_base<X>& lhs, const simd_base<X>& rhs);

template<class X> typename simd_batch_traits<X>::batch_bool_type isnan(const simd_base<X>& x);

template<class X> std::ostream& operator<<(std::ostream& out, const simd_batch<X>& rhs);

/***************************
 * generic batch operators *
 ***************************/

template<class T, std::size_t N> batch<T, N> operator&&(const batch<T, N>& lhs, const batch<T, N>& rhs);
...
template<class T_in, class T_out, std::size_t N> struct batch_cast_impl {
...
public:
	static inline batch<T_out, N> run(const batch<T_in, N>& x) { return run_impl(x, detail::make_index_sequence<N>{}); }
};

template<class T, std::size_t N> struct batch_cast_impl<T, T, N> {
	static inline batch<T, N> run(const batch<T, N>& x) { return x; }
};
...
#define XSIMD_BATCH_CAST_INTRINSIC(T_IN, T_OUT, N, INTRINSIC)                               \
	template<> struct batch_cast_impl<T_IN, T_OUT, N> {                                     \
		static inline batch<T_OUT, N> run(const batch<T_IN, N>& x) { return INTRINSIC(x); } \
	};
...
#define XSIMD_BATCH_CAST_INTRINSIC2(T_IN, T_OUT, N, INTRINSIC1, INTRINSIC2)                              \
	template<> struct batch_cast_impl<T_IN, T_OUT, N> {                                                  \
		static inline batch<T_OUT, N> run(const batch<T_IN, N>& x) { return INTRINSIC2(INTRINSIC1(x)); } \
	};

// Shorthand for defining an implicit batch_cast implementation
#define XSIMD_BATCH_CAST_IMPLICIT(T_IN, T_OUT, N)                                                 \
	template<> struct batch_cast_impl<T_IN, T_OUT, N> {                                           \
		static inline batch<T_OUT, N> run(const batch<T_IN, N>& x) { return batch<T_OUT, N>(x); } \
	};
...
template<class B_in, class B_out> struct bitwise_cast_impl;
...
#define XSIMD_BITWISE_CAST_INTRINSIC(T_IN, N_IN, T_OUT, N_OUT, INTRINSIC)                          \
	template<> struct bitwise_cast_impl<batch<T_IN, N_IN>, batch<T_OUT, N_OUT>> {                  \
		static inline batch<T_OUT, N_OUT> run(const batch<T_IN, N_IN>& x) { return INTRINSIC(x); } \
	};


// Backwards-compatible interface to bitwise_cast_impl
template<class B, std::size_t N = simd_batch_traits<B>::size> B bitwise_cast(const batch<float, N>& x);

template<class B, std::size_t N = simd_batch_traits<B>::size> B bitwise_cast(const batch<double, N>& x);

template<class B, std::size_t N = simd_batch_traits<B>::size> B bitwise_cast(const batch<int32_t, N>& x);

template<class B, std::size_t N = simd_batch_traits<B>::size> B bitwise_cast(const batch<int64_t, N>& x);

template<class T, std::size_t N> batch<T, N> bitwise_cast(const batch_bool<T, N>& src);
...
#define XSIMD_DEFINE_LOAD_STORE(TYPE, N, CVT_TYPE, ALIGNMENT)                                                \
	...
	{                                                                                                        \
		...
		unroller<N>([&](std::size_t i) { tmp[i] = static_cast<TYPE>(src[i]); });                             \
		return load_aligned(tmp);                                                                            \
	}                                                                                                        \
	inline batch<TYPE, N>& batch<TYPE, N>::load_unaligned(const CVT_TYPE* src) { return load_aligned(src); } \
	inline void batch<TYPE, N>::store_aligned(CVT_TYPE* dst) const                                           \
	{                                                                                                        \
		alignas(ALIGNMENT) TYPE tmp[N];                                                                      \
		store_aligned(tmp);                                                                                  \
		unroller<N>([&](std::size_t i) { dst[i] = static_cast<CVT_TYPE>(tmp[i]); });                         \
	}                                                                                                        \
	inline void batch<TYPE, N>::store_unaligned(CVT_TYPE* dst) const { return store_aligned(dst); }
...
template<> struct get_int_type<long> {
	using type = int32_t;
};

template<> struct get_int_type<unsigned long> {
	using type = uint32_t;
};

template<class T> using get_int_type_t = typename get_int_type<T>::type;
} // namespace detail
 ...
 */
template<class X> inline auto simd_base<X>::operator()() -> batch_reference { return derived_cast().get(); }

/**
 * Returns a constant reference to the batch type used for computation.
 */
template<class X> inline auto simd_base<X>::operator()() const -> const_batch_reference { return derived_cast().get(); }

/**
 * Returns a reference to the actual derived type of simd_base.
 */
template<class X> inline X& simd_base<X>::derived_cast() { return *static_cast<X*>(this); }

/**
 * Returns a constant reference to the actual derived type of simd_base.
 */
template<class X> inline const X& simd_base<X>::derived_cast() const { return *static_cast<const X*>(this); }
 ...
 */
template<class X> inline X simd_batch<X>::broadcast(value_type v) { return X(v); }
 ...
 * @return a new batch instance
 */
template<class X> template<class T> inline X simd_batch<X>::from_unaligned(T* src)
{
	X res;
 ...
 * @return a new batch instance
 */
template<class X> template<class T> inline X simd_batch<X>::from_aligned(T* src)
{
	X res;
 ...
 */
template<class X> inline X& simd_batch<X>::operator+=(const X& rhs)
{
	(*this)() = (*this)() + rhs;
	return (*this)();
}
 ...
 * @return a reference to \c this.
 */
template<class X> inline X& simd_batch<X>::operator+=(const value_type& rhs)
{
	(*this)() = (*this)() + X(rhs);
	return (*this)();
}
 ...
 * @return a reference to \c this.
 */
template<class X> inline X& simd_batch<X>::operator++()
{
	(*this)() += value_type(1);
	return (*this)();
}
 ...
 * @return a reference to \c this.
 */
template<class X> inline X& simd_batch<X>::operator++(int)
{
	X tmp = (*this)();
	(*this)() += value_type(1);
	return tmp;
...
template<class X> inline X& simd_batch<X>::load_aligned(const char* src)
{
	return (*this)().load_aligned(reinterpret_cast<const char_itype*>(src));
}

template<class X> inline X& simd_batch<X>::load_unaligned(const char* src)
{
	return (*this)().load_unaligned(reinterpret_cast<const char_itype*>(src));
}

template<class X> void simd_batch<X>::store_aligned(char* dst) const
{
	return (*this)().store_aligned(reinterpret_cast<char_itype*>(dst));
}

template<class X> void simd_batch<X>::store_unaligned(char* dst) const
{
	return (*this)().store_unaligned(reinterpret_cast<char_itype*>(dst));
}

template<class X> inline auto simd_batch<X>::get() -> batch_reference { return this->derived_cast(); }

template<class X> inline auto simd_batch<X>::get() const -> const_batch_reference { return this->derived_cast(); }

template<class X> inline auto simd_batch<X>::operator[](std::size_t index) -> value_type&
{
	return m_array[index & (size - 1)];
}

template<class X> inline auto simd_batch<X>::operator[](std::size_t index) const -> const value_type&
{
	return m_array[index & (size - 1)];
}

template<class X> inline auto simd_batch<X>::begin() -> iterator { return m_array; }

template<class X> inline auto simd_batch<X>::end() -> iterator { return m_array + size; }

template<class X> inline auto simd_batch<X>::begin() const -> const_iterator { return cbegin(); }

template<class X> inline auto simd_batch<X>::end() const -> const_iterator { return cend(); }

template<class X> inline auto simd_batch<X>::cbegin() const -> const_iterator { return m_array; }

template<class X> inline auto simd_batch<X>::cend() const -> const_iterator { return m_array + size; }

template<class X> inline auto simd_batch<X>::rbegin() -> reverse_iterator { return reverse_iterator(end()); }

template<class X> inline auto simd_batch<X>::rend() -> reverse_iterator { return reverse_iterator(begin()); }

template<class X> inline auto simd_batch<X>::rbegin() const -> const_reverse_iterator { return crbegin(); }

template<class X> inline auto simd_batch<X>::rend() const -> const_reverse_iterator { return crend(); }

template<class X> inline auto simd_batch<X>::crbegin() const -> const_reverse_iterator
{
	return const_reverse_iterator(end());
}

template<class X> inline auto simd_batch<X>::crend() const -> const_reverse_iterator
{
	return const_reverse_iterator(begin());
}

#define XSIMD_UNARY_OP(OP, FUNC)                                                     \
	template<class X> inline batch_type_t<X> operator OP(const simd_base<X>& rhs)    \
	{                                                                                \
		using value_type = typename simd_batch_traits<X>::value_type;                \
...
#define XSIMD_BINARY_OP(OP, FUNC)                                                                                      \
	template<class X, class Y> inline batch_type_t<X> operator OP(const simd_base<X>& lhs, const simd_base<Y>& rhs)    \
	{                                                                                                                  \
		using value_type = typename simd_batch_traits<X>::value_type;                                                  \
		using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;                                   \
		return kernel::FUNC(lhs(), rhs());                                                                             \
                                                                                                                       ...
                                                                                                                       \
	template<class X>                                                                                                  \
	inline batch_type_t<X> operator OP(const typename simd_batch_traits<X>::value_type & lhs, const simd_base<X>& rhs) \
	{                                                                                                                  \
		return batch_type_t<X>(lhs) OP rhs();                                                                          \
	}                                                                                                                  \
                                                                                                                       \
	template<class X>                                                                                                  \
	inline batch_type_t<X> operator OP(const simd_base<X>& lhs, const typename simd_batch_traits<X>::value_type & rhs) \
	{                                                                                                                  \
...
#define XSIMD_BINARY_BOOL_OP(OP, FUNC)                                                 \
	...
	inline typename simd_batch_traits<X>::batch_bool_type operator OP(                 \
	    const simd_base<X>& lhs, const simd_base<X>& rhs)                              \
	{                                                                                  \
		using value_type = typename simd_batch_traits<X>::value_type;                  \
		using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;   \
		return kernel::FUNC(lhs(), rhs());                                             \
 ...
 */
template<class X> inline batch_type_t<X> operator-(const simd_base<X>& rhs);
 ...
 * @return \c rhs.
 */
template<class X> inline X operator+(const simd_batch<X>& rhs) { return rhs(); }
 ...
 */
template<class X, class Y> batch_type_t<X> operator+(const simd_base<X>& lhs, const simd_base<Y>& rhs);
 ...
 */
template<class X> inline batch_type_t<X> sadd(const simd_base<X>& lhs, const simd_base<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline batch_type_t<X> ssub(const simd_base<X>& lhs, const simd_base<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X>
typename simd_batch_traits<X>::batch_bool_type operator==(const simd_base<X>& lhs, const simd_base<X>& rhs);
 ...
 */
template<class X, class Y> inline batch_type_t<X> operator&(const simd_base<X>& lhs, const simd_base<Y>& rhs);
 ...
 */
template<class X> batch_type_t<X> operator~(const simd_base<X>& rhs);
 ...
 */
template<class X> inline batch_type_t<X> bitwise_andnot(const simd_batch<X>& lhs, const simd_batch<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline typename simd_batch_traits<X>::batch_bool_type operator!(const simd_base<X>& rhs)
{
 ...
 */
template<class X> inline batch_type_t<X> min(const simd_base<X>& lhs, const simd_base<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline batch_type_t<X> max(const simd_base<X>& lhs, const simd_base<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 * @return a batch of the smaller values.
 */
template<class X> inline batch_type_t<X> fmin(const simd_batch<X>& lhs, const simd_batch<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 * @return a batch of the larger values.
 */
template<class X> inline batch_type_t<X> fmax(const simd_batch<X>& lhs, const simd_batch<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline real_batch_type_t<X> abs(const simd_base<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 * @return the asbolute values of \c rhs.
 */
template<class X> inline batch_type_t<X> fabs(const simd_base<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline batch_type_t<X> sqrt(const simd_base<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline batch_type_t<X> fma(const simd_base<X>& x, const simd_base<X>& y, const simd_base<X>& z)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline batch_type_t<X> fms(const simd_base<X>& x, const simd_base<X>& y, const simd_base<X>& z)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline batch_type_t<X> fnma(const simd_base<X>& x, const simd_base<X>& y, const simd_base<X>& z)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline batch_type_t<X> fnms(const simd_base<X>& x, const simd_base<X>& y, const simd_base<X>& z)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline typename simd_batch_traits<X>::value_type hadd(const simd_base<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
...
inline batch_type_t<X> select(const batch_bool_constant<typename simd_batch_traits<X>::value_type, Masks...>& cond,
    const simd_base<X>& a,
    const simd_base<X>& b)
{
	using value_type = typename simd_batch_traits<X>::value_type;
 ...
 */
template<class X> inline batch_type_t<X> zip_lo(const simd_base<X>& lhs, const simd_base<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline batch_type_t<X> zip_hi(const simd_base<X>& lhs, const simd_base<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline typename simd_batch_traits<X>::batch_bool_type isnan(const simd_base<X>& x)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline std::ostream& operator<<(std::ostream& out, const simd_batch<X>& rhs)
{
	out << '(';
...
template<class T, std::size_t N> inline batch<T, N> operator&&(const batch<T, N>& lhs, const batch<T, N>& rhs)
{
...
template<class T_out, class T_in, std::size_t N> inline batch<T_out, N> batch_cast(const batch<T_in, N>& x)
{
...
template<class B, std::size_t N> inline B bitwise_cast(const batch<float, N>& x)
{
	return bitwise_cast_impl<batch<float, N>, B>::run(x);
}

template<class B, std::size_t N> inline B bitwise_cast(const batch<double, N>& x)
{
	return bitwise_cast_impl<batch<double, N>, B>::run(x);
}

template<class B, std::size_t N> inline B bitwise_cast(const batch<int32_t, N>& x)
{
	return bitwise_cast_impl<batch<int32_t, N>, B>::run(x);
}

template<class B, std::size_t N> inline B bitwise_cast(const batch<int64_t, N>& x)
{
	return bitwise_cast_impl<batch<int64_t, N>, B>::run(x);
}

template<class T, std::size_t N> inline batch<T, N> bitwise_cast(const batch_bool<T, N>& src)
{
