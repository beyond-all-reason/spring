...
template<class T> class bool_proxy {
public:
	bool_proxy(T& ref);
 ...
 * @sa simd_batch
 */
template<class X> class simd_batch_bool {
public:
	...
	template<class P> X& load_aligned(const P& src);
	template<class P> X& load_unaligned(const P& src);

	template<class P> void store_aligned(P& dst) const;
	template<class P> void store_unaligned(P& dst) const;
...
private:
	template<class P, std::size_t... I> X& load_impl(detail::index_sequence<I...>, const P& src);

	template<class P> void store_impl(P& dst) const;
};

template<class X> X operator&(const simd_batch_bool<X>& lhs, const simd_batch_bool<X>& rhs);
...
template<class X> X bitwise_andnot(const simd_batch_bool<X>& lhs, const simd_batch_bool<X>& rhs);

template<class X> X operator==(const simd_batch_bool<X>& lhs, const simd_batch_bool<X>& rhs);

template<class X> X operator!=(const simd_batch_bool<X>& lhs, const simd_batch_bool<X>& rhs);

template<class X> bool all(const simd_batch_bool<X>& rhs);

template<class X> bool any(const simd_batch_bool<X>& rhs);

template<class X> X operator&&(const simd_batch_bool<X>& lhs, const simd_batch_bool<X>& rhs);

template<class X> X operator&&(const simd_batch_bool<X>& lhs, bool rhs);

template<class X> X operator&&(bool lhs, const simd_batch_bool<X>& rhs);
...
template<class X> std::ostream& operator<<(std::ostream& out, const simd_batch_bool<X>& rhs);
...
template<class T> inline bool_proxy<T>& bool_proxy<T>::operator=(bool rhs)
{
	m_ref = static_cast<T>(rhs);
	return *this;
}

template<class T> inline bool_proxy<T>::operator bool() const { return static_cast<bool>(m_ref); }
 ...
 */
template<class X> inline X& simd_batch_bool<X>::operator&=(const X& rhs)
{
	(*this)() = (*this)() & rhs;
	return (*this)();
}
 ...
 */
template<class X> inline X& simd_batch_bool<X>::operator()() { return *static_cast<X*>(this); }

/**
 * Returns a constant reference to the actual derived type of the simd_batch_bool.
 */
template<class X> const X& simd_batch_bool<X>::operator()() const { return *static_cast<const X*>(this); }

//@}

template<class X> inline X& simd_batch_bool<X>::load_aligned(const bool* src)
{
	return load_impl(detail::make_index_sequence<size>(), src);
}

template<class X> inline X& simd_batch_bool<X>::load_unaligned(const bool* src) { return load_aligned(src); }

template<class X> inline void simd_batch_bool<X>::store_aligned(bool* dst) const { store_impl(dst); }

template<class X> inline void simd_batch_bool<X>::store_unaligned(bool* dst) const { store_impl(dst); }

template<class X> template<class P> inline X& simd_batch_bool<X>::load_aligned(const P& src)
{
	return load_impl(detail::make_index_sequence<size>(), src);
}

template<class X> template<class P> inline X& simd_batch_bool<X>::load_unaligned(const P& src)
{
	return load_aligned(src);
}

template<class X> template<class P> inline void simd_batch_bool<X>::store_aligned(P& dst) const { store_impl(dst); }

template<class X> template<class P> inline void simd_batch_bool<X>::store_unaligned(P& dst) const { store_impl(dst); }
...
template<class X> template<class P> inline void simd_batch_bool<X>::store_impl(P& dst) const
{
 ...
 */
template<class X> inline X operator&(const simd_batch_bool<X>& lhs, const simd_batch_bool<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
 ...
 */
template<class X> inline X operator~(const simd_batch_bool<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_bool_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 */
template<class X> inline bool all(const simd_batch_bool<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_bool_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 * @return a boolean scalar.
 */
template<class X> inline bool any(const simd_batch_bool<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
	using kernel = detail::batch_bool_kernel<value_type, simd_batch_traits<X>::size>;
 ...
 * @return the result of the logical and.
 */
template<class X> inline X operator&&(const simd_batch_bool<X>& lhs, bool rhs) { return lhs() & X(rhs); }
 ...
 * @return the result of the logical and.
 */
template<class X> inline X operator&&(bool lhs, const simd_batch_bool<X>& rhs) { return X(lhs) & rhs(); }
 ...
 */
template<class X> inline X operator!(const simd_batch_bool<X>& rhs) { return rhs() == X(false); }
 ...
 */
template<class X> inline std::ostream& operator<<(std::ostream& out, const simd_batch_bool<X>& rhs)
{
	out << '(';
