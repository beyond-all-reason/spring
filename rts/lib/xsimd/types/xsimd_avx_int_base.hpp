...
template<class T, std::size_t N> class avx_int_batch_bool : public simd_batch_bool<batch_bool<T, N>> {
public:
	avx_int_batch_bool();
...
template<class T, std::size_t N> class avx_int_batch : public simd_batch<batch<T, N>> {
public:
...
template<class... Args> inline __m256i int_init(std::integral_constant<std::size_t, 1>, Args... args)
{
	return _mm256_setr_epi8(args...);
}

template<class... Args> inline __m256i int_init(std::integral_constant<std::size_t, 2>, Args... args)
{
	return _mm256_setr_epi16(args...);
}

template<class... Args> inline __m256i int_init(std::integral_constant<std::size_t, 4>, Args... args)
{
	return _mm256_setr_epi32(args...);
}

template<class... Args> inline __m256i int_init(std::integral_constant<std::size_t, 8>, Args... args)
{
	return _mm256_setr_epi64x(args...);
}

template<class T> inline __m256i int_set(std::integral_constant<std::size_t, 1>, T v) { return _mm256_set1_epi8(v); }

template<class T> inline __m256i int_set(std::integral_constant<std::size_t, 2>, T v) { return _mm256_set1_epi16(v); }

template<class T> inline __m256i int_set(std::integral_constant<std::size_t, 4>, T v) { return _mm256_set1_epi32(v); }

template<class T> inline __m256i int_set(std::integral_constant<std::size_t, 8>, T v) { return _mm256_set1_epi64x(v); }
} // namespace avx_detail
...
template<class T, std::size_t N> inline avx_int_batch_bool<T, N>::avx_int_batch_bool() {}
...
template<class T, std::size_t N> inline avx_int_batch_bool<T, N>::operator __m256i() const { return m_value; }

template<class T, std::size_t N> inline bool_proxy<T> avx_int_batch_bool<T, N>::operator[](std::size_t index)
{
	return bool_proxy<T>(m_array[index & (N - 1)]);
}

template<class T, std::size_t N> inline bool avx_int_batch_bool<T, N>::operator[](std::size_t index) const
{
	return static_cast<bool>(m_array[index & (N - 1)]);
}

template<class T, std::size_t N> inline __m256i avx_int_batch_bool<T, N>::get_value() const { return m_value; }
...
namespace detail {
template<class T, std::size_t N> struct avx_int_batch_bool_kernel {
...
#if XSIMD_X86_INSTR_SET >= XSIMD_X86_AVX2_VERSION
		switch (sizeof(T)) {
		case 1: return _mm256_cmpeq_epi8(lhs, rhs);
		case 2: return _mm256_cmpeq_epi16(lhs, rhs);
		case 4: return _mm256_cmpeq_epi32(lhs, rhs);
		case 8: return _mm256_cmpeq_epi64(lhs, rhs);
		}
#else
		switch (sizeof(T)) {
		case 1: {
	...
	static bool all(const batch_type& rhs) { return _mm256_testc_si256(rhs, batch_type(true)) != 0; }

	static bool any(const batch_type& rhs) { return !_mm256_testz_si256(rhs, rhs); }
};
} // namespace detail
...
template<class T, std::size_t N> inline avx_int_batch<T, N>::avx_int_batch() {}
...
template<class T, std::size_t N> inline batch<T, N>& avx_int_batch<T, N>::operator=(const __m256i& rhs)
{
	this->m_value = rhs;
	return (*this)();
}

template<class T, std::size_t N> inline batch<T, N>& avx_int_batch<T, N>::operator=(const batch_bool_type& rhs)
{
	this->m_value = detail::bitwise_and_impl(rhs, batch<T, N>(1));
	return (*this)();
}

template<class T, std::size_t N> inline avx_int_batch<T, N>::operator __m256i() const { return this->m_value; }

template<class T, std::size_t N> inline batch<T, N>& avx_int_batch<T, N>::load_aligned(const T* src)
{
	this->m_value = _mm256_load_si256((__m256i const *)src);
	return (*this)();
}

template<class T, std::size_t N> inline batch<T, N>& avx_int_batch<T, N>::load_unaligned(const T* src)
{
...
template<class T, std::size_t N> inline void avx_int_batch<T, N>::store_aligned(T* dst) const
{
	_mm256_store_si256((__m256i*)dst, this->m_value);
}

template<class T, std::size_t N> inline void avx_int_batch<T, N>::store_unaligned(T* dst) const
{
	_mm256_storeu_si256((__m256i*)dst, this->m_value);
}

template<class T, std::size_t N> inline void avx_int_batch<T, N>::store_aligned(flipped_sign_type_t<T>* dst) const
{
	_mm256_store_si256((__m256i*)dst, this->m_value);
}

template<class T, std::size_t N> inline void avx_int_batch<T, N>::store_unaligned(flipped_sign_type_t<T>* dst) const
{
	_mm256_storeu_si256((__m256i*)dst, this->m_value);
}

namespace detail {
template<class B> struct avx_int_kernel_base {
	using batch_type = B;
