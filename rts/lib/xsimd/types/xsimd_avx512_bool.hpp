...
template<class MASK> class bool_mask_proxy {
public:
...
template<class MASK, class B> class batch_bool_avx512 : public simd_batch_bool<B> {
public:
	batch_bool_avx512();
...
private:
	...
	template<class... Args> B& load_values(Args... args);
...
template<class T, std::size_t N> class avx512_fallback_batch_bool : public simd_batch_bool<batch_bool<T, N>> {
public:
	avx512_fallback_batch_bool();
...
private:
	template<class... Args> batch_bool<T, N>& load_values(Args... b);
...
template<class MASK> inline bool_mask_proxy<MASK>::operator bool() const { return ((m_ref >> m_idx) & MASK(1)) != 0; }

template<class MASK> inline bool_mask_proxy<MASK>& bool_mask_proxy<MASK>::operator=(bool rhs)
{
...
template<class MASK, class T> inline batch_bool_avx512<MASK, T>::batch_bool_avx512() {}
...
namespace detail {
template<class T> constexpr T get_init_value_impl(const bool (& /*init*/)[sizeof(T) * 8]) { return T(0); }

template<class T, std::size_t IX, std::size_t... I> constexpr T get_init_value_impl(const bool (&init)[sizeof(T) * 8])
{
...
template<class MASK, class T> inline batch_bool_avx512<MASK, T>::operator MASK() const { return m_value; }

template<class MASK, class T> inline bool_mask_proxy<MASK> batch_bool_avx512<MASK, T>::operator[](std::size_t idx)
{
	std::size_t s = simd_batch_traits<T>::size - 1;
	return bool_mask_proxy<MASK>(m_value, idx & s);
}

template<class MASK, class T> inline bool batch_bool_avx512<MASK, T>::operator[](std::size_t idx) const
{
	std::size_t s = simd_batch_traits<T>::size - 1;
...
template<class MASK, class T> template<class... Args> inline T& batch_bool_avx512<MASK, T>::load_values(Args... b)
{
	return load_array({{b...}});
}

namespace detail {
template<class T, std::size_t N> struct batch_bool_kernel_avx512 {
	using batch_type = batch_bool<T, N>;
	using mt = typename mask_type<N>::type;

	static batch_type bitwise_and(const batch_type& lhs, const batch_type& rhs) { return mt(lhs) & mt(rhs); }

	static batch_type bitwise_or(const batch_type& lhs, const batch_type& rhs) { return mt(lhs) | mt(rhs); }

	static batch_type bitwise_xor(const batch_type& lhs, const batch_type& rhs) { return mt(lhs) ^ mt(rhs); }

	static batch_type bitwise_not(const batch_type& rhs) { return ~mt(rhs); }

	static batch_type bitwise_andnot(const batch_type& lhs, const batch_type& rhs) { return mt(lhs) ^ mt(rhs); }

	static batch_type equal(const batch_type& lhs, const batch_type& rhs) { return (~mt(lhs)) ^ mt(rhs); }

	static batch_type not_equal(const batch_type& lhs, const batch_type& rhs) { return mt(lhs) ^ mt(rhs); }

	static bool all(const batch_type& rhs) { return mt(rhs) == mt(-1); }

	static bool any(const batch_type& rhs) { return mt(rhs) != mt(0); }
};
} // namespace detail
...
template<class T, std::size_t N> inline avx512_fallback_batch_bool<T, N>::avx512_fallback_batch_bool() {}
...
template<class T, std::size_t N> inline avx512_fallback_batch_bool<T, N>::operator __m512i() const { return m_value; }
...
template<class T, std::size_t N> inline bool_proxy<T> avx512_fallback_batch_bool<T, N>::operator[](std::size_t idx)
{
	return bool_proxy<T>(m_array[idx & (N - 1)]);
}

template<class T, std::size_t N> inline bool avx512_fallback_batch_bool<T, N>::operator[](std::size_t idx) const
{
...
{
	m_value = avx512_detail::int_init(
	    std::integral_constant<std::size_t, sizeof(T)>{}, static_cast<T>(-static_cast<bool>(b))...);
	return (*this)();
}

namespace detail {
template<class T, std::size_t N> struct avx512_fallback_batch_bool_kernel {
	using batch_type = batch_bool<T, N>;

	static batch_type bitwise_and(const batch_type& lhs, const batch_type& rhs) { return _mm512_and_si512(lhs, rhs); }

	static batch_type bitwise_or(const batch_type& lhs, const batch_type& rhs) { return _mm512_or_si512(lhs, rhs); }

	static batch_type bitwise_xor(const batch_type& lhs, const batch_type& rhs) { return _mm512_xor_si512(lhs, rhs); }
	...
	static batch_type equal(const batch_type& lhs, const batch_type& rhs) { return ~(lhs ^ rhs); }
