...
template<> struct simd_batch_traits<batch_bool<int8_t, 32>> {
...
template<> struct simd_batch_traits<batch_bool<uint8_t, 32>> {
...
template<> class batch_bool<int8_t, 32> : public avx_int_batch_bool<int8_t, 32> {
public:
	using avx_int_batch_bool::avx_int_batch_bool;
};

template<> class batch_bool<uint8_t, 32> : public avx_int_batch_bool<uint8_t, 32> {
public:
	using avx_int_batch_bool::avx_int_batch_bool;
};

namespace detail {
template<> struct batch_bool_kernel<int8_t, 32> : public avx_int_batch_bool_kernel<int8_t, 32> {};

template<> struct batch_bool_kernel<uint8_t, 32> : public avx_int_batch_bool_kernel<uint8_t, 32> {};
} // namespace detail
...
template<> struct simd_batch_traits<batch<int8_t, 32>> {
	using value_type = int8_t;
	static constexpr std::size_t size = 32;
...
template<> struct simd_batch_traits<batch<uint8_t, 32>> {
	using value_type = uint8_t;
	static constexpr std::size_t size = 32;
...
template<> class batch<int8_t, 32> : public avx_int_batch<int8_t, 32> {
public:
...
template<> class batch<uint8_t, 32> : public avx_int_batch<uint8_t, 32> {
public:
...
namespace detail {
template<class T> struct int8_batch_kernel : avx_int_kernel_base<batch<T, 32>> {
	...
	static batch_type mul(const batch_type& lhs, const batch_type& rhs)
	{
		...
		batch_type lower =
		    _mm256_slli_epi16(_mm256_mullo_epi16(_mm256_srli_si256(lhs, 1), _mm256_srli_si256(rhs, 1)), 8);
	...
	static batch_type div(const batch_type& lhs, const batch_type& rhs)
	{
		...
		auto to_float = [](__m256i val) {
			...
			if (std::is_signed<T>::value) {
				return _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm256_extractf128_si256(val, 0)));
			}
			else {
				return _mm256_cvtepi32_ps(_mm256_cvtepu8_epi32(_mm256_extractf128_si256(val, 0)));
			}
		};
		...
		auto to_int8 = [](__m256 x, __m256 y) {
			...
			return _mm_unpacklo_epi32(
			    _mm256_extractf128_si256(e, 0), _mm256_extractf128_si256(e, 1)); // 0123 4567 89AB CDEF
		};
		...
		batch<float, 8> res_2 =
		    _mm256_div_ps(to_float(_mm256_permute4x64_epi64(lhs, 0x01)), to_float(_mm256_permute4x64_epi64(rhs, 0x01)));
		batch<float, 8> res_3 =
		    _mm256_div_ps(to_float(_mm256_permute4x64_epi64(lhs, 0x02)), to_float(_mm256_permute4x64_epi64(rhs, 0x02)));
		batch<float, 8> res_4 =
		    _mm256_div_ps(to_float(_mm256_permute4x64_epi64(lhs, 0x03)), to_float(_mm256_permute4x64_epi64(rhs, 0x03)));

		return batch_type(insert(_mm256_castsi128_si256(to_int8(res_1, res_2)), to_int8(res_3, res_4)));
#else
		XSIMD_MACRO_UNROLL_BINARY(/);
#endif
	}
	...
	static value_type hadd(const batch_type& lhs)
	{
		...
		value_type res = 0;
		unroller<32>([&](std::size_t i) { res += tmp_lhs[i]; });
		return res;
	}
	...
	static batch_type zip_lo(const batch_type& lhs, const batch_type& rhs) { return _mm256_unpacklo_epi8(lhs, rhs); }

	static batch_type zip_hi(const batch_type& lhs, const batch_type& rhs) { return _mm256_unpackhi_epi8(lhs, rhs); }
};

template<> struct batch_kernel<int8_t, 32> : int8_batch_kernel<int8_t> {
...
template<> struct batch_kernel<uint8_t, 32> : int8_batch_kernel<uint8_t> {
	static batch_bool_type lt(const batch_type& lhs, const batch_type& rhs)
	{
#if XSIMD_X86_INSTR_SET >= XSIMD_X86_AVX2_VERSION
