...
template<typename T, std::size_t N> struct simd_batch_traits<batch_bool<T, N>> {
	using value_type = T;
...
template<typename T, std::size_t N> class batch_bool : public simd_batch_bool<batch_bool<T, N>> {
public:
	batch_bool();
	explicit batch_bool(bool b);

	// Constructor from N boolean parameters
	template<typename... Args, typename Enable = detail::is_array_initializer_t<bool, N, Args...>>
...
private:
	template<class... Args> batch_bool<T, N>& load_values(Args... args);
...
template<typename T, std::size_t N> struct simd_batch_traits<batch<T, N>> {
	using value_type = T;
	static constexpr std::size_t size = N;
...
template<typename T, std::size_t N> class batch : public simd_batch<batch<T, N>> {
public:
	...
	template<typename... Args, typename Enable = typename detail::is_array_initializer<T, N, Args...>::type>
...
private:
	template<typename U> batch& load_unaligned_impl(const U* src);
	template<typename U> void store_unaligned_impl(U* src) const;
};

template<typename T, std::size_t N> batch<T, N> operator<<(const batch<T, N>& lhs, int32_t rhs);
template<typename T, std::size_t N> batch<T, N> operator>>(const batch<T, N>& lhs, int32_t rhs);
...
template<class T, std::size_t N>
class batch_bool<std::complex<T>, N> : public simd_complex_batch_bool<batch_bool<std::complex<T>, N>> {
public:
...
template<class T, std::size_t N>
class batch<std::complex<T>, N> : public simd_complex_batch<batch<std::complex<T>, N>> {
public:
	...
	template<typename... Args, typename Enable = typename detail::is_array_initializer<T, N, Args...>::type>
	batch(Args... exactly_N_scalars)
	    : base_type(real_batch{exactly_N_scalars.real()...}, real_batch{exactly_N_scalars.imag()...})
	{
	}
	...
	template<class U>
	typename std::enable_if<detail::is_complex<U>::value, self_type&>::type load_aligned(const U* src);
	template<class U>
	typename std::enable_if<detail::is_complex<U>::value, self_type&>::type load_unaligned(const U* src);
	...
	template<class U> void store_aligned(U* dst) const;
	template<class U> void store_unaligned(U* dst) const;
};
...
template<class T, std::size_t N, bool i3ec>
class batch<xtl::xcomplex<T, T, i3ec>, N> : public simd_complex_batch<batch<xtl::xcomplex<T, T, i3ec>, N>> {
public:
...
template<std::size_t N> batch<int32_t, N> to_int(const batch<float, N>& x);
template<std::size_t N> batch<int64_t, N> to_int(const batch<double, N>& x);

template<std::size_t N> batch<float, N> to_float(const batch<int32_t, N>& x);
template<std::size_t N> batch<double, N> to_float(const batch<int64_t, N>& x);

/**************************
 * boolean cast functions *
 **************************/

template<std::size_t N> batch_bool<int32_t, N> bool_cast(const batch_bool<float, N>& x);
template<std::size_t N> batch_bool<int64_t, N> bool_cast(const batch_bool<double, N>& x);
template<std::size_t N> batch_bool<float, N> bool_cast(const batch_bool<int32_t, N>& x);
template<std::size_t N> batch_bool<double, N> bool_cast(const batch_bool<int64_t, N>& x);
...
#define XSIMD_FALLBACK_UNARY_OP(RESULT_TYPE, OPERATOR, X) XSIMD_FALLBACK_MAPPING_LOOP(RESULT_TYPE, (OPERATOR X[i]))
...
#define XSIMD_FALLBACK_BATCH_BITWISE_UNARY_OP(OPERATOR, X)                                                           \
	XSIMD_FALLBACK_MAPPING_LOOP(batch, detail::from_unsigned_integer<T>(OPERATOR detail::to_unsigned_integer(X[i])))

#define XSIMD_FALLBACK_BATCH_BITWISE_BINARY_OP(OPERATOR, X, Y)                                            \
	XSIMD_FALLBACK_MAPPING_LOOP(batch, detail::from_unsigned_integer<T>(detail::to_unsigned_integer(X[i]) \
	                                           OPERATOR detail::to_unsigned_integer(Y[i])))

#define XSIMD_FALLBACK_BATCH_UNARY_FUNC(FUNCTION, X) XSIMD_FALLBACK_MAPPING_LOOP(batch, FUNCTION(X[i]))

#define XSIMD_FALLBACK_BATCH_BINARY_FUNC(FUNCTION, X, Y) XSIMD_FALLBACK_MAPPING_LOOP(batch, FUNCTION(X[i], Y[i]))
...
#define XSIMD_FALLBACK_BOOL_CAST(T_OUT, X) return batch_bool<T_OUT, N>(static_cast<std::array<bool, N>>(X));

/***********************************
 * batch_bool<T, N> implementation *
 ***********************************/

template<typename T, std::size_t N> inline batch_bool<T, N>::batch_bool() {}
...
template<typename T, std::size_t N> inline batch_bool<T, N>& batch_bool<T, N>::operator=(const std::array<bool, N>& rhs)
{
	m_value = rhs;
	return *this;
}

template<typename T, std::size_t N> inline batch_bool<T, N>::operator std::array<bool, N>() const { return m_value; }

template<typename T, std::size_t N> inline const bool& batch_bool<T, N>::operator[](std::size_t index) const
{
	return m_value[index];
}

template<typename T, std::size_t N> inline bool& batch_bool<T, N>::operator[](std::size_t index)
{
	return m_value[index];
}

template<typename T, std::size_t N> inline const std::array<bool, N>& batch_bool<T, N>::get_value() const
{
	return m_value;
}
...
namespace detail {
template<class T, std::size_t N> struct batch_bool_kernel {
	...
	static batch_type bitwise_not(const batch_type& rhs) { XSIMD_FALLBACK_UNARY_OP(batch_bool, !, rhs) }
...
template<typename T, std::size_t N> inline batch<T, N>::batch() {}
...
namespace detail {
template<bool integral> struct all_bits {
	template<class T> static T get(T) { return ~T(0); }
};

template<> struct all_bits<false> {
	template<class T> static T get(T)
	{
		T res(0);
...
template<typename T, std::size_t N> inline batch<T, N>::batch(const std::array<bool, N>& src)
{
...
template<typename T, std::size_t N> inline batch<T, N>::batch(const batch_bool_type& rhs)
{
	std::transform(rhs.get_value().cbegin(), rhs.get_value().cend(), this->m_value.begin(),
	    [](bool b) -> T { return b ? T(1) : T(0); });
}

template<typename T, std::size_t N> inline batch<T, N>& batch<T, N>::operator=(const std::array<T, N>& rhs)
{
	this->m_value = rhs;
	return *this;
}

template<typename T, std::size_t N> inline batch<T, N>& batch<T, N>::operator=(const std::array<bool, N>& rhs)
{
	using all_bits = detail::all_bits<std::is_integral<T>::value>;
	std::transform(
	    rhs.cbegin(), rhs.cend(), this->m_value.begin(), [](bool b) -> T { return b ? all_bits::get(T(0)) : T(0); });
	return *this;
}

template<typename T, std::size_t N> inline batch<T, N>& batch<T, N>::operator=(const batch_bool_type& rhs)
{
	std::transform(rhs.get_value().cbegin(), rhs.get_value().cend(), this->m_value.begin(),
	    [](bool b) -> T { return b ? T(1) : T(0); });
	return *this;
...
template<typename T, std::size_t N> inline batch<T, N>::operator std::array<T, N>() const { return this->m_value; }

#define FALLBACK_DEFINE_LOAD_STORE(TYPE)                                                                 \
	template<typename T, std::size_t N> inline batch<T, N>& batch<T, N>::load_aligned(const TYPE* src)   \
	{                                                                                                    \
		return this->load_unaligned_impl(src);                                                           \
	}                                                                                                    \
	template<typename T, std::size_t N> inline batch<T, N>& batch<T, N>::load_unaligned(const TYPE* src) \
	{                                                                                                    \
		return this->load_unaligned_impl(src);                                                           \
	}                                                                                                    \
	template<typename T, std::size_t N> inline void batch<T, N>::store_aligned(TYPE* dst) const          \
	{                                                                                                    \
		this->store_unaligned_impl(dst);                                                                 \
	}                                                                                                    \
	template<typename T, std::size_t N> inline void batch<T, N>::store_unaligned(TYPE* dst) const        \
	{                                                                                                    \
		this->store_unaligned_impl(dst);                                                                 \
	}
...
template<typename T, std::size_t N> template<typename U> inline void batch<T, N>::store_unaligned_impl(U* dst) const
{
	for (std::size_t i = 0; i < N; ++i) {
...
template<typename T, std::size_t N> inline T& batch<T, N>::operator[](std::size_t index)
{
	return this->m_value[index % base_type::size];
}

template<typename T, std::size_t N> inline const T& batch<T, N>::operator[](std::size_t index) const
{
	return this->m_value[index % base_type::size];
}
...
template<class T, std::size_t N> struct batch_kernel {
	...
	static batch_type neg(const batch_type& rhs) { XSIMD_FALLBACK_UNARY_OP(batch, -, rhs) }

	static batch_type add(const batch_type& lhs, const batch_type& rhs) { XSIMD_FALLBACK_BINARY_OP(batch, +, lhs, rhs) }

	static batch_type sub(const batch_type& lhs, const batch_type& rhs) { XSIMD_FALLBACK_BINARY_OP(batch, -, lhs, rhs) }
	...
	static batch_type mul(const batch_type& lhs, const batch_type& rhs) { XSIMD_FALLBACK_BINARY_OP(batch, *, lhs, rhs) }

	static batch_type div(const batch_type& lhs, const batch_type& rhs) { XSIMD_FALLBACK_BINARY_OP(batch, /, lhs, rhs) }

	static batch_type mod(const batch_type& lhs, const batch_type& rhs) { XSIMD_FALLBACK_BINARY_OP(batch, %, lhs, rhs) }
	...
	static batch_type bitwise_not(const batch_type& rhs) { XSIMD_FALLBACK_BATCH_BITWISE_UNARY_OP(~, rhs) }

	static batch_type bitwise_andnot(const batch_type& lhs, const batch_type& rhs)
	{
		XSIMD_FALLBACK_MAPPING_LOOP(batch, detail::from_unsigned_integer<T>(~(detail::to_unsigned_integer(lhs[i]) &
	...
	static batch_type abs(const batch_type& rhs) { XSIMD_FALLBACK_BATCH_UNARY_FUNC(std::abs, rhs) }

	static batch_type fabs(const batch_type& rhs) { XSIMD_FALLBACK_BATCH_UNARY_FUNC(std::fabs, rhs) }

	static batch_type sqrt(const batch_type& rhs) { XSIMD_FALLBACK_BATCH_UNARY_FUNC(std::sqrt, rhs) }
	...
	static batch_type fms(const batch_type& x, const batch_type& y, const batch_type& z) { return fma(x, y, -z); }

	static batch_type fnma(const batch_type& x, const batch_type& y, const batch_type& z) { return fma(-x, y, z); }

	static batch_type fnms(const batch_type& x, const batch_type& y, const batch_type& z) { return fma(-x, y, -z); }
	...
	static batch_type haddp(const batch_type* row) { XSIMD_FALLBACK_MAPPING_LOOP(batch, hadd(row[i])) }
	...
	static batch_type
	select(const batch_bool_constant<value_type, Values...>& cond, const batch_type& a, const batch_type& b)
	{
		XSIMD_FALLBACK_MAPPING_LOOP(batch, (cond[i] ? a[i] : b[i]))
	}

	static batch_bool_type isnan(const batch_type& x) { XSIMD_FALLBACK_MAPPING_LOOP(batch_bool, std::isnan(x[i])) }
...
template<typename T, std::size_t N> inline batch<T, N> operator<<(const batch<T, N>& lhs, int32_t rhs)
{
...
namespace detail {
template<std::size_t N, class B, class U> inline std::pair<B, B> load_complex_impl(const U* src)
{
...
template<std::size_t N, class B, class U> inline void store_complex_impl(const B& real, const B& imag, U* dst)
{
	using value_type = typename U::value_type;
...
template<class T, std::size_t N> template<class U> inline void batch<std::complex<T>, N>::store_aligned(U* dst) const
{
	detail::store_complex_impl<N>(this->m_real, this->m_imag, dst);
}

template<class T, std::size_t N> template<class U> inline void batch<std::complex<T>, N>::store_unaligned(U* dst) const
{
	store_aligned(dst);
}
...
template<std::size_t N> inline batch<int32_t, N> to_int(const batch<float, N>& x)
{
	XSIMD_FALLBACK_BATCH_STATIC_CAST(int32_t, x)
}

template<std::size_t N> inline batch<int64_t, N> to_int(const batch<double, N>& x)
{
	XSIMD_FALLBACK_BATCH_STATIC_CAST(int64_t, x)
}

template<std::size_t N> inline batch<float, N> to_float(const batch<int32_t, N>& x)
{
	XSIMD_FALLBACK_BATCH_STATIC_CAST(float, x)
}

template<std::size_t N> inline batch<double, N> to_float(const batch<int64_t, N>& x)
{
...
template<std::size_t N> inline batch_bool<int32_t, N> bool_cast(const batch_bool<float, N>& x)
{
	XSIMD_FALLBACK_BOOL_CAST(int32_t, x)
}

template<std::size_t N> inline batch_bool<int64_t, N> bool_cast(const batch_bool<double, N>& x)
{
	XSIMD_FALLBACK_BOOL_CAST(int64_t, x)
}

template<std::size_t N> inline batch_bool<float, N> bool_cast(const batch_bool<int32_t, N>& x)
{
	XSIMD_FALLBACK_BOOL_CAST(float, x)
}

template<std::size_t N> inline batch_bool<double, N> bool_cast(const batch_bool<int64_t, N>& x)
{
...
struct bitwise_cast_impl<batch<T_in, N_in>, batch<T_out, sizeof(T_in) * N_in / sizeof(T_out)>> {
private:
	static_assert(sizeof(T_in) * N_in % sizeof(T_out) == 0, "The input and output batches must have the same size");
