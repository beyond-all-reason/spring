...
template<> struct simd_batch_traits<batch_bool<int64_t, 2>> {
...
template<> struct simd_batch_traits<batch_bool<uint64_t, 2>> {
...
template<> class batch_bool<int64_t, 2> : public sse_int_batch_bool<int64_t, 2> {
public:
	using sse_int_batch_bool::sse_int_batch_bool;
};

template<> class batch_bool<uint64_t, 2> : public sse_int_batch_bool<uint64_t, 2> {
public:
	using sse_int_batch_bool::sse_int_batch_bool;
};

namespace detail {
template<> struct batch_bool_kernel<int64_t, 2> : public sse_int_batch_bool_kernel<int64_t> {};

template<> struct batch_bool_kernel<uint64_t, 2> : public sse_int_batch_bool_kernel<uint64_t> {};
} // namespace detail
...
template<> class batch<int64_t, 2> : public sse_int_batch<int64_t, 2> {
public:
...
template<> class batch<uint64_t, 2> : public sse_int_batch<uint64_t, 2> {
public:
...
#define SSE_DEFINE_LOAD_STORE_INT64(TYPE, CVT_TYPE)                                                          \
	...
	inline batch<TYPE, 2>& batch<TYPE, 2>::load_unaligned(const CVT_TYPE* src) { return load_aligned(src); } \
	inline void batch<TYPE, 2>::store_aligned(CVT_TYPE* dst) const                                           \
	{                                                                                                        \
		sse_detail::store_aligned_int64(this->m_value, dst);                                                 \
	}                                                                                                        \
	inline void batch<TYPE, 2>::store_unaligned(CVT_TYPE* dst) const { store_aligned(dst); }
...
namespace detail {
template<class T> struct sse_int64_batch_kernel : sse_int_kernel_base<batch<T, 2>> {
	using batch_type = batch<T, 2>;
	using value_type = T;
	using batch_bool_type = batch_bool<T, 2>;

	static batch_type add(const batch_type& lhs, const batch_type& rhs) { return _mm_add_epi64(lhs, rhs); }

	static batch_type sub(const batch_type& lhs, const batch_type& rhs) { return _mm_sub_epi64(lhs, rhs); }
	...
	static batch_type mul(const batch_type& lhs, const batch_type& rhs) { XSIMD_MACRO_UNROLL_BINARY(*); }
	...
	static batch_type min(const batch_type& lhs, const batch_type& rhs) { return select(lhs < rhs, lhs, rhs); }

	static batch_type max(const batch_type& lhs, const batch_type& rhs) { return select(lhs > rhs, lhs, rhs); }
	...
	static batch_type zip_lo(const batch_type& lhs, const batch_type& rhs) { return _mm_unpacklo_epi64(lhs, rhs); }

	static batch_type zip_hi(const batch_type& lhs, const batch_type& rhs) { return _mm_unpackhi_epi64(lhs, rhs); }
};

template<> struct batch_kernel<int64_t, 2> : sse_int64_batch_kernel<int64_t> {
...
template<> struct batch_kernel<uint64_t, 2> : sse_int64_batch_kernel<uint64_t> {
	static batch_bool_type lt(const batch_type& lhs, const batch_type& rhs)
	{
...
inline batch<int64_t, 2> operator<<(const batch<int64_t, 2>& lhs, int32_t rhs) { return _mm_slli_epi64(lhs, rhs); }
...
inline batch<uint64_t, 2> operator<<(const batch<uint64_t, 2>& lhs, int32_t rhs) { return _mm_slli_epi64(lhs, rhs); }

inline batch<uint64_t, 2> operator>>(const batch<uint64_t, 2>& lhs, int32_t rhs) { return _mm_srli_epi64(lhs, rhs); }
