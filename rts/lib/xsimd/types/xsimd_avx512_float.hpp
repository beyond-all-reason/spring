...
template<> struct simd_batch_traits<batch_bool<float, 16>> {
...
template<> class batch_bool<float, 16> : public batch_bool_avx512<__mmask16, batch_bool<float, 16>> {
public:
...
namespace detail {
template<> struct batch_bool_kernel<float, 16> : batch_bool_kernel_avx512<float, 16> {};
} // namespace detail
...
template<> struct simd_batch_traits<batch<float, 16>> {
	using value_type = float;
...
template<> class batch<float, 16> : public simd_batch<batch<float, 16>> {
public:
...
inline batch<float, 16>::batch(float i0,
    float i1,
    float i2,
    float i3,
    float i4,
...
inline batch<float, 16>::operator __m512() const { return this->m_value; }
...
inline void batch<float, 16>::store_aligned(float* dst) const { _mm512_store_ps(dst, this->m_value); }

inline void batch<float, 16>::store_unaligned(float* dst) const { _mm512_storeu_ps(dst, this->m_value); }
...
namespace detail {
template<> struct batch_kernel<float, 16> {
	using batch_type = batch<float, 16>;
	using value_type = float;
	using batch_bool_type = batch_bool<float, 16>;
	...
	static batch_type add(const batch_type& lhs, const batch_type& rhs) { return _mm512_add_ps(lhs, rhs); }
	...
	static batch_type mul(const batch_type& lhs, const batch_type& rhs) { return _mm512_mul_ps(lhs, rhs); }

	static batch_type div(const batch_type& lhs, const batch_type& rhs) { return _mm512_div_ps(lhs, rhs); }
	...
	static batch_type bitwise_and(const batch_type& lhs, const batch_type& rhs) { return _mm512_and_ps(lhs, rhs); }

	static batch_type bitwise_or(const batch_type& lhs, const batch_type& rhs) { return _mm512_or_ps(lhs, rhs); }

	static batch_type bitwise_xor(const batch_type& lhs, const batch_type& rhs) { return _mm512_xor_ps(lhs, rhs); }
	...
	static batch_type min(const batch_type& lhs, const batch_type& rhs) { return _mm512_min_ps(lhs, rhs); }

	static batch_type max(const batch_type& lhs, const batch_type& rhs) { return _mm512_max_ps(lhs, rhs); }
	...
	static batch_type abs(const batch_type& rhs)
	{
		...
		__m512i res_asi = _mm512_and_epi32(_mm512_set1_epi32(0x7FFFFFFF), rhs_asi);
		return *reinterpret_cast<__m512*>(&res_asi);
	}

	static batch_type fabs(const batch_type& rhs) { return abs(rhs); }

	static batch_type sqrt(const batch_type& rhs) { return _mm512_sqrt_ps(rhs); }
...
#define XSIMD_AVX512_HADDP_STEP2(I, a, b, c, d)                                                        \
	...
	{                                                                                                  \
		...
		batch<float, 16> resx3 = _mm512_add_ps(tmp5, tmp6);                                            \
                                                                                                       \
		halfx##I = _mm256_hadd_ps(_mm512_extractf32x8_ps(resx3, 0), _mm512_extractf32x8_ps(resx3, 1)); \
	}
...
#undef XSIMD_AVX512_HADDP_STEP2
	...
	static batch_type zip_lo(const batch_type& lhs, const batch_type& rhs) { return _mm512_unpacklo_ps(lhs, rhs); }

	static batch_type zip_hi(const batch_type& lhs, const batch_type& rhs) { return _mm512_unpackhi_ps(lhs, rhs); }

	static batch_bool_type isnan(const batch_type& x) { return _mm512_cmp_ps_mask(x, x, _CMP_UNORD_Q); }
};
} // namespace detail
