...
template<class C, class R, std::size_t N, std::size_t Align> struct complex_batch_bool_traits {
	using value_type = C;
 ...
 */
template<class X> class simd_complex_batch_bool : public simd_batch_bool<X> {
public:
...
template<class C, class R, std::size_t N, std::size_t Align> struct complex_batch_traits {
	using value_type = C;
	static constexpr std::size_t size = N;
...
template<class T> struct is_ieee_compliant;
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
template<class T> struct is_ieee_compliant<xtl::xcomplex<T, T, false>> : std::false_type {};
#endif
 ...
 */
template<class X> class simd_complex_batch : public simd_base<X> {
public:
	...
	explicit simd_complex_batch(const real_value_type* v)
	    : simd_complex_batch(real_batch(v))
	{
	}

	simd_complex_batch(const real_batch& re, const real_batch& im);

	simd_complex_batch(const real_value_type* re, const real_value_type* im)
	    : simd_complex_batch(real_batch(re), real_batch(im))
	{
	}
	...
	template<class T> X& load_aligned(const T* real_src, const T* imag_src);
	template<class T> X& load_unaligned(const T* real_src, const T* imag_src);

	template<class T> void store_aligned(T* real_dst, T* imag_dst) const;
	template<class T> void store_unaligned(T* real_dst, T* imag_dst) const;

	template<class T> typename std::enable_if<detail::is_complex<T>::value, X&>::type load_aligned(const T* src);
	template<class T> typename std::enable_if<detail::is_complex<T>::value, X&>::type load_unaligned(const T* src);

	template<class T> typename std::enable_if<!detail::is_complex<T>::value, X&>::type load_aligned(const T* src);
	template<class T> typename std::enable_if<!detail::is_complex<T>::value, X&>::type load_unaligned(const T* src);

	template<class T>
	inline typename std::enable_if<detail::is_complex<T>::value, void>::type store_aligned(T* dst) const;
	template<class T>
	inline typename std::enable_if<detail::is_complex<T>::value, void>::type store_unaligned(T* dst) const;
...
template<class X> X operator+(const simd_complex_batch<X>& rhs);

template<class X> X operator-(const simd_complex_batch<X>& rhs);

template<class X> X operator+(const simd_complex_batch<X>& lhs, const simd_complex_batch<X>& rhs);
template<class X> X operator+(const simd_complex_batch<X>& lhs, const typename simd_batch_traits<X>::value_type& rhs);
template<class X> X operator+(const typename simd_batch_traits<X>::value_type& lhs, const simd_complex_batch<X>& rhs);
template<class X> X operator+(const simd_complex_batch<X>& lhs, const typename simd_batch_traits<X>::real_batch& rhs);
template<class X> X operator+(const typename simd_batch_traits<X>::real_batch& lhs, const simd_complex_batch<X>& rhs);
template<class X>
...
template<class X> typename simd_batch_traits<X>::value_type hadd(const simd_complex_batch<X>& rhs);
...
template<class X>
typename simd_batch_traits<X>::batch_bool_type operator==(const simd_complex_batch<X>& lhs,
...
template<class X> std::ostream& operator<<(std::ostream& out, const simd_complex_batch<X>& rhs);
...
template<class X> inline auto simd_complex_batch_bool<X>::value() const -> const real_batch& { return m_value; }

template<class X> inline bool simd_complex_batch_bool<X>::operator[](std::size_t index) const { return m_value[index]; }

namespace detail {
template<class T, std::size_t N> struct batch_bool_complex_kernel {
	using batch_type = batch_bool<T, N>;

	static batch_type bitwise_and(const batch_type& lhs, const batch_type& rhs) { return lhs.value() & rhs.value(); }
	...
	static bool any(const batch_type& rhs) { return xsimd::any(rhs.value()); }
};

template<class T, std::size_t N>
struct batch_bool_kernel<std::complex<T>, N> : batch_bool_complex_kernel<std::complex<T>, N> {};
 ...
 */
template<class X> inline auto simd_complex_batch<X>::real() -> real_batch& { return m_real; }

/**
 * Returns a batch for the imaginary part.
 */
template<class X> inline auto simd_complex_batch<X>::imag() -> real_batch& { return m_imag; }

/**
 * Returns a const batch for the real part
 */
template<class X> inline auto simd_complex_batch<X>::real() const -> const real_batch& { return m_real; }

/**
 * Returns a const batch for the imaginary part.
 */
template<class X> inline auto simd_complex_batch<X>::imag() const -> const real_batch& { return m_imag; }
 ...
 */
template<class X> inline X& simd_complex_batch<X>::operator+=(const X& rhs)
{
	m_real += rhs.real();
	m_imag += rhs.imag();
	return (*this)();
 ...
 * @return a reference to \c this.
 */
template<class X> inline X& simd_complex_batch<X>::operator+=(const value_type& rhs) { return (*this)() += X(rhs); }
 ...
 * @return a reference to \c this.
 */
template<class X> inline X& simd_complex_batch<X>::operator+=(const real_batch& rhs)
{
	m_real += rhs;
	return (*this)();
}
 ...
 * @return a reference to \c this.
 */
template<class X> inline X& simd_complex_batch<X>::operator+=(const real_value_type& rhs)
{
...
namespace detail {
template<class X, bool ieee_compliant> struct complex_batch_multiplier {
	using real_batch = typename simd_batch_traits<X>::real_batch;
 ...
 */
template<class X> template<class T> inline X& simd_complex_batch<X>::load_aligned(const T* real_src, const T* imag_src)
{
 ...
 */
template<class X> template<class T> inline void simd_complex_batch<X>::store_aligned(T* real_dst, T* imag_dst) const
{
 ...
 */
template<class X> template<class T> inline void simd_complex_batch<X>::store_unaligned(T* real_dst, T* imag_dst) const
{
...
template<class X>
template<class T>
inline typename std::enable_if<detail::is_complex<T>::value, X&>::type simd_complex_batch<X>::load_aligned(const T* src)
{
...
template<class X>
template<class T>
inline typename std::enable_if<detail::is_complex<T>::value, X&>::type simd_complex_batch<X>::load_unaligned(
    const T* src)
{
	using tmp_value_type = typename T::value_type;
	const tmp_value_type* rbuf = reinterpret_cast<const tmp_value_type*>(src);
...
template<class X>
template<class T>
inline typename std::enable_if<!detail::is_complex<T>::value, X&>::type simd_complex_batch<X>::load_aligned(
    const T* src)
{
...
template<class X>
template<class T>
inline typename std::enable_if<detail::is_complex<T>::value, void>::type simd_complex_batch<X>::store_aligned(
    T* dst) const
{
...
template<class X>
template<class T>
inline typename std::enable_if<detail::is_complex<T>::value, void>::type simd_complex_batch<X>::store_unaligned(
    T* dst) const
{
	real_batch hi = (*this)().get_complex_high();
	real_batch lo = (*this)().get_complex_low();
...
template<class X> inline auto simd_complex_batch<X>::operator[](std::size_t index) const -> value_type
{
	return value_type(m_real[index], m_imag[index]);
}

template<class X> inline auto simd_complex_batch<X>::get() -> batch_reference { return this->derived_cast(); }

template<class X> inline auto simd_complex_batch<X>::get() const -> const_batch_reference
{
 ...
 */
template<class X> inline X operator+(const simd_complex_batch<X>& rhs) { return rhs(); }
 ...
 * @return the opposite of \c rhs.
 */
template<class X> inline X operator-(const simd_complex_batch<X>& rhs) { return X(-rhs().real(), -rhs().imag()); }
 ...
 */
template<class X> inline X operator+(const simd_complex_batch<X>& lhs, const simd_complex_batch<X>& rhs)
{
	X tmp(lhs());
	return tmp += rhs();
}
 ...
 */
template<class X> inline typename simd_batch_traits<X>::value_type hadd(const simd_complex_batch<X>& rhs)
{
	using value_type = typename simd_batch_traits<X>::value_type;
 ...
 */
template<class X>
inline typename simd_batch_traits<X>::batch_bool_type operator==(const simd_complex_batch<X>& lhs,
    const simd_complex_batch<X>& rhs)
{
 ...
 */
template<class X> inline std::ostream& operator<<(std::ostream& out, const simd_complex_batch<X>& rhs)
{
	out << '(';
