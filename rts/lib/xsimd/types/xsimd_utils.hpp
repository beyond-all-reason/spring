...
template<class T, std::size_t N> struct as_integer<batch<T, N>> {
	using type = batch<typename as_integer<T>::type, N>;
};

template<class T> using as_integer_t = typename as_integer<T>::type;
...
template<class T, std::size_t N> struct as_unsigned_integer<batch<T, N>> {
...
template<class T> struct flipped_sign_type : detail::flipped_sign_type_impl<T, std::is_signed<T>::value> {};
...
template<class T, std::size_t N> struct as_float<batch<T, N>> {
...
template<class T, std::size_t N> struct as_logical<batch<T, N>> {
...
namespace detail {
template<class UI, class I, class F> union generic_caster {
	UI ui;
	I i;
	F f;
...
template<class T> using caster_t = typename caster<T>::type;
} // namespace detail
...
namespace detail {
template<typename T> union unsigned_convertor {
	T data;
	as_unsigned_integer_t<T> bits;
};

template<typename T> as_unsigned_integer_t<T> to_unsigned_integer(const T& input)
{
...
template<typename T> T from_unsigned_integer(const as_unsigned_integer_t<T>& input)
{
	unsigned_convertor<T> convertor;
...
#define XSIMD_MACRO_UNROLL_BINARY(FUNC)                                                                       \
	...
	unroller<size>([&](std::size_t i) { tmp_res[i] = tmp_lhs[i] FUNC tmp_rhs[i]; });                          \
	return batch_type(&tmp_res[0], aligned_mode());

template<class F, std::size_t... I> inline void unroller_impl(F&& f, detail::index_sequence<I...>)
{
	static_cast<void>(std::initializer_list<int>{(f(I), 0)...});
}

template<std::size_t N, class F> inline void unroller(F&& f) { unroller_impl(f, detail::make_index_sequence<N>{}); }
...
constexpr std::array<T, sizeof...(Is)> array_from_scalar_impl(const T& scalar, index_sequence<Is...>)
{
...
template<typename T, std::size_t N> constexpr std::array<T, N> array_from_scalar(const T& scalar)
{
...
template<typename T, std::size_t... Is>
constexpr std::array<T, sizeof...(Is)> array_from_pointer_impl(const T* c_array, index_sequence<Is...>)
{
	return std::array<T, sizeof...(Is)>{c_array[Is]...};
}

template<typename T, std::size_t N> constexpr std::array<T, N> array_from_pointer(const T* c_array)
{
...
template<typename T, typename... Args> using is_all_convertible = all_true<std::is_convertible<Args, T>::value...>;

template<typename T, std::size_t N, typename... Args>
using is_array_initializer = std::enable_if<(sizeof...(Args) == N) && is_all_convertible<T, Args...>::value>;
