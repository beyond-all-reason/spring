...
template<> struct simd_batch_traits<batch<uint64_t, 2>> {
...
template<> class batch<uint64_t, 2> : public simd_batch<batch<uint64_t, 2>> {
public:
	...
	template<class... Args, class Enable = detail::is_array_initializer_t<uint64_t, 2, Args...>> batch(Args... args);
...
inline batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const int32_t* src) { return load_aligned(src); }
...
inline batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const uint32_t* src) { return load_aligned(src); }
...
inline batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const int64_t* src) { return load_aligned(src); }
...
inline batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const uint64_t* src) { return load_aligned(src); }
...
#else
	this->m_value = uint64x2_t{static_cast<uint64_t>(src[0]), static_cast<uint64_t>(src[1])};
#endif
	return *this;
}

inline batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const float* src) { return load_aligned(src); }
...
inline batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const double* src) { return load_aligned(src); }
...
inline void batch<uint64_t, 2>::store_unaligned(int32_t* dst) const { store_aligned(dst); }
...
inline void batch<uint64_t, 2>::store_unaligned(uint32_t* dst) const { store_aligned(dst); }
...
inline void batch<uint64_t, 2>::store_unaligned(int64_t* dst) const { store_aligned(dst); }

inline void batch<uint64_t, 2>::store_aligned(uint64_t* dst) const { vst1q_u64(dst, this->m_value); }

inline void batch<uint64_t, 2>::store_unaligned(uint64_t* dst) const { store_aligned(dst); }
...
inline void batch<uint64_t, 2>::store_unaligned(float* dst) const { store_aligned(dst); }
...
inline void batch<uint64_t, 2>::store_unaligned(double* dst) const { store_aligned(dst); }

inline batch<uint64_t, 2>::operator storage_type() const { return this->m_value; }

namespace detail {
template<> struct batch_kernel<uint64_t, 2> : neon_int_kernel_base<batch<uint64_t, 2>> {
	...
	static batch_type add(const batch_type& lhs, const batch_type& rhs) { return vaddq_u64(lhs, rhs); }

	static batch_type sub(const batch_type& lhs, const batch_type& rhs) { return vsubq_u64(lhs, rhs); }

	static batch_type sadd(const batch_type& lhs, const batch_type& rhs) { return vqaddq_u64(lhs, rhs); }

	static batch_type ssub(const batch_type& lhs, const batch_type& rhs) { return vqsubq_u64(lhs, rhs); }
	...
	static batch_type bitwise_andnot(const batch_type& lhs, const batch_type& rhs) { return vbicq_u64(lhs, rhs); }
...
{
	switch (n) {
	case 0: return lhs; XSIMD_REPEAT_64(vshlq_n_u64);
	default: break;
	}
...
{
	switch (n) {
	case 0: return lhs; XSIMD_REPEAT_64(vshrq_n_u64);
	default: break;
	}
	return batch<uint64_t, 2>(uint64_t(0));
}
