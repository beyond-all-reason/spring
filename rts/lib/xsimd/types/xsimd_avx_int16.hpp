...
template<> struct simd_batch_traits<batch_bool<int16_t, 16>> {
...
template<> struct simd_batch_traits<batch_bool<uint16_t, 16>> {
...
template<> class batch_bool<int16_t, 16> : public avx_int_batch_bool<int16_t, 16> {
public:
	using avx_int_batch_bool::avx_int_batch_bool;
};

template<> class batch_bool<uint16_t, 16> : public avx_int_batch_bool<uint16_t, 16> {
public:
	using avx_int_batch_bool::avx_int_batch_bool;
};

namespace detail {
template<> struct batch_bool_kernel<int16_t, 16> : public avx_int_batch_bool_kernel<int16_t, 16> {};

template<> struct batch_bool_kernel<uint16_t, 16> : public avx_int_batch_bool_kernel<uint16_t, 16> {};
} // namespace detail
...
template<> struct simd_batch_traits<batch<int16_t, 16>> {
	using value_type = int16_t;
	static constexpr std::size_t size = 16;
...
template<> struct simd_batch_traits<batch<uint16_t, 16>> {
	using value_type = uint16_t;
	static constexpr std::size_t size = 16;
...
template<> class batch<int16_t, 16> : public avx_int_batch<int16_t, 16> {
public:
...
template<> class batch<uint16_t, 16> : public avx_int_batch<uint16_t, 16> {
public:
...
namespace detail {
template<class T> struct int16_batch_kernel : avx_int_kernel_base<batch<T, 16>> {
...
// #if XSIMD_X86_INSTR_SET >= XSIMD_X86_AVX2_VERSION
	...
	static value_type hadd(const batch_type& lhs)
	{
		...
		value_type res = 0;
		unroller<16>([&](std::size_t i) { res += tmp_lhs[i]; });
		return res;
	}
	...
	static batch_type zip_lo(const batch_type& lhs, const batch_type& rhs) { return _mm256_unpacklo_epi16(lhs, rhs); }

	static batch_type zip_hi(const batch_type& lhs, const batch_type& rhs) { return _mm256_unpackhi_epi16(lhs, rhs); }
};

template<> struct batch_kernel<int16_t, 16> : int16_batch_kernel<int16_t> {};

template<> struct batch_kernel<uint16_t, 16> : int16_batch_kernel<uint16_t> {
	static batch_bool_type lt(const batch_type& lhs, const batch_type& rhs)
	{
#if XSIMD_X86_INSTR_SET >= XSIMD_X86_AVX2_VERSION
