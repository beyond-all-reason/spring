...
template<> struct simd_batch_traits<batch<uint8_t, 16>> {
...
template<> class batch<uint8_t, 16> : public simd_batch<batch<uint8_t, 16>> {
public:
	...
	template<class... Args, class Enable = detail::is_array_initializer_t<uint8_t, 16, Args...>> batch(Args... args);
...
inline batch<uint8_t, 16>& batch<uint8_t, 16>::load_unaligned(const int8_t* src) { return load_aligned(src); }
...
inline batch<uint8_t, 16>& batch<uint8_t, 16>::load_unaligned(const uint8_t* src) { return load_aligned(src); }

inline void batch<uint8_t, 16>::store_aligned(int8_t* dst) const { vst1q_s8(dst, vreinterpretq_s8_u8(this->m_value)); }

inline void batch<uint8_t, 16>::store_unaligned(int8_t* dst) const { store_aligned(dst); }

inline void batch<uint8_t, 16>::store_aligned(uint8_t* dst) const { vst1q_u8(dst, this->m_value); }

inline void batch<uint8_t, 16>::store_unaligned(uint8_t* dst) const { store_aligned(dst); }

XSIMD_DEFINE_LOAD_STORE_INT8(uint8_t, 16, 16)
XSIMD_DEFINE_LOAD_STORE_LONG(uint8_t, 16, 16)

inline batch<uint8_t, 16>::operator uint8x16_t() const { return this->m_value; }

namespace detail {

template<> struct batch_kernel<uint8_t, 16> : neon_int_kernel_base<batch<uint8_t, 16>> {
	using batch_type = batch<uint8_t, 16>;
	using value_type = uint8_t;
	using batch_bool_type = batch_bool<uint8_t, 16>;

	static batch_type neg(const batch_type& rhs) { return vreinterpretq_u8_s8(vnegq_s8(vreinterpretq_s8_u8(rhs))); }

	static batch_type add(const batch_type& lhs, const batch_type& rhs) { return vaddq_u8(lhs, rhs); }

	static batch_type sub(const batch_type& lhs, const batch_type& rhs) { return vsubq_u8(lhs, rhs); }

	static batch_type sadd(const batch_type& lhs, const batch_type& rhs) { return vqaddq_u8(lhs, rhs); }

	static batch_type ssub(const batch_type& lhs, const batch_type& rhs) { return vqsubq_u8(lhs, rhs); }

	static batch_type mul(const batch_type& lhs, const batch_type& rhs) { return vmulq_u8(lhs, rhs); }

	static batch_type div(const batch_type& lhs, const batch_type& rhs)
	{
		return neon_detail::unroll_op<16, uint8x16_t, uint8_t>(
	...
	static batch_bool_type eq(const batch_type& lhs, const batch_type& rhs) { return vceqq_u8(lhs, rhs); }

	static batch_bool_type neq(const batch_type& lhs, const batch_type& rhs) { return !(lhs == rhs); }

	static batch_bool_type lt(const batch_type& lhs, const batch_type& rhs) { return vcltq_u8(lhs, rhs); }

	static batch_bool_type lte(const batch_type& lhs, const batch_type& rhs) { return vcleq_u8(lhs, rhs); }
	...
	static batch_type min(const batch_type& lhs, const batch_type& rhs) { return vminq_u8(lhs, rhs); }

	static batch_type max(const batch_type& lhs, const batch_type& rhs) { return vmaxq_u8(lhs, rhs); }
...
{
	switch (n) {
	case 0: return lhs; XSIMD_REPEAT_8(vshlq_n_u8);
	default: break;
	}
...
{
	switch (n) {
	case 0: return lhs; XSIMD_REPEAT_8(vshrq_n_u8);
	default: break;
	}
	return batch<uint8_t, 16>(uint8_t(0));
}
