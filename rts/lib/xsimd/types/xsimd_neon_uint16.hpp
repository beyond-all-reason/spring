...
template<> class batch<uint16_t, 8> : public simd_batch<batch<uint16_t, 8>> {
public:
	...
	template<class... Args, class Enable = detail::is_array_initializer_t<uint16_t, 8, Args...>> batch(Args... args);
...
inline batch<uint16_t, 8>& batch<uint16_t, 8>::load_unaligned(const int16_t* src) { return load_aligned(src); }
...
inline batch<uint16_t, 8>& batch<uint16_t, 8>::load_unaligned(const uint16_t* src) { return load_aligned(src); }
...
inline void batch<uint16_t, 8>::store_unaligned(int16_t* dst) const { store_aligned(dst); }

inline void batch<uint16_t, 8>::store_aligned(uint16_t* dst) const { vst1q_u16(dst, this->m_value); }

inline void batch<uint16_t, 8>::store_unaligned(uint16_t* dst) const { store_aligned(dst); }

XSIMD_DEFINE_LOAD_STORE_INT16(uint16_t, 8, 8)
XSIMD_DEFINE_LOAD_STORE_LONG(uint16_t, 8, 8)

inline batch<uint16_t, 8>::operator uint16x8_t() const { return this->m_value; }

namespace detail {
template<> struct batch_kernel<uint16_t, 8> : neon_int_kernel_base<batch<uint16_t, 8>> {
	...
	static batch_type add(const batch_type& lhs, const batch_type& rhs) { return vaddq_u16(lhs, rhs); }

	static batch_type sub(const batch_type& lhs, const batch_type& rhs) { return vsubq_u16(lhs, rhs); }

	static batch_type sadd(const batch_type& lhs, const batch_type& rhs) { return vqaddq_u16(lhs, rhs); }

	static batch_type ssub(const batch_type& lhs, const batch_type& rhs) { return vqsubq_u16(lhs, rhs); }

	static batch_type mul(const batch_type& lhs, const batch_type& rhs) { return vmulq_u16(lhs, rhs); }

	static batch_type div(const batch_type& lhs, const batch_type& rhs)
	{
		return neon_detail::unroll_op<8, uint16x8_t, uint16_t>(
	...
	static batch_bool_type eq(const batch_type& lhs, const batch_type& rhs) { return vceqq_u16(lhs, rhs); }

	static batch_bool_type neq(const batch_type& lhs, const batch_type& rhs) { return !(lhs == rhs); }

	static batch_bool_type lt(const batch_type& lhs, const batch_type& rhs) { return vcltq_u16(lhs, rhs); }

	static batch_bool_type lte(const batch_type& lhs, const batch_type& rhs) { return vcleq_u16(lhs, rhs); }
	...
	static batch_type min(const batch_type& lhs, const batch_type& rhs) { return vminq_u16(lhs, rhs); }

	static batch_type max(const batch_type& lhs, const batch_type& rhs) { return vmaxq_u16(lhs, rhs); }
...
{
	switch (n) {
	case 0: return lhs; XSIMD_REPEAT_16(vshlq_n_u16);
	default: break;
	}
...
{
	switch (n) {
	case 0: return lhs; XSIMD_REPEAT_16(vshrq_n_u16);
	default: break;
	}
	return batch<uint16_t, 8>(uint16_t(0));
}
