...
inline typename std::enable_if<std::is_integral<T1>::value, batch_type_t<B>>::type pow(const simd_base<B>& t0,
    const T1& t1);
 ...
 */
template<class B> batch_type_t<B> cbrt(const simd_base<B>& x);
 ...
 */
template<class B> batch_type_t<B> hypot(const simd_base<B>& x, const simd_base<B>& y);
...
namespace detail {
template<class B> struct pow_kernel {
...
template<class B> inline batch_type_t<B> pow(const simd_base<B>& x, const simd_base<B>& y)
{
...
template<class B, class T = typename B::value_type> struct cbrt_kernel;

template<class B> struct cbrt_kernel<B, float> {
	static inline B compute(const B& a)
	{
		...
		i_type e;
		B x = frexp(z, e);
		x = horner<B, 0x3ece0609, 0x3f91eb77, 0xbf745265, 0x3f0bf0fe, 0xbe09e49a>(x);
...
template<class B> struct cbrt_kernel<B, double> {
	static inline B compute(const B& a)
	{
		B z = abs(a);
#ifndef XSIMD_NO_DENORMALS
		...
		i_type e;
		B x = frexp(z, e);
		x = horner<B, 0x3fd9c0c12122a4feull, 0x3ff23d6ee505873aull, 0xbfee8a4ca3ba37b8ull, 0x3fe17e1fc7e59d58ull,
...
template<class B> inline batch_type_t<B> cbrt(const simd_base<B>& x)
{
...
template<class B> inline batch_type_t<B> hypot(const simd_base<B>& x, const simd_base<B>& y)
{
