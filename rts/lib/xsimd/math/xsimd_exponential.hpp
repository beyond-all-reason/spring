 ...
 */
template<class B> batch_type_t<B> exp(const simd_base<B>& x);
 ...
 */
template<class B> batch_type_t<B> exp2(const simd_base<B>& x);
...
namespace detail {
template<class B, class Tag, class T = typename B::value_type> struct exp_kernel;

template<class B, class Tag> struct exp_kernel<B, Tag, float> {
...
template<class B, class Tag> struct exp_kernel<B, Tag, double> {
	/* origin: boost/simd/arch/common/detail/simd/expo_base.hpp */
	/*
	 * ====================================================
	 * copyright 2016 NumScale SAS
...
template<class B> inline batch_type_t<B> exp(const simd_base<B>& x)
{
	return detail::exp_kernel<batch_type_t<B>, exp_tag>::compute(x());
}

template<class B> inline batch_type_t<B> exp2(const simd_base<B>& x)
{
	return detail::exp_kernel<batch_type_t<B>, exp2_tag>::compute(x());
}

template<class B> inline batch_type_t<B> exp10(const simd_base<B>& x)
{
...
namespace detail {
template<class B, class T = typename B::value_type> struct expm1_kernel;

template<class B> inline B expm1_real_impl(const B& x)
{
	return select(x < logeps<B>(), B(-1.), select(x > maxlog<B>(), infinity<B>(), expm1_kernel<B>::compute_impl(x)));
}

template<class B> struct expm1_kernel<B, float> {
	...
	static inline B compute(const B& a) { return expm1_real_impl(a); }
};

template<class B> struct expm1_kernel<B, double> {
	/* origin: boost/simd/arch/common/detail/generic/expm1_kernel.hpp */
	/*
	 * ====================================================
	 * copyright 2016 NumScale SAS
	...
	static inline B compute_impl(const B& a)
	{
		...
		B x = hi - lo;
		B hxs = x * x * B(0.5);
		B r = horner<B, 0X3FF0000000000000ULL, 0XBFA11111111110F4ULL, 0X3F5A01A019FE5585ULL, 0XBF14CE199EAADBB7ULL,
...
template<class B> inline batch_type_t<B> expm1(const simd_base<B>& x)
{
