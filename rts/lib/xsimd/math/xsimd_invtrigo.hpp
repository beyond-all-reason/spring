...
namespace xsimd {
namespace detail {
template<class B, class T = typename B::value_type> struct invtrigo_kernel_impl;
 ...
 * ====================================================
 */
template<class B> struct invtrigo_kernel_impl<B, float> {
	static inline B asin(const B& a)
	{
		...
		B z1 = horner<B, 0x3e2aaae4, 0x3d9980f6, 0x3d3a3ec7, 0x3cc617e3, 0x3d2cb352>(z);
		z1 = fma(z1, z * x, x);
	...
	static inline B kernel_atan(const B& x, const B& recx)
	{
		...
		const B z = xx * xx;
		B z1 = horner<B, 0xbeaaaa2aul, 0x3e4c925ful, 0xbe0e1b85ul, 0x3da4f0d1ul>(z);
		z1 = fma(xx, z1 * z, xx);
 ...
 * ====================================================
 */
template<class B> struct invtrigo_kernel_impl<B, double> {
	static inline B asin(const B& a)
	{
		B x = abs(a);
		...
		B vp =
		    zz1 *
		    horner<B, 0x403c896240f3081dull, 0xc03991aaac01ab68ull, 0x401bdff5baf33e6aull, 0xbfe2079259f9290full,
		        0x3f684fc3988e9f08ull>(zz1) /
		    horner1<B, 0x40756709b0b644beull, 0xc077fe08959063eeull, 0x40626219af6a7f42ull, 0xc035f2a2b6bf5d8cull>(zz1);
		zz1 = sqrt(zz1 + zz1);
		B z = pio4<B>() - zz1;
		...
		B zz2 = a * a;
		z = zz2 *
		    horner<B, 0xc020656c06ceafd5ull, 0x40339007da779259ull, 0xc0304331de27907bull, 0x4015c74b178a2dd9ull,
		        0xbfe34341333e5c16ull, 0x3f716b9b0bd48ad3ull>(zz2) /
		    horner1<B, 0xc04898220a3607acull, 0x4061705684ffbf9dull, 0xc06265bb6d3576d7ull, 0x40519fc025fe9054ull,
		        0xc02d7b590b5e0eabull>(zz2);
		zz2 = fma(x, z, x);
		return select(x > B(1.), nan<B>(), select(small_cond, x, select(x > ct1, zz1, zz2)) ^ bitofsign(a));
	}
	...
	static inline B kernel_atan(const B& x, const B& recx)
	{
		...
		xx = select(flag2, (x - B(1.)) / (x + B(1.)), xx);
		B z = xx * xx;
		z *= horner<B, 0xc0503669fd28ec8eull, 0xc05eb8bf2d05ba25ull, 0xc052c08c36880273ull, 0xc03028545b6b807aull,
		         0xbfec007fa1f72594ull>(z) /
		     horner1<B, 0x4068519efbbd62ecull, 0x407e563f13b049eaull, 0x407b0e18d2e2be3bull, 0x4064a0dd43b8fa25ull,
...
template<class B> struct invtrigo_kernel {
	static inline B asin(const B& a) { return invtrigo_kernel_impl<B>::asin(a); }
	...
	static inline B kernel_atan(const B& x, const B& recx) { return invtrigo_kernel_impl<B>::kernel_atan(x, recx); }
};
} // namespace detail
} // namespace xsimd
