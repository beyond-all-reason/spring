...
#define XSIMD_DEFINE_CONSTANT(NAME, SINGLE, DOUBLE)                                             \
	template<class T> constexpr T NAME() noexcept { return T(NAME<typename T::value_type>()); } \
	template<> constexpr float NAME<float>() noexcept { return SINGLE; }                        \
	template<> constexpr double NAME<double>() noexcept { return DOUBLE; }

#define XSIMD_DEFINE_CONSTANT_HEX(NAME, SINGLE, DOUBLE)                                                    \
	template<class T> constexpr T NAME() noexcept { return T(NAME<typename T::value_type>()); }            \
	template<> constexpr float NAME<float>() noexcept { return detail::caster32_t(uint32_t(SINGLE)).f; }   \
	template<> constexpr double NAME<double>() noexcept { return detail::caster64_t(uint64_t(DOUBLE)).f; }

XSIMD_DEFINE_CONSTANT(infinity, (std::numeric_limits<float>::infinity()), (std::numeric_limits<double>::infinity()))
XSIMD_DEFINE_CONSTANT(invlog_2,
    1.442695040888963407359924681001892137426645954152986f,
...
XSIMD_DEFINE_CONSTANT(log_2,
    0.6931471805599453094172321214581765680755001343602553f,
...
template<> constexpr int32_t mask1frexp<float>() noexcept { return 0x7f800000; }

template<> constexpr int64_t mask1frexp<double>() noexcept { return 0x7ff0000000000000; }
...
template<> constexpr int32_t mask2frexp<float>() noexcept { return 0x3f000000; }

template<> constexpr int64_t mask2frexp<double>() noexcept { return 0x3fe0000000000000; }
...
template<> constexpr int32_t maxexponent<float>() noexcept { return 127; }

template<> constexpr int64_t maxexponent<double>() noexcept { return 1023; }
...
template<> constexpr int32_t maxexponentm1<float>() noexcept { return 126; }

template<> constexpr int64_t maxexponentm1<double>() noexcept { return 1022; }

/**********************
 * nmb implementation *
 **********************/

template<class T> constexpr int32_t nmb() noexcept { return nmb<typename T::value_type>(); }

template<> constexpr int32_t nmb<float>() noexcept { return 23; }

template<> constexpr int32_t nmb<double>() noexcept { return 52; }

/***********************
 * zero implementation *
 ***********************/

template<class T> constexpr T zero() noexcept { return T(typename T::value_type(0)); }
...
template<class T> struct minvalue_impl {
	static constexpr T get_value() noexcept { return std::numeric_limits<typename T::value_type>::min(); }
};
...
template<> struct minvalue_impl<float> {
	static constexpr float get_value() noexcept { return detail::caster32_t(uint32_t(0xff7fffff)).f; }
};

template<> struct minvalue_impl<double> {
	static constexpr double get_value() noexcept { return detail::caster64_t(uint64_t(0xffefffffffffffff)).f; }
};
} // namespace detail
...
template<class T> constexpr T maxvalue() noexcept { return T(std::numeric_limits<typename T::value_type>::max()); }
