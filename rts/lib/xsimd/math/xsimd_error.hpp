 ...
 */
template<class B> batch_type_t<B> erf(const simd_base<B>& x);
 ...
 * @return the error function of \c x.
 */
template<class B> batch_type_t<B> erfc(const simd_base<B>& x);
 ...
 * ====================================================
 */
template<class B, class T = typename B::value_type> struct erf_kernel;

template<class B> struct erf_kernel<B, float> {
	// computes erf(a0)/a0
...
template<class B> struct erf_kernel<B, double> {
	// computes erf(a0)/a0
...
template<class B, class T = typename B::value_type> struct erf_impl;

template<class B> struct erf_impl<B, float> {
...
template<class B> struct erf_impl<B, double> {
	static inline B compute(const B& a)
	{
		B x = abs(a);
		B xx = x * x;
...
template<class B> inline batch_type_t<B> erf(const simd_base<B>& x)
{
...
namespace detail {
template<class B, class T = typename B::value_type> struct erfc_impl;

template<class B> struct erfc_impl<B, float> {
	static inline B compute(const B& a)
	{
		B x = abs(a);
		auto test0 = a < B(0.);
...
template<class B> struct erfc_impl<B, double> {
	static inline B compute(const B& a)
	{
		B x = abs(a);
		B xx = x * x;
...
template<class B> inline batch_type_t<B> erfc(const simd_base<B>& x)
{
