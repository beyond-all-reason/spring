 ...
 */
template<class B> batch_type_t<B> tgamma(const simd_base<B>& x);
 ...
 */
template<class B> batch_type_t<B> lgamma(const simd_base<B>& x);
...
template<class B, class T = typename B::value_type> struct stirling_kernel;

template<class B> struct stirling_kernel<B, float> {
	static inline B compute(const B& x) { return horner<B, 0x3daaaaab, 0x3b638e39, 0xbb2fb930, 0xb970b359>(x); }

	static inline B split_limit() { return B(detail::caster32_t(uint32_t(0x41d628f6)).f); }

	static inline B large_limit() { return B(detail::caster32_t(uint32_t(0x420c28f3)).f); }
};

template<class B> struct stirling_kernel<B, double> {
	...
	static inline B split_limit() { return B(detail::caster64_t(uint64_t(0x4061e083ba3443d4)).f); }

	static inline B large_limit() { return B(detail::caster64_t(uint64_t(0x4065800000000000)).f); }
};
...
template<class B> inline B stirling(const B& a)
{
...
namespace detail {
template<class B, class T = typename B::value_type> struct tgamma_kernel;

template<class B> struct tgamma_kernel<B, float> {
	static inline B compute(const B& x)
	{
		return horner<B,
...
template<class B> struct tgamma_kernel<B, double> {
	static inline B compute(const B& x)
	{
		return horner<B,
...
namespace detail {
template<class B> B tgamma_large_negative(const B& a)
{
	B st = stirling(a);
	B p = floor(a);
...
template<class B, class BB> B tgamma_other(const B& a, const BB& test)
{
...
template<class B> inline B tgamma_impl(const B& a)
{
...
template<class B> inline batch_type_t<B> tgamma(const simd_base<B>& x) { return detail::tgamma_impl(x()); }
 ...
 */
namespace detail {
template<class B, class T = typename B::value_type> struct lgamma_kernel;

template<class B> struct lgamma_kernel<B, float> {
...
template<class B> struct lgamma_kernel<B, double> {
 ...
 */
namespace detail {
template<class B, class T = typename B::value_type> struct lgamma_impl;

template<class B> struct lgamma_impl<B, float> {
...
template<class B> struct lgamma_impl<B, double> {
	static inline B compute(const B& a)
	{
		auto inf_result = (a <= B(0.)) && is_flint(a);
		B x = select(inf_result, nan<B>(), a);
...
template<class B> inline batch_type_t<B> lgamma(const simd_base<B>& x)
{
