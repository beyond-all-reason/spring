...
template<class B> batch_type_t<B> fmod(const simd_base<B>& x, const simd_base<B>& y);

template<class B> batch_type_t<B> remainder(const simd_base<B>& x, const simd_base<B>& y);
...
template<class X, class Y> batch_type_t<X> nextafter(const simd_base<X>& from, const simd_base<Y>& to);

/****************************
 * Classification functions *
 ****************************/

template<class B> typename simd_batch_traits<B>::batch_bool_type isfinite(const simd_base<B>& x);

template<class B> typename simd_batch_traits<B>::batch_bool_type isinf(const simd_base<B>& x);

template<class B> typename simd_batch_traits<B>::batch_bool_type is_flint(const simd_base<B>& x);

template<class B> typename simd_batch_traits<B>::batch_bool_type is_odd(const simd_base<B>& x);

template<class B> typename simd_batch_traits<B>::batch_bool_type is_even(const simd_base<B>& x);
...
template<class T> struct get_batch_value_type {
	using type = T;
};

template<class T, std::size_t N> struct get_batch_value_type<batch<T, N>> {
	using type = T;
};

template<class B> using get_batch_value_type_t = typename get_batch_value_type<B>::type;

template<class B, bool is_int = std::is_integral<get_batch_value_type_t<B>>::value> struct remainder_kernel {
	using batch_type = B;
	using size_type = std::size_t;
	static constexpr size_type double_size = batch_type::size;

	static inline batch_type fmod(const batch_type& x, const batch_type& y) { return fnma(trunc(x / y), y, x); }
	...
	template<class IB> static inline batch_type to_double(const IB& b, size_type offset)
	{
		batch_type res;
	...
	template<class IB> static inline void to_int(const batch_type& src, IB& dst, size_type offset)
	{
		for (size_type i = 0; i < double_size; ++i) {
			dst[i + offset] = src[i];
		}
...
template<> struct remainder_kernel<double, false> {
	using size_type = std::size_t;

	static inline double fmod(double x, double y) { return std::fmod(x, y); }

	static inline double remainder(double x, double y) { return std::remainder(x, y); }

	template<class IB> static inline double to_double(const IB& b, size_type offset)
	{
		double res = b[offset];
		return res;
	}

	template<class IB> static inline void to_int(double src, IB& dst, size_type offset) { dst[offset] = src; }
};

template<class B> struct remainder_kernel<B, true> {
	using batch_type = B;
	...
	static inline batch_type fmod(const batch_type& x, const batch_type& y)
	{
		...
		for (size_type i = 0; i < int_size; i += double_size) {
			double_batch tmp = double_kernel::fmod(double_kernel::to_double(x, i), double_kernel::to_double(y, i));
	...
	static inline batch_type remainder(const batch_type& x, const batch_type& y)
	{
		batch_type res;
		for (size_type i = 0; i < int_size; i += double_size) {
			double_batch tmp = double_kernel::remainder(double_kernel::to_double(x, i), double_kernel::to_double(y, i));
			double_kernel::to_int(tmp, res, i);
		}
		return res;
	}
 ...
 */
template<class B> inline batch_type_t<B> fmod(const simd_base<B>& x, const simd_base<B>& y)
{
 ...
 */
template<class B> inline batch_type_t<B> remainder(const simd_base<B>& x, const simd_base<B>& y)
{
 ...
 */
template<class B> inline batch_type_t<B> fdim(const simd_base<B>& x, const simd_base<B>& y)
{
 ...
 */
template<class B> inline batch_type_t<B> clip(const simd_base<B>& x, const simd_base<B>& lo, const simd_base<B>& hi)
{
...
namespace detail {
template<class T, std::size_t N, bool is_int = std::is_integral<T>::value> struct nextafter_kernel {
	using batch_type = batch<T, N>;

	static inline batch_type next(const batch_type& b) noexcept { return b; }

	static inline batch_type prev(const batch_type& b) noexcept { return b; }
};

template<class T, std::size_t N> struct bitwise_cast_batch;

template<std::size_t N> struct bitwise_cast_batch<float, N> {
	using type = batch<int32_t, N>;
};

template<std::size_t N> struct bitwise_cast_batch<double, N> {
	using type = batch<int64_t, N>;
};

template<class T, std::size_t N> struct nextafter_kernel<T, N, false> {
	using batch_type = batch<T, N>;
...
template<class X, class Y> inline batch_type_t<X> nextafter(const simd_base<X>& from, const simd_base<Y>& to)
{
 ...
 */
template<class B> inline typename simd_batch_traits<B>::batch_bool_type isfinite(const simd_base<B>& x)
{
	return (x - x) == batch_type_t<B>(0.);
}

namespace detail {
template<class T, std::size_t N, bool is_int = std::is_integral<T>::value> struct isinf_kernel {
	using batch_type = batch<T, N>;
...
template<class T, std::size_t N> struct isinf_kernel<T, N, true> {
	using batch_type = batch<T, N>;
	using batch_bool_type = typename simd_batch_traits<batch_type>::batch_bool_type;

	static inline batch_bool_type run(const batch_type&) { return batch_bool_type(false); }
};
} // namespace detail
 ...
 * @return a batch of booleans.
 */
template<class B> inline typename simd_batch_traits<B>::batch_bool_type isinf(const simd_base<B>& x)
{
...
template<class B> inline typename simd_batch_traits<B>::batch_bool_type is_flint(const simd_base<B>& x)
{
...
template<class B> inline typename simd_batch_traits<B>::batch_bool_type is_odd(const simd_base<B>& x)
{
	return is_even(x - batch_type_t<B>(1.));
}

template<class B> inline typename simd_batch_traits<B>::batch_bool_type is_even(const simd_base<B>& x)
{
