 ...
 */
template<class B> batch_type_t<B> log(const simd_base<B>& x);
 ...
 */
template<class B> batch_type_t<B> log10(const simd_base<B>& x);
...
namespace detail {
template<class B, class T = typename B::value_type> struct log_kernel;

template<class B> struct log_kernel<B, float> {
...
template<class B> struct log_kernel<B, double> {
	/* origin: boost/simd/arch/common/simd/function/log.hpp */
	/*
	 * ====================================================
	 * copyright 2016 NumScale SAS
	...
	static inline B compute(const B& a)
	{
		...
		B t1 = w * horner<B, 0x3fd999999997fa04ll, 0x3fcc71c51d8e78afll, 0x3fc39a09d078c69fll>(w);
		B t2 = z * horner<B, 0x3fe5555555555593ll, 0x3fd2492494229359ll, 0x3fc7466496cb03dell, 0x3fc2f112df3e5244ll>(w);
		B R = t2 + t1;
		B r = fma(dk, log_2hi<B>(), fma(s, (hfsq + R), dk * log_2lo<B>()) - hfsq + f);
#ifndef XSIMD_NO_INFINITIES
		B zz = select(isnez, select(a == infinity<B>(), infinity<B>(), r), minusinfinity<B>());
...
template<class B> inline batch_type_t<B> log(const simd_base<B>& x)
{
...
namespace detail {
template<class B, class T = typename B::value_type> struct log2_kernel;

template<class B> struct log2_kernel<B, float> {
...
template<class B> struct log2_kernel<B, double> {
	/* origin: FreeBSD /usr/src/lib/msun/src/e_log2f.c */
	/*
	 * ====================================================
	 * Copyright (C) 1993 by Sun Microsystems, Inc. All rights reserved.
...
template<class B> inline batch_type_t<B> log2(const simd_base<B>& x)
{
...
namespace detail {
template<class B, class T = typename B::value_type> struct log10_kernel;

template<class B> struct log10_kernel<B, float> {
	...
	static inline B compute(const B& a)
	{
		const B ivln10hi(4.3432617188e-01f), ivln10lo(-3.1689971365e-05f), log10_2hi(3.0102920532e-01f),
		...
		B r = fma(dk, log10_2hi,
		    fma(hibits, ivln10hi, fma(lobits, ivln10hi, fma(lobits + hibits, ivln10lo, dk * log10_2lo))));
#ifndef XSIMD_NO_INFINITIES
		B zz = select(isnez, select(a == infinity<B>(), infinity<B>(), r), minusinfinity<B>());
#else
		B zz = select(isnez, r, minusinfinity<B>());
...
template<class B> struct log10_kernel<B, double> {
	/* origin: FreeBSD /usr/src/lib/msun/src/e_log10f.c */
	/*
	 * ====================================================
	 * Copyright (C) 1993 by Sun Microsystems, Inc. All rights reserved.
	...
	static inline B compute(const B& a)
	{
		const B ivln10hi(4.34294481878168880939e-01), ivln10lo(2.50829467116452752298e-11),
...
template<class B> inline batch_type_t<B> log10(const simd_base<B>& x)
{
...
namespace detail {
template<class B, class T = typename B::value_type> struct log1p_kernel;

template<class B> struct log1p_kernel<B, float> {
...
template<class B> struct log1p_kernel<B, double> {
	/* origin: boost/simd/arch/common/simd/function/log1p.hpp */
	/*
	 * ====================================================
	 * copyright 2016 NumScale SAS
...
template<class B> inline batch_type_t<B> log1p(const simd_base<B>& x)
{
