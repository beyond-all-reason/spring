...
template<class B> batch_type_t<B> average(const simd_base<B>& x1, const simd_base<B>& x2);
 ...
 */
template<class B> batch_type_t<B> sinh(const simd_base<B>& x);
 ...
 */
template<class B> batch_type_t<B> cosh(const simd_base<B>& x);
 ...
 */
template<class B> batch_type_t<B> asinh(const simd_base<B>& x);
 ...
 */
template<class B> batch_type_t<B> acosh(const simd_base<B>& x);
 ...
 */
template<class B> batch_type_t<B> atanh(const simd_base<B>& x);
...
template<class B, bool cond = std::is_floating_point<typename B::value_type>::value> struct average_impl {
	static inline B compute(const B& x1, const B& x2) { return (x1 & x2) + ((x1 ^ x2) >> 1); }
};

template<class B> struct average_impl<B, true> {
	static inline B compute(const B& x1, const B& x2) { return fma(x1, B(0.5), x2 * B(0.5)); }
};
} // namespace detail

template<class B> inline batch_type_t<B> average(const simd_base<B>& x1, const simd_base<B>& x2)
{
...
template<class B, class T = typename B::value_type> struct sinh_kernel_impl;

template<class B> struct sinh_kernel_impl<B, float> {
...
template<class B> struct sinh_kernel_impl<B, double> {
	static inline B compute(const B& x)
	{
		B sqrx = x * x;
		return fma(x,
...
template<class B> struct sinh_kernel {
 ...
 * ====================================================
 */
template<class B> inline batch_type_t<B> sinh(const simd_base<B>& a)
{
...
namespace detail {
template<class B> struct cosh_kernel {
	static inline B compute(const B& a)
	{
		using b_type = B;
		b_type x = abs(a);
...
template<class B> inline batch_type_t<B> cosh(const simd_base<B>& a)
{
 ...
 * ====================================================
 */
template<class B, class T = typename B::value_type> struct tanh_kernel_impl;

template<class B> struct tanh_kernel_impl<B, float> {
	...
	static inline B cotanh(const B& x) { return B(1.) / tanh(x); }
};

template<class B> struct tanh_kernel_impl<B, double> {
	static inline B tanh(const B& x)
	{
		B sqrx = x * x;
...
template<class B> inline batch_type_t<B> tanh(const simd_base<B>& a)
{
...
template<class B, class T = typename B::value_type> struct asinh_kernel;

template<class B> struct asinh_kernel<B, float> {
	static inline B compute(const B& a)
	{
		B x = abs(a);
		auto lthalf = x < B(0.5);
		...
		B z(0.);
		if (any(lthalf)) {
			z = horner<B, 0x3f800000, 0xbe2aa9ad, 0x3d9949b1, 0xbd2ee581, 0x3ca4d6e6>(x2) * x;
			if (all(lthalf))
				return z ^ bts;
		}
...
template<class B> struct asinh_kernel<B, double> {
	static inline B compute(const B& a)
	{
		B x = abs(a);
...
template<class B> inline batch_type_t<B> asinh(const simd_base<B>& x)
{
...
namespace detail {
template<class B> struct acosh_kernel {
	static inline B compute(const B& a)
	{
		using b_type = B;
		b_type x = a - b_type(1.);
...
template<class B> inline batch_type_t<B> acosh(const simd_base<B>& a)
{
...
namespace detail {
template<class B> struct atanh_kernel {
	static inline B compute(const B& a)
	{
		using b_type = B;
		b_type x = abs(a);
...
template<class B> inline batch_type_t<B> atanh(const simd_base<B>& a)
{
