...
template<class T, size_t Align> class aligned_stack_buffer {
public:
...
inline void* aligned_alloc_stack(size_t size, size_t alignment)
{
	return reinterpret_cast<void*>(
	           reinterpret_cast<size_t>(XSIMD_ALLOCA(size + alignment)) & ~(size_t(alignment - 1))) +
	       alignment;
}
} // namespace detail
    ...
    : m_size(n)
{
	...
	if (sizeof(T) * n <= XSIMD_STACK_ALLOCATION_LIMIT) {
		m_ptr = reinterpret_cast<pointer>((reinterpret_cast<size_t>(XSIMD_ALLOCA(n + A)) & ~(size_t(A - 1))) + A);
...
template<class T, size_t A> inline aligned_stack_buffer<T, A>::~aligned_stack_buffer()
{
...
template<class T, size_t A> inline auto aligned_stack_buffer<T, A>::size() const noexcept -> size_type
{
	return m_size;
}

template<class T, size_t A> inline auto aligned_stack_buffer<T, A>::operator[](size_type i) -> reference
{
	return m_ptr[i];
}

template<class T, size_t A> inline auto aligned_stack_buffer<T, A>::operator[](size_type i) const -> const_reference
{
	return m_ptr[i];
}

template<class T, size_t A> inline aligned_stack_buffer<T, A>::operator pointer() noexcept { return m_ptr; }
} // namespace xsimd
