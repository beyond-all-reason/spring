 ...
 */
template<class T, size_t Align> class aligned_allocator {
public:
	using value_type = T;
	using pointer = T*;
	...
	template<class U> aligned_allocator(const aligned_allocator<U, Align>& rhs) noexcept;
 ...
 * Default constructor.
 */
template<class T, size_t A> inline aligned_allocator<T, A>::aligned_allocator() noexcept {}

/**
 * Copy constructor.
 */
template<class T, size_t A> inline aligned_allocator<T, A>::aligned_allocator(const aligned_allocator&) noexcept {}
 ...
 * Destructor.
 */
template<class T, size_t A> inline aligned_allocator<T, A>::~aligned_allocator() {}
 ...
 */
template<class T, size_t A> inline auto aligned_allocator<T, A>::address(reference r) noexcept -> pointer { return &r; }
 ...
 */
template<class T, size_t A> inline auto aligned_allocator<T, A>::allocate(size_type n, const void*) -> pointer
{
 ...
 */
template<class T, size_t A> inline void aligned_allocator<T, A>::deallocate(pointer p, size_type) { aligned_free(p); }
 ...
 */
template<class T, size_t A> inline auto aligned_allocator<T, A>::max_size() const noexcept -> size_type
{
 ...
 */
template<class T, size_t A> inline auto aligned_allocator<T, A>::size_max() const noexcept -> size_type
{
	return size_type(-1) / sizeof(T);
}
 ...
 */
template<class T, size_t A> template<class U> inline void aligned_allocator<T, A>::destroy(U* p) { p->~U(); }
...
inline void* aligned_malloc(size_t size, size_t alignment) { return detail::xaligned_malloc(size, alignment); }

inline void aligned_free(void* ptr) { detail::xaligned_free(ptr); }
