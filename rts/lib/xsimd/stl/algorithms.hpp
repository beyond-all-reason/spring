...
namespace xsimd {
template<class I1, class I2, class O1, class UF> void transform(I1 first, I2 last, O1 out_first, UF&& f)
{
...
namespace detail {
struct plus {
	template<class X, class Y> auto operator()(X&& x, Y&& y) -> decltype(x + y) { return x + y; }
};
} // namespace detail
...
template<class Iterator1,
    ...
    size_t N = sizeof...(BinaryFunction)>
InitArray reduce(Iterator1 first, Iterator2 last, InitArray init, BinaryFunction&&... binfun)
{
	...
	for (size_t i = 0; i < N; ++i) xsimd::load_aligned(ptr, batch_init[i]);
	...
	const auto ReduceAcross = [&arr, &init, &batch_init, first, align_end, size](auto&& binFunScalar, size_t fi) {
		...
		for (auto x: arr) init[fi] = binFunScalar(init[fi], x);
