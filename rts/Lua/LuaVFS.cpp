...
int LuaVFS::Include(lua_State* L, bool synced)
{
	...
	if ((loadCode = LoadFileWithModes(fileName, fileData, mode)) != 1) {
		...
		const auto buf = fmt::format(
		    "[LuaVFS::{}(synced={})][loadvfs] file={} status={} cenv={} vfsmode={} {}", __func__, synced, fileName,
		    loadCode, hasCustomEnv, mode, hint
		);
	...
	if ((luaError = luaL_loadbuffer(L, fileData.c_str(), fileData.size(), fileName.c_str())) != 0) {
		const auto buf = fmt::format(
		    "[LuaVFS::{}(synced={})][loadbuf] file={} error={} ({}) cenv={} vfsmode={}", __func__, synced, fileName,
		    luaError, lua_tostring(L, -1), hasCustomEnv, mode
		);
		lua_pushlstring(L, buf.c_str(), buf.size());
		lua_error(L);
	}
	...
	if (lua_setfenv(L, -2) == 0)
		luaL_error(
		    L, "[LuaVFS::%s(synced=%d)][setfenv] file=%s type=%d cenv=%d", __func__, synced, fileName.c_str(),
	...
	if ((luaError = lua_pcall(L, 0, LUA_MULTRET, 0)) != 0) {
		const auto buf = fmt::format(
		    "[LuaVFS::{}(synced={})][pcall] file={} error={} ({}) ptop={} cenv={} vfsmode={}", __func__, synced,
		    fileName, luaError, lua_tostring(L, -1), paramTop, hasCustomEnv, mode
		);
		lua_pushlstring(L, buf.c_str(), buf.size());
		lua_error(L);
	}
...
int LuaVFS::MapArchive(lua_State* L)
{
	...
	if (args >= 2) {
		...
		if (argChecksum != hexChecksum)
			luaL_error(
			    L, "[VFS::%s] incorrect checksum for archive: %s (got: %s, expected: %s)", __func__,
