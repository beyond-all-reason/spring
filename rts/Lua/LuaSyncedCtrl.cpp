...
static int SetSolidObjectBlocking(lua_State* L, CSolidObject* o)
{
	...
	o->UpdateCollidableStateBit(CSolidObject::CSTATE_BIT_PROJECTILES, luaL_optboolean(L, 3, o->HasCollidableStateBit(CSolidObject::CSTATE_BIT_PROJECTILES)));
...
static int SetSolidObjectRotation(lua_State* L, T* o)
{
	...
	angles[CMatrix44f::ANGLE_Y] = luaL_checkfloat(L, 2);
	angles[CMatrix44f::ANGLE_R] = luaL_checkfloat(L, 3);
...
static int SetSolidObjectHeadingAndUpDir(lua_State* L, T* o)
{
	...
	const float3 newUpDir = float3(luaL_checkfloat(L, 2), luaL_checkfloat(L, 3), luaL_checkfloat(L, 5)).SafeNormalize();
...
static int SetSolidObjectDirection(lua_State* L, CSolidObject* o, const char* func)
{
	...
	const float3 newFrontDir = float3(luaL_checkfloat(L, 2), luaL_checkfloat(L, 2), luaL_checkfloat(L, 3)).SafeNormalize();
...
static int SetWorldObjectVelocity(lua_State* L, CWorldObject* o)
{
	...
	speed.y = std::clamp(luaL_checkfloat(L, 2), -MAX_UNIT_SPEED, MAX_UNIT_SPEED);
	speed.z = std::clamp(luaL_checkfloat(L, 3), -MAX_UNIT_SPEED, MAX_UNIT_SPEED);
...
static int SetSolidObjectPieceCollisionVolumeData(lua_State* L, CSolidObject* obj)
{
	...
	const float3 scales(luaL_checkfloat(L, 3), luaL_checkfloat(L, 5), luaL_checkfloat(L, 6));
	...
	vol->SetIgnoreHits(!luaL_checkboolean(L, 2));
	return 0;
}
...
static int SetSolidObjectPieceVisible(lua_State* L, CSolidObject* obj)
{
	...
	lmp->SetScriptVisible(luaL_checkboolean(L, 2));
	return 0;
}
...
int LuaSyncedCtrl::SetAlly(lua_State* L)
{
	...
	teamHandler.SetAlly(firstAllyTeamID, secondAllyTeamID, luaL_checkboolean(L, 2));
	return 0;
}
...
int LuaSyncedCtrl::AddTeamResource(lua_State* L)
{
	...
	const float value = max(0.0f, luaL_checkfloat(L, 2));
...
int LuaSyncedCtrl::UseTeamResource(lua_State* L)
{
	...
	if (lua_isstring(L, 2)) {
		...
		const float value = std::max(0.0f, luaL_checkfloat(L, 2));
...
int LuaSyncedCtrl::ShareTeamResource(lua_State* L)
{
	...
	const char* type = luaL_checkstring(L, 2);
	float amount = luaL_checkfloat(L, 3);
...
//FIXME?	CheckAllowGameChanges(L);
	...
	ParseCobArgs(L, 3, numArgs, cobArgs);

	const int numRetVals = std::min(lua_toint(L, 2), std::min(MAX_LUA_COB_ARGS, cobArgs[0]));
	int retCode = 0;
...
int LuaSyncedCtrl::DestroyUnit(lua_State* L)
{
	...
	const bool reclaimed = luaL_optboolean(L, 2, false);
...
int LuaSyncedCtrl::TransferUnit(lua_State* L)
{
	...
	bool given = true;
	if (FullCtrl(L) && lua_isboolean(L, 2))
		given = lua_toboolean(L, 2);
...
int LuaSyncedCtrl::SetUnitResourcing(lua_State* L)
{
	...
	if (lua_israwstring(L, 2)) {
		SetUnitResourceParam(unit, lua_tostring(L, 2), luaL_checkfloat(L, 2));
...
int LuaSyncedCtrl::SetUnitStorage(lua_State* L)
{
	...
	if (lua_israwstring(L, 2)) {
		SetUnitStorageParam(unit, lua_tostring(L, 2), luaL_checkfloat(L, 2));
	} else if (lua_istable(L, 2)) {
		constexpr int tableIdx = 2;
...
int LuaSyncedCtrl::SetUnitStockpile(lua_State* L)
{
	...
	if (lua_isnumber(L, 2))
		unit->stockpileWeapon->buildPercent = std::clamp(lua_tofloat(L, 2), 0.0f, 1.0f);
...
int LuaSyncedCtrl::SetUnitUseWeapons(lua_State* L)
{
	...
	unit->allowUseWeapons = luaL_optboolean(L, 2, unit->allowUseWeapons);
	return 0;
}
...
int LuaSyncedCtrl::SetUnitWeaponDamages(lua_State* L)
{
	...
	if (lua_istable(L, 2)) {
		...
		// key, value
		if ((lua_isnumber(L, 2) || lua_israwstring(L, 2)) && lua_isnumber(L, 3)) {
...
int LuaSyncedCtrl::SetUnitArmored(lua_State* L)
{
	...
	unit->armoredMultiple = luaL_optfloat(L, 2, unit->armoredMultiple);
...
int LuaSyncedCtrl::SetUnitLosMask(lua_State* L)
{
	...
	const unsigned char  newMask = ParseLosBits(L, 2, oldMask);
...
int LuaSyncedCtrl::SetUnitLosState(lua_State* L)
{
	...
	const unsigned char  newState = ParseLosBits(L, 2, oldState);
...
int LuaSyncedCtrl::SetUnitCloak(lua_State* L)
{
	...
	if (lua_israwnumber(L, 2)) {
		unit->decloakDistance = lua_tofloat(L, 2);
		return 0;
	}
...
int LuaSyncedCtrl::SetUnitMetalExtraction(lua_State* L)
{
	...
	const float range = luaL_optfloat(L, 2, mex->GetExtractionRange());
	mex->ResetExtraction();
...
int LuaSyncedCtrl::SetUnitBuildParams(lua_State* L)
{
	...
	switch (hashString(luaL_checkstring(L, 2))) {
		...
		case hashString("buildDistance"): {
			builder->buildDistance = luaL_optfloat(L, 2, builder->buildDistance);
		} break;
		case hashString("buildRange3D"): {
			builder->range3D = luaL_optboolean(L, 2, builder->range3D);
		} break;
		default: {} break;
	};
...
int LuaSyncedCtrl::SetUnitBuildSpeed(lua_State* L)
{
	...
	if (lua_isnumber(L, 2)) {
		builder->repairSpeed    = INV_GAME_SPEED * max(0.0f, lua_tofloat(L, 2));
	}
	if (lua_isnumber(L, 3)) {
		builder->reclaimSpeed   = INV_GAME_SPEED * max(0.0f, lua_tofloat(L, 3));
	}
	if (lua_isnumber(L, 5)) {
...
int LuaSyncedCtrl::SetUnitShieldRechargeDelay(lua_State* L)
{
	...
	if (lua_isnumber(L, 2)) {
		const auto seconds = lua_tofloat(L, 2);
...
int LuaSyncedCtrl::SetUnitFlanking(lua_State* L)
{
	...
	if (key == "mode") {
		unit->flankingBonusMode = luaL_checkint(L, 2);
	}
	else if (key == "dir") {
		...
		unit->flankingBonusMobilityAdd = luaL_checkfloat(L, 2);
	}
	else if (key == "minDamage") {
		const float minDamage = luaL_checkfloat(L, 2);
		...
		const float maxDamage = luaL_checkfloat(L, 2);
...
int LuaSyncedCtrl::SetUnitTarget(lua_State* L)
{
	...
	if (args >= 4 && !lua_isboolean(L, 2)) {
		...
		const bool manualFire = luaL_optboolean(L, 2, false);
		const bool userTarget = luaL_optboolean(L, 3, false);
...
int LuaSyncedCtrl::SetUnitRadiusAndHeight(lua_State* L)
{
	...
	const float newHeight = std::max(1.0f, luaL_optfloat(L, 2, unit->height));
...
int LuaSyncedCtrl::SetUnitPieceMatrix(lua_State* L)
{
	...
	if (LuaUtils::ParseFloatArray(L, 2, &mat.m[0], 16) == -1)
		return 0;
...
int LuaSyncedCtrl::SetUnitSensorRadius(lua_State* L)
{
	...
	const int radius = std::clamp(luaL_checkint(L, 2), 0, MAX_UNIT_SENSOR_RADIUS);
...
int LuaSyncedCtrl::SetUnitPosErrorParams(lua_State* L)
{
	...
	unit->posErrorVector.y = luaL_optfloat(L, 2, unit->posErrorVector.y);
	unit->posErrorVector.z = luaL_optfloat(L, 3, unit->posErrorVector.z);
...
int LuaSyncedCtrl::SetUnitMoveGoal(lua_State* L)
{
	...
	const float3 pos(luaL_checkfloat(L, 2), luaL_checkfloat(L, 2), luaL_checkfloat(L, 3));
...
int LuaSyncedCtrl::SetUnitLandGoal(lua_State* L)
{
	...
	const float3 landPos(luaL_checkfloat(L, 2), luaL_checkfloat(L, 2), luaL_checkfloat(L, 3));
...
int LuaSyncedCtrl::SetFactoryBuggerOff(lua_State* L)
{
	...
	f->boOffset      = luaL_optfloat(  L, 2, f->boOffset     );
	f->boRadius      = luaL_optfloat(  L, 3, f->boRadius     );
...
int LuaSyncedCtrl::AddUnitDamage(lua_State* L)
{
	...
	const int paralyze    = luaL_optint(L, 2, 0);
	const int attackerID  = luaL_optint(L, 3, -1);
...
int LuaSyncedCtrl::AddUnitResource(lua_State* L)
{
	...
	switch (type[0]) {
		case 'm': { unit->AddMetal (std::max(0.0f, luaL_checkfloat(L, 2))); } break;
		case 'e': { unit->AddEnergy(std::max(0.0f, luaL_checkfloat(L, 2))); } break;
		default: {} break;
	}
...
int LuaSyncedCtrl::UseUnitResource(lua_State* L)
{
	...
	if (lua_isstring(L, 2)) {
		...
		switch (type[0]) {
			case 'm': { lua_pushboolean(L, unit->UseMetal (std::max(0.0f, lua_tofloat(L, 2)))); return 1; } break;
			case 'e': { lua_pushboolean(L, unit->UseEnergy(std::max(0.0f, lua_tofloat(L, 2)))); return 1; } break;
			default : {                                                                                   } break;
		}
...
int LuaSyncedCtrl::AddGrass(lua_State* L)
{
	...
	const uint8_t grassValue = static_cast<uint8_t>(luaL_optint(L, 2, 1));
...
int LuaSyncedCtrl::SetFeatureResources(lua_State* L)
{
	...
	feature->resources.energy = std::clamp(luaL_checknumber(L, 2), 0.0f, feature->defResources.energy);

	feature->reclaimTime = std::clamp(luaL_optnumber(L, 3, feature->reclaimTime), 1.0f, 1000000.0f);
...
int LuaSyncedCtrl::SetFeatureResurrect(lua_State* L)
{
	...
	feature->resurrectProgress = std::clamp(luaL_optnumber(L, 3, feature->resurrectProgress), 0.0f, 1.0f);
	return 0;
}
...
int LuaSyncedCtrl::SetFeatureMoveCtrl(lua_State* L)
{
	...
	if ((moveCtrl.enabled = luaL_optboolean(L, 2, moveCtrl.enabled))) {
		...
		// set vectors
		for (int i = 0; i < 3; i++) {
			moveCtrl.velVector[i] = luaL_optfloat(L, 2 + i, moveCtrl.velVector[i]);
			moveCtrl.accVector[i] = luaL_optfloat(L, 6 + i, moveCtrl.accVector[i]);
		}
	} else {
		// set masks
		for (int i = 0; i < 3; i++) {
			moveCtrl.velocityMask[i] = (luaL_optfloat(L, 2 + i, moveCtrl.velocityMask[i]) != 0.0f);
...
int LuaSyncedCtrl::SetFeatureRadiusAndHeight(lua_State* L)
{
	...
	const float newHeight = std::max(1.0f, luaL_optfloat(L, 2, feature->height));
...
int LuaSyncedCtrl::SetProjectilePosition(lua_State* L)
{
	...
	proj->pos.y = luaL_optfloat(L, 2, 0.0f);
	proj->pos.z = luaL_optfloat(L, 3, 0.0f);
...
int LuaSyncedCtrl::SetProjectileTarget(lua_State* L)
{
	...
	switch (lua_gettop(L)) {
		...
		case 4: {
			...
			wpro->SetTargetObject(nullptr);
			wpro->SetTargetPos(float3(luaL_checkfloat(L, 2), luaL_checkfloat(L, 2), luaL_checkfloat(L, 3)));
...
int LuaSyncedCtrl::SetPieceProjectileParams(lua_State* L)
{
	...
	pproj->spinAngle = luaL_optfloat(L, 2, pproj->spinAngle);
	pproj->spinSpeed = luaL_optfloat(L, 3, pproj->spinSpeed);
		...
		int& x1, int& z1, int& x2, int& z2, int resolution, int maxX, int maxZ)
{
	...
	if (args == 3) {
		...
		factor    = luaL_checkfloat(L, 2);
	}
	else if (args == 5) {
		fx1    = luaL_checkfloat(L, 1);
		fz1    = luaL_checkfloat(L, 2);
		fx2    = luaL_checkfloat(L, 2);
		fz2    = luaL_checkfloat(L, 3);
...
int LuaSyncedCtrl::AddHeightMap(lua_State* L)
{
	...
	const float h  = luaL_checkfloat(L, 2);
...
int LuaSyncedCtrl::SetMapSquareTerrainType(lua_State* L)
{
	...
	const int ntt = luaL_checkint(L, 2);
...
int LuaSyncedCtrl::SetTerrainTypeData(lua_State* L)
{
	...
	if (args >= 3 && lua_isnumber(L, 2)) ttSpeedModChanged |= (ctt. kbotSpeed != (tt-> kbotSpeed = lua_tofloat(L, 2)));
	if (args >= 4 && lua_isnumber(L, 3)) ttSpeedModChanged |= (ctt.hoverSpeed != (tt->hoverSpeed = lua_tofloat(L, 3)));
...
int LuaSyncedCtrl::SetSquareBuildingMask(lua_State* L)
{
	...
	const int mask = luaL_checkint(L, 2);
...
int LuaSyncedCtrl::UnitDetachFromAir(lua_State* L)
{
	...
	float3 pos;
	if (args >= 4) {
		pos = float3(luaL_checkfloat(L, 2), luaL_checkfloat(L, 2), luaL_checkfloat(L, 3));
	} else {
		pos = transportee->pos;
		pos.y = CGround::GetHeightAboveWater(pos.x, pos.z);
	}
...
{
	const float3 pos = {luaL_checkfloat(L, 1      ), luaL_checkfloat(L, 2      ), luaL_checkfloat(L, 2      )};
	const float3 dir = {luaL_optfloat  (L, 3, 0.0f), luaL_optfloat  (L, 5, 0.0f), luaL_optfloat  (L, 6, 0.0f)};
...
{
	const float3 pos = {luaL_optfloat(L, 2, 0.0f), luaL_optfloat(L, 2, 0.0f), luaL_optfloat(L, 3, 0.0f)};
...
int LuaSyncedCtrl::SpawnSFX(lua_State* L)
{
	...
	const float3 pos = {luaL_checkfloat(L, 2), luaL_checkfloat(L, 3), luaL_checkfloat(L, 5)};
...
int LuaSyncedCtrl::SetExperienceGrade(lua_State* L)
{
	...
	if (gs->cheatEnabled) {
		...
		if (lua_isnumber(L, 2))
			globalUnitParams.expHealthScale = lua_tofloat(L, 2);

		if (lua_isnumber(L, 3))
			globalUnitParams.expReloadScale = lua_tofloat(L, 3);
	}
	return 0;
}
...
int LuaSyncedCtrl::SetRadarErrorParams(lua_State* L)
{
	...
	losHandler->SetBaseRadarErrorSize(luaL_optnumber(L, 2, losHandler->GetBaseRadarErrorSize()));
	losHandler->SetBaseRadarErrorMult(luaL_optnumber(L, 3, losHandler->GetBaseRadarErrorMult()));
	return 0;
}
...
int LuaSyncedCtrl::EditUnitCmdDesc(lua_State* L)
{
	...
	ParseCommandDescription(L, 2, cmdDesc);
...
int LuaSyncedCtrl::InsertUnitCmdDesc(lua_State* L)
{
	...
	if ((args >= 3) && (!lua_isnumber(L, 2) || !lua_istable(L, 2)))
