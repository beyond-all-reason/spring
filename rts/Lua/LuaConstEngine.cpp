 ...
 * @field NegativeGetUnitCurrentCommand boolean Whether Spring.GetUnitCurrentCommand allows negative indices to look
 * from the end
 ...
 * @field noAutoShowMetal boolean Whether the engine switches to the metal view when selecting a "build metal extractor"
 * command (yes if false)
 * @field maxPiecesPerModel integer How many pieces supported for 3d models?
 * @field gunshipCruiseAltitudeMultiplier number For gunships, the cruiseAltitude from the unit def is multiplied by
 * this much
 * @field noRefundForConstructionDecay boolean Whether there is no refund for construction decay (100% metal back if
 * false)
 * @field noRefundForFactoryCancel boolean Whether there is no refund for factory cancel (100% metal back if false)
 * @field noOffsetForFeatureID boolean Whether featureID from various interfaces (targetID for Reclaim commands, ownerID
 * from SpringGetGroundDecalOwner, etc) needs to be offset by `Game.maxUnits`
 */
 ...
 * @field versionFull string "Major.Minor.PatchSet" for releases, "Major.Minor.PatchSet-CommitNum-gHash branch"
 * otherwise. Will also include (buildFlags), if there're any.
...
bool LuaConstEngine::PushEntries(lua_State* L)
{
	...
	LuaPushNamedNumber(
	    L, "gunshipCruiseAltitudeMultiplier", 1.5f); // see https://github.com/beyond-all-reason/spring/issues/1028
