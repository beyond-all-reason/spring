		...
		for (lua_pushnil(L); lua_next(L, tableIdx) != 0; lua_pop(L, 1)) {
			...
			if (iter == p.activeUniforms.end()) {
				if (globalRendering->glDebug || globalRendering->glDebugErrors)
					LOG_L(
					    L_WARNING, "[%s] uniform \"%s\" from table \"%s\" not active in shader", __func__, uniformName,
					    fieldName
					);
				continue;
			}
			...
			default: {
				LOG_L(
				    L_WARNING, "[%s] value for uniform \"%s\" from table \"%s\" (GL-type 0x%x) set as int", __func__,
