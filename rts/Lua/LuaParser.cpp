...
bool LuaParser::Execute()
{
	...
	if ((errorNum = luaL_loadbuffer(L, code.c_str(), code.size(), codeLabel.c_str())) != 0) {
		SNPRINTF(
		    errorBuf, sizeof(errorBuf), "[loadbuf] error %d (\"%s\") in %s", errorNum, lua_tostring(L, -1),
		    codeLabel.c_str()
		);
		LUA_CLOSE(&L);
...

	{
		...
		if ((errorNum = lua_pcall(L, 0, 1, 0)) != 0) {
			SNPRINTF(
			    errorBuf, sizeof(errorBuf), "[pcall] error %d (\"%s\") in %s", errorNum, lua_tostring(L, -1),
			    fileName.c_str()
			);
			LUA_CLOSE(&L);
