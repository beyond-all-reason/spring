...
int LuaOpenGL::DispatchCompute(lua_State* L)
{
	...
	if (numGroupX < 0 && numGroupX > maxNumGroups[0] || numGroupY < 0 && numGroupY > maxNumGroups[1] ||
		...
		luaL_error(
		    L, "%s Incorrect number of work groups specified x: 0 > %d < %d; y: 0 > %d < %d; z: 0 > %d < %d", __func__,
		    numGroupX, maxNumGroups[0], numGroupY, maxNumGroups[1], numGroupZ, maxNumGroups[2]
		);
...
int LuaOpenGL::BindImageTexture(lua_State* L)
{
	...
	if (unit < 0 || unit > maxUnit)
		luaL_error(
		    L, "%s Invalid image unit specified %d. The level must be >=0 and <= %d (spec. guaranteed 8)", __func__,
		    unit, maxUnit
		);
	...
	if (access != GL_READ_ONLY && access != GL_WRITE_ONLY && access != GL_READ_WRITE)
		luaL_error(
		    L, "%s Invalid access specified %d. The access must be GL_READ_ONLY or GL_WRITE_ONLY or GL_READ_WRITE.",
		    __func__, access
		);
...
int LuaOpenGL::CreateTextureAtlas(lua_State* L)
{
	...
	if (std::min(xsize, ysize) < minSize)
		luaL_error(
		    L, "gl.%s() The specified atlas dimensions (%d, %d) are too small. The minimum side size is %d", __func__,
		    xsize, ysize, minSize
		);

	if (xsize > maxSizeX || ysize > maxSizeY)
		luaL_error(
		    L, "gl.%s() The specified atlas dimensions (%d, %d) are too large. The maximum side sizes are (%d, %d)",
...
int LuaOpenGL::DeleteTextureAtlas(lua_State* L)
{
	...
	if (idStr[0] != LuaAtlasTextures::prefix)
		luaL_error(
		    L,
		    "gl.%s() Call is only suitable for destroying Texture Atlases. Use "
		    "gl.DeleteTexture/gl.DeleteTextureFBO for other texture types",
		    __func__
		);
...
int LuaOpenGL::AddAtlasTexture(lua_State* L)
{
	...
	if (texID <= 0 || texSizeX <= 0 || texSizeY <= 0) {
		luaL_error(
		    L, "gl.%s() Requested Lua texture %s has invalid size {%d,%d}", __func__, luaTexStr.c_str(), texSizeX,
		    texSizeY
		);
	}
	...
	atlas->AddTexFromMem(
	    subAtlasTexName, texSizeX, texSizeY, CTextureAtlas::TextureType::RGBA32,
	    buffer.data()
	); // whelp double copy
...
int LuaOpenGL::GetEngineAtlasTextures(lua_State* L)
{
	...
	default:
		luaL_error(
		    L, "[%s] Invalid engine atlas (%s) is specified (only $explosions and $groundfx are supported)", __func__,
		    atlasName.c_str()
		);
		return 0;
	}
...
int LuaOpenGL::Clear(lua_State* L)
{
	...
	case 5: {
		...
		case GL_COLOR_BUFFER_BIT: {
			glClearColor(
			    (GLfloat)lua_tonumber(L, 2), (GLfloat)lua_tonumber(L, 3), (GLfloat)lua_tonumber(L, 4),
