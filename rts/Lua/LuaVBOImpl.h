...
public:
	...
	size_t Upload(const sol::stack_table& luaTblData,
	...
	sol::as_table_t<std::vector<lua_Number>> Download(sol::optional<int> attribIdxOpt,
	    sol::optional<int> elemOffsetOpt,
	...
	size_t
	InstanceDataFromUnitDefIDs(int id, int attrID, sol::optional<int> teamIdOpt, sol::optional<int> elemOffsetOpt);
	size_t InstanceDataFromUnitDefIDs(const sol::stack_table& ids,
	    int attrID,
	    sol::optional<int> teamIdOpt,
	    sol::optional<int> elemOffsetOpt);
	size_t
	InstanceDataFromFeatureDefIDs(int id, int attrID, sol::optional<int> teamIdOpt, sol::optional<int> elemOffsetOpt);
	size_t InstanceDataFromFeatureDefIDs(const sol::stack_table& ids,
	    int attrID,
	    sol::optional<int> teamIdOpt,
	    sol::optional<int> elemOffsetOpt);
...
private:
	...
	int BindBufferRangeImpl(GLuint index,
	    const sol::optional<int> elemOffsetOpt,
	    const sol::optional<int> elemCountOpt,
	    const sol::optional<GLenum> targetOpt,
	    bool bind);
...
private:
	...
	template<typename TObj> static SInstanceData InstanceDataFromGetData(int id, int attrID, uint8_t defTeamID);

	template<typename TObj>
	size_t InstanceDataFromImpl(int id, int attrID, uint8_t defTeamID, const sol::optional<int>& elemOffsetOpt);

	template<typename TObj>
	size_t InstanceDataFromImpl(const sol::stack_table& ids,
	    int attrID,
	    uint8_t defTeamID,
	    const sol::optional<int>& elemOffsetOpt);

	template<typename Iterable>
	size_t MatrixDataFromProjectileIDsImpl(const Iterable& ids,
	    int attrID,
	    sol::optional<int> elemOffsetOpt,
	    const char* func);
	...
	template<typename TIn> size_t UploadImpl(const std::vector<TIn>& dataVec, uint32_t elemOffset, int attribIdx)
	{
		return UploadImpl(dataVec, elemOffset, [attribIdx](int attrID) {
			return attribIdx == -1 ||
			       attribIdx ==
			           attrID; // copy data if specific attribIdx is not requested or requested and matches attrID
		});
	}

	template<typename TIn>
	size_t
	UploadImpl(const std::vector<TIn>& dataVec, uint32_t elemOffset, const std::initializer_list<int>& attribIdxs)
	{
	...
	template<typename T> static T MaybeFunc(const sol::table& tbl, const std::string& key, T defValue);

	template<typename TIn, typename TOut, typename TIter>
	bool TransformAndWrite(int& bytesWritten,
	    GLubyte*& mappedBuf,
	...
	template<typename TIn>
	bool TransformAndRead(int& bytesRead,
	    GLubyte*& mappedBuf,
	    const int mappedBufferSizeInBytes,
	    const int size,
...
private:
	static constexpr uint32_t uboMinIndex =
	    5 + 1; // glBindBufferBase(GL_UNIFORM_BUFFER, 5, uboGroundLighting.GetId()); //DecalsDrawerGL4
	static constexpr uint32_t ssboMinIndex =
	    3 + 1; // glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 3, uboDecalsStructures.GetId()); //DecalsDrawerGL4
private:
