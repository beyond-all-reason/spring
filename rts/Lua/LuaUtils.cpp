...
int LuaUtils::IsEngineMinVersion(lua_State* L)
{
	...
	lua_pushboolean(
	    L, std::tuple(
	           StringToInt(SpringVersion::GetMajor()), StringToInt(SpringVersion::GetMinor()),
	           StringToInt(SpringVersion::GetCommits())
...
bool LuaUtils::IsProjectileVisible(lua_State* L, const CProjectile* pro)
{
	...
	return !(
	    (CLuaHandle::GetHandleReadAllyTeam(L) != pro->GetAllyteamID()) &&
