...
int LuaMathExtra::clamp(lua_State* L)
{
	...
	if (lbound > ubound) {
		luaL_error(
		    L, "Invalid math.%s parameters, lower bound(%f) is greater than upper bound(%f)", __func__, lbound, ubound);
		return 0;
	}
 ...
 * For example math.smoothstep(10, 25, 15) is 0.259, because 15 is 0.333 of the way from 10 to 25, and smoothstep(0.333)
 * is 0.259
 * @param edge0 number
 * @param edge1 number
 * @param v number
