		...
		case -2: {
			if ((u.loc = glGetUniformLocation(matShader.openglID, u.name)) == -1) {
				LOG_L(
				    ...
				    "material %s",
				    __func__, objId, objType, uuid, u.name, deferredPass ? "deferred" : "forward", GetMatTypeName(type)
				);
				continue;
			}
		} break;
...
int LuaMaterial::Compare(const LuaMaterial& a, const LuaMaterial& b)
{
	...
	if ((cmp = LuaMatShader::Compare(
	         a.shaders[LuaMatShader::LUASHADER_PASS_FWD], b.shaders[LuaMatShader::LUASHADER_PASS_FWD]
	     )) != 0)
		return cmp;
	if ((cmp = LuaMatShader::Compare(
	         a.shaders[LuaMatShader::LUASHADER_PASS_DFR], b.shaders[LuaMatShader::LUASHADER_PASS_DFR]
	     )) != 0)
		return cmp;
	...
	if ((cmp = LuaMatUniforms::Compare(
	         a.uniforms[LuaMatShader::LUASHADER_PASS_FWD], b.uniforms[LuaMatShader::LUASHADER_PASS_FWD]
	     )) != 0)
		return cmp;
	if ((cmp = LuaMatUniforms::Compare(
	         a.uniforms[LuaMatShader::LUASHADER_PASS_DFR], b.uniforms[LuaMatShader::LUASHADER_PASS_DFR]
	     )) != 0)
		return cmp;
...
void LuaMatUniforms::Validate(LuaMatShader* s)
{
	...
	while (uniformIndx < numUniforms) {
		...
		LOG_L(
		    L_WARNING, fmts[1], __func__, au.name, uniformLoc, GLUniformTypeToString(au.type),
