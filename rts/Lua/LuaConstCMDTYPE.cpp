...
#define PUSH_CMDTYPE(cmd) LuaInsertDualMapPair(L, #cmd, CMDTYPE_##cmd)
	...
	/*** @field CMDTYPE.ICON_UNIT_OR_MAP integer Expect 1 parameters in return (unitid) or 3 parameters in return
	 * (mappos). */
	PUSH_CMDTYPE(ICON_UNIT_OR_MAP);
	/*** @field CMDTYPE.ICON_FRONT integer Expect 3 or 6 parameters in return (middle and right side of front if a front
	 * was defined). */
	PUSH_CMDTYPE(ICON_FRONT);
	/*** @field CMDTYPE.ICON_UNIT_OR_AREA integer Expect 1 parameter in return (unitid) or 4 parameters in return
	 * (mappos+radius). */
	...
	PUSH_CMDTYPE(PREV);
	/*** @field CMDTYPE.ICON_UNIT_FEATURE_OR_AREA integer Expect 1 parameter in return (unitid or
	 * Game.maxUnits+featureid) or 4 parameters in return (mappos+radius). */
	...
	PUSH_CMDTYPE(CUSTOM);
	/*** @field CMDTYPE.ICON_UNIT_OR_RECTANGLE integer Expect 1 parameter in return (unitid) or 3 parameters in return
	 * (mappos) or 6 parameters in return (startpos+endpos). */
