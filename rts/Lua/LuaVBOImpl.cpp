...
bool LuaVBOImpl::FillAttribsTableImpl(const sol::table& attrDefTable)
{
	...
	for (const auto& kv : attrDefTable) {
		...
		if (!IsTypeValid(type)) {
			LOG_L(
			    L_ERROR, "[LuaVBOImpl::%s] Invalid attribute type [%u] for selected buffer type [%u]", __func__, type,
			    defTarget
			);
			continue;
		}
...
bool LuaVBOImpl::FillAttribsNumberImpl(const int numVec4Attribs)
{
	...
	if (numVec4Attribs > attributesCountMax) {
		LuaUtils::SolLuaError(
		    "[LuaVBOImpl::%s] Invalid number of vec4 arguments [%d], exceeded maximum of [%u]", __func__,
		    numVec4Attribs, attributesCountMax
		);
	}
 ...
 * @see VBO:Define
 */
size_t LuaVBOImpl::Upload(
    const sol::stack_table& luaTblData,
...
)
{
	...
	if (luaStartIndex > luaTblDataSize) {
		LuaUtils::SolLuaError(
		    "[LuaVBOImpl::%s] Invalid luaStartIndex [%u] exceeds table size [%u]", __func__, luaStartIndex,
		    luaTblDataSize
		);
	}
	...
	if (luaFinishIndex > luaTblDataSize) {
		LuaUtils::SolLuaError(
		    "[LuaVBOImpl::%s] Invalid luaFinishIndex [%u] exceeds table size [%u]", __func__, luaFinishIndex,
		    luaTblDataSize
		);
	}
	...
	if (luaStartIndex > luaFinishIndex) {
		LuaUtils::SolLuaError(
		    "[LuaVBOImpl::%s] Invalid luaStartIndex [%u] is greater than luaFinishIndex [%u]", __func__, luaStartIndex,
		    luaFinishIndex
		);
	}
 ...
 */
sol::as_table_t<std::vector<lua_Number>> LuaVBOImpl::Download(
    sol::optional<int> attribIdxOpt,
    sol::optional<int> elemOffsetOpt,
...
)
{
	...
	if (elemOffset + elemCount > elementsCount) {
		LuaUtils::SolLuaError(
		    "[LuaVBOImpl::%s] Invalid elemOffset [%u] + elemCount [%u] >= elementsCount [%u]", __func__, elemOffset,
		    elemCount, elementsCount
		);
	}
...
size_t LuaVBOImpl::MatrixDataFromProjectileIDsImpl(
    const Iterable& ids,
    int attrID,
    sol::optional<int> elemOffsetOpt,
    const char* func
)
{
	...
	default:
		LuaUtils::SolLuaError(
		    "[LuaVBOImpl::%s] Buffer attribute %d must have floating type, got (%u) type instead", func, attrID,
		    attr0.type
		);
		break;
	}
...
template<typename TObj> SInstanceData LuaVBOImpl::InstanceDataFromGetData(int id, int attrID, uint8_t defTeamID)
{
	...
	const uint32_t uniIndex = static_cast<uint32_t>(modelsUniformsStorage.GetObjOffset(obj)
	); // doesn't need to exist for defs and model. Don't check for validity
...
template<typename TObj>
size_t LuaVBOImpl::InstanceDataFromImpl(
    const sol::stack_table& ids,
    int attrID,
    uint8_t defTeamID,
...
size_t LuaVBOImpl::UploadImpl(const std::vector<TIn>& dataVec, uint32_t elemOffset, AttribTestFunc attribTestFunc)
{
	...
	const auto uploadToGPU = [this, buffDataWithOffset, bufferOffsetInBytes,
	                          mappedBufferSizeInBytes](int bytesWritten) -> int {
		vbo->Bind();
#if 1
...
#define TRANSFORM_AND_WRITE(T)                                                                                     \
	{                                                                                                              \
		if (!TransformAndWrite<TIn, T>(                                                                            \
		        bytesWritten, buffDataWithOffset, mappedBufferSizeInBytes, basicTypeSize, bdvIter, dataVec.cend(), \
		        copyData                                                                                           \
		    )) {                                                                                                   \
 ...
 */
size_t LuaVBOImpl::InstanceDataFromUnitDefIDs(
    int id,
    int attrID,
 ...
 * @return integer attrID
 */
size_t LuaVBOImpl::InstanceDataFromFeatureDefIDs(
    int id,
    int attrID,
    sol::optional<int> teamIdOpt,
    sol::optional<int> elemOffsetOpt
...
int LuaVBOImpl::BindBufferRangeImpl(
    GLuint bindingIndex,
    const sol::optional<int> elemOffsetOpt,
...
)
{
	...
	if (target != GL_UNIFORM_BUFFER && target != GL_SHADER_STORAGE_BUFFER) {
		LuaUtils::SolLuaError(
		    "[LuaVBOImpl::%s] (Un)binding target can only be equal to [%u] or [%u]", __func__, GL_UNIFORM_BUFFER,
	...
	case GL_UNIFORM_BUFFER: {
		if (bindingIndex < uboMinIndex || bindingIndex >= globalRendering->glslMaxUniformBufferBindings)
			LuaUtils::SolLuaError(
			    "[LuaVBOImpl::%s] Invalid (Un)binding index [%u]. Index must be within [%u : %d)", __func__,
			    uboMinIndex, globalRendering->glslMaxUniformBufferBindings
			);
	} break;
	case GL_SHADER_STORAGE_BUFFER: {
		if (bindingIndex < ssboMinIndex || bindingIndex >= globalRendering->glslMaxStorageBufferBindings)
			LuaUtils::SolLuaError(
			    "[LuaVBOImpl::%s] Invalid (Un)binding index [%u]. Index must be within [%u : %d)", __func__,
			    ssboMinIndex, globalRendering->glslMaxStorageBufferBindings
			);
	} break;
	default:
		LuaUtils::SolLuaError(
 ...
 */
int LuaVBOImpl::BindBufferRange(
    const GLuint index,
    const sol::optional<int> elemOffsetOpt,
    const sol::optional<int> elemCountOpt,
    const sol::optional<GLenum> targetOpt
 ...
 */
int LuaVBOImpl::UnbindBufferRange(
    const GLuint index,
    const sol::optional<int> elemOffsetOpt,
    const sol::optional<int> elemCountOpt,
    const sol::optional<GLenum> targetOpt
...
void LuaVBOImpl::DumpDefinition()
{
	...
	ss << fmt::format(
	    "Count of elements={}\nSize of one element={}\nTotal buffer size={}", elementsCount, elemSizeInBytes,
	    vbo->GetSize()
	);
...
void LuaVBOImpl::AllocGLBuffer(size_t byteSize)
{
	if (defTarget == GL_UNIFORM_BUFFER && bufferSizeInBytes > UBO_SAFE_SIZE_BYTES) {
		LuaUtils::SolLuaError(
		    "[LuaVBOImpl::%s] Exceeded [%u] safe UBO buffer size limit of [%u] bytes", __func__, bufferSizeInBytes,
		    LuaVBOImpl::UBO_SAFE_SIZE_BYTES
		);
	}

	if (bufferSizeInBytes > BUFFER_SANE_LIMIT_BYTES) {
		LuaUtils::SolLuaError(
		    "[LuaVBOImpl::%s] Exceeded [%u] sane buffer size limit of [%u] bytes", __func__, bufferSizeInBytes,
		    LuaVBOImpl::BUFFER_SANE_LIMIT_BYTES
		);
	}
...
bool LuaVBOImpl::TransformAndWrite(
    int& bytesWritten,
    GLubyte*& mappedBuf,
...
bool LuaVBOImpl::TransformAndRead(
    int& bytesRead,
    GLubyte*& mappedBuf,
    const int mappedBufferSizeInBytes,
    const int count,
