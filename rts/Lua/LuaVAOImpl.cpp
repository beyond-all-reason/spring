...
void LuaVAOImpl::AttachBufferImpl(
    const std::shared_ptr<LuaVBOImpl>& luaVBO,
...
LuaVAOImpl::DrawCheckResult LuaVAOImpl::DrawCheck(GLenum mode, const DrawCheckInput& inputs, bool indexed)
{
	...
	if (result.instCount > 0) {
		...
		if (result.baseInstance > 0)
			LuaUtils::SolLuaError(
			    "[LuaVAOImpl::%s]: Requested baseInstance [%d] and but zero instance count", __func__,
			    result.baseInstance
			);
	}
	...
	if (result.instCount == 0) {
		...
		if (result.baseInstance > 0)
			glDrawElementsInstancedBaseVertexBaseInstance(
			    mode, result.drawCount, indexType, INT2PTR(indElemOffsetInBytes), result.instCount, result.baseVertex,
			    result.baseInstance
			);
		else {
			if (result.baseVertex == 0)
