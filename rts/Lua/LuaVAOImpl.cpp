...
LuaVAOImpl::~LuaVAOImpl() { Delete(); }

bool LuaVAOImpl::Supported()
{
	static bool supported = VBO::IsSupported(GL_ARRAY_BUFFER) && VAO::IsSupported() && GLAD_GL_ARB_instanced_arrays &&
	                        GLAD_GL_ARB_draw_elements_base_vertex && GLAD_GL_ARB_multi_draw_indirect;
	return supported;
}

void LuaVAOImpl::AttachBufferImpl(const std::shared_ptr<LuaVBOImpl>& luaVBO,
    std::shared_ptr<LuaVBOImpl>& thisLuaVBO,
    GLenum reqTarget)
{
	if (thisLuaVBO) {
	...
	if (luaVBO->defTarget != reqTarget) {
		LuaUtils::SolLuaError(
		    "[LuaVAOImpl::%s] LuaVBO should have been created with [%u] target, got [%u] target instead", __func__,
		    reqTarget, luaVBO->defTarget);
	}
...
template<typename TObj> const SIndexAndCount LuaVAOImpl::GetDrawIndicesImpl(int id)
{
	const TObj* obj =
	    LuaUtils::SolIdToObject<TObj>(id, __func__); // wrong ids are handles in LuaUtils::SolIdToObject<>()
	return GetDrawIndicesImpl<TObj>(obj);
}

template<typename TObj> const SIndexAndCount LuaVAOImpl::GetDrawIndicesImpl(const TObj* obj)
{
...
template<typename TObj> int LuaVAOImpl::AddObjectsToSubmissionImpl(int id)
{
	DrawCheckInput inputs{
	    std::nullopt, std::nullopt, std::nullopt, static_cast<int>(submitCmds.size() + 1), std::nullopt};
...
template<typename TObj> int LuaVAOImpl::AddObjectsToSubmissionImpl(const sol::stack_table& ids)
{
	const std::size_t idsSize = ids.size(); // size() is very costly to do in the loop

	DrawCheckInput inputs{
	    std::nullopt, std::nullopt, std::nullopt, static_cast<int>(submitCmds.size() + idsSize), std::nullopt};
...
template<typename TObj> SDrawElementsIndirectCommand LuaVAOImpl::DrawObjectGetCmdImpl(int id)
{
	const auto& indexAndCount = LuaVAOImpl::GetDrawIndicesImpl<TObj>(id);

	return SDrawElementsIndirectCommand{indexAndCount.count, 1u, indexAndCount.index, 0u, baseInstance++};
}
...
{
	if (vao && (vertLuaVBO && vertLuaVBO->GetId() == oldVertVBOId) &&
	    (indxLuaVBO && indxLuaVBO->GetId() == oldIndxVBOId) && (instLuaVBO && instLuaVBO->GetId() == oldInstVBOId))
...
#define INT2PTR(x) (reinterpret_cast<void*>(static_cast<intptr_t>(x)))
	...
	const auto glVertexAttribPointerFunc = [](GLuint index, GLint size, GLenum type, GLboolean normalized,
	...
	if (vertLuaVBO)
		for (const auto& va: vertLuaVBO->bufferAttribDefsVec) {
			...
			glVertexAttribPointerFunc(
			    va.first, attr.size, attr.type, attr.normalized, vertLuaVBO->elemSizeInBytes, INT2PTR(attr.pointer));
	...
	if (instLuaVBO) {
		...
		for (const auto& va: instLuaVBO->bufferAttribDefsVec) {
			...
			glEnableVertexAttribArray(va.first);
			glVertexAttribPointerFunc(
			    va.first, attr.size, attr.type, attr.normalized, instLuaVBO->elemSizeInBytes, INT2PTR(attr.pointer));
...
LuaVAOImpl::DrawCheckResult LuaVAOImpl::DrawCheck(GLenum mode, const DrawCheckInput& inputs, bool indexed)
{
	...
	if (indexed) {
		if (!indxLuaVBO)
			LuaUtils::SolLuaError(
			    "[LuaVAOImpl::%s]: No index buffer is attached. Did you succesfully call vao:AttachIndexBuffer()?",
			    __func__);
		...
		if (result.drawCount <= 0)
			LuaUtils::SolLuaError(
			    "[LuaVAOImpl::%s]: Non-positive number of draw elements %d is requested", __func__, result.drawCount);

		if (result.drawCount > indxLuaVBO->elementsCount - result.baseIndex)
			LuaUtils::SolLuaError(
			    "[LuaVAOImpl::%s]: Requested number of elements %d with offset %d exceeds buffer size %u", __func__,
			    result.drawCount, result.baseIndex, indxLuaVBO->elementsCount);
	}
	else {
		if (!vertLuaVBO) {
			if (!inputs.drawCount.has_value())
				LuaUtils::SolLuaError("[LuaVAOImpl::%s]: In case vertex buffer is not attached, the drawCount param "
			...
			if (result.drawCount > vertLuaVBO->elementsCount - result.baseIndex)
				LuaUtils::SolLuaError(
				    "[LuaVAOImpl::%s]: Requested number of vertices %d with offset %d exceeds buffer size %u", __func__,
				    result.drawCount, result.baseIndex, vertLuaVBO->elementsCount);
		}
	}

	result.baseInstance = std::max(inputs.baseInstance.value_or(0), 0);
	result.instCount =
	    std::max(inputs.instCount.value_or(0), 0); // 0 - forces ordinary version, while 1 - calls *Instanced()

	if (result.instCount > 0) {
		if (instLuaVBO && (result.instCount > instLuaVBO->elementsCount - result.baseInstance))
			LuaUtils::SolLuaError(
			    "[LuaVAOImpl::%s]: Requested number of instances %d with offset %d exceeds buffer size %u", __func__,
			    result.instCount, result.baseInstance, instLuaVBO->elementsCount);
	}
	else {
		if (result.baseInstance > 0)
			LuaUtils::SolLuaError("[LuaVAOImpl::%s]: Requested baseInstance [%d] and but zero instance count", __func__,
			    result.baseInstance);
	}
 ...
 * @return nil
 */
void LuaVAOImpl::DrawArrays(GLenum mode,
    sol::optional<int> vertCountOpt,
    sol::optional<int> vertexFirstOpt,
    sol::optional<int> instanceCountOpt,
    sol::optional<int> instanceFirstOpt)
{
	DrawCheckInput inputs{vertCountOpt, std::nullopt, vertexFirstOpt, instanceCountOpt, instanceFirstOpt};
	...
	if (result.instCount == 0)
		...
		if (result.baseInstance > 0)
			glDrawArraysInstancedBaseInstance(
			    mode, result.baseIndex, result.drawCount, result.instCount, result.baseInstance);
		else
 ...
 * @return nil
 */
void LuaVAOImpl::DrawElements(GLenum mode,
    sol::optional<int> indCountOpt,
    sol::optional<int> indElemOffsetOpt,
    sol::optional<int> instanceCountOpt,
    sol::optional<int> baseVertexOpt,
    sol::optional<int> instanceFirstOpt)
{
	DrawCheckInput inputs{indCountOpt, baseVertexOpt, indElemOffsetOpt, instanceCountOpt, instanceFirstOpt};
	...
	if (result.instCount == 0) {
		if (result.baseVertex == 0)
			...
			glDrawElementsBaseVertex(
			    mode, result.drawCount, indexType, INT2PTR(indElemOffsetInBytes), result.baseVertex);
	}
	else {
		if (result.baseInstance > 0)
			glDrawElementsInstancedBaseVertexBaseInstance(mode, result.drawCount, indexType,
			    INT2PTR(indElemOffsetInBytes), result.instCount, result.baseVertex, result.baseInstance);
		else {
			if (result.baseVertex == 0)
				glDrawElementsInstanced(
				    mode, result.drawCount, indexType, INT2PTR(indElemOffsetInBytes), result.instCount);
			else
				glDrawElementsInstancedBaseVertex(mode, result.drawCount, indexType, INT2PTR(indElemOffsetInBytes),
				    result.instCount, result.baseVertex);
		}
	}
#undef INT2PTR
...
int LuaVAOImpl::AddFeaturesToSubmission(const sol::stack_table& ids)
{
	return AddObjectsToSubmissionImpl<CFeature>(ids);
}
...
int LuaVAOImpl::AddUnitDefsToSubmission(const sol::stack_table& ids)
{
	return AddObjectsToSubmissionImpl<UnitDef>(ids);
}
...
int LuaVAOImpl::AddFeatureDefsToSubmission(const sol::stack_table& ids)
{
	return AddObjectsToSubmissionImpl<FeatureDef>(ids);
}
...
void LuaVAOImpl::Submit()
{
	...
	glMultiDrawElementsIndirect(
	    GL_TRIANGLES, GL_UNSIGNED_INT, submitCmds.data(), submitCmds.size(), sizeof(SDrawElementsIndirectCommand));
	vao->Unbind();
