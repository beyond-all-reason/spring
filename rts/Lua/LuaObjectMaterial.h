...
public:
	...
	void PushLODCount(unsigned int tmpCount)
	{
		lodStack.push_back(lodCount);
		lodCount = tmpCount;
	}

	void PopLODCount()
	{
		lodCount = lodStack.back();
		lodStack.pop_back();
	}
	...
	bool AddObjectForLOD(CSolidObject* o, LuaObjType objType, LuaMatType matType, float lodDist)
	{
		...
		LuaObjectLODMaterial* lodMat =
		    objMat->GetMaterial(SetCurrentLOD(CalcCurrentLOD(objType, lodDist, objMat->GetLastLOD())));
		...
		if ((lodMat != nullptr) && lodMat->IsActive()) {
			...
			default: {
				assert(false);
			} break; // gcc needs always a default case, else it's not able to optimize the switch
			}
	...
	static void SetGlobalLODFactor(LuaObjType objType, float lodFactor) { GLOBAL_LOD_FACTORS[objType] = lodFactor; }
