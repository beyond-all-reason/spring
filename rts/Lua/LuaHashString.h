...
public:
	LuaHashString(const char* s)
	    : hash(lua_calchash(s, slen = strlen(s)))
	{
...
public:
	inline void Push(lua_State* L) const { lua_pushhstring(L, hash, str, slen); }
...
#define HSTR_PUSH_CSTRING(L, name, val)                                   \
	{                                                                     \
		HSTR_PUSH(L, name);                                               \
		lua_pushhstring(L, COMPILE_TIME_HASH(val), val, sizeof(val) - 1); \
		lua_rawset(L, -3);                                                \
	}
