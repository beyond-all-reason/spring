...
static int SetTextColorShared(lua_State* L, bool outline)
{
	...
	if (args < 2)
		luaL_error(
		    L, "[%s] insufficient arguments to font:SetText%sColor(font,table|number*)", __func__,
		    outline ? "Outline" : ""
		);
	...
	if (lua_istable(L, 2)) {
		...
		luaL_error(
		    L, "[%s] incorrect arguments to font:SetText%sColor(font,table|number*)", __func__, outline ? "Outline" : ""
		);
	}
