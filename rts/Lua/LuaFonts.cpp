...
{
	RECOIL_DETAILED_TRACY_ZONE;
	auto f = CglFont::LoadFont(
	    luaL_checkstring(L, 1), luaL_optint(L, 2, 14), luaL_optint(L, 3, 2), luaL_optfloat(L, 4, 15.0f));
	if (f == nullptr)
		return 0;
...
int LuaFonts::PrintWorld(lua_State* L)
{
	...
	const string text(luaL_checkstring(L, 2), lua_strlen(L, 2));
	float3 pos = {luaL_checkfloat(L, 3), luaL_checkfloat(L, 4), luaL_checkfloat(L, 5)};
...
static int SetTextColorShared(lua_State* L, bool outline)
{
	...
	if (args < 2)
		luaL_error(L, "[%s] insufficient arguments to font:SetText%sColor(font,table|number*)", __func__,
		    outline ? "Outline" : "");
	...
	if (lua_istable(L, 2)) {
		...
		luaL_error(L, "[%s] incorrect arguments to font:SetText%sColor(font,table|number*)", __func__,
		    outline ? "Outline" : "");
	}
