...
int LuaZipFileWriter::meta_open(lua_State* L)
{
	...
	f->fileOpen =
	    (Z_OK == zipOpenNewFileInZip(
	                 f->zip, name.c_str(), nullptr, nullptr, 0, nullptr, 0, nullptr, Z_DEFLATED, Z_BEST_COMPRESSION
	             ));
...
)
{
	...
	if (!dataDirsAccess.InWriteDir(normFolderPath)) {
		SNPRINTF(
		    buf, sizeof(buf), "[%s] cannot zip \"%s\": outside writable data-directory", __func__,
