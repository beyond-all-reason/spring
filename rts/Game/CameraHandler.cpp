...
CONFIG(int, CamTransitionMode)
    ...
    .description(
        strformat("Defines the function used for camera transitions. Options are:\n%i = Exponential Decay\n%i = Spring "
...
CONFIG(int, CamSpringHalflife)
    .defaultValue(100)
    .description("For Spring Dampened camera. It is the time in milliseconds at which the camera should be "
                 "approximately halfway towards the goal.")
    .minimumValue(0);
...
void CCameraHandler::Init()
{
	...
	configHandler->NotifyOnChange(this, {"CamModeName", "WindowedEdgeMove", "FullscreenEdgeMove", "CamTimeFactor",
	                                        "CamTimeExponent", "CamTransitionMode", "CamSpringHalflife"});
...
void CameraTransitionExpDecay(const CCameraController* currCam,
    CCameraHandler::CamTransitionState& camTransState,
    float nsecs)
{
	...
	if (camera->useInterpolate > 0) {
		camTransState.timeStart =
		    globalRendering->lastSwapBuffersEnd.toMilliSecsf() + 1000.0f / std::fmax(globalRendering->FPS, 1.0f);
	}
...
void CameraTransitionTimedSpringDampened(const CCameraController* currCam,
    CCameraHandler::CamTransitionState& camTransState,
    float nsecs)
{
	if (nsecs == 0.0f) {
...
void CameraTransitionSpringDampened(const CCameraController* currCam,
    CCameraHandler::CamTransitionState& camTransState,
    float nsecs)
{
	if (nsecs == 0.0f) {
...
static constexpr decltype(&CameraTransitionExpDecay) cameraTransitionFunction[] = {
    CameraTransitionExpDecay, CameraTransitionSpringDampened, CameraTransitionTimedSpringDampened,
...
void UpdateTransitionExpDecay(const CCameraController* currCam, CCameraHandler::CamTransitionState& camTransState)
{
	...
	float timeRatio =
	    (camTransState.timeEnd - camTransState.timeStart != 0.0f) ?
	...
	if (vsync == 0 && camera->useInterpolate == 1) {
		tweenFact = 0.08f * (camTransState.timeEnd - camTransState.timeStart) /
		            drawFPS; // should be 0.25 at 120ms deltat and 60hz
	}
	/*
	LOG_L(L_INFO, "CamUpdate: %.3f, 1/FPS = %.4f, DF=%d, timeRatio = %.3f, tween = %.3f, swap = %.3f i=%d",
	globalRendering->lastFrameStart.toMilliSecsf(), drawFPS, globalRendering->drawFrame, timeRatio, tweenFact,
	lastswaptime, useInterpolate); LOG_L(L_INFO, "CamEnd: %.3f, CamStart = %.3f, deltat = %.3f CamExp = %.3f",
	    camTransState.timeEnd,
	    camTransState.timeStart,
	    camTransState.timeEnd - camTransState.timeStart,
	    camTransState.timeExponent
	    );
	LOG_L(L_INFO, "CSX = %.2f, CSZ = %.2f, CTX = %.2f CTZ = %.2f", camTransState.startPos.x, camTransState.startPos.z,
	camTransState.tweenPos.x, camTransState.tweenPos.z );
	*/
...
void UpdateTransitionTimedSpringDampened(const CCameraController* currCam,
    CCameraHandler::CamTransitionState& camTransState)
{
	float3 currentPos = camera->GetPos();
	float3 currentRot = camera->GetRot();
	...
	timed_spring_damper_exact_vector(currentPos, camTransState.posVelocity, camTransState.startPos, targetPos,
	    camTransState.timeEnd, camTransState.halflife, damping, eydt, dt);
	timed_spring_damper_exact_vector(currentRot, camTransState.rotVelocity, camTransState.startRot, targetRot,
	    camTransState.timeStart, camTransState.halflife, damping, eydt, dt);
