...
void GameControllerTextInput::Draw()
{
	...
	if (drawTextCaret) {
		...
		const float caretRelPos = fontSize *
		                          font->GetTextWidth(userString.substr(0, userPrompt.length() + prvCaretIdx)) *
		                          globalRendering->pixelX;
	...
	font->glPrint(inputTextPosX, inputTextPosY, fontSize,
	    FONT_DESCENDER | (FONT_OUTLINE * guihandler->GetOutlineFonts()) | FONT_NORM, userString);
	font->End();
...
bool GameControllerTextInput::ConsumeReleasedKey(int keyCode, int scanCode) const
{
	...
	return (keyCode == SDLK_RETURN || keyCode == SDLK_BACKSPACE || keyCode == SDLK_DELETE || keyCode == SDLK_HOME ||
	        keyCode == SDLK_END || keyCode == SDLK_RIGHT || keyCode == SDLK_LEFT);
}
