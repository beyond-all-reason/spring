...
void CEndGameBox::Draw()
{
	...
	font->glPrint(box.x1 + exitBox.x1 + 0.025f, box.y1 + exitBox.y1 + 0.005f, 1.0f,
	    FONT_SCALE | FONT_NORM | FONT_BUFFERED, "Exit");
	font->glPrint(box.x1 + playerBox.x1 + 0.015f, box.y1 + playerBox.y1 + 0.005f, 0.7f,
	    FONT_SCALE | FONT_NORM | FONT_BUFFERED, "Player stats");
	font->glPrint(box.x1 + sumBox.x1 + 0.015f, box.y1 + sumBox.y1 + 0.005f, 0.7f,
	    FONT_SCALE | FONT_NORM | FONT_BUFFERED, "Team stats");
	font->glPrint(box.x1 + difBox.x1 + 0.015f, box.y1 + difBox.y1 + 0.005f, 0.7f,
	    FONT_SCALE | FONT_NORM | FONT_BUFFERED, "Team delta stats");
	if (dispMode != 0) {
		font->glPrint(box.x1 + graphScaleBox.x1 + 0.015f, box.y1 + graphScaleBox.y1 + 0.005f, 0.7f,
		    FONT_SCALE | FONT_NORM | FONT_BUFFERED, "Log scale");
	}

	if (winners.empty()) {
		font->glPrint(
		    box.x1 + 0.25f, box.y1 + 0.65f, 1.0f, FONT_SCALE | FONT_NORM | FONT_BUFFERED, "Game result was undecided");
	}
	else {
		...
		if (neverPlayed) {
			...
			font->glPrint(
			    box.x1 + 0.25f, box.y1 + 0.65f, 1.0f, FONT_SCALE | FONT_NORM | FONT_BUFFERED, winnersText.str());
		}
		else {
			winnersText.str("");
	...
	if (dispMode == 0) {
		...
		for (int a = 0; a < playerHandler.ActivePlayers(); ++a) {
			...
			if (game->totalGameTime > 0) {
				...
				for (int i = 1; i < 5; i++) SNPRINTF(values[i], 100, "%i", 0);
			}
		...
		rbT.AddVertex({
		    {box.x1 + 0.15f, box.y1 + 0.08f, 0.0f},
            0.0f, 0.0f
        }); // tl
		rbT.AddVertex({
		    {box.x1 + 0.69f, box.y1 + 0.08f, 0.0f},
            4.0f, 0.0f
        }); // tr
		rbT.AddVertex({
		    {box.x1 + 0.69f, box.y1 + 0.62f, 0.0f},
            4.0f, 4.0f
        }); // br
		...
		rbT.AddVertex({
		    {box.x1 + 0.15f, box.y1 + 0.62f, 0.0f},
            0.0f, 4.0f
        }); // bl
		rbT.AddVertex({
		    {box.x1 + 0.15f, box.y1 + 0.08f, 0.0f},
		...
		if (mx > bxmin && mx < bxmax && my > bymin && my < bymax) {
			...
			rbC.AddVertex({
			    {box.x1 + 0.01f, box.y1 + 0.55f - (sel * 0.02f), 0.0f},
                {0.7f, 0.2f, 0.2f, guiAlpha}
            }); // tl
			rbC.AddVertex({
			    {box.x1 + 0.01f, box.y1 + 0.55f - (sel * 0.02f) + 0.02f, 0.0f},
                {0.7f, 0.2f, 0.2f, guiAlpha}
            }); // bl
			rbC.AddVertex({
			    {box.x1 + 0.12f, box.y1 + 0.55f - (sel * 0.02f) + 0.02f, 0.0f},
                {0.7f, 0.2f, 0.2f, guiAlpha}
            }); // br
			...
			rbC.AddVertex({
			    {box.x1 + 0.12f, box.y1 + 0.55f - (sel * 0.02f), 0.0f},
                {0.7f, 0.2f, 0.2f, guiAlpha}
            }); // tr
			rbC.AddVertex({
			    {box.x1 + 0.01f, box.y1 + 0.55f - (sel * 0.02f), 0.0f},
		...
		for (int a = 0; a < 5; ++a) {
			...
			font->glPrint(box.x1 + 0.12f, box.y1 + 0.07f + (a * 0.135f), 0.8f, FONT_SCALE | FONT_NORM | FONT_BUFFERED,
			    FloatToSmallString(yLabelNum));
			font->glFormat(box.x1 + 0.135f + (a * 0.135f), box.y1 + 0.057f, 0.8f,
			    FONT_SCALE | FONT_NORM | FONT_BUFFERED, "%02i:%02i", mins, secs);
		}

		font->glPrint(box.x1 + 0.55f, box.y1 + 0.65f, 0.8f, FONT_SCALE | FONT_NORM | FONT_BUFFERED, stats[stat1].name);
		font->glPrint(box.x1 + 0.55f, box.y1 + 0.63f, 0.8f, FONT_SCALE | FONT_NORM | FONT_BUFFERED,
		    (stat2 != -1) ? stats[stat2].name : "");

		rbC.AddVertex({
		    {box.x1 + 0.50f, box.y1 + 0.66f, 0.0f},
            {1.0f, 1.0f, 1.0f, 0.8f}
        });
		rbC.AddVertex({
		    {box.x1 + 0.55f, box.y1 + 0.66f, 0.0f},
            {1.0f, 1.0f, 1.0f, 0.8f}
        });
		...
		rbC.AddVertex({
		    {box.x1 + 0.50f, box.y1 + 0.64f, 0.0f},
            {1.0f, 1.0f, 1.0f, 0.8f}
        });
		rbC.AddVertex({
		    {box.x1 + 0.55f, box.y1 + 0.64f, 0.0f},
            {1.0f, 1.0f, 1.0f, 0.8f}
        });
...
void CEndGameBox::addVertices(TypedRenderBuffer<VA_TYPE_C>& rbC,
    ...
    const uint8_t (&color)[4])
{
	...
	for (size_t a = 0, n = numPoints - 1; a < n; ++a) {
		...
		rbC.AddVertex({
		    {box.x1 + 0.15f + (a)*scalex, box.y1 + 0.08f + v0 * scaley, 0.0f},
            color
        });
		rbC.AddVertex({
		    {box.x1 + 0.15f + (a + 1) * scalex, box.y1 + 0.08f + v1 * scaley, 0.0f},
            color
        });
		v0 = v1;
	}
...
std::string CEndGameBox::GetTooltip(int x, int y)
{
	...
	if (dispMode == 0) {
		if ((mx > box.x1 + 0.02f) && (mx < box.x1 + 0.1f * 6)) {
			static const std::string tips[] = {"Player Name", "Mouse clicks per minute",
			    "Mouse movement in pixels per minute", "Keyboard presses per minute", "Unit commands per minute",
...
void CEndGameBox::FillTeamStats()
{
	...
	for (int team = 0; team < teamHandler.ActiveTeams(); team++) {
		...
		for (const auto& si: pteam->statHistory) {
			...
			stats[11].AddStat(
			    team, si.metalProduced + si.metalReceived - (si.metalUsed + si.metalSent + si.metalExcess));
			stats[12].AddStat(
			    team, si.energyProduced + si.energyReceived - (si.energyUsed + si.energySent + si.energyExcess));

			stats[13].AddStat(team, si.unitsProduced + si.unitsReceived + si.unitsCaptured -
			                            (si.unitsDied + si.unitsSent + si.unitsOutCaptured));
