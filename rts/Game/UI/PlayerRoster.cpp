...
void PlayerRoster::SetCompareFunc(SortType cmpType)
{
	...
	case TeamID: {
		compareFunc = CompareTeamIDs;
	} break;
	case PlayerName: {
		compareFunc = ComparePlayerNames;
	} break;
	case PlayerCPU: {
		compareFunc = ComparePlayerCPUs;
	} break;
	case PlayerPing: {
		compareFunc = ComparePlayerPings;
	} break;
	case Disabled: {
		compareFunc = CompareDummy;
	} break;
	default: {
		compareFunc = CompareAllies;
	} break;
...
bool PlayerRoster::SetSortTypeByName(const std::string& name)
{
	...
	case 'a':
	case 'A': {
		SetCompareFunc(compareType = Allies);
		return true;
	} break;
	case 't':
	case 'T': {
		SetCompareFunc(compareType = TeamID);
		return true;
	} break;
	case 'n':
	case 'N': {
		SetCompareFunc(compareType = PlayerName);
		return true;
	} break;
	case 'c':
	case 'C': {
		SetCompareFunc(compareType = PlayerCPU);
		return true;
	} break;
	case 'p':
	case 'P': {
		SetCompareFunc(compareType = PlayerPing);
		return true;
	} break;
	default: {
	} break;
...
bool PlayerRoster::SetSortTypeByCode(SortType type)
{
	...
	case PlayerPing: {
		SetCompareFunc(compareType = type);
		return true;
	} break;
	case Disabled: {
		SetCompareFunc(compareType = type);
...
const std::vector<int>& PlayerRoster::GetIndices(bool includePathingFlag, bool callerBlockResort)
{
	...
	if (!callerBlockResort) {
		...
		std::sort(playerIndices.begin(), playerIndices.end(),
		    [&](int aIdx, int bIdx) { return (compareFunc(aIdx, bIdx) <= 0); });
	}
...
static inline int CompareBasics(const CPlayer* a, const CPlayer* b)
{
	...
	if (gs->PreSimFrame()) {
		if (a->ping == PATHING_FLAG && b->ping != PATHING_FLAG)
			return -1;
		if (a->ping != PATHING_FLAG && b->ping == PATHING_FLAG)
			return +1;
	}
...
static int CompareAllies(const int aIdx, const int bIdx)
{
	...
	if ((aAlly == myATeam) && (bAlly != myATeam))
		return -1;
	if ((aAlly != myATeam) && (bAlly == myATeam))
		return +1;
