...
void CCursorIcons::DrawCursors() const
{
	...
	for (const Icon& icon: icons) {
		...
		rb.AddQuadTriangles({cursorMat * ICON_VERTS[0], ICON_TXCDS[0].x, ICON_TXCDS[0].y, iconColor}, // tl
	...
	for (const auto& text: texts) {
		...
		if (vpPos.z <= 1.0f) {
			...
			if (guihandler->GetOutlineFonts()) {
				font->glPrint(
				    x, y, fontScale, FONT_OUTLINE | FONT_CENTER | FONT_TOP | FONT_SCALE | FONT_NORM, text.text);
			}
			else {
...
const CMouseCursor* CCursorIcons::GetCursor(int cmd) const
{
	...
	case CMD_DEATHWAIT:
		cursorName = "Wait";
		break; // there is a "DeathWait" cursor, but to prevent cheating, we have to use the same like for CMD_WAIT
