...
static void DrawBufferStats(const float2 pos)
{
	...
	rb.AddVertex({
	    {drawArea.x - 10.0f * globalRendering->pixelX, drawArea.y - 10.0f * globalRendering->pixelY, 0.0f},
	    bgColor
    }); // TL
	rb.AddVertex({
	    {drawArea.x - 10.0f * globalRendering->pixelX, drawArea.w + 10.0f * globalRendering->pixelY, 0.0f},
	    bgColor
    }); // BL
	rb.AddVertex({
	    {drawArea.z + 10.0f * globalRendering->pixelX, drawArea.w + 10.0f * globalRendering->pixelY, 0.0f},
	    bgColor
    }); // BR
	...
	rb.AddVertex({
	    {drawArea.z + 10.0f * globalRendering->pixelX, drawArea.y - 10.0f * globalRendering->pixelY, 0.0f},
	    bgColor
    }); // TR
	rb.AddVertex({
	    {drawArea.x - 10.0f * globalRendering->pixelX, drawArea.y - 10.0f * globalRendering->pixelY, 0.0f},
...

	{
		...
		font->glFormat(pos.x, pos.y - 0.005f, 0.5f, FONT_TOP | DBG_FONT_FLAGS | FONT_BUFFERED, FMT, "FONTS",
		    lfMetrics[0], lfMetrics[1], lfMetrics[2], lfMetrics[3]);
		...
		for (const auto& rb: RenderBuffer::GetAllStandardRenderBuffers()) {
			...
			font->glFormat(pos.x, pos.y - (0.025f + bias), 0.5f, FONT_TOP | DBG_FONT_FLAGS | FONT_BUFFERED, FMT,
			    rbNameShort, rb->SumElems(), rb->SumIndcs(), rb->NumSubmits(false), rb->NumSubmits(true));
			bias += 0.02f;
		}
...
static void DrawTimeSlices(std::deque<TimeSlice>& frames,
    ...
    const float4& sliceColor)
{
	...
	for (const TimeSlice& ts: frames) {
		...
		rb.AddVertex({
		    {mx1, y1 + 3.0f * globalRendering->pixelX, 0.0f},
            sliceCol
        }); // bl
		rb.AddVertex({
		    {mx1, y2 - 3.0f * globalRendering->pixelX, 0.0f},
            sliceCol
        }); // tl
		rb.AddVertex({
		    {mx2, y2 - 3.0f * globalRendering->pixelX, 0.0f},
            sliceCol
        }); // tr
		...
		rb.AddVertex({
		    {mx2, y1 + 3.0f * globalRendering->pixelX, 0.0f},
            sliceCol
        }); // br
		rb.AddVertex({
		    {mx1, y1 + 3.0f * globalRendering->pixelX, 0.0f},
...
static void DrawThreadBarcode(TypedRenderBuffer<VA_TYPE_C>& rb)
{
...

	{
		// background
		rb.AddVertex({
		    {drawArea[0] - 10.0f * globalRendering->pixelX, drawArea[1] - 10.0f * globalRendering->pixelY, 0.0f},
		    barColor
        }); // tl
		rb.AddVertex({
		    {drawArea[0] - 10.0f * globalRendering->pixelX, drawArea[3] + 10.0f * globalRendering->pixelY, 0.0f},
		    barColor
        }); // bl
		rb.AddVertex({
		    {drawArea[2] + 10.0f * globalRendering->pixelX, drawArea[3] + 10.0f * globalRendering->pixelY, 0.0f},
		    barColor
        }); // br
		...
		rb.AddVertex({
		    {drawArea[2] + 10.0f * globalRendering->pixelX, drawArea[1] - 10.0f * globalRendering->pixelY, 0.0f},
		    barColor
        }); // tr
		rb.AddVertex({
		    {drawArea[0] - 10.0f * globalRendering->pixelX, drawArea[1] - 10.0f * globalRendering->pixelY, 0.0f},
	...
	{
		// title
		font->glFormat(drawArea[0], drawArea[3], 0.7f, FONT_TOP | DBG_FONT_FLAGS | FONT_BUFFERED,
		    "ThreadPool (%.1f seconds :: %u threads)", MAX_THREAD_HIST_TIME, numThreads);
	}
	{
	...
	}
	{
		...
		rb.AddVertex({
		    {xf + 5.0f * globalRendering->pixelX, drawArea[3], 0.0f},
            feederColor
        }); // br
		...
		rb.AddVertex({
		    {xf + 5.0f * globalRendering->pixelX, drawArea[1], 0.0f},
            feederColor
        }); // tr
		rb.AddVertex({
		    {xf, drawArea[1], 0.0f},
...
static void DrawFrameBarcode(TypedRenderBuffer<VA_TYPE_C>& rb)
{
...

	{
		...
		rb.AddVertex({
		    {drawArea[0] - 10.0f * globalRendering->pixelX, drawArea[3] + 20.0f * globalRendering->pixelY, 0.0f},
		    barColor
        }); // bl
		rb.AddVertex({
		    {drawArea[2] + 10.0f * globalRendering->pixelX, drawArea[3] + 20.0f * globalRendering->pixelY, 0.0f},
		    barColor
        }); // br
	...
	// title and legend
	font->glFormat(drawArea[0], drawArea[3] + 10 * globalRendering->pixelY, 0.7f,
	    FONT_TOP | DBG_FONT_FLAGS | FONT_BUFFERED,
	    "Frame Grapher (%.2fsec)"
	    "\xff\xff\x80\xff  GC"
...

	{
		...
		rb.AddVertex({
		    {xf + 10 * globalRendering->pixelX, drawArea[3], 0.0f},
            feederColor
        }); // br
		...
		rb.AddVertex({
		    {xf + 10 * globalRendering->pixelX, drawArea[1], 0.0f},
            feederColor
        }); // tr
		rb.AddVertex({
		    {xf, drawArea[1], 0.0f},
	...
	}
	{
		...
		rb.AddVertex({
		    {xs1, drawArea[3] + 2.0f * globalRendering->pixelY, 0.0f},
            feederColor
        }); // tl
		rb.AddVertex({
		    {xs1, drawArea[3] + 10.0f * globalRendering->pixelY, 0.0f},
            feederColor
        }); // bl
		rb.AddVertex({
		    {xs2, drawArea[3] + 10.0f * globalRendering->pixelY, 0.0f},
            feederColor
        }); // br
		...
		rb.AddVertex({
		    {xs2, drawArea[3] + 2.0f * globalRendering->pixelY, 0.0f},
            feederColor
        }); // tr
		rb.AddVertex({
		    {xs1, drawArea[3] + 2.0f * globalRendering->pixelY, 0.0f},
...
static void DrawProfiler(TypedRenderBuffer<VA_TYPE_C>& rb)
{
	...
	{
		rb.AddVertex({
		    {MIN_X_COOR, MIN_Y_COOR - sortedProfiles.size() * LINE_HEIGHT - 0.010f, 0.0f},
            winColor
        }); // tl
		rb.AddVertex({
		    {MIN_X_COOR, MIN_Y_COOR + LINE_HEIGHT + 0.005f, 0.0f},
            winColor
        }); // bl
		rb.AddVertex({
		    {MAX_X_COOR, MIN_Y_COOR + LINE_HEIGHT + 0.005f, 0.0f},
            winColor
        }); // br
		...
		rb.AddVertex({
		    {MAX_X_COOR, MIN_Y_COOR - sortedProfiles.size() * LINE_HEIGHT - 0.010f, 0.0f},
            winColor
        }); // tr
		rb.AddVertex({
		    {MIN_X_COOR, MIN_Y_COOR - sortedProfiles.size() * LINE_HEIGHT - 0.010f, 0.0f},
	...
	// table header
	{
		...
		font->glPrint(fStartX += 0.04f, fStartY, textSize,
		    FONT_SHADOW | FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_RIGHT | FONT_BUFFERED, "sum-time");

		// print percent of CPU time used within the last 500ms
		font->glPrint(fStartX += 0.06f, fStartY, textSize,
		    FONT_SHADOW | FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_RIGHT | FONT_BUFFERED, "cur-%usage");
		font->glPrint(fStartX += 0.04f, fStartY, textSize,
		    FONT_SHADOW | FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_RIGHT | FONT_BUFFERED, "max-%usage");
		font->glPrint(fStartX += 0.04f, fStartY, textSize,
		    FONT_SHADOW | FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_RIGHT | FONT_BUFFERED, "lag");

		// print timer name
		font->glPrint(fStartX += 0.01f, fStartY, textSize,
		    FONT_SHADOW | FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_BUFFERED, "title");
	}
	...
	for (const auto& p: sortedProfiles) {
		...
		// print total-time running since application start
		font->glFormat(fStartX += 0.04f, fStartY, textSize,
		    FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_RIGHT | FONT_BUFFERED, "%.2fs", profileData.total.toSecsf());

		// print percent of CPU time used within the last 500ms
		font->glFormat(fStartX += 0.06f, fStartY, textSize,
		    FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_RIGHT | FONT_BUFFERED, "%.2f%%",
		    profileData.stats.y * 100.0f);
		font->glFormat(fStartX += 0.04f, fStartY, textSize,
		    FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_RIGHT | FONT_BUFFERED, "\xff\xff%c%c%.2f%%",
		    profileData.newPeak ? 1 : 255, profileData.newPeak ? 1 : 255, profileData.stats.z * 100.0f);
		font->glFormat(fStartX += 0.04f, fStartY, textSize,
		    FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_RIGHT | FONT_BUFFERED, "\xff\xff%c%c%.0fms",
		    profileData.newLagPeak ? 1 : 255, profileData.newLagPeak ? 1 : 255, profileData.stats.x);

		// print timer name
		font->glPrint(
		    fStartX += 0.01f, fStartY, textSize, FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_BUFFERED, p.first);
	}
...

	{
		...
		for (const auto& p: sortedProfiles) {
			...
			if (tr.showGraph) {
				rb.AddVertex(
				    {boxMat * float3(LINE_HEIGHT + selOffset, -i * LINE_HEIGHT - selOffset, 0.0f), actColor}); // tl
				rb.AddVertex({boxMat * float3(LINE_HEIGHT + selOffset, -i * LINE_HEIGHT - boxSize + selOffset, 0.0f),
				    actColor}); // bl
				rb.AddVertex(
				    {boxMat * float3(LINE_HEIGHT + boxSize - selOffset, -i * LINE_HEIGHT - boxSize + selOffset, 0.0f),
				        actColor}); // br

				rb.AddVertex(
				    {boxMat * float3(LINE_HEIGHT + boxSize - selOffset, -i * LINE_HEIGHT - boxSize + selOffset, 0.0f),
				        actColor}); // br
				rb.AddVertex({boxMat * float3(LINE_HEIGHT + boxSize - selOffset, -i * LINE_HEIGHT - selOffset, 0.0f),
				    actColor}); // tr
				rb.AddVertex(
				    {boxMat * float3(LINE_HEIGHT + selOffset, -i * LINE_HEIGHT - selOffset, 0.0f), actColor}); // tl
			}
...
static void DrawInfoText(TypedRenderBuffer<VA_TYPE_C>& rb)
{
	...
	rb.AddVertex({
	    {0.01f - 10.0f * globalRendering->pixelX, 0.02f - 10.0f * globalRendering->pixelY, 0.0f},
        bgColor
    }); // tl
	rb.AddVertex({
	    {0.01f - 10.0f * globalRendering->pixelX, 0.17f + 20.0f * globalRendering->pixelY, 0.0f},
        bgColor
    }); // bl
	rb.AddVertex({
	    {MIN_X_COOR - 0.05f + 10.0f * globalRendering->pixelX, 0.17f + 20.0f * globalRendering->pixelY, 0.0f},
	    bgColor
    }); // br
	...
	rb.AddVertex({
	    {MIN_X_COOR - 0.05f + 10.0f * globalRendering->pixelX, 0.02f - 10.0f * globalRendering->pixelY, 0.0f},
	    bgColor
    }); // tr
	rb.AddVertex({
	    {0.01f - 10.0f * globalRendering->pixelX, 0.02f - 10.0f * globalRendering->pixelY, 0.0f},
	...
	constexpr const char* sopFmtStr =
	    "[9] SOP-allocated memory: {U,F,P,W}={%.1f/%.1f, %.1f/%.1f, %.1f/%.1f, %.1f/%.1f}KB";
	...
	font->glFormat(
	    0.01f, 0.04f, 0.5f, DBG_FONT_FLAGS | FONT_BUFFERED, ctrFmtStr, globalRendering->drawFrame, gs->frameNum);
	...
	font->glFormat(0.01f, 0.06f, 0.5f, DBG_FONT_FLAGS | FONT_BUFFERED, avgFmtStr,
	    ...
	    (globalRendering->CalcGLDeltaTime(
	         CGlobalRendering::FRAME_REF_TIME_QUERY_IDX, CGlobalRendering::FRAME_END_TIME_QUERY_IDX) *
	        0.001f) *
	        0.001f);

	font->glFormat(
	    0.01f, 0.08f, 0.5f, DBG_FONT_FLAGS | FONT_BUFFERED, spdFmtStr, gs->speedFactor, gs->wantedSpeedFactor);
	font->glFormat(0.01f, 0.10f, 0.5f, DBG_FONT_FLAGS | FONT_BUFFERED, sfxFmtStr, ph->GetActiveProjectiles(true).size(),
	    ph->GetActiveProjectiles(false).size(), ph->GetCurrentParticles(), ph->GetParticleSaturation(true));

	{
		...
		case NOPFS_TYPE: {
			font->glFormat(
			    0.01f, 0.12f, 0.5f, DBG_FONT_FLAGS | FONT_BUFFERED, pfsFmtStr, "NO", pfsUpdates.x, pfsUpdates.y);
		} break;
		case HAPFS_TYPE: {
			font->glFormat(
			    0.01f, 0.12f, 0.5f, DBG_FONT_FLAGS | FONT_BUFFERED, pfsFmtStr, "HA", pfsUpdates.x, pfsUpdates.y);
		} break;
		case QTPFS_TYPE: {
			font->glFormat(
			    0.01f, 0.12f, 0.5f, DBG_FONT_FLAGS | FONT_BUFFERED, pfsFmtStr, "QT", pfsUpdates.x, pfsUpdates.y);
		} break;
		default: {
		} break;
		}
...

	{
		...
		font->glFormat(
		    0.01f, 0.14f, 0.5f, DBG_FONT_FLAGS | FONT_BUFFERED, luaFmtStr, allocMegs, kiloAlloc, allocTime, numStates);
	}

	{
		...
		font->glFormat(0.01f, 0.16f, 0.5f, DBG_FONT_FLAGS | FONT_BUFFERED, gpuFmtStr,
		    (vidMemInfo.x - vidMemInfo.y) / 1024.0f, vidMemInfo.x / 1024.0f);
	}

	font->glFormat(0.01f, 0.18f, 0.5f, DBG_FONT_FLAGS | FONT_BUFFERED, sopFmtStr, unitMemPool.alloc_size() / 1024.0f,
	    unitMemPool.freed_size() / 1024.0f, featureMemPool.alloc_size() / 1024.0f,
	    featureMemPool.freed_size() / 1024.0f, projMemPool.alloc_size() / 1024.0f, projMemPool.freed_size() / 1024.0f,
	    weaponMemPool.alloc_size() / 1024.0f, weaponMemPool.freed_size() / 1024.0f);
}
...
bool ProfileDrawer::IsAbove(int x, int y)
{
	...
	return (mx >= MIN_X_COOR && mx <= MAX_X_COOR &&
	        my >= (MIN_Y_COOR - CTimeProfiler::GetInstance().GetNumSortedProfiles() * LINE_HEIGHT) && my <= MIN_Y_COOR);
}
...
static void DiscardOldTimeSlices(std::deque<TimeSlice>& frames, const spring_time curTime, const spring_time maxTime)
{
	RECOIL_DETAILED_TRACY_ZONE;
	// remove old entries
