...
public:
	...
	virtual void SetParam(int index, const std::string& value) { assert(false); }
...
protected:
	Filter(const std::string& name, int args)
	    : numArgs(args)
	{
...
#define DECLARE_FILTER_EX(name, args, condition, extra, init)                        \
	struct name##_Filter : public Filter{name##_Filter() : Filter(#name, args){init; \
	}                                                                                \
...
#define DECLARE_FILTER(name, condition) DECLARE_FILTER_EX(name, 0, condition, , )
...
DECLARE_FILTER(Waiting,
    !unit->commandAI->commandQue.empty() && (unit->commandAI->commandQue.front().GetID() == CMD_WAIT))
DECLARE_FILTER(Guarding,
    !unit->commandAI->commandQue.empty() && (unit->commandAI->commandQue.front().GetID() == CMD_GUARD))
...
DECLARE_FILTER_EX(WeaponRange, 1, unit->maxRange > minRange, float minRange;
    void SetParam(int index, const std::string& value) override { minRange = atof(value.c_str()); }, minRange = 0.0f;)

DECLARE_FILTER_EX(AbsoluteHealth, 1, unit->health > minHealth, float minHealth;
    void SetParam(int index, const std::string& value) override { minHealth = atof(value.c_str()); }, minHealth = 0.0f;)

DECLARE_FILTER_EX(
    RelativeHealth, 1, unit->health / unit->maxHealth > minHealth, float minHealth;
...
DECLARE_FILTER_EX(
    InPrevSel, 0, prevTypes.find(unit->unitDef->id) != prevTypes.end(), spring::unordered_set<int> prevTypes;
...
DECLARE_FILTER_EX(NameContain, 1, unit->unitDef->humanName.find(name) != std::string::npos, std::string name;
    void SetParam(int index, const std::string& value) override { name = value; }, )

// Only used for the _Not_IdMatches case, the positive case is handled differently
DECLARE_FILTER_EX(IdMatches, 1, unit->unitDef->name.compare(name) == 0, std::string name;
    void SetParam(int index, const std::string& value) override { name = value; }, )

DECLARE_FILTER_EX(Category, 1, unit->category == cat, unsigned int cat;
    void SetParam(int index, const std::string& value)
        override { cat = CCategoryHandler::Instance()->GetCategory(value); },
    cat = 0;)

DECLARE_FILTER_EX(InGroup, 1, uiGroupHandlers[gu->myTeam].GetUnitGroupNum(unit->id) == groupNum, int groupNum;
    void SetParam(int index, const std::string& value) override { groupNum = atoi(value.c_str()); }, groupNum = -1;)
...
void CSelectionKeyHandler::DoSelection(std::string selectString)
{
	...
	case hashString("FromMouseC"): {
		...
		const float gndDist =
		    CGround::LineGroundCol(camera->GetPos(), camera->GetPos() + mouse->dir * camera->GetFarPlaneDist(), false);
