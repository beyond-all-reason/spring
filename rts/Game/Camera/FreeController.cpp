...
CONFIG(float, CamFreeGroundOffset)
    ...
    .description("Determines ground handling for the free camera.\n0 - the camera can move anywhere,\n< 0 - the camera "
                 "is always offset from the ground height by -CamFreeGroundOffset\n> 0 - the camera can be \"locked\" "
                 "to the ground by using SHIFT UP_ARROW. (and will use CamFreeGroundOffset as the offset). To release "
                 "the lock, simply press SHIFT DOWN_ARROW.");
...
void CFreeController::Update()
{
	...
	if (!tracking) {
		if (goForward) {
			const float3 tmpVel((camera->GetDir() * vel.x) + (UpVector * vel.y) + (camera->GetRight() * vel.z));
			vel = tmpVel;
		}
		else {
			float3 forwardNoY(camera->GetDir().x, 0.0f, camera->GetDir().z);
			forwardNoY.ANormalize();
			const float3 tmpVel((forwardNoY * vel.x) + (UpVector * vel.y) + (camera->GetRight() * vel.z));
			vel = tmpVel;
		}
	}
	...
	if (KeyInput::GetKeyModState(KMOD_SHIFT)) {
		...
		else if ((gndOffset > 0.0f) && (ctrlVelY < 0.0f) && (pos.y < (gndHeight + gndOffset))) {
			gndLock = true;
		}
	}
...
float3 CFreeController::GetDir() const { return camera->GetDir(); }
