...
void CSelectedUnitsHandler::GiveCommand(const Command& c, bool fromUser)
{
	...
	if (!selectedUnits.empty()) {
		...
		Channels[ChannelType::CHANNEL_UNITREPLY]->PlayRandomSample(ud->sounds.ok, u);
	}
}
...
void CSelectedUnitsHandler::HandleUnitBoxSelection(const float4& planeRight, const float4& planeLeft, const float4& planeTop, const float4& planeBottom)
{
	...
	switch (numUnits) {
		...
		case 1: {
			Channels[ChannelType::CHANNEL_UNITREPLY]->PlayRandomSample(unit->unitDef->sounds.select, unit);
		} break;
		default: {
			Channels[ChannelType::CHANNEL_UI]->PlaySample(soundMultiselID);
		} break;
	}
}
