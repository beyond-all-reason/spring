...
struct SRectangle {
	...
	// rect inside *this
	bool Inside(SRectangle&& rect) const { return x1 <= rect.x1 && y1 <= rect.y1 && x2 >= rect.x2 && y2 >= rect.y2; }
	...
	{
		return SRectangle(x1 / divisor, z1 / divisor, x2 / divisor, z2 / divisor);
	}
	...
	SRectangle operator+(const SRectangle& other) const
	{
		// SRectangle ret(*this);
		// return ret += other;
		return SRectangle(x1 + other.x1, z1 + other.z1, x2 + other.x2, z2 + other.z2);
	}
	...
	SRectangle operator>>(uint32_t shift) const
	{
		// SRectangle ret(*this);
		// return ret >>= shift;
		return SRectangle(x1 >> shift, z1 >> shift, x2 >> shift, z2 >> shift);
	}

	template<typename T> SRectangle operator*(const T v) const { return SRectangle(x1 * v, z1 * v, x2 * v, z2 * v); }
...
template<typename T> struct TRectangle {
	...
	TRectangle(T _x1, T _y1, T _x2, T _y2)
	    : x1(_x1)
	    , y1(_y1)
	    , x2(_x2)
	    , y2(_y2)
