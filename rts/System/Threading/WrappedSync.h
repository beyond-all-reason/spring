...
namespace spring {
template<typename Mutex> class WrappedSync {
public:
	WrappedSync()
	{
		sync = {std::make_unique<spring::mutex_wrapper<spring::noop_mutex>>(),
	...
	std::scoped_lock<spring::mutex_wrapper_concept> GetScopedLock()
	{
		return std::scoped_lock(*sync[needThreadSafety]);
	}

	std::unique_lock<spring::mutex_wrapper_concept> GetUniqueLock()
	{
		return std::unique_lock(*sync[needThreadSafety]);
	}
