...
public:
	...
	auto AcquireScoped()
	{
		return spring::ScopedResource([this]() { return Acquire(); }(), [this](int tidx) {
			if (tidx >= 0)
				Release(tidx);
		});
	}
...
private:
	int Acquire()
	{
		...
		while (true) {
			...
			if (busyMask.compare_exchange_weak(
			        currMask, desiredMask, std::memory_order_acq_rel, std::memory_order_relaxed)) {
				return pos;
			}
