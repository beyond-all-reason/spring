...
float3 float3::PickNonParallel() const
{
	...
	auto [mi, Mi] = std::minmax_element(
	    std::begin(xyz), std::end(xyz), [](const auto& a, const auto& b) { return math::fabs(a) < math::fabs(b); });
	float3 npVec{0.0f};
...
float3 float3::fabs(const float3 v) { return {std::fabs(v.x), std::fabs(v.y), std::fabs(v.z)}; }

float3 float3::sign(const float3 v) { return {Sign(v.x), Sign(v.y), Sign(v.z)}; }
