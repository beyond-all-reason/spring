...
public:
	...
	bool HasClient(CEventClient* ec) const { return (std::find(handles.begin(), handles.end(), ec) != handles.end()); }
...
public:
	...
	void UnitSeismicPing(const CUnit* unit, int allyTeam, const float3& pos, float strength);
	...
	void
	FeatureDamaged(const CFeature* feature, const CUnit* attacker, float damage, int weaponDefID, int projectileID);
	...
	void StockpileChanged(const CUnit* unit, const CWeapon* weapon, int oldCount);
	...
	bool
	AllowUnitTransportLoad(const CUnit* transporter, const CUnit* transportee, const float3& loadPos, bool allowed);
	bool
...
public:
	...
	std::string WorldTooltip(const CUnit* unit, const CFeature* feature, const float3* groundPos);

	bool MapDrawCmd(int playerID, int type, const float3* pos0, const float3* pos1, const std::string* label);
...
private:
	void SetupEvent(const std::string& ciName, EventClientList* list, int props);
...
inline void
CEventHandler::UnitConstructionDecayed(const CUnit* unit, float timeSinceLastBuild, float iterationPeriod, float part)
{
	ITERATE_UNIT_ALLYTEAM_EVENTCLIENTLIST(UnitConstructionDecayed, unit, timeSinceLastBuild, iterationPeriod, part)
}

inline void CEventHandler::UnitFromFactory(const CUnit* unit, const CUnit* factory, bool userOrders){
    ITERATE_UNIT_ALLYTEAM_EVENTCLIENTLIST(UnitFromFactory, unit, factory, userOrders)}


UNIT_CALLIN_NO_PARAM(UnitCloaked) UNIT_CALLIN_NO_PARAM(UnitDecloaked)
...
inline void
CEventHandler::UnitCommand(const CUnit* unit, const Command& command, int playerNum, bool fromSynced, bool fromLua)
{
...
inline void CEventHandler::UnitStunned(const CUnit* unit, bool stunned)
{
	ITERATE_UNIT_ALLYTEAM_EVENTCLIENTLIST(UnitStunned, unit, stunned)
}

inline void CEventHandler::UnitExperience(const CUnit* unit, float oldExperience)
{
	ITERATE_UNIT_ALLYTEAM_EVENTCLIENTLIST(UnitExperience, unit, oldExperience)
}

inline void CEventHandler::UnitSeismicPing(const CUnit* unit, int allyTeam, const float3& pos, float strength)
{
	ITERATE_UNIT_ALLYTEAM_EVENTCLIENTLIST(UnitSeismicPing, unit, allyTeam, pos, strength)
}

inline void CEventHandler::UnitLoaded(const CUnit* unit, const CUnit* transport)
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		if (ec->GetFullRead() || (ecAllyTeam == unit->allyteam) || (ecAllyTeam == transport->allyteam)) {
			ec->UnitLoaded(unit, transport);
		}
	}
}

inline void CEventHandler::UnitUnloaded(const CUnit* unit, const CUnit* transport)
{
...
inline void CEventHandler::FeatureDamaged(const CFeature* feature,
    const CUnit* attacker,
    float damage,
    int weaponDefID,
    int projectileID)
...
inline void CEventHandler::StockpileChanged(const CUnit* unit, const CWeapon* weapon, int oldCount)
{
