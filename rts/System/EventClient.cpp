...
CEventClient::~CEventClient() { eventHandler.RemoveClient(this); }
...
bool CEventClient::WantsEvent(const std::string& eventName)
{
	...
	const auto iter =
	    std::lower_bound(autoLinkedEvents.begin(), autoLinkedEvents.end(), LinkPair{eventName, false}, comp);
...
bool CEventClient::GameSetup(const std::string& state,
    bool& ready,
