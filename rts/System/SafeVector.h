...
public:
	...
	safe_vector(size_type size, T value)
	    : std::vector<T>(size, value)
	    , showError(true)
	{
	}

	safe_vector(const safe_vector<T>& vec)
	    : std::vector<T>(vec)
	    , showError(true)
	{
	}
...
template<typename T> struct DeduceType<safe_vector<T>> {
	static std::unique_ptr<IType> Get() { return std::unique_ptr<IType>(new DynamicArrayType<safe_vector<T>>()); }
};
} // namespace creg
