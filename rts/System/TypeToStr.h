...
namespace spring {
template<unsigned N> struct CharN {
	char str[N];
};
...
template<> struct type_name_holder<std::string> {
...
template<typename T> constexpr auto TypeToCharN() { return impl::type_name_holder<T>::value; }

template<typename T> constexpr auto TypeToStr() -> std::string_view
{
	return std::string_view{impl::type_name_holder<T>::value.str};
}

template<typename T> constexpr auto TypeToCStr() -> const char* { return impl::type_name_holder<T>::value.str; }
} // namespace spring
