...
public:
	...
	inline bool CanReadAllyTeam(int allyTeam) { return (GetFullRead() || (GetReadAllyTeam() == allyTeam)); }
...
protected:
	...
	template<class T> void RegisterLinkedEvents(T* foo)
	{
...
#undef SETUP_EVENT
		...
		std::stable_sort(autoLinkedEvents.begin(), autoLinkedEvents.end(),
		    [](const LinkPair& a, const LinkPair& b) { return (a.first < b.first); });
	}
...
public:
	...
	virtual void UnitDamaged(const CUnit* unit,
	...
	virtual void UnitSeismicPing(const CUnit* unit, int allyTeam, const float3& pos, float strength) {}
	...
	virtual void StockpileChanged(const CUnit* unit, const CWeapon* weapon, int oldCount) {}
	...
	virtual bool AllowCommand(const CUnit* unit, const Command& cmd, int playerNum, bool fromSynced, bool fromLua)
	{
		return true;
	}

	virtual std::pair<bool, bool>
	AllowUnitCreation(const UnitDef* unitDef, const CUnit* builder, const BuildInfo* buildInfo)
	{
		return {true, true};
	}
	...
	virtual bool AllowStartPosition(int playerID,
	    int teamID,
	...
	virtual bool AllowWeaponTarget(unsigned int attackerID,
	...
	virtual bool AllowWeaponInterceptTarget(const CUnit* interceptorUnit,
	...
	virtual bool UnitPreDamaged(const CUnit* unit,
	    const CUnit* attacker,
	    float damage,
	    int weaponDefID,
	    int projectileID,
	...
	virtual bool ShieldPreDamaged(const CProjectile* projectile,
	...
	virtual bool
	GameSetup(const std::string& state, bool& ready, const std::vector<std::pair<int, std::string>>& playerStates);

	virtual std::string WorldTooltip(const CUnit* unit, const CFeature* feature, const float3* groundPos);
	...
	virtual void
	DrawWorldPreParticles(bool drawAboveWater, bool drawBelowWater, bool drawReflection, bool drawRefraction)
	{
	}
