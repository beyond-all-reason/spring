...
std::int64_t GetTicksNative()
{
	...
	if (highResMode) {
		...
		if (tickFreq.QuadPart >= std::int64_t(double(1e9)))
			return (FromNanoSecs<std::uint64_t>(
			    std::max(double(0.0), currTick.QuadPart / (tickFreq.QuadPart * double(1e-9)))));
		if (tickFreq.QuadPart >= std::int64_t(double(1e6)))
			return (FromMicroSecs<std::uint64_t>(
			    std::max(double(0.0), currTick.QuadPart / (tickFreq.QuadPart * double(1e-6)))));
		if (tickFreq.QuadPart >= std::int64_t(double(1e3)))
			return (FromMilliSecs<std::uint64_t>(
			    std::max(double(0.0), currTick.QuadPart / (tickFreq.QuadPart * double(1e-3)))));
...
void spring_time::sleep(bool forceThreadSleep)
{
	...
	if (toMicroSecsi() < (avgThreadSleepTimeMicroSecs + avgThreadYieldTimeMicroSecs * 5)) {
		...
		while ((gettime() - s) < *this) thread_yield();
...
{
	auto tp =
	    chrono::time_point<chrono::high_resolution_clock, chrono::nanoseconds>(chrono::nanoseconds(toNanoSecsi()));
