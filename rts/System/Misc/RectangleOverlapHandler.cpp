...
CR_REG_METADATA(CRectangleOverlapHandler, (CR_MEMBER(rectangles), CR_MEMBER(frontIdx), CR_MEMBER(needsUpdate)))
...
bool CRectangleOverlapHandler::HandleMerge(SRectangle& rect1, SRectangle& rect2)
{
	...
	if (edgesInRect12.count() == 3) {
		if (sharedEdges.count() == 3 || ((sharedEdges.count() == 2) && ((sharedEdges[0] && sharedEdges[1]) ||
		                                                                   (sharedEdges[2] && sharedEdges[3])))) {
			//  __________
			// |  | |     |
			// |  | |     |
			// |__|_|_____|
