...
public:
	...
	template<typename T>
	explicit spring_time(const T millis)
	    : x(spring_clock::FromMilliSecs(millis))
	{
	}

	spring_time& operator+=(const spring_time st)
	{
		x += st.x;
		return *this;
	}
	...
	spring_time& operator%=(const spring_time mt)
	{
		x %= mt.x;
		return *this;
	}

	spring_time& operator*=(const int n)
	{
		x *= n;
		return *this;
	}

	spring_time& operator*=(const float n)
	{
		x *= n;
		return *this;
	}
	...
	static spring_time gettime(bool init = false)
	{
		assert(xs != 0 || init);
		return spring_time_native(spring_clock::GetTicks());
	}

	static spring_time getstarttime()
	{
		assert(xs != 0);
		return spring_time_native(xs);
	}

	static spring_time getelapsedtime() { return (gettime() - getstarttime()); }

	static void setstarttime(const spring_time t)
	{
		assert(xs == 0);
		xs = t.x;
		assert(xs != 0);
...
private:
	...
	static spring_time spring_time_native(const int64 n)
	{
		spring_time s;
		s.x = n;
		return s;
