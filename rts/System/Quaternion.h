...
public:
	...
	constexpr CQuaternion& Conjugate()
	{
		x = -x;
		y = -y;
		z = -z;
...
public:
	...
	CQuaternion operator*(float a) const { return CQuaternion(x * a, y * a, z * a, r * a); }
	...
	float3 operator*(const float3& arg) const { return Rotate(arg); }

	float4 operator*(const float4& arg) const { return Rotate(arg); }
