...
template<typename TVal> class FreeListMap {
...
public:
	...
	template<typename ShuffleFunc> std::size_t Add(const TVal& val, ShuffleFunc&& shuffleFunc)
	{
	...
	template<typename ShuffleFunc> void ShuffleFreeIDs(ShuffleFunc&& shuffleFunc)
	{
...
private:
	template<typename ShuffleFunc> std::size_t GetId(ShuffleFunc&& shuffleFunc)
	{
		if (freeIDs.empty())
			return nextId++;
...
template<typename TVal, typename TKey = int> class FreeListMapCompact {
...
public:
	...
	template<typename ShuffleFunc> TKey Add(const TVal& val, ShuffleFunc&& shuffleFunc)
	{
...
private:
	template<typename ShuffleFunc> TKey GetKey(ShuffleFunc&& shuffleFunc)
	{
		if (freeKeys.empty())
			return kpMap.size();
...
CR_REG_METADATA_TEMPLATE_1TYPED(spring::FreeListMap, TVal, (CR_MEMBER(values), CR_MEMBER(freeIDs), CR_MEMBER(nextId)))

CR_BIND_TEMPLATE_2TYPED(spring::FreeListMapCompact, TVal, Tkey, )
CR_REG_METADATA_TEMPLATE_2TYPED(spring::FreeListMapCompact,
    TVal,
    TKey,
    (CR_MEMBER(vault), CR_MEMBER(freeKeys), CR_MEMBER(kpMap), CR_MEMBER(pkMap)))
