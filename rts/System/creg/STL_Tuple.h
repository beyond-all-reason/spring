...
namespace creg {
template<typename... Ts> class TupleType : public IType {
public:
...
template<typename... Ts> struct DeduceType<std::tuple<Ts...>> {
	static std::unique_ptr<IType> Get() { return std::unique_ptr<IType>(new TupleType<Ts...>()); }
};
