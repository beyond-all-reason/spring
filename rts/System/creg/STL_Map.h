...
template<typename TKey, typename TValue> struct MapInserter<std::multimap<TKey, TValue>> {
...
template<typename T> struct MapType : public IType {
	typedef typename T::iterator iterator;
	...
	{
		return "map<" + DeduceType<typename T::key_type>::Get()->GetName() + ", " +
		       DeduceType<typename T::mapped_type>::Get()->GetName() + ">";
	}
};

// Map type
template<typename TKey, typename TValue> struct DeduceType<std::map<TKey, TValue>> {
	static std::unique_ptr<IType> Get() { return std::unique_ptr<IType>(new MapType<std::map<TKey, TValue>>()); }
};

// Multimap
template<typename TKey, typename TValue> struct DeduceType<std::multimap<TKey, TValue>> {
	static std::unique_ptr<IType> Get() { return std::unique_ptr<IType>(new MapType<std::multimap<TKey, TValue>>()); }
};

// Hash map
template<typename TKey, typename TValue> struct DeduceType<spring::unordered_map<TKey, TValue>> {
...
template<typename TKey, typename TValue> struct DeduceType<spring::unsynced_map<TKey, TValue>> {
	static std::unique_ptr<IType> Get()
	{
...
template<typename T> struct PairType : public IType {
...
// std::pair
template<typename TFirst, typename TSecond> struct DeduceType<std::pair<TFirst, TSecond>> {
	static std::unique_ptr<IType> Get() { return std::unique_ptr<IType>(new PairType<std::pair<TFirst, TSecond>>()); }
};
} // namespace creg
