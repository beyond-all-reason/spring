...
#define ASSERT_SIZE(structName)                                                                   \
	static_assert(sizeof(creg_##structName) == sizeof(structName), #structName " Size mismatch");
...
public:
	...
	void SetContext(void* newLcd, lua_Alloc newfrealloc, lua_CFunction newPanic)
	{
		context = newLcd;
		frealloc = newfrealloc;
		panic = newPanic;
	}
...
void freeProtector(void* m) { assert(false); }
...
#define sizeCclosure(n) (lua_cast(int, sizeof(CClosure)) + lua_cast(int, sizeof(TValue) * ((n) - 1)))

#define sizeLclosure(n) (lua_cast(int, sizeof(LClosure)) + lua_cast(int, sizeof(TValue*) * ((n) - 1)))
...
#define CR_CLOSURE_HEADER() CR_COMMON_HEADER(), CR_MEMBER(isC), CR_MEMBER(nupvalues), CR_IGNORED(gclist), CR_MEMBER(env)
...
namespace creg {
template<> class ObjectPointerType<creg_GCObject> : public IType {
public:
	ObjectPointerType()
	    : IType(sizeof(creg_GCObject*))
	{
	}
	...
	void Serialize(ISerializer* s, void* instance) override
	{
		...
		switch (tt) {
		case LUA_TSTRING: {
			c = creg_TString::StaticClass();
			break;
		}
		case LUA_TUSERDATA: {
			c = creg_Udata::StaticClass();
			break;
		}
		case LUA_TFUNCTION: {
			bool isC;
		...
		case LUA_TTABLE: {
			c = creg_Table::StaticClass();
			break;
		}
		case LUA_TPROTO: {
			c = creg_Proto::StaticClass();
			break;
		}
		case LUA_TUPVAL: {
			c = creg_UpVal::StaticClass();
			break;
		}
		case LUA_TTHREAD: {
			c = creg_lua_State::StaticClass();
			break;
		}
		default: {
			assert(false);
	...
	std::string GetName() const override { return "creg_GCObject*"; }
};
} // namespace creg
...
CR_REG_METADATA(creg_Node, (CR_MEMBER(i_val), CR_IGNORED(i_key), CR_SERIALIZER(Serialize)))
...
CR_REG_METADATA(creg_LocVar, (CR_MEMBER(varname), CR_MEMBER(startpc), CR_MEMBER(endpc)))
...
CR_REG_METADATA(creg_CClosure,
    (CR_CLOSURE_HEADER(), CR_IGNORED(f), CR_IGNORED(upvalue), CR_SERIALIZER(Serialize), CR_GETSIZE(GetSize)))


CR_BIND_POOL(creg_LClosure, , luaContext.alloc, freeProtector)
CR_REG_METADATA(creg_LClosure,
    (CR_CLOSURE_HEADER(), CR_MEMBER(p), CR_IGNORED(upvals), CR_SERIALIZER(Serialize), CR_GETSIZE(GetSize)))


CR_BIND_POOL(creg_Udata, , luaContext.alloc, freeProtector)
CR_REG_METADATA(creg_Udata, (CR_IGNORED(u), CR_SERIALIZER(Serialize), CR_GETSIZE(GetSize)))
...
CR_REG_METADATA(creg_LG, (CR_MEMBER(l), CR_MEMBER(g)))

template<typename T, typename C> inline void SerializeCVector(creg::ISerializer* s, T** vecPtr, C count)
{
...
template<typename T> void SerializePtr(creg::ISerializer* s, T** t)
{
	creg::ObjectPointerType<T> opt;
	opt.Serialize(s, t);
}

template<typename T> void SerializeInstance(creg::ISerializer* s, T* t)
{
...
void SerializeLightUserData(creg::ISerializer* s, void** p)
{
	...
	if (!s->IsWriting()) {
		switch (defType) {
		case DT_Feature: {
			*p = (void*)featureDefHandler->GetFeatureDefByID(idx);
			break;
		}
		case DT_Unit: {
			*p = (void*)unitDefHandler->GetUnitDefByID(idx);
			break;
		}
		case DT_Weapon: {
			*p = (void*)weaponDefHandler->GetWeaponDefByID(idx);
			break;
		}
		}
	}
...
void creg_TValue::Serialize(creg::ISerializer* s)
{
	...
	case LUA_TBOOLEAN: {
		s->SerializeInt(&value.b, sizeof(value.b));
		return;
	}
	case LUA_TLIGHTUSERDATA: {
		SerializeLightUserData(s, &value.p);
		return;
	} // No support for light user data atm
	case LUA_TNUMBER: {
		s->SerializeInt(&value.n, sizeof(value.n));
		return;
	}
	case LUA_TSTRING: {
		SerializePtr(s, &value.gc);
		return;
	}
	case LUA_TTABLE: {
		SerializePtr(s, &value.gc);
...
void creg_Table::PostLoad()
{
	...
	for (int i = 0; i < sizenode; ++i) {
		...
		if (tt != LUA_TNIL && tt != LUA_TBOOLEAN && tt != LUA_TNUMBER && tt != LUA_TSTRING && tt != LUA_TDEADKEY) {
			reorder = true;
			break;
		}
	}
...
size_t creg_TString::GetSize() { return sizeof(creg_TString) + u.tsv.len + 1; }
...
size_t creg_CClosure::GetSize() { return sizeCclosure(nupvalues); }
...
size_t creg_LClosure::GetSize() { return sizeLclosure(nupvalues); }
...
size_t creg_Udata::GetSize() { return sizeof(creg_Udata) + u.uv.len; }

void creg_stringtable::Serialize(creg::ISerializer* s) { SerializeCVector(s, &hash, size); }

inline creg_Proto* GetProtoFromCallInfo(CallInfo* ci) { return ((creg_TValue*)ci->func)->value.gc->cl.l.p; }
...
void CopyLuaContext(lua_State* L) { luaContext.SetContext(G(L)->ud, G(L)->frealloc, G(L)->panic); }
} // namespace creg
