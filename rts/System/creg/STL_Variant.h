...
template<typename Func, typename Variant, size_t N = 0> void runtime_get(Func&& func, Variant& var, size_t idx)
{
	if (N == idx) {
...
template<typename Func, typename Variant, size_t N = 0> void runtime_set(Func&& func, Variant& var, size_t idx)
{
	if (N == idx) {
...
namespace creg {
template<typename... Ts> class VariantType : public IType {
public:
...
template<typename... Ts> struct DeduceType<std::variant<Ts...>> {
	static std::unique_ptr<IType> Get() { return std::unique_ptr<IType>(new VariantType<Ts...>()); }
};
