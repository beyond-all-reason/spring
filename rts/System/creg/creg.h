...
public:
	...
	void
	AddMember(const char* name, std::unique_ptr<IType> type, unsigned int offset, int alignment, ClassMemberFlag flags);
 ...
 */
#define CR_DECLARE_SUB(cl) struct cl##MemberRegistrator;
...
#define CR_BIND(TCls, ctor_args)                                                                           \
	...
	creg::Class TCls::creg_class(#TCls, creg::CF_None, nullptr, &TCls::_CregRegisterMembers, sizeof(TCls), \
	    alignof(TCls), std::is_polymorphic<TCls>::value, TCls::creg_isStruct, TCls::_ConstructInstance,    \
	    TCls::_DestructInstance, nullptr, nullptr);
...
#define CR_BIND_POOL(TCls, ctor_args, poolAlloc, poolFree)                                                 \
	...
	creg::Class TCls::creg_class(#TCls, creg::CF_None, nullptr, &TCls::_CregRegisterMembers, sizeof(TCls), \
	    alignof(TCls), std::is_polymorphic<TCls>::value, TCls::creg_isStruct, TCls::_ConstructInstance,    \
	    TCls::_DestructInstance, TCls::_Alloc, TCls::_Free);
...
#define CR_BIND_DERIVED(TCls, TBase, ctor_args)                                                                       \
	void TCls::_ConstructInstance(void* d) { new (d) MyType ctor_args; }                                              \
	void TCls::_DestructInstance(void* d) { ((MyType*)d)->~MyType(); }                                                \
	creg::Class TCls::creg_class(#TCls, creg::CF_None, &TBase::creg_class, &TCls::_CregRegisterMembers, sizeof(TCls), \
	    alignof(TCls), std::is_polymorphic<TCls>::value, TCls::creg_isStruct, TCls::_ConstructInstance,               \
	    TCls::_DestructInstance, nullptr, nullptr);
...
#define CR_BIND_DERIVED_SUB(TSuper, TCls, TBase, ctor_args)                                                         \
	...
	creg::Class TSuper::TCls::creg_class(#TSuper "::" #TCls, creg::CF_None, &TBase::creg_class,                     \
	    &TSuper::TCls::_CregRegisterMembers, sizeof(TSuper::TCls), alignof(TCls), std::is_polymorphic<TCls>::value, \
	    TCls::creg_isStruct, TSuper::TCls::_ConstructInstance, TSuper::TCls::_DestructInstance, nullptr, nullptr);
...
#define CR_BIND_TEMPLATE_1TYPED(TCls, TArg, ctor_args)                                                     \
	...
	creg::Class TCls<TArg>::creg_class(TOSTRING(TCls) "<" TOSTRING(TArg) ">", creg::CF_None, nullptr,      \
	    &TCls<TArg>::_CregRegisterMembers, sizeof(TCls<TArg>), alignof(TCls<TArg>),                        \
	    std::is_polymorphic<TCls<TArg>>::value, TCls<TArg>::creg_isStruct, TCls<TArg>::_ConstructInstance, \
	    TCls<TArg>::_DestructInstance, nullptr, nullptr);

/** @def CR_BIND_TEMPLATE_2TYPED
 *  @see CR_BIND
 */
#define CR_BIND_TEMPLATE_2TYPED(TCls, TArg1, TArg2, ctor_args)                                             \
	template<typename TArg1, typename TArg2> void TCls<TArg1, TArg2>::_ConstructInstance(void* d)          \
	{                                                                                                      \
		new (d) MyType ctor_args;                                                                          \
	}                                                                                                      \
	template<typename TArg1, typename TArg2> void TCls<TArg1, TArg2>::_DestructInstance(void* d)           \
	{                                                                                                      \
		((MyType*)d)->~MyType();                                                                           \
	}                                                                                                      \
	template<typename TArg1, typename TArg2>                                                               \
	creg::Class TCls<TArg1, TArg2>::creg_class(TOSTRING(TCls) "<" TOSTRING(TArg1) "," TOSTRING(TArg2) ">", \
	    creg::CF_None, nullptr, &TCls<TArg1, TArg2>::_CregRegisterMembers, sizeof(TCls<TArg1, TArg2>),     \
	    alignof(TCls<TArg1, TArg2>), std::is_polymorphic<TCls<TArg1, TArg2>>::value,                       \
	    TCls<TArg1, TArg2>::creg_isStruct, TCls<TArg1, TArg2>::_ConstructInstance,                         \
	    TCls<TArg1, TArg2>::_DestructInstance, nullptr, nullptr);
...
#define CR_BIND_DERIVED_INTERFACE(TCls, TBase)                                                                  \
	creg::Class TCls::creg_class(#TCls, creg::CF_Abstract, &TBase::creg_class, &TCls::_CregRegisterMembers,     \
	    sizeof(TCls&), alignof(TCls&), std::is_polymorphic<TCls>::value, TCls::creg_isStruct, nullptr, nullptr, \
	    nullptr, nullptr);
...
#define CR_BIND_INTERFACE(TCls)                                                                                     \
	creg::Class TCls::creg_class(#TCls, creg::CF_Abstract, nullptr, &TCls::_CregRegisterMembers, sizeof(TCls&),     \
	    alignof(TCls&), std::is_polymorphic<TCls>::value, TCls::creg_isStruct, nullptr, nullptr, nullptr, nullptr);
...
#define CR_MEMBER(Member)                                                                \
	class_->AddMember(#Member, creg::GetType(null->Member), offsetof_creg(Type, Member), \
	    alignof(decltype(Type::Member)), (creg::ClassMemberFlag)currentMemberFlags)

/** @def CR_IGNORED
 * Registers a member variable that isn't saved/loaded
 */
#define CR_IGNORED(Member)                                                                                           \
	class_->AddMember(#Member, creg::IType::CreateIgnoredType(sizeof(Type::Member)), offsetof_creg(Type, Member),    \
	    alignof(decltype(Type::Member)), (creg::ClassMemberFlag)currentMemberFlags) // NOLINT{misc-sizeof-container}
 ...
 */
#define CR_MEMBER_UN(Member) CR_IGNORED(Member)

/** @def CR_SETFLAG
 * Set a flag for a class/struct.
 * @param Flag the class flag @see ClassFlag
 */
#define CR_SETFLAG(Flag) class_->SetFlag(creg::Flag)
 ...
 * @see ClassMemberFlag
 */
#define CR_MEMBER_SETFLAG(Member, Flag) class_->SetMemberFlag(#Member, creg::Flag)

#define CR_MEMBER_BEGINFLAG(Flag) currentMemberFlags |= (int)creg::Flag

#define CR_MEMBER_ENDFLAG(Flag) currentMemberFlags &= ~(int)creg::Flag
...
#define CR_SERIALIZER(SerializeFunc)                                                                                 \
	(class_->SetSerialize([](void* object, creg::ISerializer* s) { static_cast<Type*>(object)->SerializeFunc(s); }))
...
#define CR_POSTLOAD(PostLoadFunc)                                                           \
	(class_->SetPostLoad([](void* object) { static_cast<Type*>(object)->PostLoadFunc(); }))
...
#define CR_GETSIZE(GetSizeFunc)                                                                  \
	(class_->SetGetSize([](void* object) { return static_cast<Type*>(object)->GetSizeFunc(); }))
...
#define CR_PREALLOC(GetContainerFunc)                                                                         \
	(class_->SetPrealloc([](void* object) { return (void*)static_cast<Type*>(object)->GetContainerFunc(); }))
