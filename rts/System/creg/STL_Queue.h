...
template<class T, class S, class C> struct PQueueType : public IType {
	PQueueType()
	    : IType(sizeof(std::priority_queue<T, S, C>))
	{
	}
...
// List type
template<class T, class S, class C> struct DeduceType<std::priority_queue<T, S, C>> {
	static std::unique_ptr<IType> Get() { return std::unique_ptr<IType>(new PQueueType<T, S, C>()); }
};
} // namespace creg
