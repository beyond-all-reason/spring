...
struct float4 : public float3 {
	...
	constexpr float4(const float3& f, const float w = 0.0f)
	    : float3(f)
	    , w(w)
	{
	}

	constexpr float4(const float* f)
	    : float3(f[0], f[1], f[2])
	    , w(f[3])
	{
	}

	constexpr float4(const float x, const float y, const float z, const float w = 0.0f)
	    : float3(x, y, z)
	    , w(w)
	{
	}
	...
	float dot4(const float4& f) const { return (x * f.x) + (y * f.y) + (z * f.z) + (w * f.w); }

	std::string str() const { return std::format("float4({:.3f}, {:.3f}, {:.3f}, {:.3f})", x, y, z, w); }
