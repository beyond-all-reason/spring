...
{
	auto buf =
	    fmt::format("[{}:{}][{}] {}", log_util_levelToString(log_util_getNearestLevel(level)), level, section, record);
...
struct TracySinkRegistrator {
	TracySinkRegistrator() { log_backend_registerSink(&log_sink_record_tracy); }

	~TracySinkRegistrator() { log_backend_unregisterSink(&log_sink_record_tracy); }
