...
struct LogFileDetails {
	LogFileDetails(FILE* outStream = nullptr,
	...
	FILE* GetOutStream() const { return outStream; }

	bool IsLogging(int level, const char* section) const
	{
		return ((level >= minLevel) &&
		        (sections.empty() || (sections.find("," + std::string(section) + ",") != std::string::npos)));
	}

	bool FlushOnWrite(int level) const { return (level >= flushLevel); }
...
public:
	...
	LogFilesMap& GetLogFiles() { return logFiles; }
...
inline bool isActivelyLogging() { return (!getLogFiles().empty()); }
...
void log_file_addLogFile(const char* filePath, const char* sections, int minLevel, int flushLevel)
{
	...
	const auto pred = [](const log_file::LogFilePair& a, const log_file::LogFilePair& b) {
		return (a.first < b.first);
	};
	const auto iter =
	    std::lower_bound(logFiles.begin(), logFiles.end(), log_file::LogFilePair{filePathStr, nullptr}, pred);
...
void log_file_removeLogFile(const char* filePath)
{
	...
	const auto iter =
	    std::lower_bound(logFiles.begin(), logFiles.end(), log_file::LogFilePair{filePath, nullptr}, pred);
