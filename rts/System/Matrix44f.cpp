...
CMatrix44f::CMatrix44f(const CMatrix44f& mat) { memcpy(&m[0], &mat.m[0], sizeof(CMatrix44f)); }
...
{
	return md[0][3] * md[1][2] * md[2][1] * md[3][0] - md[0][2] * md[1][3] * md[2][1] * md[3][0] -
	       md[0][3] * md[1][1] * md[2][2] * md[3][0] + md[0][1] * md[1][3] * md[2][2] * md[3][0] +
	       md[0][2] * md[1][1] * md[2][3] * md[3][0] - md[0][1] * md[1][2] * md[2][3] * md[3][0] -
	       md[0][3] * md[1][2] * md[2][0] * md[3][1] + md[0][2] * md[1][3] * md[2][0] * md[3][1] +
	       md[0][3] * md[1][0] * md[2][2] * md[3][1] - md[0][0] * md[1][3] * md[2][2] * md[3][1] -
	       md[0][2] * md[1][0] * md[2][3] * md[3][1] + md[0][0] * md[1][2] * md[2][3] * md[3][1] +
	       md[0][3] * md[1][1] * md[2][0] * md[3][2] - md[0][1] * md[1][3] * md[2][0] * md[3][2] -
	       md[0][3] * md[1][0] * md[2][1] * md[3][2] + md[0][0] * md[1][3] * md[2][1] * md[3][2] +
	       md[0][1] * md[1][0] * md[2][3] * md[3][2] - md[0][0] * md[1][1] * md[2][3] * md[3][2] -
	       md[0][2] * md[1][1] * md[2][0] * md[3][3] + md[0][1] * md[1][2] * md[2][0] * md[3][3] +
	       md[0][2] * md[1][0] * md[2][1] * md[3][3] - md[0][0] * md[1][2] * md[2][1] * md[3][3] -
	       md[0][1] * md[1][0] * md[2][2] * md[3][3] + md[0][0] * md[1][1] * md[2][2] * md[3][3];
}
...
CMatrix44f& CMatrix44f::RotateEulerXYZ(const float3& angles)
{
	...
	if (angles[ANGLE_P] != 0.0f) {
		RotateX(angles[ANGLE_P]);
	}
	if (angles[ANGLE_Y] != 0.0f) {
		RotateY(angles[ANGLE_Y]);
	}
	if (angles[ANGLE_R] != 0.0f) {
		RotateZ(angles[ANGLE_R]);
	}
	return *this;
}
...
{
	return col[0].equals(rhs.col[0]) && col[1].equals(rhs.col[1]) && col[2].equals(rhs.col[2]) &&
...
#define ADD_COLUMN(col) _mm_store_ps(&r.md[col][0], _mm_add_ps(_mm_load_ps(&md[col][0]), _mm_load_ps(&mat.md[col][0])))
...
std::tuple<float3, CQuaternion, float3> CMatrix44f::DecomposeIntoTRS() const
{
	...
	assert(!epscmp(scaling[0], 0.0f, float3::cmp_eps()) && !epscmp(scaling[1], 0.0f, float3::cmp_eps()) &&
	...
	return std::make_tuple(float3(c3.x, c3.y, c3.z), // translate
	    CQuaternion::MakeFrom(tmpMat),               // rotate (quat)
	    scaling                                      // scale
	);
}

template<typename T> static inline T CalculateCofactor(const T m[4][4], const int ei, const int ej)
{
	size_t ai, bi, ci;
	switch (ei) {
	case 0: {
	...
	const T val = (m[ai][aj] * ((m[bi][bj] * m[ci][cj]) - (m[bi][cj] * m[ci][bj]))) +
...
bool CMatrix44f::InvertInPlace()
{
	...
	const float det =
	    (md[0][0] * cofac[0][0]) + (md[0][1] * cofac[0][1]) + (md[0][2] * cofac[0][2]) + (md[0][3] * cofac[0][3]);
...
CMatrix44f CMatrix44f::Invert(bool* status) const
{
	...
	const float det = (md[0][0] * cofac.md[0][0]) + (md[0][1] * cofac.md[0][1]) + (md[0][2] * cofac.md[0][2]) +
	                  (md[0][3] * cofac.md[0][3]);
