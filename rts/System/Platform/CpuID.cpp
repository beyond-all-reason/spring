...
void CPUID::EnumerateCores()
{
	...
	std::ranges::stable_sort(processorCaches.groupCaches
	    // sort larger to the bottom.
	    ,
	    [](const auto& lh, const auto& rh) -> bool { return lh.cacheSizes[2] > rh.cacheSizes[2]; });

	std::ranges::for_each(processorCaches.groupCaches, [](const auto& cache) -> void {
		LOG("Found logical processors (mask 0x%08x) using L3 cache (sized %dKB) ", cache.groupMask,
		    cache.cacheSizes[2] / 1024);
	});
