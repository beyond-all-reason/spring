...
template<class I> void int_to_string(I p, once_char_type* buf)
{
...
inline void name_once_mutex(once_char_type* mutex_name, void* flag_address)
{
	...
	once::int_to_string(reinterpret_cast<std::ptrdiff_t>(flag_address), mutex_name + once_mutex_name_fixed_length);
	once::int_to_string(GetCurrentProcessId(), mutex_name + once_mutex_name_fixed_length + sizeof(void*) * 2);
}
...
inline HANDLE open_once_event(once_char_type* mutex_name, void* flag_address)
{
	...
	return OpenEventA(SYNCHRONIZE | EVENT_MODIFY_STATE, FALSE, mutex_name);
}
...
struct once_context {
	...
	~once_context() { CloseHandle(event_handle); }
};
...
inline void commit_once_region(once_flag& flag, once_context& ctx) noexcept
{
	...
	if (!ctx.event_handle && (InterlockedCompareExchange(&flag.count, 0, 0) > 1)) {
...
inline void call_once(once_flag& flag, void (*f)())
{
	...
	while (InterlockedCompareExchange(&flag.status, 0, 0) != ctx.function_complete_flag_value) {
...
// #endif
template<typename Function> inline void call_once(once_flag& flag, Function&& f)
{
	// Try for a quick win: if the procedure has already been called
	// just skip through:
	once::once_context ctx;
...
inline void call_once(once_flag& flag, Function&& f, A&& a, ArgTypes&&... args)
{
	...
	while (InterlockedCompareExchange(&flag.status, 0, 0) != ctx.function_complete_flag_value) {
		if (once::enter_once_region(flag, ctx)) {
			std::bind(std::forward<Function>(f), std::forward<A>(a), std::forward<ArgTypes>(args)...)();
			once::commit_once_region(flag, ctx);
			break;
		}
		if (!ctx.counted) {
