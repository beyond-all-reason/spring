 ...
 * These suspend/resume controls are exposed via the ThreadControls object that is provided by the caller and
 * initialized by the thread. The thread is guaranteed to be in a running and initialized state when this function
 * returns.
 *
 * The threadCtls object is an optional return parameter that gives access to the Suspend/Resume controls under Linux.
 *
 */
spring::thread CreateNewThread(std::function<void()> taskFunc,
    std::shared_ptr<Threading::ThreadControls>* threadCtls = nullptr);
...
struct Error {
	...
	Error(const char* _caption, const char* _message, unsigned int _flags)
	    : flags(_flags)
	{
