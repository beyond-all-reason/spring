...
std::string GetProcessExecutablePath() { return FileSystem::GetDirectory(GetProcessExecutableFile()); }
...
std::string GetModulePath(const std::string& moduleName) { return FileSystem::GetDirectory(GetModuleFile(moduleName)); }
...
std::string GetPlatformStr() { return (Platform::GetOSFamilyStr() + " " + Platform::GetWordSizeStr()); }
...
std::string GetSysInfoHash()
{
	...
	std::snprintf(
	    reinterpret_cast<char*>(sysInfo.data()), sysInfo.size(), "%s\n%s\n%s\n", oss.data(), hws.data(), wss.data());
...
std::array<char, 18> GetHexMacAddr()
{
	...
	snprintf(hexAddr.data(), hexAddr.size(), "%02x:%02x:%02x:%02x:%02x:%02x", rawAddr[0], rawAddr[1], rawAddr[2],
	    rawAddr[3], rawAddr[4], rawAddr[5]);
	return hexAddr;
}
} // namespace Platform
