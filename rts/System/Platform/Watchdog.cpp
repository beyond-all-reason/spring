...
static void HangDetectorLoop()
{
	...
	while (!hangDetectorThreadInterrupted) {
		...
		if (hangDetected) {
			...
			LOG_L(L_WARNING, "\t(in threads: {%s,%s,%s,%s}={%d,%d,%d,%d})", threadNames[WDT_MAIN],
			    threadNames[WDT_LOAD], threadNames[WDT_AUDIO], threadNames[WDT_VFSI], hangThreads[WDT_MAIN],
			    hangThreads[WDT_LOAD], hangThreads[WDT_AUDIO], hangThreads[WDT_VFSI]);
...
void ClearTimer(const char* name, bool disable)
{
	...
	if (i == threadNumTable.end() || (num = i->second) >= WDT_COUNT ||
	    (threadInfo = registeredThreads[num])->numreg == 0) {
...
void Uninstall()
{
	LOG_L(L_INFO, "[WatchDog::%s][1] hangDetectorThread=%p (joinable=%d)", __func__, &hangDetectorThread,
	    hangDetectorThread.joinable());
	...
	memset(registeredThreadsData, 0, sizeof(registeredThreadsData));
	for (unsigned int i = 0; i < WDT_COUNT; ++i) registeredThreads[i] = &registeredThreadsData[WDT_COUNT];
	memset(threadSlots, 0, sizeof(threadSlots));
