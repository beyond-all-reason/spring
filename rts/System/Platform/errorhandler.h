...
#define CATCH_SPRING_ERRORS_BASE                                                                                       \
	catch (const content_error& e) { ErrorMessageBox(e.what(), "Spring: caught content_error: ", MBF_OK | MBF_EXCL); } \
	catch (const opengl_error& e) { ErrorMessageBox(e.what(), "Spring: caught opengl_error: ", MBF_OK | MBF_CRASH); }  \
	catch (const user_error& e) { ErrorMessageBox(e.what(), "Spring: caught user_error: ", MBF_OK | MBF_EXCL); }       \
	catch (const unsupported_error& e)                                                                                 \
	{                                                                                                                  \
		ErrorMessageBox(e.what(), "Spring: caught unsupported_error: ", MBF_OK | MBF_CRASH);                           \
	}                                                                                                                  \
	catch (const network_error& e) { ErrorMessageBox(e.what(), "Spring: caught network_error: ", MBF_OK | MBF_EXCL); }
...
#define CATCH_SPRING_ERRORS_EXTENDED                                                                     \
	...
	catch (const char* e) { ErrorMessageBox(e, "Spring: caught legacy exception", MBF_OK | MBF_CRASH); }

/**
 * Spring's common error handler.
 */
#ifdef NO_CATCH_EXCEPTIONS
#define CATCH_SPRING_ERRORS CATCH_SPRING_ERRORS_BASE
#else
