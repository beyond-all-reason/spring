...
static const char* oglLibWarning = "This stacktrace indicates a problem with your graphics card driver, please try "
 ...
 */
static unsigned int FindBaseMemoryAddresses(const StackTrace& stacktrace,
    std::array<PathAddrPair, 1024>& baseMemAddrPaths)
{
	...
	std::for_each(
	    stacktrace.begin(), stacktrace.begin() + std::min(stacktrace.size(), baseMemAddrPaths.size()), AddressPred);
	...
	std::for_each(begUniquePaths, begUniquePaths + (numAddrPairs = endUniquePaths - begUniquePaths),
	    [&](const PathAddrPair& p) { notFoundPaths[&p - begUniquePaths] = p; });
...
static void TranslateStackTrace(StackTrace& stacktrace, const int logLevel)
{
	...
	for (StackFrame& sf: stacktrace) {
		...
		LOG_L(L_DEBUG, "symbol=\"%s\" relPath=\"%s\" absPath=\"%s\" addr=0x%lx", sf.symbol.c_str(), relPath.c_str(),
		    absPath.c_str(), sf.addr);
	...
	for (unsigned int j = 0, numAddrPairs = FindBaseMemoryAddresses(stacktrace, baseMemAddrPaths); j < numAddrPairs;
	    j++) {
		...
		if (modulePath.empty()) {
			LOG_L(L_ERROR, "[%s][6] addrPair=%u/%u modulePath=N/A (addr: 0x%lx) symbolFile=%s", __func__, j,
			    numAddrPairs, baseMemAddrPair.second, symbolFile.c_str());
		}
		else {
			LOG_L(L_DEBUG, "[%s][6] addrPair=%u/%u modulePath=%s (addr: 0x%lx) symbolFile=%s", __func__, j,
			    numAddrPairs, modulePath.c_str(), baseMemAddrPair.second, symbolFile.c_str());
		}
		...
		while (ReadPipe(cmdOutputPipe, lineBuf, sizeof(lineBuf)) != nullptr) {
			if (lineBuf[0] == '0' && lineBuf[1] == 'x') {
				...
				if (sscanf(lineBuf, "0x%lx", &parsedAddr) != 1 || parsedAddr != stackAddr) {
					LOG_L(L_WARNING, "[%s] mismatched address from addr2line for line \"%s\": 0x%lx != 0x%lx", __func__,
					    lineBuf, parsedAddr, stackAddr);
					break;
				}
			}
...
static void LogStacktrace(const int logLevel, StackTrace& stacktrace)
{
	...
	for (const StackFrame& sf: stacktrace) {
		for (const StackEntry& se: sf.entries) {
			if (se.inlined) {
				LOG_I(logLevel, "  <%02u> %*s  %s", sf.level, int(colFileLine), se.filelineAbbrev.c_str(),
				    se.funcnameAbbrev.c_str());
			}
			else {
				LOG_I(logLevel, "[%02u]   %*s  %s", sf.level, int(colFileLine), se.filelineAbbrev.c_str(),
				    se.funcnameAbbrev.c_str());
...
static sigaction_t& GetSigAction(sigact_handler_t sigact_handler)
{
	...
	if (sigact_handler == nullptr) {
		// the default signal handler uses the old sa_handler interface, so just identify this case with sigact_handler
		// == null
...
static void
Stacktrace(pthread_t* hThread = nullptr, const char* threadName = nullptr, const int logLevel = LOG_LEVEL_ERROR)
{
 ...
 * Since the thread to be traced may be running, it requires a ThreadControls object in order to suspend/resume the
 * thread.
...
{
	LOG_L(
	    L_ERROR, "Halted Stacktrace for Spring %s (%s) using libunwind:", (SpringVersion::GetFull()).c_str(), signame);
	...
	// Print out the translated StackTrace. Ignore the frames that occur inside the signal handler (before its line in
	// the trace) -- they are likely some kind of padding or just garbage.
...
void HandleSignal(int signal, siginfo_t* siginfo, void* pctx)
{
	...
	case SIGFPE: {
		signame = "SIGFPE";
	} break; // causes endless loop, process never gets past the signal trigger
	case SIGABRT: {
		signame = "SIGABRT";
	} break; // same
	case SIGBUS: {
