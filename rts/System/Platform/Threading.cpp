...
uint32_t GetPreferredMainThreadMask()
{
	...
	auto preferredCache = std::ranges::find_if(
	    pc.groupCaches, [affinityMask](const auto& gc) -> bool { return !!(affinityMask & gc.groupMask); });

	std::call_once(preferredMaskDetailsLogFlag, [&]() {
		if (preferredCache != pc.groupCaches.end())
			LOG("[Threading] Preferred performance cache mask is: 0x%08x (L3 sized: %dKB)", preferredCache->groupMask,
			    preferredCache->cacheSizes[2] / 1024);
		else
			LOG_L(L_WARNING, "[Threading] Failed to find a preferred performance cache mask");
	});

	const uint32_t policy =
	    affinityMask & ((preferredCache != pc.groupCaches.end()) ? preferredCache->groupMask : 0xffffffff);
#else
	/* Allow any core; keep it a "proper" mask though
	 * since that has less risk of blowing up than 0 or 0xFF..FF */
	cpu_topology::ProcessorMasks pm = springproc::CPUID::GetInstance().GetAvailableProcessorAffinityMask();
...
int GetPhysicalCpuCores() { return springproc::CPUID::GetInstance().GetNumPhysicalCores(); }

int GetPerformanceCpuCores() { return springproc::CPUID::GetInstance().GetNumPerformanceCores(); }

bool HasHyperThreading() { return springproc::CPUID::GetInstance().HasHyperThreading(); }
...
NativeThreadHandle GetCurrentThread()
{
	...
	::DuplicateHandle(
	    ::GetCurrentProcess(), ::GetCurrentThread(), ::GetCurrentProcess(), &hThread, 0, TRUE, DUPLICATE_SAME_ACCESS);
	return hThread;
#else
	return pthread_self();
#endif
...
{
	return NativeThreadIdsEqual(threadID, nativeThreadIDs[THREAD_IDX_LOAD]);
}
...
{
	return NativeThreadIdsEqual(threadID, nativeThreadIDs[THREAD_IDX_VFSI]);
}
...
{
	return NativeThreadIdsEqual(threadID, nativeThreadIDs[THREAD_IDX_WDOG]);
}
