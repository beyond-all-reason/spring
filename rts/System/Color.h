...
struct SColor {
	...
	constexpr SColor operator+(const SColor& o) const { return {r + o.r, g + o.g, b + o.b, a + o.a}; }
	...
	template<std::size_t N> constexpr SColor operator*(const float (&s)[N]) const
	{
	...
	template<std::size_t N> constexpr SColor& operator*=(const float (&s)[N])
	{
	...
	constexpr operator float4() const
	{
		...
		return float4(static_cast<float>(r) / div, static_cast<float>(g) / div, static_cast<float>(b) / div,
