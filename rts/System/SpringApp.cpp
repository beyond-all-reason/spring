...
CONFIG(int, MaxFontTries).defaultValue(std::numeric_limits<int>::max()).description("Represents the maximum number of attempts to search for font replacement using the FontConfig library (lower = foreign glyphs may fail to render, higher = foreign glyphs can lag the game).");
