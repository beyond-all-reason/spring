...
float spring_damper_damping(float halflife) { return halflife_to_damping(halflife) / 2.0f; }

float spring_damper_eydt(float y, float dt) { return fast_negexp(y * dt); }

void simple_spring_damper_exact(float& x, float& v, float x_goal, float damping, float eydt, float dt)
{
	float j0 = x - x_goal;
...
void simple_spring_damper_exact_vector_part(float& cur, float& v, float goal, float damping, float eydt, float dt)
{
	float xj0 = cur - goal;
...
void simple_spring_damper_exact_vector(float3& cur, float3& v, float3 goal, float damping, float eydt, float dt)
{
...
void timed_spring_damper_exact_vector(float3& x,
    float3& v,
    float3& xi,
    float3 goal,
    float t_goal,
    ...
    float apprehension)
{
	...
	float x_goal_future = t_goal_future < t_goal ? xi.x + xv_goal * t_goal_future : goal.x;
	simple_spring_damper_exact(x.x, v.x, x_goal_future, damping, eydt, dt);
	xi.x += xv_goal * dt;

	float yv_goal = (goal.y - xi.y) / min_time;
	float y_goal_future = t_goal_future < t_goal ? xi.y + yv_goal * t_goal_future : goal.y;
	simple_spring_damper_exact(x.y, v.y, y_goal_future, damping, eydt, dt);
	xi.y += yv_goal * dt;

	float zv_goal = (goal.z - xi.z) / min_time;
	float z_goal_future = t_goal_future < t_goal ? xi.z + zv_goal * t_goal_future : goal.z;
