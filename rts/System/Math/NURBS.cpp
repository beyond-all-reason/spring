...
bool isValidNURBS(int degree, const std::vector<float4>& controlPoints, const std::vector<float>& knots, float t)
{
	float last = knots[0];
	int mult = 1;

	if (controlPoints.size() <= degree || t < 0.0 || t > 1.0 || knots.size() != controlPoints.size() + degree + 1) {
		return false;
	}
...
float3 SolveNURBS(int degree, const std::vector<float4>& controlPoints, const std::vector<float>& knots, float t)
{
...
std::vector<float3>
SolveNURBSCurve(int degree, const std::vector<float4>& controlPoints, const std::vector<float>& knots, float segments)
{
