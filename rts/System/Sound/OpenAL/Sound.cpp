...
CSound::CSound()
{
	configHandler->NotifyOnChange(
	    this, {"snd_volmaster", "snd_eaxpreset", "snd_filter", "UseEFX", "snd_volgeneral", "snd_volunitreply",
	              "snd_volbattle", "snd_volui", "snd_volmusic", "PitchAdjust"});
}

CSound::~CSound() { configHandler->RemoveObserver(this); }
...
void CSound::Init()
{
	...
	soundThread =
	    Threading::CreateNewThread(std::bind(&CSound::UpdateThread, this, configHandler->GetInt("MaxSounds")));
}
...
#undef LOAD_PROC
	...
	if (alcLoopbackOpenDeviceSOFT == nullptr || alcIsRenderFormatSupportedSOFT == nullptr ||
	    alcRenderSamplesSOFT == nullptr)
		return;
	...
	if (!configHandler->GetBool("UseSDLAudio")) {
		...
		LOG("[Sound::%s] UseSDLAudio is set, rendering openal-soft audio to SDL buffer and let SDL audio handle the "
		    "hardware",
		    __func__);
	}
	...
	if (SDL_GetNumAudioDevices(0) <= 0) {
		LOG("[Sound::%s] UseSDLAudio is set, but no SDL sound devices for playback can be found. Falling back to "
		    "openal-soft backends",
		    __func__);
		return;
	}
	 ...
	 *
	 * Note that proper support for 3D surround sounds sounds hard, for example as of 2023 counter-strike has very weak
	 * support for it, apparently noticeably worse than just stereo according to players, despite being in a more
	 * relevant genre. */
	selectedDeviceName = "";
	for (int channelsDesired: {2, 1}) {
		...
		if (!deviceName.empty()) {
			LOG("[Sound::%s] opening configured device \"%s\"", __func__, deviceName.c_str());
			sdlDeviceID = SDL_OpenAudioDevice(deviceName.c_str(), 0, &desiredSpec, &obtainedSpec,
			    SDL_AUDIO_ALLOW_ANY_CHANGE & ~SDL_AUDIO_ALLOW_CHANNELS_CHANGE);
			selectedDeviceName = deviceName;
		}

		if (sdlDeviceID == 0) {
			LOG("[Sound::%s] opening default device", __func__);
			sdlDeviceID = SDL_OpenAudioDevice(
			    nullptr, 0, &desiredSpec, &obtainedSpec, SDL_AUDIO_ALLOW_ANY_CHANGE & ~SDL_AUDIO_ALLOW_CHANNELS_CHANGE);
		...
		LOG("[Sound::%s] SDL returned %d channels, when we asked for %d. Closing previously opened device.", __func__,
	...
	if ((frameSize = obtainedSpec.channels * SDL_AUDIO_BITSIZE(obtainedSpec.format) / 8) <= 0) {
		LOG("[Sound::%s] failed to obtain valid SDL spec: numChannels=%d formatBits=%d", __func__,
		    obtainedSpec.channels, SDL_AUDIO_BITSIZE(obtainedSpec.format));
		Cleanup();
		return;
	}
	...
	LOG("[Sound::%s] device=%p(%s) context=%p  numChannels=%d frameSize=%d", __func__, curDevice,
	    selectedDeviceName.c_str(), curContext, obtainedSpec.channels, frameSize);
#endif
}
...
void CSound::InitThread(int cfgMaxSounds)
{
...

	{
...

		{
			...
			if (devices.size()) {
				for (const std::string& deviceName: devices) LOG("    [%s]", deviceName.c_str());
			}
			else {
				LOG("    [N/A]");
			}
...
void CSound::UpdateThread(int cfgMaxSounds)
{
...

	{
		...
		LOG("[Sound::%s][3] #sources=%u #items=%u", __func__, uint32_t(soundSources.size()),
		    uint32_t(soundItems.size()));
...
{
	// call from sound thread, cause OpenAL calls tend to cause L2 misses and so are slow (no reason to call them from
	// mainthread)
	if (!updateListener)
		return;
	...
	// Note: soundsource related doppler effects are not deactivated by this! Flying cannon shoots still change their
	// frequencies. Note2: by not updating the listener velocity soundsource related velocities are calculated wrong, so
	// even if the camera is moving with a cannon shoot the frequency gets changed.
	/*
...
void CSound::GenSources(int alMaxSounds)
{
	...
	for (int i = 0; i < alMaxSounds; i++) {
		...
		LOG_L(
		    L_WARNING, "[Sound::%s] alMaxSounds=%d but numSources=%d", __func__, alMaxSounds, int(soundSources.size()));
		break;
	}
}
...
std::vector<std::string> CSound::GetSoundDevices()
{
	...
	if (hasAllEnumExt || hasDefEnumExt) {
		const char* deviceSpecStr =
		    alcGetString(nullptr, hasAllEnumExt ? ALC_ALL_DEVICES_SPECIFIER : ALC_DEVICE_SPECIFIER);
