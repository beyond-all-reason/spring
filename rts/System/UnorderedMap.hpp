...
auto map_try_get(Map&& map, const typename MapPureT::key_type& key)
{
	...
	using ReturnType =
	    std::conditional_t<mapIsConst, const typename MapPureT::mapped_type, typename MapPureT::mapped_type>;
...
template<typename K, typename V> class unordered_bimap {
public:
	...
	unordered_bimap(const std::initializer_list<std::pair<K, V>>& list)
	    : kv_map(list)
	{
