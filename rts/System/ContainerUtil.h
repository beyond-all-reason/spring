...
template<typename T, typename UnaryPredicate> static bool VectorEraseIf(std::vector<T>& v, UnaryPredicate p)
{
...
template<typename T> static bool VectorErase(std::vector<T>& v, const T& e)
{
...
template<typename T, typename C> VUS static bool VectorEraseUniqueSorted(std::vector<T>& v, const T& e, const C& c)
{
...
template<typename T> VUS static bool VectorInsertUnique(std::vector<T>& v, const T& e, bool checkIfUnique = false)
{
...
template<typename T, typename Pred, typename = typename std::enable_if_t<std::is_pointer_v<std::remove_cv_t<T>>>>
VUS typename std::vector<T>::iterator VectorInsertSorted(std::vector<T>& vec, T item, Pred pred)
{
	return vec.insert(std::upper_bound(vec.begin(), vec.end(), item, pred), item);
}

template<typename T> VUS typename std::vector<T>::iterator VectorInsertSorted(std::vector<T>& vec, T&& item)
{
	return vec.insert(std::upper_bound(vec.begin(), vec.end(), item), item);
}

template<typename T, typename = typename std::enable_if_t<std::is_pointer_v<std::remove_cv_t<T>>>>
VUS typename std::vector<T>::iterator VectorInsertSorted(std::vector<T>& vec, T item)
{
	return vec.insert(std::upper_bound(vec.begin(), vec.end(), item), item);
}

template<typename T, typename Pred> VUS static bool VectorInsertUniqueSorted(std::vector<T>& v, const T& e, Pred pred)
{
...
template<typename T> static T VectorBackPop(std::vector<T>& v)
{
	T e = std::move(v.back());
	v.pop_back();
	return e;
