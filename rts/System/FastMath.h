...
inline float sqrt_builtin(float x) { return __builtin_sqrtf(x); }
 ...
 * Use with care.
 */
inline float apxsqrt(float x) { return x * fastmath::isqrt_nosse(x); }
 ...
 */
inline float apxsqrt2(float x) { return x * fastmath::isqrt2_nosse(x); }
 ...
 */
inline float cos(float x) { return fastmath::sin(x + math::HALFPI); }
 ...
 */
template<typename T> inline T floor(T f)
{
