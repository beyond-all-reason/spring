...
class RapidEntry {
public:
	RapidEntry() { value.resize(entries); }
	...
	const std::string& GetTag() const { return value[0]; }

	const std::string& GetPackageHash() const { return value[1]; }

	const std::string& GetParentGameName() const { return value[2]; }

	const std::string& GetName() const { return value[3]; }
...
template<typename Lambda> static bool GetRapidEntry(const std::string& file, RapidEntry* re, Lambda p)
{
	...
	while (gzgets(in, buf, bufsize) != nullptr) {
		...
		if (len <= 2)
			continue; // line to short/invalid, skip
		if (buf[len - 1] == '\n')
			len--;
		if (buf[len - 1] == '\r')
			len--;
...
std::string GetRapidPackageFromTag(const std::string& tag)
{
	...
	for (const std::string& file: dataDirsAccess.FindFiles("rapid", "versions.gz", FileQueryFlags::RECURSE)) {
		RapidEntry re;
		if (GetRapidEntry(
		        dataDirsAccess.LocateFile(file), &re, [&](const RapidEntry& re) { return re.GetTag() == tag; })) {
...
std::string GetRapidTagFromPackage(const std::string& pkg)
{
	for (const std::string& file: dataDirsAccess.FindFiles("rapid", "versions.gz", FileQueryFlags::RECURSE)) {
		RapidEntry re;
		if (GetRapidEntry(
		        dataDirsAccess.LocateFile(file), &re, [&](const RapidEntry& re) { return re.GetPackageHash() == pkg; }))
			return re.GetTag();
	}
