...
std::vector<std::string> FileSystemMisc::GetSplashScreenFiles()
{
	...
	std::vector<std::string> splashScreenFiles =
	    dataDirsAccess.FindFiles(FileSystem::IsAbsolutePath(ssd) ? ssd : cwd + ssd, "*.{png,jpg}", 0);

	if (splashScreenFiles.empty()) {
		auto logoPath = FileSystem::EnsurePathSepAtEnd(FileSystem::GetNormalizedPath(
		    FileSystem::EnsurePathSepAtEnd(FileSystemAbstraction::GetSpringExecutableDir()) + "base"));
