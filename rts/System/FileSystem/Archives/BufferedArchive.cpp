...
CBufferedArchive::~CBufferedArchive()
{
	...
	LOG_L(L_INFO, "[%s][name=%s] %u bytes cached in %u files, %u bytes cached in %u files", __func__,
	    archiveFile.c_str(), cachedSize, fileCount, uncachedSize, static_cast<uint32_t>(fileCache.size() - fileCount));
}
...
bool CBufferedArchive::GetFile(uint32_t fid, std::vector<std::uint8_t>& buffer)
{
	...
	if (!globalConfig.vfsCacheArchiveFiles || noCache) {
		if ((ret = GetFileImpl(fid, buffer)) != 1)
			LOG_L(L_ERROR, "[BufferedArchive::%s(fid=%u)][noCache=%d,vfsCache=%d] name=%s ret=%d size=" _STPF_,
			    __func__, fid, static_cast<int>(noCache), static_cast<int>(globalConfig.vfsCacheArchiveFiles),
			    archiveFile.c_str(), ret, buffer.size());
	...
	if ((ret = GetFileImpl(fid, buffer)) != 1)
		LOG_L(L_ERROR, "[BufferedArchive::%s(fid=%u)][noCache=%d,vfsCache=%d] name=%s ret=%d size=" _STPF_, __func__,
		    fid, static_cast<int>(noCache), static_cast<int>(globalConfig.vfsCacheArchiveFiles), archiveFile.c_str(),
		    ret, buffer.size());
