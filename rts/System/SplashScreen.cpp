    ...
    const std::function<bool()>& testDoneFunc)
{
	...
	if (splashScreenFile.empty() || !bmp.Load(splashScreenFile)) {
		...
		quadElems[0].x = 0.5f - 0.125f * 0.5f;
		quadElems[0].y = 0.5f + 0.125f * 0.5f * globalRendering->aspectRatio;
		quadElems[1].x = 0.5f - 0.125f * 0.5f;
		quadElems[1].y = 0.5f - 0.125f * 0.5f * globalRendering->aspectRatio;
		quadElems[2].x = 0.5f + 0.125f * 0.5f;
		quadElems[2].y = 0.5f - 0.125f * 0.5f * globalRendering->aspectRatio;
		quadElems[3].x = 0.5f + 0.125f * 0.5f;
		quadElems[3].y = 0.5f + 0.125f * 0.5f * globalRendering->aspectRatio;
	}
	...
	const float textWidth[3] = {
	    font->GetTextWidth(fmtStrs[0]), font->GetTextWidth(fmtStrs[4]), font->GetTextWidth(versionStrBuf)};
	...
	for (spring_time t0 = spring_now(), t1 = t0; !testDoneFunc(); t1 = spring_now()) {
		...
		rb.AddQuadTriangles({quadElems[0].x, quadElems[0].y, quadElems[0].s, quadElems[0].t},
		...
		font->glFormat(coors.x - (normWidth[0] * 0.475f), coors.y - (coors.w * coors.z * 1.0f), coors.z, fontFlags,
		    fmtStrs[1], CArchiveScanner::GetNumScannedArchives());
		font->glFormat(coors.x - (normWidth[0] * 0.475f), coors.y - (coors.w * coors.z * 2.0f), coors.z, fontFlags,
		    fmtStrs[2], (t1 - t0).toMilliSecsf());
		font->End();
		...
		font->SetTextColor(color.x, color.y, color.z, color.w);
		font->glFormat(coors.x - (normWidth[2] * 0.5f), coors.y * 0.5f - (coors.w * coors.z * 1.0f), coors.z,
		    fontFlags | FONT_OUTLINE, versionStrBuf);
		font->glFormat(coors.x - (normWidth[1] * 0.5f), coors.y * 0.5f - (coors.w * coors.z * 2.0f), coors.z,
		    fontFlags | FONT_OUTLINE, fmtStrs[4]);
		font->End();
