 ...
 */
template<typename T> static inline void AssertDebugger(const T& x, const char* msg = "assert")
{
 ...
 * @brief Check sync of the argument x.
 */
template<typename T> static inline void Assert(const T& x, const char* msg = "assert") { Assert(&x, sizeof(T), msg); }
