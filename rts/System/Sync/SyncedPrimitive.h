...
template<class T> struct SyncedPrimitive {
private:
	T x;
...
public:
	...
	/* assignment */
	template<class U>
	SyncedPrimitive(const SyncedPrimitive<U>& f)
	    : x(f.x)
	{
		Sync("copy");
	}

	template<class U> SyncedPrimitive& operator=(const SyncedPrimitive<U>& f)
	{
		x = f.x;
		Sync("=");
		return *this;
...
#define DO(T)                               \
	...
	SyncedPrimitive& operator=(const T f)   \
	{                                       \
		x = f;                              \
		Sync("=" #T);                       \
		return *this;                       \
