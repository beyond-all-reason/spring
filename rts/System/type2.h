...
template<typename t> struct type2 {
	...
	constexpr type2(const t nx, const t ny)
	    : x(nx)
	    , y(ny)
	{
	}
	...
	t Norm() const { return t(math::sqrt(x * x + y * y)); }

	float Dot(const type2<t>& f) const { return x * f.x + y * f.y; }
...
template<typename t> struct itype2 : public type2<t> {
	...
	constexpr itype2(const t nx, const t ny)
	    : type2<t>(nx, ny)
	{
	}

	constexpr itype2(const type2<int>& v)
	    : type2<t>(v.x, v.y)
	{
	}
	...
	bool operator<(const type2<int>& f) const
	{
		return (type2<t>::y == f.y) ? (type2<t>::x < f.x) : (type2<t>::y < f.y);
	}
