...
char32_t utf8::GetNextChar(const std::string& text, int& pos, bool advance)
{
	...
	if (clo > 4 || clo == 0)
		clo = 1; // ignore >=5 byte ones cause of RFC 3629
...
std::vector<std::uint8_t> zlib::deflate(const std::vector<std::uint8_t>& inflData)
{
	return (zlib::deflate(inflData.data(), inflData.size()));
}
...
std::vector<std::uint8_t> zlib::inflate(const std::vector<std::uint8_t>& deflData)
{
	return (zlib::inflate(deflData.data(), deflData.size()));
}
