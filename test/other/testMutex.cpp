...
static void futex_init(futex* m) { *m = 0; }

static void futex_destroy(futex* m) { *m = 0; }
...
TEST_CASE("Mutex")
{
	...
	spring_time tCrit =
	    Test("critical section", [&] { EnterCriticalSection(&cs); }, [&] { LeaveCriticalSection(&cs); });
...
TEST_CASE("ConditionVariable")
{
	...
	LOG("[spring::condition_variable::wait_for] accuracy:={ err: %+.4fms %+.4fms erravg: %.4fms } ",
	    emin.toMilliSecsf(), emax.toMilliSecsf(), eavg * 1e-6);
}
