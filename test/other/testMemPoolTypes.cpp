...
template<typename T> struct AllocFixture {
	AllocFixture()
	    : mempool(std::make_unique<T>())
	{
	}

	TestData* alloc() { return mempool->template alloc<TestData>(instanceCounter); }

	void free(TestData* obj) { return mempool->free(obj); }

	std::unique_ptr<T> mempool;
	uint64_t instanceCounter = 0;
};

TEMPLATE_TEST_CASE_METHOD(AllocFixture,
    "test allocator when full",
    "[class][template]",
...
TEMPLATE_TEST_CASE_METHOD(AllocFixture,
    "test bounded allocator base functionality",
    "[class][template]",
...
TEMPLATE_TEST_CASE_METHOD(AllocFixture,
    "test unbounded allocator base functionality",
    "[class][template]",
...
TEMPLATE_TEST_CASE_METHOD(AllocFixture,
    "test reuse allocator's memory in LIFO order",
    "[class][template]",
    (StaticMemPoolT<TEST_ALLOCATOR_SIZE, TestData>),
    (FixedDynMemPoolT<1, TEST_ALLOCATOR_SIZE, TestData>),
    (DynMemPoolT<TestData>))
{
	...
	std::for_each(allocated.rbegin(), allocated.rend(), [&](auto* obj) { mempool.free(obj); });
