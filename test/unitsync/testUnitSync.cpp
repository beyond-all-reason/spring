...
{
	const std::string source = "for k, v in pairs(Test) do\n"
...
#define CHECK_ERROR_MESSAGE(s)                 \
	if ((s = us::GetNextError()) != nullptr) { \
		FAIL_CHECK(s);                         \
	}
...
TEST_CASE("UnitSync")
{
	...
	if ((errmsg = us::GetNextError()) == nullptr) {
		FAIL_CHECK("No error on GetWritableDataDirectory before init"); // there's an error cause we called
		                                                                // GetWritableDataDirectory() before Init()!
	}
	...
	if ((errmsg = us::GetNextError()) == nullptr) {
		FAIL_CHECK("No error on GetWritableDataDirectory before init"); // there's an error cause we called
		                                                                // GetWritableDataDirectory() after UnInit()!
	}
}
