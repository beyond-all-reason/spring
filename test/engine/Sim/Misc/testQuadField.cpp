...
template<typename T> T Clamp(T val, T min, T max) { return std::min(std::max(val, min), max); }
...
TEST_CASE("QuadField")
{
	...
	for (int n = 0; n < TEST_RUNS; ++n) {
		...
		// #2: raytrace via QuadField
		{
			...
			assert(std::adjacent_find(qfQuery.quads->begin(), qfQuery.quads->end()) ==
			       qfQuery.quads->end()); // check for duplicates
