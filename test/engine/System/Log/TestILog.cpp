...
static inline void
test_log_sl(int line, std::stringstream& logStream, bool enabled, const char* section, int level, const char* fmt, ...)
{
	if (enabled) {
		...
		va_end(arguments);
		std::string expected =
		    std::string() + (!LOG_SECTION_IS_DEFAULT(section) ? (std::string("[") + std::string(section) + "] ") : "") +
		    ((level != LOG_LEVEL_INFO) ? (std::string(log_util_levelToString(level)) + ": ") : "") + logMessage + "\n";
...
#define TLOG_S(section, fmt, ...) TLOG_SL(section, L_INFO, fmt, ##__VA_ARGS__)

#define TLOG_L(level, fmt, ...) TLOG_SL(LOG_SECTION_DEFAULT, level, fmt, ##__VA_ARGS__)

#define TLOG(fmt, ...) TLOG_SL(LOG_SECTION_DEFAULT, L_INFO, fmt, ##__VA_ARGS__)
...
static bool TestDefaultSection1() { return (LOG_SECTION_EQUAL("", LOG_SECTION_DEFAULT)); }
