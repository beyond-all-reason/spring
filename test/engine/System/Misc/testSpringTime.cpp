...
struct SpringClock {
	...
	static inline int64_t Get() { return spring_time::gettime().toNanoSecsi(); }
};

template<class Clock> struct TestProcessor {
	static inline float Run()
	{
		...
		for (int i = 0; i < testRuns; ++i) {
			...
			if (tick > 0)
				lowTick = std::min<int64_t>(tick, lowTick);
			lastTick = curTick;
		}
		...
		LOG("[%17s] maxTick: %3.6fms minTick: %3.6fms avgTick: %3.6fms minNonNullTick: %3.6fms totalTestRuntime: "
		    "%4.0fms",
		    Clock::GetName().c_str(), maxMsTick, minMsTick, avgMsTick, minNonNullMsTick,
		    (spring_time::gettime() - startTestTime).toMilliSecsf());
		return avgMsTick;
	}
};
...
void sleep_posix_nanosec(int time)
{
	struct timespec tim, tim2;
	tim.tv_sec = 0;
	tim.tv_nsec = time;
	if (nanosleep(&tim, &tim2) != 0)
		nanosleep(&tim2, nullptr);
}
#endif


void BenchmarkSleepFnc(const std::string& name,
    void (*sleep)(int time),
    const int runs,
    const float toMilliSecondsScale)
{
	...
	for (int i = 0; i < runs; ++i) {
		...
		if ((diff > tmax) || !spring_istime(tmax))
			tmax = diff;
		if ((diff < tmin) || !spring_istime(tmin))
			tmin = diff;
	...
	if (toMilliSecondsScale != 0) {
		for (int i = 0; i < 100; ++i) {
			...
			if ((diff > emax) || !emax.isDuration())
				emax = diff;
			if ((diff < emin) || !emin.isDuration())
				emin = diff;
			eavg = float(i * eavg + std::abs(diff.toNanoSecsf())) / (i + 1);
		}
	}

	LOG("[%35s] accuracy:={ err: %+.4fms %+.4fms erravg: %.4fms } min sleep time:={ min: %.6fms avg: %.6fms max: "
	    "%.6fms }",
	    name.c_str(), emin.toMilliSecsf(), emax.toMilliSecsf(), eavg * 1e-6, tmin.toMilliSecsf(), tavg * 1e-6,
	    tmax.toMilliSecsf());
}
