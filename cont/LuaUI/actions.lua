...
function actionHandler:AddAction(widget, cmd, func, data, types)
	...
	if text then
		tSuccess = add(self.textActions)
	end
	if keyPress then
		pSuccess = add(self.keyPressActions)
	end
	if keyRepeat then
		RSuccess = add(self.keyRepeatActions)
	end
	if keyRelease then
		rSuccess = add(self.keyReleaseActions)
	end
...
function actionHandler:RemoveAction(widget, cmd, types)
	...
	if text then
		tSuccess = remove(self.textActions)
	end
	if keyPress then
		pSuccess = remove(self.keyPressActions)
	end
	if keyRepeat then
		RSuccess = remove(self.keyRepeatActions)
	end
	if keyRelease then
		rSuccess = remove(self.keyReleaseActions)
	end
...
function actionHandler:TextAction(line)
	...
	local _, _, match = string.find(line, "[^%s]+[%s]+(.*)")
	if match == nil then
		match = "" -- no args
	end
	return TryAction(self.textActions, cmd, match, words, false, nil)
end
...
actionHandler.HaveSyncAction = function()
	return (next(self.syncActions) ~= nil)
end
