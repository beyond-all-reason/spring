	...
	if (FLAGS_teamstats || FLAGS_stats) {
		...
		for (unsigned teamNum = 0; teamNum < statvec.size(); ++teamNum) {
			...
			for (unsigned i = 0; i < statvec[teamNum].size(); ++i) {
				std::wcout << L"-- Team statistics for player " << teamNum << L", game second " << time << L" --"
				           << std::endl;
				wstringstream buf;
...
{
#define REGISTER_CMD(cmdDefine) cmdIdToName[cmdDefine] = #cmdDefine;
...
void TrafficDump(CDemoReader& reader, bool trafficStats)
{
	...
	while (!reader.ReachedEnd()) {
		...
		case NETMSG_PLAYERNAME:
			...
			break;
		case NETMSG_SETPLAYERNUM: std::cout << "SETPLAYERNUM: Playernum: " << (unsigned)buffer[1] << std::endl; break;
		case NETMSG_QUIT: std::cout << "QUIT" << std::endl; break;
		case NETMSG_STARTPLAYING: std::cout << "STARTPLAYING" << std::endl; break;
		case NETMSG_STARTPOS:
			std::cout << "STARTPOS: Playernum: " << (unsigned)buffer[1] << " Team: " << (unsigned)buffer[2]
			          << " Readyness: " << (unsigned)buffer[3] << std::endl;
			break;
		...
		case NETMSG_PLAYERINFO:
			std::cout << "NETMSG_PLAYERINFO: Player:" << (int)buffer[1] << " Ping: " << *(uint16_t*)&buffer[6]
			          << std::endl;
			break;
		case NETMSG_LUAMSG: {
			std::cout << "LUAMSG length:" << packet->length << " Player:" << (unsigned)buffer[3]
			          << " Script: " << *(uint16_t*)&buffer[4] << " Mode: " << (unsigned)buffer[6] << " Msg: ";
		...
		case NETMSG_COMMAND:
			...
			if (*(unsigned short*)(buffer + 1) != packet->length)
				std::cout << "      packet length error: expected: " << *(unsigned short*)(buffer + 1)
				          << " got: " << packet->length << std::endl;
			break;
		case NETMSG_SELECT:
		...
		case NETMSG_MAPDRAW:
			...
			case MAPDRAW_POINT:
				...
				break;
			case MAPDRAW_LINE:
				std::cout << " LINE x1:" << *(uint32_t*)&buffer[4] << " z1:" << *(uint32_t*)&buffer[8]
				          << " x2:" << *(uint32_t*)&buffer[12] << " z2:" << *(uint32_t*)&buffer[16];
				break;
			case MAPDRAW_ERASE:
			...
			std::cout << std::endl;
			break;
		case NETMSG_PATH_CHECKSUM: std::cout << "NETMSG_PATH_CHECKSUM" << std::endl; break;
		case NETMSG_INTERNAL_SPEED: std::cout << "NETMSG_INTERNAL_SPEED" << std::endl; break;
		case NETMSG_PLAYERLEFT: std::cout << "NETMSG_PLAYERLEFT" << std::endl; break;
		case NETMSG_GAMEDATA: std::cout << "NETMSG_GAMEDATA" << std::endl; break;
		...
		case NETMSG_SHARE:
			...
			std::cout << std::endl;
			break;
		case NETMSG_CCOMMAND: std::cout << "NETMSG_CCOMMAND: " << std::endl; break;
		case NETMSG_PAUSE:
			std::cout << "NETMSG_PAUSE: Player " << (unsigned)buffer[1] << " paused: " << (unsigned)buffer[2]
			          << std::endl;
			break;
		case NETMSG_SYNCRESPONSE:
			...
			std::cout << std::endl;
			break;
		case NETMSG_DIRECT_CONTROL: std::cout << "NETMSG_DIRECT_CONTROL: " << std::endl; break;
		case NETMSG_SETSHARE: std::cout << "NETMSG_SETSHARE: " << std::endl; break;
		case NETMSG_CLIENTDATA: {
			const uint16_t playerNum = (int)buffer[3];
			const uint16_t totalSize = *(unsigned short*)(buffer + 1);
			std::cout << "NETMSG_CLIENTDATA: Player " << (unsigned)playerNum << " totalSize: " << totalSize
			          << std::endl;
			break;
		}
		case NETMSG_AI_STATE_CHANGED: std::cout << "NETMSG_AI_STATE_CHANGED: " << std::endl; break;
		case NETMSG_PLAYERSTAT: std::cout << "NETMSG_PLAYERSTAT: " << std::endl; break;
		default: std::cout << "MSG: " << cmd << std::endl;
		}
		delete packet;
	}
...
template<typename T> void PrintSep(std::ofstream& file, T value) { file << value << ";"; }
