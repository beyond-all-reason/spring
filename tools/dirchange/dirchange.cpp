...
#if defined _WIN64 || defined __LP64__ || defined __ppc64__ || defined __ILP64__ || defined __SILP64__ || \
    defined __LLP64__ || defined(__sparcv9)
#define __arch64__
#else
#define __arch32__
#endif
	...
	for (moreFiles = (int)handle; moreFiles != -1; moreFiles = _findnext(handle, &files)) {
		...
		if ((files.attrib & 16) != 0) {
			...
			if (files.name[0] != '.') {
				...
				string newStorePath;
				if (storePath.empty())
					newStorePath = files.name;
				else
					newStorePath = storePath + "/" + files.name;
	...
	while ((ep = readdir(dp)) != NULL) {
		...
		if (S_ISDIR(info.st_mode)) {
			...
			if (ep->d_name[0] != '.') {
				...
				string newStorePath;
				if (storePath.empty())
					newStorePath = ep->d_name;
				else
					newStorePath = storePath + "/" + ep->d_name;
				Scan(newPath, newStorePath);
			}
		}
		else {
	...
	for (list<file_t>::iterator i = filelist.begin(); i != filelist.end(); ++i) {
		...
		while (x < buffer.size()) {
			...
			if (!incl[a]) {
				...
				while (headernamesrc[j] != '"' && headernamesrc + j < &buffer.back()) headerpath += headernamesrc[j++];
				...
				//	printf ("In file %s:  Replaced %s with %s\n", i->path.c_str(), headername.c_str(),
				//fh->second->localpath.c_str());
