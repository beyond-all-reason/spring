...
#define CATCH_CPP_AI_EXCEPTION(RETURN_VAR)                            \
	catch (int err) { RETURN_VAR = err; }                             \
	catch (const std::exception& e) { RETURN_VAR = ERROR_SHIFT + 2; } \
	catch (const std::string& s) { RETURN_VAR = ERROR_SHIFT + 3; }    \
	catch (const char* s) { RETURN_VAR = ERROR_SHIFT + 4; }           \
	catch (...) { RETURN_VAR = ERROR_SHIFT + 5; }
