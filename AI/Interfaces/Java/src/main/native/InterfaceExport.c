...
static struct SSkirmishAILibrary jvmSSkirmishAILibrary = {NULL, NULL, NULL,
                                                          NULL};

EXPORT(int)
initStatic(int _interfaceId, const struct SAIInterfaceCallback *_callback) {
  simpleLog_initcallback(_interfaceId, "Java Interface", _callback->Log_logsl,
                         LOG_LEVEL_INFO);
  ...
  const char *const myShortName = callback->AIInterface_Info_getValueByKey(
      interfaceId, AI_INTERFACE_PROPERTY_SHORT_NAME);
  const char *const myVersion = callback->AIInterface_Info_getValueByKey(
      interfaceId, AI_INTERFACE_PROPERTY_VERSION);

  if (myShortName == NULL || myVersion == NULL) {
    simpleLog_logL(LOG_LEVEL_ERROR, "Couldn't fetch AI Name / Version \"%d\"",
                   _interfaceId);
    return -1;
  }
...
getLevelOfSupportFor(const char *engineVersion,
                     int engineAIInterfaceGeneratedVersion) {
  return LOS_Unknown;
}
...
int CALLING_CONV proxy_skirmishAI_init(
    int skirmishAIId, const struct SSkirmishAICallback *aiCallback) {
  int ret = -1;
  ...
  if (className != NULL)
    ret = java_initSkirmishAIClass(shortName, version, className, skirmishAIId)
              ? 0
              : 1;
...
int CALLING_CONV proxy_skirmishAI_handleEvent(int skirmishAIId, int topicId,
                                              const void *data) {
...
loadSkirmishAILibrary(const char *const shortName, const char *const version) {
  ...
  if (jvmSSkirmishAILibrary.init == NULL) {
    jvmSSkirmishAILibrary.getLevelOfSupportFor =
        &proxy_skirmishAI_getLevelOfSupportFor;
    /*
                    jvmSSkirmishAILibrary.getInfo = proxy_skirmishAI_getInfo;
                    jvmSSkirmishAILibrary.getOptions =
       proxy_skirmishAI_getOptions;
    */
...
EXPORT(int)
unloadSkirmishAILibrary(const char *const shortName,
                        const char *const version) {
  const char *const className = callback->SkirmishAI_Info_getValueByKey(
      interfaceId, shortName, version, JAVA_SKIRMISH_AI_PROPERTY_CLASS_NAME);
