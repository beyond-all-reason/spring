...
typedef jint(JNICALL JNI_CreateJavaVM_t)(JavaVM **vm, void **jniEnv,
                                         void *vmArgs);
typedef jint(JNICALL JNI_GetCreatedJavaVMs_t)(JavaVM **vms, jsize vms_sizeMax,
                                              jsize *vms_size);
...
static bool java_createClassPath(char *classPathStr,
                                 const size_t classPathStr_sizeMax) {
  ...
  const bool located = callback->DataDirs_locatePath(
      interfaceId, mainJarPath, sizeof(mainJarPath),
      JAVA_AI_INTERFACE_LIBRARY_FILE_NAME, false, false, false, false);

  if (!located) {
    simpleLog_logL(LOG_LEVEL_ERROR, "Couldn't find %s",
  ...
  jarDirs[jarDirs_size++] =
      util_allocStrCatFSPath(2, jarsDataDir, "jresources");
  ...
  size_t jd, jf;
  for (jd = 0; (jd < jarDirs_size) && (classPath_size < classPath_sizeMax);
       ++jd) {
    if (util_fileExists(jarDirs[jd])) {
      ...
      const size_t jarFiles_size =
          util_listFiles(jarDirs[jd], ".jar", jarFiles, true, jarFiles_sizeMax);

      for (jf = 0; (jf < jarFiles_size) && (classPath_size < classPath_sizeMax);
           ++jf) {
        classPath[classPath_size++] =
            util_allocStrCatFSPath(2, jarDirs[jd], jarFiles[jf]);
        FREE(jarFiles[jf]);
      }
...
static size_t java_createAIClassPath(const char *shortName, const char *version,
  ...
  const char *const skirmDD = callback->SkirmishAI_Info_getValueByKey(
      interfaceId, shortName, version, SKIRMISH_AI_PROPERTY_DATA_DIR);
  if (skirmDD == NULL) {
  ...
  // {spring-data-dir}/{SKIRMISH_AI_DATA_DIR}/{ai-name}/{ai-version}/SkirmishAI.jar
  jarFiles[jarFiles_size++] =
      util_allocStrCatFSPath(2, skirmDD, "SkirmishAI.jar");
  ...
  const char *const skirmDDCommon = callback->SkirmishAI_Info_getValueByKey(
      interfaceId, shortName, version, SKIRMISH_AI_PROPERTY_DATA_DIR_COMMON);

  if (skirmDDCommon != NULL) {
    // add to classpath:
    // {spring-data-dir}/Skirmish/MyJavaAI/common/${x}/
    jarDirs[jarDirs_size++] =
        util_allocStrCatFSPath(2, skirmDDCommon, "jconfig");
    jarDirs[jarDirs_size++] =
        util_allocStrCatFSPath(2, skirmDDCommon, "config");
    jarDirs[jarDirs_size++] =
        util_allocStrCatFSPath(2, skirmDDCommon, "jresources");
    jarDirs[jarDirs_size++] =
        util_allocStrCatFSPath(2, skirmDDCommon, "resources");
    jarDirs[jarDirs_size++] =
        util_allocStrCatFSPath(2, skirmDDCommon, "jscript");
    jarDirs[jarDirs_size++] =
        util_allocStrCatFSPath(2, skirmDDCommon, "script");
  ...
  size_t jf;
  for (jf = 0;
       (jf < jarFiles_size) && (classPathParts_size < classPathParts_sizeMax);
       ++jf) {
  ...
  size_t jd, sjf;
  for (jd = 0;
       (jd < jarDirs_size) && (classPathParts_size < classPathParts_sizeMax);
       ++jd) {
    if (jarDirs[jd] != NULL && util_fileExists(jarDirs[jd])) {
      ...
      classPathParts[classPathParts_size++] =
          util_allocStrCat(2, jarDirs[jd], "/");

      // add the jars in the dir
      const size_t subJarFiles_sizeMax =
          classPathParts_sizeMax - classPathParts_size;
      char **subJarFiles = (char **)calloc(subJarFiles_sizeMax, sizeof(char *));
      const size_t subJarFiles_size = util_listFiles(
          jarDirs[jd], ".jar", subJarFiles, true, subJarFiles_sizeMax);

      for (sjf = 0; (sjf < subJarFiles_size) &&
                    (classPathParts_size < classPathParts_sizeMax);
           ++sjf) {
        // .../[*].jar
        classPathParts[classPathParts_size++] =
...
static jobject java_createAIClassLoader(JNIEnv *env, const char *shortName,
                                        const char *version) {
  static const size_t classPathParts_sizeMax = 512;
  char **classPathParts =
      (char **)calloc(classPathParts_sizeMax, sizeof(char *));
  const size_t classPathParts_size = java_createAIClassPath(
  ...
  if (o_cppURLs != NULL) {
    ...
    for (cpp = 0; cpp < classPathParts_size; ++cpp) {
      ...
      char *str_fileUrl =
          util_allocStrCat(2, FILE_URL_PREFIX, classPathParts[cpp]);
      ...
      const bool inserted =
          jniUtil_insertURLIntoArray(env, o_cppURLs, cpp, jurl_fileUrl);
      if (!inserted) {
        simpleLog_logL(
            LOG_LEVEL_ERROR,
  ...
  if (o_cppURLs != NULL) {
    if ((o_jClsLoader = jniUtil_createURLClassLoader(env, o_cppURLs)) != NULL)
      o_jClsLoader =
          jniUtil_makeGlobalRef(env, o_jClsLoader, "Skirmish AI class-loader");
  }
...
static bool java_readJvmCfgFile(struct Properties *props) {
  ...
  bool located = callback->DataDirs_locatePath(
      interfaceId, jvmPropFile, sizeof(jvmPropFile), JVM_PROPERTIES_FILE, false,
      false, false, false);
  ...
  if (!located)
    located = callback->DataDirs_locatePath(
        interfaceId, jvmPropFile, sizeof(jvmPropFile), JVM_PROPERTIES_FILE,
        false, false, false, true);

  if (located) {
    props->size = util_parsePropertiesFile(jvmPropFile, props->keys,
                                           props->values, props_sizeMax);
    read = true;
    simpleLog_logL(LOG_LEVEL_INFO, "JVM: arguments loaded from: %s",
                   jvmPropFile);
  } else {
    props->size = 0;
    read = false;
...
static bool java_createNativeLibsPath(char *libraryPath,
                                      const size_t libraryPath_sizeMax) {
  // {spring-data-dir}/{AI_INTERFACES_DATA_DIR}/Java/{version}/
  const char *const dd_r = callback->AIInterface_Info_getValueByKey(
      interfaceId, AI_INTERFACE_PROPERTY_DATA_DIR);
  ...
  char dd_lib_r[2048];
  const bool located =
      callback->DataDirs_locatePath(interfaceId, dd_lib_r, sizeof(dd_lib_r),
  ...
  // {spring-data-dir}/{AI_INTERFACES_DATA_DIR}/Java/common/
  const char *const dd_r_common = callback->AIInterface_Info_getValueByKey(
      interfaceId, AI_INTERFACE_PROPERTY_DATA_DIR_COMMON);
  ...
  if (dd_r_common != NULL) {
    ...
    const bool located = callback->DataDirs_locatePath(
        interfaceId, dd_lib_r_common, sizeof(dd_lib_r_common), NATIVE_LIBS_DIR,
...
static bool java_createJavaVMInitArgs(struct JavaVMInitArgs *vm_args,
                                      const struct Properties *jvmProps) {
  ...
  if (jvmProps != NULL) {
    const char *jniVersionFromCfg =
        java_getValueByKey(jvmProps, "jvm.jni.version");
    const char *ignoreUnrecognizedFromCfg =
        java_getValueByKey(jvmProps, "jvm.arguments.ignoreUnrecognized");

    // ### evaluate JNI version to use ###
    if (jniVersionFromCfg != NULL) {
      const unsigned long int jniVersion_tmp =
  ...
  if (jvmProps != NULL) {
    ...
    const char *clsPathFromCfg =
        java_getValueByKey(jvmProps, "jvm.option.java.class.path");
  ...
  if (jvmProps != NULL) {
    ...
    const char *libPathFromCfg =
        java_getValueByKey(jvmProps, "jvm.option.java.library.path");
  ...
  if (jvmProps != NULL) {
    // ### add string options from the JVM config file with property name
    // "jvm.option.x" ###
    int i;
    ...
    simpleLog_logL(LOG_LEVEL_WARNING,
                   "JVM: properties file (" JVM_PROPERTIES_FILE
                   ") not found; using default options.");
  ...
  vm_args->options =
      (struct JavaVMOption *)calloc(numOpts, sizeof(struct JavaVMOption));
  vm_args->nOptions = 0;

  // fill strOptions into the JVM options
  simpleLog_logL(LOG_LEVEL_INFO, "JVM: options:", numOpts);
  char dd_rw[2048] = {'\0'};
  callback->DataDirs_locatePath(interfaceId, dd_rw, sizeof(dd_rw), "", true,
                                true, true, false);
  size_t i;

  for (i = 0; i < numOpts; ++i) {
    char *tmpOptionString =
        util_allocStrReplaceStr(strOptions[i], "${home-dir}", dd_rw);
    ...
    simpleLog_logL(LOG_LEVEL_INFO, "JVM option %ul: %s", vm_args->nOptions - 1,
                   tmpOptionString);
  }
...
static JNIEnv *java_reattachCurrentThread(JavaVM *jvm) {
  ...
  // const jint res = (*jvm)->AttachCurrentThreadAsDaemon(jvm, (void**) &env,
  // NULL);
  ...
  if (res != 0) {
    env = NULL;
    simpleLog_logL(LOG_LEVEL_ERROR,
                   "Failed attaching JVM to current thread(2): %i - %s", res,
                   jniUtil_getJniRetValDescription(res));
  }
...
static JNIEnv *java_getJNIEnv(bool preload) {
  ...
  if ((res = JNI_GetCreatedJavaVMs_f(&jvm, 1, &numJVMsFound)) != 0) {
    jvm = NULL;
    simpleLog_logL(LOG_LEVEL_ERROR,
                   "Failed fetching list of running JVMs: %i - %s", res,
                   jniUtil_getJniRetValDescription(res));
    goto end;
  }
  ...
  if (numJVMsFound > 0) {
    ...
    if ((res = JNI_CreateJavaVM_f(&jvm, (void **)&env, &vm_args)) != 0 ||
        (*env)->ExceptionCheck(env)) {
      simpleLog_logL(LOG_LEVEL_ERROR, "Failed to create Java VM: %i - %s", res,
                     jniUtil_getJniRetValDescription(res));
      goto end;
    }
  }
  ...
  if (res < 0 || (*env)->ExceptionCheck(env)) {
    ...
    simpleLog_logL(LOG_LEVEL_ERROR,
                   "Failed to attach JVM to current thread: %i - %s", res,
                   jniUtil_getJniRetValDescription(res));
    goto end;
  }
  ...
  if (env == NULL || jvm == NULL || (*env)->ExceptionCheck(env) || res != 0) {
    ...
    if (jvm != NULL) {
      // never destroy the JVM, doing so prevents it from being created again
      // for the same thread res = (*jvm)->DestroyJavaVM(jvm);
    }
...
bool java_unloadJNIEnv() {
  ...
  if (res != 0) {
    simpleLog_logL(LOG_LEVEL_ERROR,
                   "JVM: Failed detaching current thread: %i - %s", res,
                   jniUtil_getJniRetValDescription(res));
    return false;
  }
...
bool java_initStatic(int _interfaceId,
  ...
  const bool located = callback->DataDirs_locatePath(
      interfaceId, jreLocationFile, sizeof(jreLocationFile), JRE_LOCATION_FILE,
      false, false, false, false);
  char jrePath[1024];
  char jvmLibPath[1024];

  if (!GetJREPath(jrePath, sizeof(jrePath), located ? jreLocationFile : NULL,
                  NULL)) {
    simpleLog_logL(LOG_LEVEL_ERROR, "Failed locating a JRE installation, you "
                                    "may specify the JAVA_HOME env var.");
    return false;
  }
  ...
  if (!GetJVMPath(jrePath, jvmType, jvmLibPath, sizeof(jvmLibPath), NULL)) {
    simpleLog_logL(LOG_LEVEL_ERROR,
                   "Failed locating the %s version of the JVM, please contact "
                   "spring devs.",
                   jvmType);
    return false;
  }
  ...
  if (!sharedLib_isLoaded(jvmSharedLib = sharedLib_load(jvmLibPath))) {
    simpleLog_logL(LOG_LEVEL_ERROR, "Failed to load the JVM at \"%s\".",
                   jvmLibPath);
    return false;
  }

  simpleLog_logL(LOG_LEVEL_NOTICE,
                 "Successfully loaded the JVM shared library at \"%s\".",
                 jvmLibPath);

  if ((JNI_GetDefaultJavaVMInitArgs_f =
           (JNI_GetDefaultJavaVMInitArgs_t *)sharedLib_findAddress(
               jvmSharedLib, "JNI_GetDefaultJavaVMInitArgs")) == NULL) {
    simpleLog_logL(LOG_LEVEL_ERROR,
                   "Failed to load the JVM, function \"%s\" not exported.",
                   "JNI_GetDefaultJavaVMInitArgs");
    return false;
  }

  if ((JNI_CreateJavaVM_f = (JNI_CreateJavaVM_t *)sharedLib_findAddress(
           jvmSharedLib, "JNI_CreateJavaVM")) == NULL) {
    simpleLog_logL(LOG_LEVEL_ERROR,
                   "Failed to load the JVM, function \"%s\" not exported.",
                   "JNI_CreateJavaVM");
    return false;
  ...
  if ((JNI_GetCreatedJavaVMs_f =
           (JNI_GetCreatedJavaVMs_t *)sharedLib_findAddress(
               jvmSharedLib, "JNI_GetCreatedJavaVMs")) == NULL) {
    simpleLog_logL(LOG_LEVEL_ERROR,
                   "Failed to load the JVM, function \"%s\" not exported.",
  ...
  const bool inited =
      (loaded && eventsJniBridge_initStatic(env, maxSkirmishAIs) == 0);
  java_establishSpringEnv();
...
static jobject
java_createAICallback(JNIEnv *env, const struct SSkirmishAICallback *aiCallback,
                      int skirmishAIId) {
  (void)aiCallback;
  ...
  if (g_cls_aiCallback == NULL) {
    ...
    if ((g_cls_aiCallback = jniUtil_findClass(env, CLS_AI_CALLBACK)) == NULL)
      return NULL;
    if ((g_cls_aiCallback = jniUtil_makeGlobalRef(env, g_cls_aiCallback,
                                                  CLS_AI_CALLBACK)) == NULL)
      return NULL;
    // get (int skirmishAIId) constructor
    if ((g_m_aiCallback_ctor_I = jniUtil_getMethodID(env, g_cls_aiCallback,
                                                     "<init>", "(I)V")) == NULL)
      return NULL;
  }
  ...
  // this should be safe since the callback objects created in
  // java_skirmishAI_init are not broken by java_skirmishAI_handleEvent, which
  // also calls java_reattachCurrentThread and mightget a different env-pointer
  // back
  if (jAICallBacks[skirmishAIId] != NULL)
    return jAICallBacks[skirmishAIId];
#endif

  jobject o_clb = (*env)->NewObject(env, g_cls_aiCallback,
                                    g_m_aiCallback_ctor_I, skirmishAIId);

  if (jniUtil_checkException(env, "Failed creating Java AI Callback instance"))
    return NULL;

  // return (jAICallBacks[skirmishAIId] = jniUtil_makeGlobalRef(env, o_clb, "AI
  // callback instance"));
  return (jAICallBacks[skirmishAIId] = o_clb);
}

static bool java_loadSkirmishAI(JNIEnv *env, const char *shortName,
                                const char *version, const char *className,
  ...
  *o_aiClassLoader = o_global_aiClassLoader;
  ...
  if (g_cls_ai_int == NULL) {
    if ((g_cls_ai_int = jniUtil_findClass(env, INT_AI)) == NULL)
      return false;
    if ((g_cls_ai_int = jniUtil_makeGlobalRef(env, g_cls_ai_int,
                                              "AI interface class")) == NULL)
      return false;
  }

  // get the AI implementation class (from SkirmishAI.jar)
  jclass cls_ai =
      jniUtil_findClassThroughLoader(env, o_global_aiClassLoader, className);

  if (cls_ai == NULL)
    return false;

  const bool implementsAIInt =
      (bool)(*env)->IsAssignableFrom(env, cls_ai, g_cls_ai_int);

  if (!implementsAIInt || (*env)->ExceptionCheck(env)) {
    simpleLog_logL(LOG_LEVEL_ERROR,
                   "AI class not assignable from interface " INT_AI ": %s",
                   className);
    simpleLog_logL(LOG_LEVEL_ERROR,
    ...
    simpleLog_logL(
        LOG_LEVEL_ERROR,
        "* The AI is not compiled for the Java AI Interface version in use");
  ...
  if (o_local_ai == NULL || (*env)->ExceptionCheck(env)) {
    simpleLog_logL(LOG_LEVEL_ERROR, "Failed fetching AI instance for class: %s",
                   className);
  ...
  // make the AI a global reference, so it will not be garbage collected even
...
bool java_initSkirmishAIClass(const char *const shortName,
                              ...
                              const char *const className, int skirmishAIId) {
  bool success = false;
  ...
  if (jAIClassNames[sai] == NULL) {
    ...
    success = java_loadSkirmishAI(env, shortName, version, className, &instance,
                                  &classLoader);
    java_establishSpringEnv();
    ...
    if (success) {
      ...
      simpleLog_logL(LOG_LEVEL_ERROR, "Class loading failed for class: %s",
                     className);
    }
  } else {
    success = true;
...
bool java_releaseSkirmishAIClass(const char *className) {
  ...
  bool successPart =
      jniUtil_deleteGlobalRef(env, jAIClassLoaders[sai], "AI class-loader");
...
int java_skirmishAI_init(int skirmishAIId,
  ...
  JNIEnv *env = java_getJNIEnv(false);
  jobject global_javaAICallback =
      java_createAICallback(env, aiCallback, skirmishAIId);
...
int java_skirmishAI_release(int skirmishAIId) { return 0; }
...
int java_skirmishAI_handleEvent(int skirmishAIId, int topic, const void *data) {
  ...
  const int res =
      eventsJniBridge_handleEvent(env, aiInstance, skirmishAIId, topic, data);
