...
const char *jniUtil_getJniRetValDescription(const jint retVal) {
  ...
  case JNI_EDETACHED: {
    return "JNI_EDETACHED - thread detached from the VM";
    break;
  }
  case JNI_EVERSION: {
...
#ifdef JNI_ENOMEM
  case JNI_ENOMEM: {
    return "JNI_ENOMEM - not enough (contiguous) memory";
    break;
  }
#endif // JNI_ENOMEM
#ifdef JNI_EEXIST
...
#endif // JNI_EINVAL
  default: {
    return "UNKNOWN - unknown/invalid JNI return value";
    break;
  }
  }
}
...
bool jniUtil_deleteGlobalRef(JNIEnv *env, jobject globalObject,
  ...
  if ((*env)->ExceptionCheck(env)) {
    simpleLog_logL(LOG_LEVEL_ERROR, "Failed to delete global reference %s.",
                   ((objDesc == NULL) ? "" : objDesc));
    (*env)->ExceptionDescribe(env);
    return false;
  }
...
jmethodID jniUtil_getMethodID(JNIEnv *env, jclass cls, const char *const name,
  ...
  const bool hasException = (*env)->ExceptionCheck(env);
  if (res == NULL || hasException) {
    simpleLog_logL(LOG_LEVEL_ERROR, "Method not found: %s(%s)", name,
                   signature);
    if (hasException) {
      (*env)->ExceptionDescribe(env);
    }
...
static bool jniUtil_initURLClass(JNIEnv *env) {
  ...
  if (g_m_url_ctor == NULL) {
    ...
    g_m_url_ctor =
        jniUtil_getMethodID(env, g_cls_url, "<init>", "(Ljava/lang/String;)V");
    if (g_m_url_ctor == NULL)
      return false;
  }
...
jobject jniUtil_createURLObject(JNIEnv *env, const char *const url) {
  ...
  if (ok) {
    ...
    if (jniUtil_checkException(env, "Failed creating Java String.")) {
      jstrUrl = NULL;
    }
    if (jstrUrl != NULL) {
      jurl = (*env)->NewObject(env, g_cls_url, g_m_url_ctor, jstrUrl);
      if (jniUtil_checkException(env, "Failed creating Java URL.")) {
        jurl = NULL;
      }
    }
  } else {
...
bool jniUtil_insertURLIntoArray(JNIEnv *env, jobjectArray arr, size_t index,
                                jobject url) {

  bool ok = true;

  (*env)->SetObjectArrayElement(env, arr, index, url);
  if (jniUtil_checkException(env, "Failed inserting Java URL into array.")) {
    ok = false;
  }
...
static bool jniUtil_initURLClassLoaderClass(JNIEnv *env) {
  ...
  if (g_m_urlClassLoader_findClass == NULL) {
    ...
    // get (URL[])	constructor
    g_m_urlClassLoader_ctor = jniUtil_getMethodID(
        env, g_cls_urlClassLoader, "<init>", "([Ljava/net/URL;)V");
    if (g_m_urlClassLoader_ctor == NULL)
      return false;

    // get the findClass(String) method
    g_m_urlClassLoader_findClass =
        jniUtil_getMethodID(env, g_cls_urlClassLoader, "findClass",
...
jobject jniUtil_createURLClassLoader(JNIEnv *env, jobject urlArray) {
  ...
  if (ok) {
    classLoader = (*env)->NewObject(env, g_cls_urlClassLoader,
                                    g_m_urlClassLoader_ctor, urlArray);
    if (jniUtil_checkException(env, "Failed creating class-loader.")) {
      return NULL;
    }
  } else {
    simpleLog_logL(
...
jclass jniUtil_findClassThroughLoader(JNIEnv *env, jobject classLoader,
  ...
  if (ok) {
    ...
    cls = (*env)->CallObjectMethod(
        env, classLoader, g_m_urlClassLoader_findClass, jstr_className);
    const bool hasException = (*env)->ExceptionCheck(env);
