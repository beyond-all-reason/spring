...
bool GetJREPathFromConfig(char *path, size_t pathSize, const char *configFile) {
  ...
  const size_t props_size = util_parsePropertiesFile(
      configFile, props_keys, props_values, props_sizeMax);
  ...
  if (fgets(line, line_sizeMax, cfp)) {
    ...
    simpleLog_logL(LOG_LEVEL_NOTICE, "Fetched JRE location from \"%s\"!",
                   configFile);
...
bool GetJREPathFromEnvVars(char *path, size_t pathSize, const char *arch) {
  ...
  for (l = 0; l < possLoc_i; ++l) {
    ...
    if (envPath != NULL) {
      ...
      if (found) {
        simpleLog_logL(LOG_LEVEL_NOTICE, "JRE found in env var \"%s\"!",
                       possLoc[l]);
        goto locSearchEnd;
      } else {
        simpleLog_logL(LOG_LEVEL_WARNING,
                       "Unusable JRE from env var \"%s\"=\"%s\"!", possLoc[l],
                       envPath);
      }
    }
  }
