...
// LIBARCH	 sparc, sparcv9, i386, amd64, or ia64 // last one is windows
// only
 ...
 */
bool GetJVMPath(const char *jrePath, const char *jvmType, char *jvmPath,
                size_t jvmPathSize, const char *arch) {
  if (arch == NULL) {
    arch = GetArchPath();
  }
...
static size_t ExecFileSystemGlob(char **pathHits, size_t pathHits_sizeMax,
  ...
  while (fgets(line, line_sizeMax, cmd_fp) &&
         (pathHits_size < pathHits_sizeMax)) {
    ...
    simpleLog_logL(LOG_LEVEL_DEBUG, "glob-hit \"%s\"!", line);
...
static bool GetJREPathInCommonLocations(char *path, size_t pathSize,
                                        const char *arch) {
  bool found = false;
  ...
  for (l = 0; l < possLoc_i; ++l) {
    const size_t globHits_size =
        ExecFileSystemGlob(globHits, globHits_sizeMax, possLoc[l]);
    for (g = 0; g < globHits_size; ++g) {
      found = GetJREPathFromBase(path, pathSize, globHits[g], arch);
      if (found) {
        simpleLog_logL(LOG_LEVEL_NOTICE,
                       "JRE found common location env var \"%s\"!", possLoc[l]);
        goto locSearchEnd;
      }
    }
  }
...
static bool GetJREPathWhichJava(char *path, size_t pathSize, const char *arch) {
  ...
  if (fgets(line, line_sizeMax, cmd_fp)) {
    if (*line == '/') { // -> absolute path
      ...
      simpleLog_logL(LOG_LEVEL_DEBUG, "which line \"%s\"!", line);

      if (line_size > suf_size &&
          strcmp(line + (line_size - suf_size), suf) == 0) {
        // line ends with suf
        simpleLog_logL(LOG_LEVEL_NOTICE, "JRE found with `which java`!");
        // remove suf
