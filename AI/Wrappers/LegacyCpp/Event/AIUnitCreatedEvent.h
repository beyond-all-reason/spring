...
public:
	CAIUnitCreatedEvent(const SUnitCreatedEvent& event)
	    : event(event)
	{
	}
	...
	void Run(IGlobalAI& ai, IGlobalAICallback* globalAICallback = NULL) { ai.UnitCreated(event.unit, event.builder); }
