...
springLegacyAI::CAIAICheats::CAIAICheats(int skirmishAIId,
...
void springLegacyAI::CAIAICheats::SetMyHandicap(float handicap)
{
	...
	sAICallback->Engine_handleCommand(
	    skirmishAIId, COMMAND_TO_ID_ENGINE, -1, COMMAND_CHEATS_SET_MY_INCOME_MULTIPLIER, &cmd);
	SetCheatsEnabled(false);
}
...
void springLegacyAI::CAIAICheats::GiveMeMetal(float amount)
{
	...
	sAICallback->Engine_handleCommand(skirmishAIId, COMMAND_TO_ID_ENGINE, -1, COMMAND_CHEATS_GIVE_ME_RESOURCE, &cmd);
	SetCheatsEnabled(false);
}
...
int springLegacyAI::CAIAICheats::CreateUnit(const char* unitDefName, float3 pos)
{
	...
	const int unitId = sAICallback->Engine_handleCommand(
	    skirmishAIId, COMMAND_TO_ID_ENGINE, -1, COMMAND_CHEATS_GIVE_ME_NEW_UNIT, &cmd);
	SetCheatsEnabled(false);
	return unitId;
}
...
int springLegacyAI::CAIAICheats::GetEnemyUnits(int* unitIds, const float3& pos, float radius, int unitIds_max)
{
	SetCheatsEnabled(true);
...
int springLegacyAI::CAIAICheats::GetNeutralUnits(int* unitIds, const float3& pos, float radius, int unitIds_max)
{
	SetCheatsEnabled(true);
...
bool springLegacyAI::CAIAICheats::GetUnitResourceInfo(int unitId, UnitResourceInfo* resourceInfo)
{
	SetCheatsEnabled(true);
