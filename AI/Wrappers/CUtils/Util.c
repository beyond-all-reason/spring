...
char *util_allocStrSubCpyByPointers(const char *toCopy, const char *fromPos,
                                    const char *toPos) {
...
char *util_allocStrCat(int numParts, const char *first, ...) {
  ...
  va_start(args, first);
  for (str = first, si = 0; si < numParts;
       ++si, ((str) = (const char *)va_arg(args, const char *))) {
    length += strlen(str);
  }
  va_end(args);
...
bool util_strToBool(const char *str) {
  ...
  if (strcmp(strT, "0") == 0 || strcmp(strT, "NO") == 0 ||
      strcmp(strT, "No") == 0 || strcmp(strT, "no") == 0 ||
      strcmp(strT, "n") == 0 || strcmp(strT, "N") == 0 ||
      strcmp(strT, "FALSE") == 0 || strcmp(strT, "False") == 0 ||
      strcmp(strT, "false") == 0 || strcmp(strT, "f") == 0 ||
      strcmp(strT, "F") == 0) {
    res = false;
  }
...
static unsigned int util_listFilesRec(const char *dir, const char *suffix,
                                      char **fileNames, bool recursive,
                                      const unsigned int maxFileNames,
  ...
  if (handle != -1L) {
    ...
    while (_findnext(handle, &fileInfo) == 0 && numFileNames < maxFileNames) {
      if (util_isFile(&fileInfo)) {
  ...
  if (recursive) {
    ...
    if (handle != -1L) {
      ...
      if (util_isNormalDir(&fileInfo)) {
        ...
        numFileNames =
            util_listFilesRec(subDir, suffix, fileNames, recursive,
                              maxFileNames, numFileNames, subRelPath);
        free(subDir);
        free(subRelPath);
      }
      while (_findnext(handle, &fileInfo) == 0 && numFileNames < maxFileNames) {
...
unsigned int util_listFiles(const char *dir, const char *suffix,
                            char **fileNames, bool recursive,
                            const unsigned int maxFileNames) {
  return util_listFilesRec(dir, suffix, fileNames, recursive, maxFileNames, 0,
                           "");
}
#else // defined _WIN32
  ...
  if (recursive) {
    ...
    for (f = 0; f < currentNumFiles && numFiles < maxFileNames; ++f) {
      ...
      if (retStat == 0 && S_ISDIR(dirStat.st_mode)) {
        numFiles = util_listFilesRec(subDir, suffix, fileNames, recursive,
                                     maxFileNames, numFiles, subRelPath);
      }
      free(subDir);
      free(subRelPath);
    }
 ...
 */
static bool util_parseProperty(const char *propLine, const char *keys[],
                               const char *values[], int propStoreIndex) {
...
int util_parsePropertiesFile(const char *propertiesFile, const char *keys[],
                             const char *values[], int maxProperties) {
...
const char *util_map_getValueByKey(unsigned int mapSize, const char **mapKeys,
                                   const char **mapValues, const char *key) {
